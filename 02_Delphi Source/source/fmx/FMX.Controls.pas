...
uses
  ...
  FMX.ActnList, FMX.Types, FMX.Styles, FMX.VirtualKeyboard, FMX.TextLayout, FMX.Graphics, FMX.Filter, FMX.Effects,
  FMX.Ani, FMX.AcceleratorKey;
...
procedure TControlActionLink.SetShortCut(Value: System.Classes.TShortCut);
var
  KeyShortcut: IKeyShortcut;
begin
  if IsShortCutLinked and Supports(Client, IKeyShortcut, KeyShortcut) then
    KeyShortcut.ShortCut := Value;
end;
...
{ TStyleCache }
  ...
  private
    ...
    FClearStyleCacheId: TMessageSubscriptionId;
    FEnabled: Boolean;
    FClearing: Boolean;
    procedure Clear;
...
procedure TControlHelper.PositionChanged(Sender: TObject);
var
...
procedure TControl.RepaintJointArea(const DestControl: TFmxObject);
var
  UpdateRect: TRectF;
begin
  if not ClipChildren and ((Canvas = nil) or (TCanvasStyle.SupportClipRects in Canvas.GetCanvasStyle)) and
     not (csDestroying in DestControl.ComponentState) then
  begin
...
procedure TControl.DoAddObject(const AObject: TFmxObject);
  ...
  var
    I: Integer;
  begin
    for I := AControl.FUpdating downto FUpdating + 1 do
      AControl.EndUpdate;

    for I := AControl.FUpdating to FUpdating - 1 do
...
begin
  ...
  try
    ...
    begin
      ...
      NeedRepaint := True;
      if FInPaintTo then
        ChildControl.SetInPaintTo(True);
    end;
...
function TControl.DoGetUpdateRect: TRectF;
  ...
  begin
    DisableDisappear := True;
    try
      P.DisableDisappear := True;
      try
        Result := P.UpdateRect;
      finally
        P.DisableDisappear := False;
      end;
    finally
      DisableDisappear := False;
    end;
...
procedure TControl.RecalculateAbsoluteMatrices;
var
...
procedure TControl.FlipChildren(const AAllLevels: Boolean);
const
    ...
    try
      try
        ...
        begin
          ...
          if (ChildControl.Align in HorizontalAlignments) and not CanFlipChild(ChildControl) then
...
procedure TControl.ParentContentChanged;
begin
  if ParentContent <> nil then
    ParentContent.Changed;
  if ParentContentObserver <> nil then
    ParentContentObserver.Changed(Self);
end;
...
begin
  if (FEffectCache <> nil) and not FApplyingEffect then
    FEffectCache.SetNeedUpdate;
  if FParentControl <> nil then
...
begin
  if (Children = nil) or (FScene = nil) or FDisableEffect or (FEffects = nil) or not HasEffect then
    Exit;

  FApplyingEffect := True;
  try
    for var Effect in FEffects do
      if Effect.Enabled then
      begin
        var EffectRect := Effect.GetRect(LocalRect);
        var State := Canvas.SaveState;
        try
          Canvas.SetMatrix(AbsoluteMatrix);
          var CacheCanvas: TCanvas;
          if Effect.BeginLayer(Canvas, EffectRect, AbsoluteOpacity, RotationAngle = 0, FEffectCache, CacheCanvas) then
          try
            var S := GetAbsoluteScale;
            var R := TRectF.Create(Effect.GetOffset, Width, Height);
            MultiplyRect(R, S.X, S.Y);
            if TEffectStyle.DisablePaintToBitmap in Effect.EffectStyle then
              CacheCanvas.ClearRect(R, TAlphaColors.Null)
            else
              PaintTo(CacheCanvas, R);
          finally
            Effect.EndLayer;
          end;
        finally
...
procedure TControl.DrawDragHighlight;
const
...
begin
  ...
  if not HasDisablePaintEffect or ApplyingEffect then
  begin
...
begin
  EnumRenderableControls(
    procedure (AControl: TControl)
    begin
      AControl.PrepareForPaint;
    end);
end;
...
begin
  ...
  begin
    ...
    begin
      if not TControl(P).Visible then
        Exit;
    end
    else if Supports(P, IControl, Control) then
    begin
...
function TControl.CanFlipChild(const AChild: TFmxObject): Boolean;
begin
  Result := True;
end;
...
begin
  if not CanRepaint or Rect.IsEmpty then
    Exit;
...
procedure TControl.EnumRenderableControls(const AConsumer: TProc<TControl>);
var
  I, J: Integer;
  R: TRectF;
  AllowPaint: Boolean;
...
procedure TControl.DoFlipChildren;
var
  I: Integer;
  TheWidth: Single;
  ChildControl: TControl;
begin
  ...
  begin
    ChildControl := Controls[I];
    if CanFlipChild(ChildControl) then
      ChildControl.Position.X := TheWidth - ChildControl.GetContainerWidth - ChildControl.Position.X ;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not SizeChanged and Moved then
        Move;
      if Moved or SizeChanged then
      begin
        RecalcUpdateRect;
...
procedure TStyledControl.NeedStyleLookup;
  ...
  begin
    Result := (Scene <> nil) and (Scene.StyleBook <> nil);
  end;
...
begin
  ...
  if FResourceLink <> nil then
  begin
    // We don't use style cache, if user uses custom styles
    if IsCustomStyle or not TStyleCache.Current.Enabled then
      KillResourceLink
    else
      RecycleResourceLink;
  end;
...
procedure TStyledControl.DoFreeStyle;
begin
  if Assigned(FOnFreeStyle) then
    FOnFreeStyle(Self);
end;
...
var
  ...
  SaveDisableAlign: Boolean;
begin
  if FIsNeedStyleLookup and (Scene <> nil) and (not InPaintTo or (FResourceLink = nil)) then
  begin
...
function TStyledControl.CanFlipChild(const AChild: TFmxObject): Boolean;
begin
...
procedure TStyledControl.InternalApplyStyle(const StyleObject: TFmxObject);
var
...
begin
  ...
  try
    ...
    begin
      ...
      StyleControl.Lock;
      ApplyStyle;
      if (FEffectCache <> nil) and not ApplyingEffect then
        FEffectCache.SetNeedUpdate;
      DoApplyStyleLookup;
    end
    else
...
procedure TStyledControl.FreeStyle;
  ...
  begin
    ...
    begin
      ...
      if ValueFromStyle.IsInstanceOf(TPersistent) and ValueFromData.IsInstanceOf(TPersistent) and
        (ValueFromStyle.AsObject <> ValueFromData.AsObject) then
        ValueFromData.AsType<TPersistent>.Assign(ValueFromStyle.AsType<TPersistent>)
      else
...
procedure TPopup.RecalculateAbsoluteMatrices;
begin
  if FUseParentScale then
    inherited
  else
  begin
    FAbsoluteMatrix := FLocalMatrix;
...
procedure TPopup.SetUseParentScale(const Value: Boolean);
begin
  if FUseParentScale <> Value then
  begin
    FUseParentScale := Value;
    FRecalcAbsolute := True;
  end;
end;
