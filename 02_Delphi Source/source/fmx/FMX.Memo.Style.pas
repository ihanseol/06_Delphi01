...
uses
  ...
  FMX.Types, FMX.Controls, FMX.TextLayout, FMX.Objects, FMX.MagnifierGlass, FMX.SpellChecker, FMX.Menus, FMX.Memo.Types,
  FMX.Presentation.Messages, FMX.Presentation.Style, FMX.Controls.Presentation, FMX.ScrollBox.Style, FMX.Text,
  FMX.Controls.Model, FMX.Text.UndoManager;
...
begin
  ...
  begin
    ...
    UpdateSelectionPointPositions;
    if not FSpellHighlightRect.IsEmpty then
      FSpellHighlightRect.Offset(OldViewportPosition.X - NewViewportPosition.X, OldViewportPosition.Y - NewViewportPosition.Y);
    if Model.CheckSpelling then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Model.Caret.Visible := True;
      FLMouseSelecting := False;
      if not FFollowTheMouse and not (ssDouble in Shift) then
      begin
        FSelected := False;
...
procedure TStyledMemo.UpdateSelectionPointPositions;
  ...
  begin
    ...
    begin
      ...
      if FShouldPlaceLeftSelectionPointBelow then
        Result.Y := Result.Y + Region[Low(Region)].Height
      else
    ...
    begin
      ...
      if FShouldPlaceLeftSelectionPointBelow then
        Result.Y := Result.Y + GetLineHeight
      else
        Result.Y := Result.Y - 2 * FLeftSelPt.GripSize;
    end;
  ...
  begin
    ...
    if FShouldPlaceLeftSelectionPointBelow then
      Result.Y := Result.Y + Model.ContentBounds.Top
    else
...
procedure TStyledMemo.LeftSelPtChangePositionHandler(Sender: TObject; var X, Y: Single);
  ...
  begin
    if FShouldPlaceLeftSelectionPointBelow then
      HitTestPoint := TPointF.Create(X, Y - FLeftSelPt.Height)
    else
      HitTestPoint := TPointF.Create(X, Y + FLeftSelPt.GripSize * 2);

    NewSelStart := FLineObjects.GetPointPosition(HitTestPoint);
  ...
  begin
    ...
    begin
      ...
      if FShouldPlaceLeftSelectionPointBelow then
        PointTmp.Y := PointTmp.Y + Region[Low(Region)].Height
      else
    ...
    begin
      ...
      if FShouldPlaceLeftSelectionPointBelow then
        PointTmp.Y := PointTmp.Y + GetLineHeight
      else
        PointTmp.Y := PointTmp.Y - 2 * FLeftSelPt.GripSize + Model.ContentBounds.Top;
    end;
...
procedure TStyledMemo.RightSelPtChangePositionHandler(Sender: TObject; var X, Y: Single);
  ...
  begin
    if FShouldPlaceLeftSelectionPointBelow then
      HitTestPoint := TPointF.Create(X, Y - FRightSelPt.Height)
    else
      HitTestPoint := TPointF.Create(X, Y - FRightSelPt.GripSize * 2);
    NewSelEnd := FLineObjects.GetPointPosition(HitTestPoint);
    GetNormalizedSelectionRange(SelStartTmp, SelEndTmp);
  ...
  begin
    ...
    PointTmp.X := PointTmp.X + Model.ContentBounds.Left;
    if FShouldPlaceLeftSelectionPointBelow then
      PointTmp.Y := PointTmp.Y + GetLineHeight + Model.ContentBounds.Top
    else
...
procedure TStyledMemo.CaretPtChangePositionHandler(Sender: TObject; var X, Y: Single);

  function GetCaretPointPos: TPointF;
  begin
    Result := GetPositionPoint(CaretPosition) - ViewportPosition;
    Result.Y := Result.Y + GetLineHeight;
    Result.X := Result.X + Model.Caret.Width;
    Result := FContent.ConvertLocalPointTo(FCaretPt.ParentControl, Result);
  end;

begin
  StartAutoScroll(X, Y);
  PutCaretTo(X, Y - FCaretPt.Height);
  X := GetCaretPointPos.X;
  Y := GetCaretPointPos.Y;
  if FLoupeService <> nil then
    SetLoupePosition(X, Y);
end;

procedure TStyledMemo.CaretPtMouseDownHandler(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
var
  CaretPoint: TPointF;
begin
  RepaintEdit;
...
procedure TStyledMemo.CaretPtMouseUpHandler(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
  StopAutoScroll;
  HideLoupe;
end;
...
procedure TStyledMemo.UpdateCaretPointPosition;
  ...
  var
    PointInContent: TPointF;
  begin
    PointInContent := Memo.ConvertLocalPointFrom(FCaretPt, TPointF.Zero);
    Result := Memo.LocalRect.Contains(PointInContent);
  end;
...
begin
  ...
  FCaretPt.Position.Point := GetCaretPointPos;
end;
...
procedure TStyledMemo.SetLoupePosition(const ASelectionPointType: TSelectionPointType);
  ...
  begin
    ...
    begin
      ...
      Result.BottomRight := ConvertLocalPointFrom(FContent, Result.BottomRight);
    end;
  end;
...
begin
  ...
  begin
    case ASelectionPointType of
      TSelectionPointType.Left:
        begin
          ZoomCenter := TPointF.Create(SelRect.Left, SelRect.CenterPoint.Y);
          LoupePos := SelRect.TopLeft - TPointF.Create(FLoupeService.GetWidth / 2, FLoupeService.GetHeight + LOUPE_OFFSET);
        end;
      TSelectionPointType.Right:
        begin
          ZoomCenter := TPointF.Create(SelRect.Right, SelRect.CenterPoint.Y);
          LoupePos := TPointF.Create(SelRect.Right, SelRect.Top) - TPointF.Create(FLoupeService.GetWidth / 2, FLoupeService.GetHeight + LOUPE_OFFSET);
        end;
    end;
