  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Child.Align in NeedAddBottom then
          LocalR.Bottom := LocalR.Bottom + Padding.Bottom;
        if Child.Align in NeedAddRight then
...
begin
  ...
  Matrix := TMatrix.CreateScaling(RoundTo(Width / FOriginalWidth, ScaleAccuracy),
                                  RoundTo(Height / FOriginalHeight, ScaleAccuracy));
...
procedure TFlowLayout.ContentChanged(const AChild: TFmxObject);
begin
...
begin
  ...
  begin
    DestCell := TCellItem(Dest);
    DestCell.FSizeStyle := Self.FSizeStyle;
    DestCell.FValue := Self.FValue;
    DestCell.FSize := Self.FSize;
  end;
end;
...
begin
  ...
  begin
    ...
    else if FSizeStyle = TSizeStyle.Percent then
      FValue := EnsureRange(Value, 0, 100)
    else
      FValue := Value;
    Changed(False);
  end;
...
function TGridPanelLayout.TCellCollection.GetItemAttr(Index, ItemIndex: Integer): string;
  ...
  begin
    case Items[Index].SizeStyle of
      ...
      TSizeStyle.Weight: Result := SCellWeightSize;
    else
      Result := string.Empty;
    end;
  end;
...
begin
  ...
  if Owner <> nil then
  begin
    Owner.NeedRecalculateCellSizes;
    Owner.FNeedAlign := True;
    if Item is TColumnItem then
      Owner.RecalcCellDimensions(nil, TColumnItem(Item))
    else if Item is TRowItem then
      Owner.RecalcCellDimensions(TRowItem(Item), nil);
    Owner.Realign;
...
procedure TGridPanelLayout.DoDeleteChildren;
begin
  if FControlCollection <> nil then
    FControlCollection.Clear;
  inherited;
end;
...
procedure TGridPanelLayout.NeedRecalculateCellSizes;
begin
  Include(FState, TState.NeedRecalculateCellSizes);
end;
...
procedure TGridPanelLayout.RecalcCellDimensions(const ABasedRow: TRowItem; const ABasedColumn: TColumnItem);
begin
  RecalcCellDimensions(LocalRect, ABasedRow, ABasedColumn);
end;

procedure TGridPanelLayout.RecalcCellDimensions(const ARect: TRectF; const ABasedRow: TRowItem; const ABasedColumn: TColumnItem);
begin
  if (csLoading in ComponentState) or (TState.RecalculatingCellSizes in FState) then
    Exit;

  Include(FState, TState.RecalculatingCellSizes);
  try
    FLayoutManager.RecalcCellDimensions(ARect, ABasedRow, ABasedColumn);
  finally
    FState := [];
  end;
end;
...
begin
  ...
  FDisableAlign := True;
  try
    if TState.NeedRecalculateCellSizes in FState then
...
{ TGridPanelLayout.TLayoutManager }

function TGridPanelLayout.TLayoutManager.CalculateMaxHeight(const ARow: Integer): Single;
var
  ControlItem: TControlItem;
  Column: Integer;
  Control: TControl;
...
begin
  Result := 0;
  for Column := 0 to FOwner.ColumnCollection.Count - 1 do
  begin
    ControlItem := FOwner.ControlCollection.ControlItems[Column, ARow];
    if (ControlItem <> nil) and (ControlItem.Control <> nil) and (ControlItem.Column = Column) and (ControlItem.Row = ARow) then
    begin
      Control := ControlItem.Control;
      LSize := Control.Height + Control.Margins.Top + Control.Margins.Bottom + FOwner.Padding.Top + FOwner.Padding.Bottom;
...
function TGridPanelLayout.TLayoutManager.CalculateMaxWidth(const AColumn: Integer): Single;
var
  ControlItem: TControlItem;
  Row: Integer;
  Control: TControl;
...
begin
  Result := 0;
  for Row := 0 to FOwner.FRowCollection.Count - 1 do
  begin
    ControlItem := FOwner.ControlCollection.ControlItems[AColumn, Row];
    if (ControlItem <> nil) and (ControlItem.Control <> nil) and (ControlItem.Column = AColumn) and (ControlItem.Row = Row) then
    begin
      Control := ControlItem.Control;
      LSize := Control.Width + Control.Margins.Left + Control.Margins.Right + FOwner.Padding.Left + FOwner.Padding.Right;
      if LSize > Result then
        Result := LSize;
    end;
  end;
...
function TGridPanelLayout.TLayoutManager.CollectColumnsInfo(const AWidth: Single): TCollectionInfo;
var
  Item: TCellItem;
  ColumnIndex: Integer;
  MaxSize: Single;
begin
  Result.FlexibleSize := AWidth;
  Result.WeightSum := 0;
  Result.WeightCount := 0;
  Result.PercentageSum := 0;
  Result.PercentageCount := 0;
  for ColumnIndex := 0 to FOwner.ColumnCollection.Count - 1 do
  begin
    Item := FOwner.ColumnCollection[ColumnIndex];
    case Item.SizeStyle of
      TSizeStyle.Absolute:
        Result.FlexibleSize := Result.FlexibleSize - Item.Value;

      TSizeStyle.Percent:
      begin
        Result.PercentageSum := Result.PercentageSum  + Item.Value;
        Inc(Result.PercentageCount);
      end;

      TSizeStyle.Weight:
      begin
        Result.WeightSum := Result.WeightSum + Item.Value;
        Inc(Result.WeightCount);
      end;

      TSizeStyle.Auto:
      begin
        MaxSize := CalculateMaxWidth(ColumnIndex);
        Result.FlexibleSize := Result.FlexibleSize - MaxSize;
        Item.Size := MaxSize;
      end;
    end;
  end;
...
function TGridPanelLayout.TLayoutManager.CollectRowsInfo(const AHeight: Single): TCollectionInfo;
var
  Item: TCellItem;
  RowIndex: Integer;
  MaxSize: Single;
begin
  Result.FlexibleSize := AHeight;
  Result.WeightSum := 0;
  Result.WeightCount := 0;
  Result.PercentageSum := 0;
  Result.PercentageCount := 0;
  for RowIndex := 0 to FOwner.RowCollection.Count - 1 do
  begin
    Item := FOwner.RowCollection[RowIndex];
    case Item.SizeStyle of
      TSizeStyle.Absolute:
        Result.FlexibleSize := Result.FlexibleSize - Item.Value;
      ...
      TSizeStyle.Auto:
      begin
        MaxSize := CalculateMaxHeight(RowIndex);
        Result.FlexibleSize := Result.FlexibleSize - MaxSize;
        Item.Size := MaxSize;
      end;
    end;
...
constructor TGridPanelLayout.TLayoutManager.Create(const AOwner: TGridPanelLayout);
begin
  FOwner := AOwner;
end;

procedure TGridPanelLayout.TLayoutManager.NormalizePercentages(const ACollection: TCellCollection;
  const AInfo: TCollectionInfo; const ABaseCell: TCellItem);
var
  I: Integer;
  Item: TCellItem;
  BaseValue: Single;
  RemainPercentages: Single;
  NormalizedRemainPercentages: Single;
  CountWithoutBase: Integer;
begin
  if ABaseCell = nil then
  begin
    BaseValue := 0;
    CountWithoutBase := AInfo.PercentageCount;
  end
  else
  begin
    BaseValue := ABaseCell.Value;
    CountWithoutBase := AInfo.PercentageCount - 1;
  end;

  RemainPercentages := AInfo.PercentageSum - BaseValue;
  NormalizedRemainPercentages := 100 - BaseValue;

  for I := 0 to ACollection.Count - 1 do
  begin
    Item := ACollection[I];
    if (Item.SizeStyle <> TSizeStyle.Percent) or (Item = ABaseCell) then
      Continue;

    if IsZero(NormalizedRemainPercentages) or IsZero(RemainPercentages) then
      Item.Value := NormalizedRemainPercentages / CountWithoutBase
    else
      Item.Value := (Item.Value / RemainPercentages) * NormalizedRemainPercentages;
  end;
end;

procedure TGridPanelLayout.TLayoutManager.RealignItems(const ACollection: TCellCollection; const AInfo: TCollectionInfo;
  const ABaseCell: TCellItem);
var
  Index: Integer;
  Item: TCellItem;
  PercentageSize: Single;
  FlexSize: Single;
begin
  PercentageSize := AInfo.FlexibleSize * AInfo.PercentageSum / 100;
  FlexSize := AInfo.FlexibleSize - PercentageSize;
  for Index := 0 to ACollection.Count - 1 do
  begin
    Item := ACollection[Index];
    case Item.SizeStyle of
      TSizeStyle.Percent:
        Item.Size := AInfo.FlexibleSize * (Item.Value / 100);

      TSizeStyle.Weight:
        Item.Size := FlexSize * (Item.Value / AInfo.WeightSum);
    end;
  end;
end;

procedure TGridPanelLayout.TLayoutManager.RecalcCellDimensions(const ARect: TRectF; const ABasedRow: TRowItem;
  const ABasedColumn: TColumnItem);
var
  ColumnsInfo: TCollectionInfo;
  RowsInfo: TCollectionInfo;
begin
  ColumnsInfo := CollectColumnsInfo(ARect.Width);
  RowsInfo := CollectRowsInfo(ARect.Height);

  if ShouldNormalizePercentages(ColumnsInfo) then
    NormalizePercentages(FOwner.ColumnCollection, ColumnsInfo, ABasedColumn);

  if ShouldNormalizePercentages(RowsInfo) then
    NormalizePercentages(FOwner.RowCollection, RowsInfo, ABasedRow);

  // Finally Calculate the size of the percentage-based columns and rows based on the remaining
  // X and Y sizes
  RealignItems(FOwner.ColumnCollection, ColumnsInfo, ABasedColumn);
  RealignItems(FOwner.RowCollection, RowsInfo, ABasedRow);
end;

function TGridPanelLayout.TLayoutManager.ShouldNormalizePercentages(const ACollectionInfo: TCollectionInfo): Boolean;
begin
  Result := ((ACollectionInfo.WeightCount = 0) and not SameValue(ACollectionInfo.PercentageSum, 100)) or
            ((ACollectionInfo.WeightCount > 0) and (CompareValue(ACollectionInfo.PercentageSum, 100) = GreaterThanValue));
end;
