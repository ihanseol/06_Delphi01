...
type
  ...
  public
    constructor Create(const AColumn: TColumn);
    destructor Destroy; override;
    procedure Changed; virtual;
    property Column: TColumn read FColumn;
  published
    property StyleLookup: string read FStyleLookup write SetStyleLookup;
    property StyledSettings: TStyledSettings read GetStyledSettings write SetStyledSettings stored IsStyledSettingsStored nodefault;
  ...
  private
    ...
    procedure SetHeaderSettings(const Value: THeaderSettings);
  protected
    { ICaption }
  ...
  published
    ...
    property HeaderSettings: THeaderSettings read FHeaderSettings write SetHeaderSettings;
  ...
  private
    FVKMessageId: TMessageSubscriptionId;
...
procedure TColumn.SetHeaderSettings(const Value: THeaderSettings);
begin
  FHeaderSettings.Assign(Value);
end;
...
var
  LCoord: TGridModel.TCoord;
begin
  LCoord.Col := EnsureRange(ACol, -1, ColumnCount - 1);
  LCoord.Row := EnsureRange(ARow, -1, RowCount - 1);
...
begin
  if (Cardinal(ACol) >= Cardinal(ColumnCount)) or (Cardinal(ARow) >= Cardinal(RowCount)) then
    Result := string.Empty
  else
...
begin
  if Cardinal(ACol) >= Cardinal(ColumnCount) then
    raise EArgumentOutOfRangeException.CreateResFmt(@SInvalidColumnIndex, [ACol]);
  if Cardinal(ARow) >= Cardinal(RowCount) then
...
var
  NeedUpdateEditor: Boolean;
begin
  if not FCellsChanging and
    (Cardinal(ACoord.Col) < Cardinal(ColumnCount)) and
    (Cardinal(ACoord.Row) < Cardinal(RowCount)) and
    (Columns[ACoord.Col].EffectiveVisible) then
  begin
...
type
  THeaderSettingsInfo = class(TTextSettingsInfo)
  private
    [Weak] FHeaderSettings: THeaderSettings;
    FEditModel: TCustomEditModel;
  protected
    procedure DoCalculatedTextSettings; override;
  public
    constructor Create(AOwner: TPersistent; ATextSettingsClass: TTextSettingsInfo.TCustomTextSettingsClass); override;
  ...
  THeaderTextSettings = class (TTextSettingsInfo.TCustomTextSettings)
  public
    constructor Create(const AOwner: TPersistent); override;
  published
    property Font;
...
constructor THeaderSettings.Create(const AColumn: TColumn);
begin
  inherited Create;
  FColumn := AColumn;
  FTextSettingsInfo := THeaderSettingsInfo.Create(Self, THeaderTextSettings);
end;
...
function THeaderSettings.GetStyledSettings: TStyledSettings;
begin
  Result := FTextSettingsInfo.StyledSettings;
end;

function THeaderSettings.GetTextSettings: TTextSettings;
begin
  Result := FTextSettingsInfo.TextSettings;
end;

function THeaderSettings.IsStyledSettingsStored: Boolean;
begin
  Result := StyledSettings <> DefaultStyledSettings;
end;

procedure THeaderSettings.SetStyledSettings(const Value: TStyledSettings);
begin
  FTextSettingsInfo.StyledSettings := Value;
end;

procedure THeaderSettings.SetStyleLookup(const Value: string);
begin
  FStyleLookup := Value;
  FColumn.Changed;
end;

procedure THeaderSettings.SetTextSettings(const Value: TTextSettings);
begin
  FTextSettingsInfo.TextSettings := Value;
end;

{ THeaderTextSettings }

constructor THeaderTextSettings.Create(const AOwner: TPersistent);
begin
  inherited;
  WordWrap := False;
...
constructor THeaderSettingsInfo.Create(AOwner: TPersistent; ATextSettingsClass: TTextSettingsInfo.TCustomTextSettingsClass);
begin
  inherited;
  FHeaderSettings := THeaderSettings(AOwner);
end;

procedure THeaderSettingsInfo.DoCalculatedTextSettings;
begin
  inherited;
  FHeaderSettings.Changed;
end;
