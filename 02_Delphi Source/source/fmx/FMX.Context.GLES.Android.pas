...
begin
  ...
  Win := ANativeWindow_fromSurface(TJNIResolver.GetJNIEnv, TJNIResolver.JavaInstanceToID(Surface));
  try
...
class procedure TCustomAndroidContext.LoadExtensions;
  ...
  begin
    ...
    if not Surface.isValid then
      RaiseContextExceptionFmt(@SCannotCreateOpenGLContextWithCode, ['JSurfaceTexture', eglGetError()]);
    NativeWindow := ANativeWindow_fromSurface(TJNIResolver.GetJNIEnv, TJNIResolver.JavaInstanceToID(Surface));
...
class procedure TCustomAndroidContext.CreateSharedContext;
  ...
  begin
    Result := (TPlatformServices.Current <> nil) and TPlatformServices.Current.SupportsPlatformService(IFMXApplicationService, ApplicationService) and
...
begin
  ...
  FNativeWindow := ANativeWindow_fromSurface(TJNIResolver.GetJNIEnv, TJNIResolver.JavaInstanceToID(TAndroidWindowHandle(Parent).Holder.getSurface));
  eglGetConfigAttrib(FSharedDisplay, Config, EGL_NATIVE_VISUAL_ID, @Format);
  ANativeWindow_setBuffersGeometry(FNativeWindow, 0, 0, Format);
