...
uses
  System.Types, System.Generics.Collections, Winapi.Windows, FMX.Types, FMX.Platform, FMX.Platform.Win, FMX.Forms;
...
{ TWinMultiDisplay }
  ...
  TWinFormPositionerService = class(TDefaultFormPositionerService)
  protected
    procedure PlaceDesktopCenter(const AForm: TCommonCustomForm); override;
  end;
...
begin
  ...
  if (AIndex < 0) or (AIndex >= GetDisplayCount) then
    ListIndexError(AIndex, GetDisplayCount - 1, Self);
...
procedure TWinFormPositionerService.PlaceDesktopCenter(const AForm: TCommonCustomForm);
var
  NewBounds: TRect;
  Service: IFMXMultiDisplayService;
  FormHandle: TWinWindowHandle;
begin
  if TPlatformServices.Current.SupportsPlatformService(IFMXMultiDisplayService, Service) and (Service.DisplayCount > 0) then
  begin
    NewBounds := Service.PhysicalDesktopRect; // px
    FormHandle := WindowHandleToPlatform(AForm.Handle);
    NewBounds := TRect.Create(TPoint.Create(NewBounds.Left + (NewBounds.Width - FormHandle.WndBounds.Width) div 2,
                                            NewBounds.Top + (NewBounds.Height - FormHandle.WndBounds.Height) div 2),
                              FormHandle.WndBounds.Width, FormHandle.WndBounds.Height);
    NewBounds := FitInRect(NewBounds, Service.PhysicalDesktopRect).Round;
    FormHandle.WndBounds := NewBounds;
  end
  else
    inherited;
end;
