...
type
  ...
  private type
    ...
    TWinMenuInfo = record
      ...
      constructor Create(const AMenuId: TMenuId; const AnItem: TMenuItem);
    end;
  private
    ...
    function AssignNewIdToLastMenu(const AParentMenu, AMenu: HMENU): TMenuId;
...
function TWinMenuService.AssignNewIdToLastMenu(const AParentMenu, AMenu: HMENU): TMenuId;
var
  MenuItemInfo: tagMENUITEMINFOW;
  LastMenuItemPos: Integer;
begin
  ...
  LastMenuItemPos := GetMenuItemCount(AParentMenu) - 1;
  if not GetMenuItemInfo(AParentMenu, LastMenuItemPos, True, MenuItemInfo) then
    RaiseLastOSError;
  MenuItemInfo.wID := Result;
  try
    if not SetMenuItemInfo(AParentMenu, LastMenuItemPos, True, MenuItemInfo) then
      RaiseLastOSError;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          begin
            PopupMenuId := AssignNewIdToLastMenu(AParent, PopupMenu);
...
constructor TWinMenuService.TWinMenuInfo.Create(const AMenuId: TMenuId; const AnItem: TMenuItem);
begin
  MenuID := AMenuId;
  FMXMenuItem := AnItem;
end;
