...
function TEffect.BeginLayer(const ATarget: TCanvas; const ADestRect: TRectF;
...
procedure TEffect.EndLayer;
var
  LBitmapCacheLayer: IBitmapCacheLayer;
begin
  if Supports(FCacheLayer, IBitmapCacheLayer, LBitmapCacheLayer) then
  begin
    LBitmapCacheLayer.EndUpdate(
      procedure(ABitmap: TBitmap)
      begin
        ProcessEffect(ABitmap.Canvas, ABitmap, ABitmap.Canvas.Matrix.ExtractScale.X);
      end);
  end;
  FCacheLayer := nil;
end;
...
function TFilterEffect.BeginLayer(const ATarget: TCanvas;
  const ADestRect: TRectF; const AOpacity: Single; const AHighSpeed: Boolean;
  var ACacheLayer: IFilterCacheLayer; out ACacheCanvas: TCanvas): Boolean;
begin
  Result := (FFilter <> nil) and FFilter.BeginLayer(ATarget, ADestRect,
    AOpacity, AHighSpeed, ACacheLayer, ACacheCanvas);
end;
...
procedure TFilterEffect.EndLayer;
begin
  if FFilter <> nil then
    FFilter.EndLayer;
end;
...
begin
  ...
  begin
    ...
    FFilter.FilterContext.CopyToBitmap(Visual, Visual.Bounds);
  end;
end;
  ...
  begin
    ...
    if Assigned(Context) then
      ...
      try
        ...
        Mat.Texture := FFilter.ValuesAsTexture['Output'];
        ...
        Ver.Vertices[1] := Point3D(Context.PixelToPixelPolygonOffset.X + Visual.Width,
          Context.PixelToPixelPolygonOffset.Y, 0);
        Ver.TexCoord0[1] := PointF(Visual.Width / Mat.Texture.Width, 0.0);
        ...
        Ver.TexCoord0[2] := PointF(Visual.Width / Mat.Texture.Width,
          Visual.Height / Mat.Texture.Height);
        Ver.Vertices[3] := Point3D(Context.PixelToPixelPolygonOffset.X,
          Context.PixelToPixelPolygonOffset.Y + Visual.Height, 0);
        Ver.TexCoord0[3] := PointF(0.0, Visual.Height / Mat.Texture.Height);
