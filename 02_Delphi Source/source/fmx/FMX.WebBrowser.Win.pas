...
uses
  System.Classes, System.Win.InternetExplorer, System.UITypes, System.Types, System.Win.IEInterfaces, System.IOUtils,
  ...
  FMX.Presentation.Factory, FMX.Consts, FMX.Helpers.Win;
...
type
  ...
  TEdgeWebBrowserModel = class;

  TEdgeWebBrowserPresentation = class(TWinPresentation)
  private
    FURL: string;
  ...
  public
    ...
    property Model: TEdgeWebBrowserModel read FModel;
    /// <summary>
    ///   If set this specifies the location of msedgewebview2.exe, used with WebView2 Fixed Version Distribution mode:
    ///   https://docs.microsoft.com/en-us/microsoft-edge/webview2/concepts/distribution#fixed-version-distribution-mode
    ///   The value can contain environment variables surrounded by % signs, e.g. %LOCALAPPDATA%.
  ...
  TEdgeWebBrowserModel = class(TDataModel)
  private
    FControl: TEdgeWebBrowser;
  protected
    property Control: TEdgeWebBrowser read FControl write FControl;
  public
    procedure DoContentLoading(const ASuccess: Boolean; const ANavigationID: UInt64);
    procedure DoCreateWebViewCompleted(const ASuccess: Boolean);
    procedure DoJavascriptEvaluated(const AErrorCode: LongInt; const AResultObjectAsJSON: string);
  ...
  TEdgeWebBrowser = class(TPresentedControl)
  private
    FCustomBrowser: ICustomBrowser;
    FNativePresentation: TEdgeWebBrowserPresentation;
    ...
    function GetModel: TEdgeWebBrowserModel;
  protected
    function DefineModelClass: TDataModelClass; override;
    procedure DoContentLoading(const ASuccess: Boolean; const ANavigationID: UInt64);
    procedure DoCreateWebViewCompleted(const ASuccess: Boolean);
  ...
  public
    ...
    procedure Refresh;
    procedure Stop;
    property Model: TEdgeWebBrowserModel read GetModel;
    property NativePresentation: TEdgeWebBrowserPresentation read FNativePresentation;
  ...
  private
    FURL: string;
    FWinBrowserControl: TEdgeWebBrowser;
...
{ TEdgeWebBrowserModel }

procedure TEdgeWebBrowserModel.DoContentLoading(const ASuccess: Boolean; const ANavigationID: UInt64);
begin
  FControl.DoContentLoading(ASuccess, ANavigationID);
end;

procedure TEdgeWebBrowserModel.DoCreateWebViewCompleted(const ASuccess: Boolean);
begin
  FControl.DoCreateWebViewCompleted(ASuccess);
end;

procedure TEdgeWebBrowserModel.DoJavascriptEvaluated(const AErrorCode: LongInt; const AResultObjectAsJSON: string);
begin
  FControl.DoJavascriptEvaluated(AErrorCode, AResultObjectAsJSON);
end;

procedure TEdgeWebBrowserModel.DoNavigationCompleted(const ASuccess: Boolean);
begin
  FControl.DoNavigationCompleted(ASuccess);
end;

procedure TEdgeWebBrowserModel.DoNavigationStarting(const AURL: string);
begin
...
constructor TEdgeWebBrowser.Create(AOwner: TComponent);
begin
  inherited;
  Model.Control := Self;
end;

function TEdgeWebBrowser.DefineModelClass: TDataModelClass;
begin
  Result := TEdgeWebBrowserModel;
end;

procedure TEdgeWebBrowser.LoadPresentation;
begin
  inherited;
  if ControlType = TControlType.Platform then
  begin
    FNativePresentation := TEdgeWebBrowserPresentation(Presentation);
    FNativePresentation.CustomBrowser := FCustomBrowser;
  end;
end;

procedure TEdgeWebBrowser.DoContentLoading(const ASuccess: Boolean; const ANavigationID: UInt64);
begin
  if Assigned(FOnContentLoading) then
    FOnContentLoading(Self, ASuccess, ANavigationID);
end;

procedure TEdgeWebBrowser.DoCreateWebViewCompleted(const ASuccess: Boolean);
begin
  if Assigned(FOnCreateWebViewCompleted) then
    FOnCreateWebViewCompleted(Self, ASuccess);
end;

procedure TEdgeWebBrowser.DoJavascriptEvaluated(const AErrorCode: LongInt; const AResultObjectAsJSON: string);
begin
  if Assigned(FOnJavascriptEvaluated) then
    FOnJavascriptEvaluated(Self, AErrorCode, AResultObjectAsJSON);
end;

procedure TEdgeWebBrowser.DoNavigationCompleted(const ASuccess: Boolean);
begin
  if Assigned(FOnNavigationCompleted) then
    FOnNavigationCompleted(Self, ASuccess);
end;

procedure TEdgeWebBrowser.DoNavigationStarting(const AURL: string);
begin
  if Assigned(FOnNavigationStarting) then
    FOnNavigationStarting(Self, AURL);
end;

function TEdgeWebBrowser.GetModel: TEdgeWebBrowserModel;
begin
  Result := inherited GetModel<TEdgeWebBrowserModel>;
end;

function TEdgeWebBrowser.CanGoBack: Boolean;
begin
  Result := FNativePresentation.CanGoBack;
end;

function TEdgeWebBrowser.CanGoForward: Boolean;
begin
  Result := FNativePresentation.CanGoForward;
end;

function TEdgeWebBrowser.CaptureBitmap: TBitmap;
begin
  Result := FNativePresentation.CaptureBitmap;
end;

procedure TEdgeWebBrowser.EvaluateJavascript(const AJavascript: string);
begin
  FNativePresentation.EvaluateJavascript(AJavascript);
end;

procedure TEdgeWebBrowser.NavigateTo(const ANavigation: TBrowserNavigation);
begin
  FNativePresentation.NavigateTo(ANavigation);
end;

procedure TEdgeWebBrowser.NavigateToString(const AHTMLContent: string);
begin
  FNativePresentation.NavigateToString(AHTMLContent);
end;

procedure TEdgeWebBrowser.NavigateURL(const AURL: string = '');
begin
  FNativePresentation.NavigateURL(AURL);
end;

function TEdgeWebBrowser.RecommendSize(const AWishedSize: TSizeF): TSizeF;
begin
  Result := AWishedSize;
end;

procedure TEdgeWebBrowser.Refresh;
begin
  FNativePresentation.Refresh;
end;

procedure TEdgeWebBrowser.SetCustomBrowser(const ACustomBrowser: ICustomBrowser);
begin
  FCustomBrowser := ACustomBrowser;
end;

procedure TEdgeWebBrowser.Stop;
begin
  FNativePresentation.Stop;
end;

{ TEdgeWebBrowserPresentation }

constructor TEdgeWebBrowserPresentation.Create(AOwner: TComponent);
const
  CLocalAppData = '%LOCALAPPDATA%'; // Do not localize
  CUserDatafolderSuffix = '.WebView2'; // Do not localize
begin
  inherited;
  FModel := TEdgeWebBrowserModel(inherited Model);
  ...
  FBrowserExecutableFolder := TGlobalEdgeBrowserSettings.BrowserExecutableFolder;
  FUserDataFolder := TGlobalEdgeBrowserSettings.UserDataFolder;
  if FUserDataFolder.IsEmpty then
    FUserDataFolder := TPath.Combine(CLocalAppData, TPath.GetFileName(ParamStr(0) + CUserDatafolderSuffix));
end;

destructor TEdgeWebBrowserPresentation.Destroy;
begin
  FreeAndNil(FDevToolsProtocolEventReceivedTokenMap);
  inherited;
  FreeAndNil(FCritSec);
end;

procedure TEdgeWebBrowserPresentation.CreateHandle;
begin
  inherited;
  CreateWebView;
end;

procedure TEdgeWebBrowserPresentation.DestroyHandle;
begin
  inherited;
  // Not implemented
end;

procedure TEdgeWebBrowserPresentation.DoContentLoading(const ASuccess: Boolean; const ANavigationID: UInt64);
begin
  // Not implemented yet
end;

procedure TEdgeWebBrowserPresentation.DoCreateWebViewCompleted(const ASuccess: Boolean);
begin
  Model.DoCreateWebViewCompleted(ASuccess);
end;

procedure TEdgeWebBrowserPresentation.DoJavascriptEvaluated(const AErrorCode: LongInt; const AResultObjectAsJSON: string);
begin
  Model.DoJavascriptEvaluated(AErrorCode, AResultObjectAsJSON);
end;

procedure TEdgeWebBrowserPresentation.DoNavigationCompleted(const ASuccess: Boolean);
begin
  Model.DoNavigationCompleted(ASuccess);
end;

procedure TEdgeWebBrowserPresentation.DoNavigationStarting(const AURL: string);
begin
  Model.DoNavigationStarting(AURL);
end;

procedure TEdgeWebBrowserPresentation.DoProcessFailed(const AFailureType: COREWEBVIEW2_PROCESS_FAILED_KIND);
begin
  // Not implemented yet
end;

procedure TEdgeWebBrowserPresentation.EvaluateJavascript(const AJavascript: string);
begin
...
function TEdgeWebBrowserPresentation.CreateCoreWebView2ControllerCompleted(AResult: HResult; const ACreatedController: ICoreWebView2Controller): HResult;
begin
  Result := S_OK;
          ...
          FWebViewController.add_AcceleratorKeyPressed(
            Callback<ICoreWebView2, ICoreWebView2AcceleratorKeyPressedEventArgs>.CreateAs<ICoreWebView2AcceleratorKeyPressedEventHandler>(
              ...
              begin
                ...
                if ProcessHResult(Args.Get_KeyEventKind(Kind)) and ProcessHResult(Args.Get_VirtualKey(LKey)) and
                   ProcessHResult(Args.Get_KeyEventLParam(LParam)) and
                   (Kind in [COREWEBVIEW2_KEY_EVENT_KIND_SYSTEM_KEY_DOWN, COREWEBVIEW2_KEY_EVENT_KIND_SYSTEM_KEY_UP]) then
                begin
                  var KeyCode: Word := LKey;
                  var KeyChar: WideChar := Chr(KeyCode);
                  var ShiftState := KeyDataToShiftState(LParam);
                  if Form.DispatchAcceleratorKey(KeyCode, KeyChar, ShiftState) or Form.DispatchDialogKey(KeyCode, KeyChar, ShiftState) then
...
procedure TEdgeWebBrowserPresentation.SelectNextControl(const AForwards: Boolean);
var
  LTabList: ITabList;
  LParent: TControl;
...
function TEdgeWebBrowserPresentation.CreateEnvironmentCompleted(AResult: HResult; const AEnvironment: ICoreWebView2Environment): HResult;
begin
  if ProcessHResult(AResult) then
  begin
...
procedure TEdgeWebBrowserPresentation.CreateWebView;
begin
...
function TEdgeWebBrowserPresentation.CanGoBack: Boolean;
begin
  Result := False;
  if FWebView <> nil then
  begin
...
function TEdgeWebBrowserPresentation.CanGoForward: Boolean;
begin
  Result := False;
  if FWebView <> nil then
  begin
...
function TEdgeWebBrowserPresentation.CaptureBitmap: TBitmap;
var
...
procedure TEdgeWebBrowserPresentation.CloseWebView;
begin
  if FWebView <> nil then
  begin
    try
...
procedure TEdgeWebBrowserPresentation.InitializeWebView;
var
...
procedure TEdgeWebBrowserPresentation.Navigate;
var
  URL, NewURL: string;
begin
  URL := FURL;
...
procedure TEdgeWebBrowserPresentation.NavigateTo(const ANavigation: TBrowserNavigation);
begin
  if FWebView <> nil then
  begin
    case ANavigation of
...
procedure TEdgeWebBrowserPresentation.NavigateToString(const AHTMLContent: string);
begin
  if FWebView <> nil then
    ProcessHResult(FWebView.NavigateToString(PChar(AHTMLContent)));
end;

procedure TEdgeWebBrowserPresentation.NavigateURL(const AURL: string = '');
begin
...
function TEdgeWebBrowserPresentation.NavigateWithWebResourceRequest(const ARequest: ICoreWebView2WebResourceRequest): Boolean;
var
...
procedure TEdgeWebBrowserPresentation.PMSetSize(var AMessage: TDispatchMessageWithValue<TSizeF>);
begin
  inherited;
  Resize;
end;

function TEdgeWebBrowserPresentation.ProcessHResult(AHResult: HResult): Boolean;
begin
  FCritSec.Enter;
  try
...
function TEdgeWebBrowserPresentation.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := inherited QueryInterface(IID, Obj);
...
procedure TEdgeWebBrowserPresentation.Refresh;
begin
  if FWebView <> nil then
    FWebView.Reload;
end;

procedure TEdgeWebBrowserPresentation.Resize;
begin
...
procedure TEdgeWebBrowserPresentation.Stop;
begin
  if FWebView <> nil then
    FWebView.Stop;
end;

function TEdgeWebBrowserPresentation.DefineModelClass: TDataModelClass;
begin
  Result := TEdgeWebBrowserModel;
end;
...
var
  LMessage: TMessage;
begin
  if HandleAllocated and (FBrowserEngine.Instance <> nil) and (FBrowserEngine.Instance.MiscStatus and OLEMISC_SIMPLEFRAME = 0) then
  begin
...
function TWindowsBrowserEngine.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := inherited QueryInterface(IID ,Obj);
end;
...
begin
  inherited;
  FWinBrowserControl := TEdgeWebBrowser.Create(nil);
...
initialization
  TPresentationProxyFactory.Current.Register(TEdgeWebBrowser, TControlType.Platform, TWinPresentationProxy<TEdgeWebBrowserPresentation>);
finalization
  TPresentationProxyFactory.Current.Unregister(TEdgeWebBrowser, TControlType.Platform, TWinPresentationProxy<TEdgeWebBrowserPresentation>);
end.
