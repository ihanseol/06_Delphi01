...
uses
  ...
  FMX.Ani, FMX.Presentation.Style, FMX.Presentation.Style.Common;
...
begin
  Result := not AObject.Equals(FMasterContent) and not AObject.Equals(ResourceLink) and not (AObject is TAnimation)
            ...
            and not (AObject is TNativeScene);
end;
...
begin
  ...
  FreeAndNil(FShadowOptions);
  FreeAndNil(FDrawerOptions);
  FreeAndNil(FPopoverOptions);
  FreeAndNil(FSplitViewOptions);
  FreeAndNil(FNavigationPaneOptions);
  inherited;
end;
