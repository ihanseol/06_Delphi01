...
begin
  ...
  FEndAngle := DefaultEndAngle;
end;

function TArc.IsStartAngleStored: Boolean;
begin
  Result := not SameValue(FStartAngle, DefaultStartAngle, Single.Epsilon);
end;

function TArc.IsEndAngleStored: Boolean;
begin
  Result := not SameValue(FEndAngle, DefaultEndAngle, Single.Epsilon);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      Canvas.MeasureText(R, Text, False, FillTextFlags, TTextAlign.Leading, TTextAlign.Leading);
      TextRect := LocalRect;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Stroke.Kind <> TBrushKind.None) and (FWrapMode in [TPathWrapMode.Original, TPathWrapMode.Stretch]) then
...
begin
  ...
  if FindStyleResource('Background', Background) then
  begin
    FBackgroundRect.TopLeft := Background.ConvertLocalPointTo(Self, Background.LocalRect.TopLeft);
    FBackgroundRect.BottomRight := Background.ConvertLocalPointTo(Self, Background.LocalRect.BottomRight);
  end
  else
    FBackgroundRect := LocalRect;
  FStylized := True;
end;
...
begin
  ...
  Height := FGripSize * 2;
  SetAcceptsControls(False);
  FBackgroundRect := TRectF.Create(-FGripSize, -FGripSize, FGripSize, FGripSize);
...
begin
  if FStylized and FAutodetectPointLocation then
  begin
    LBackgroundRect := FBackgroundRect;
    LBackgroundRect.Inflate(TouchTargetExpansion.Left, TouchTargetExpansion.Top,
                            TouchTargetExpansion.Right, TouchTargetExpansion.Bottom);
    Result := LBackgroundRect.Contains(TPointF.Create(X, Y));
  end
  else
  begin
...
procedure TSelectionPoint.MouseMove(Shift: TShiftState; X, Y: Single);
  ...
  function ConvertLocalPointTo(const AControl: TFmxObject; const APoint: TPointF): TPointF;
  var
    ControlScene: IScene;
  begin
    if AControl is TControl then
      Result := inherited ConvertLocalPointTo(TControl(AControl), APoint)
    else if Supports(AControl, IScene, ControlScene) then
    begin
      Result := LocalToScreen(APoint);
      Result := ControlScene.ScreenToLocal(Result);
    end;
  end;

  function GetParentSize: TSizeF;
  begin
    if ParentControl <> nil then
      Result := ParentControl.Size.Size
    else if Parent is TCommonCustomForm then
    begin
      Result.Width := TCommonCustomForm(Parent).Width;
      Result.Height := TCommonCustomForm(Parent).Height;
    end
    ...
    begin
      ...
      Result := TSizeF.Create(Single.MaxValue, Single.MaxValue);
  end;
...
begin
  ...
  if FPressed then
  begin
    P := ConvertLocalPointTo(Parent, TPointF.Create(X, Y));
    P.Offset(FGripCenter.Point);
    if ParentBounds then
    begin
      ParentSize := GetParentSize;
      P.X := EnsureRange(P.X, 0, ParentSize.Width);
      P.Y := EnsureRange(P.Y, 0, ParentSize.Height);
    end;
...
begin
  FBackgroundRect := GetBackgroundRectForNonStyle;
  inherited FreeStyle;
  FStylized := False;
end;

function TSelectionPoint.GetBackgroundRectForNonStyle: TRectF;
begin
  Result := TRectF.Create(-FGripSize, -FGripSize, FGripSize, FGripSize);
end;

procedure TSelectionPoint.DoGesture(const EventInfo: TGestureEventInfo; var Handled: Boolean);
begin
  inherited;
...
begin
  ...
  if FStylized and (ResourceControl <> nil) then
  begin
    if AutodetectPointLocation then
      Result.Inflate(-FBackgroundRect.Left, -FBackgroundRect.Top,
                      FBackgroundRect.Right, FBackgroundRect.Bottom)
    else
...
begin
  ...
  begin
    ...
    LRect := LBounds[0];
    LRect.Intersect(ALayout.TextRect);
