...
uses
  ...
  FMX.StdCtrls, FMX.Layouts, FMX.Edit, FMX.Consts, FMX.TextLayout, FMX.Controls;
...
var
  ...
  function CreateButton(const AParent: TCOntrol; const AText: string; const AModalResult: TModalResult): TButton;
  begin
    Result := TButton.Create(Form);
    Result.Width := ButtonWidth;
    Result.Margins.Left := BorderSize.X;
    Result.Parent := AParent;
    Result.Position.X := AParent.Width;
...
begin
  ...
  try
    ...
    Button := CreateButton(Layout, LocalizedButtonCaption(TMsgDlgBtn.mbOK), mrOk);
    Button.Default := True;
    CreateButton(Layout, LocalizedButtonCaption(TMsgDlgBtn.mbCancel), mrCancel);
