...
uses
  ...
  FMX.Controls.Presentation, FMX.ScrollBox, FMX.Controls, FMX.Types, FMX.Text, FMX.Text.UndoManager;
...
uses
  ...
  FMX.Platform, FMX.Consts, FMX.Clipboard, FMX.Memo.Style.New
...
type
  ...
  private
    [Weak] FModel: TCustomMemoModel;
    FTextLengthWithoutLineBreaks: Integer;
    FTextLength: Integer;
  ...
  public
    property TextLengthWithoutLineBreaks: Integer read FTextLengthWithoutLineBreaks;
...
function TCustomMemoModel.GetText: string;
begin
  Result := FLines.Text;
end;
...
begin
  ...
  if Insertion = Lines.LineBreak then
    TextFragment := Lines.LineBreak
  else
    TextFragment := Insertion;
  ...
  if TInsertOption.CanUndo in Options then
    SendMessage<TFragmentInserted>(PM_MEMO_UNDO_MANAGER_INSERT_TEXT, TFragmentInserted.Create(PosToTextPos(LPosition),
      TextFragment, TInsertOption.UndoPairedWithPrev
...
begin
  ...
  begin
    if FLines is TMemoLines then
      FSelLength := EnsureRange(Value, 0, TMemoLines(Lines).TextLength)
    else
...
begin
  if FLines is TMemoLines then
    FSelStart := EnsureRange(Value, 0, TMemoLines(Lines).TextLength)
  else
...
function TCustomMemo.IsAddToContent(const AObject: TFmxObject): Boolean;
begin
  Result := inherited and not (AObject.TagString = 'Selector');
end;
...
procedure TCustomMemo.PasteFromClipboard;
  ...
  begin
    ...
    if Insertion = Model.Lines.LineBreak then
      TextFragment := Model.Lines.LineBreak
    else
      TextFragment := Insertion;

    Model.SendMessage<TFragmentInserted>(PM_MEMO_UNDO_MANAGER_INSERT_TEXT,
      TFragmentInserted.Create(Model.PosToTextPos(ACaret), TextFragment, False, False));
...
begin
  FTextChanged := FTextLengthWithoutLineBreaks > 0;
  inherited;
  FTextLengthWithoutLineBreaks := 0;
  FTextLength := 0;
...
begin
  Dec(FTextLengthWithoutLineBreaks, Strings[Index].Length);
  Dec(FTextLength, Strings[Index].Length - LineBreak.Length);
  inherited;
...
begin
  ...
  begin
    LAvailableLength := FModel.MaxLength - FTextLengthWithoutLineBreaks;
  ...
  Inc(FTextLengthWithoutLineBreaks, LText.Length);
  Inc(FTextLength, LText.Length + LineBreak.Length);
...
begin
  ...
  begin
    ...
    FTextLengthWithoutLineBreaks := FTextLengthWithoutLineBreaks - LineLength + LText.Length;
