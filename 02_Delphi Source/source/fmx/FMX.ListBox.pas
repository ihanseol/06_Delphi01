...
{ TListBoxItem }
  ...
  protected
    ...
    property VisualIndex: Integer read FVisualIndex;
  public
...
{ TCustomListBox }
  ...
  strict private
    ...
    FInvalidVisualIndiciesFrom: Integer;
  ...
  protected
    ...
    procedure NeedInvalidateVisualIndiciesFrom(const AFrom: Integer);
    procedure RecalculateVisualIndiciesFrom(const AFrom: Integer);
    /// <summary> Should be called when you change an instance or reference to instance of <b>TBaseImageList</b> or the
    /// <b>ImageIndex</b> property
    /// <para>See also <b>FMX.ActnList.IGlyph</b></para></summary>
    procedure ImagesChanged; virtual;
...
uses
  ...
  FMX.Forms, FMX.Utils, FMX.Platform, FMX.Platform.Metrics;
...
begin
  ...
  begin
    ...
    // It's important to set handler after changing IsChecked property, because when we change IsChecked,
    // it forces a invoking OnChange event.
...
begin
  ...
  begin
    ...
    LListBox.NeedInvalidateVisualIndiciesFrom(Index);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Item := ItemByIndex(i);
        if Item.Visible and Odd(Item.VisualIndex div Columns) then
        begin
...
begin
  ...
  if (FHeaderCompartment.TagObject is TLayout) and (TLayout(FHeaderCompartment.TagObject).Owner = Self) then
    FHeaderCompartment.TagObject.Free;
  ...
  if (FFooterCompartment.TagObject is TLayout) and (TLayout(FFooterCompartment.TagObject).Owner = Self) then
    FFooterCompartment.TagObject.Free;
...
procedure TCustomListBox.RecalculateVisualIndiciesFrom(const AFrom: Integer);
var
  I: Integer;
  NextVisualIndex: Integer;
  Item: TListBoxItem;
begin
  if (1 <= AFrom ) and (AFrom < Count) then
    NextVisualIndex := ListItems[AFrom - 1].VisualIndex + 1
  else
    NextVisualIndex := 0;
  ...
  begin
    ...
    begin
      Item.FVisualIndex := NextVisualIndex;
      Inc(NextVisualIndex);
    end;
  end;
  FInvalidVisualIndiciesFrom := -1;
end;
...
begin
  ...
  begin
    ...
    inherited;
    NeedInvalidateVisualIndiciesFrom(AObject.Index);
...
begin
  ...
  begin
    ...
    if not TListBoxItem(AObject).IsInflated then
      FToInflate.Add(TListBoxItem(AObject));
    inherited;
    NeedInvalidateVisualIndiciesFrom(Index);
    if not IsUpdating then
    begin
      UpdateVisibleItems;
      SortItems;
...
begin
  Result := LocalRect;
  if IsUpdating or (ContentLayout = nil) then
    Exit;
...
begin
  ...
  if FInvalidVisualIndiciesFrom >= 0 then
    RecalculateVisualIndiciesFrom(FInvalidVisualIndiciesFrom);
...
procedure TCustomListBox.NeedInvalidateVisualIndiciesFrom(const AFrom: Integer);
begin
  if FInvalidVisualIndiciesFrom = -1 then
    FInvalidVisualIndiciesFrom := AFrom
  else
    FInvalidVisualIndiciesFrom := Min(FInvalidVisualIndiciesFrom, AFrom);
end;
...
begin
  inherited;
  Item := ItemByPoint(X, Y);
  if (Item <> nil) and IsItemVisibleInViewport(Item) and not AniCalculations.Moved then
  begin
...
begin
  inherited;
  if (Parent is TPopup) and TPopup(Parent).IsOpen and (FComboBox <> nil) and not AniCalculations.Moved then
  begin
    LItemIndex := ItemIndex;
...
var
  ...
  DeltaItemIndex: Integer;
begin
  inherited;
  if Handled or FDisableMouseWheel then
    Exit;
  ...
  DeltaItemIndex := IfThen(WheelDelta < 0, 1, -1);
  NewItemIndex := EnsureRange(ItemIndex + DeltaItemIndex, 0, Count - 1);

  if NewItemIndex <> ItemIndex then
  begin
    TLinkObservers.PositionLinkPosChanging(Observers);
...
begin
  ...
  if FListPicker <> nil then
    FListPicker.ListStyleLookup := Value;
end;
...
begin
  ...
  if FListPicker <> nil then
    FListPicker.PreferredPlacement := TPickerPlacement.Create(Value);
end;
...
begin
  ...
  AListPicker.PreferredPlacement := TPickerPlacement.Create(Placement);
...
begin
  ...
  if ListBox.SupportsPlatformService(IFMXPlatformPropertiesService, PropertiesService) then
  begin
    ItemStyle := PropertiesService.GetValue('ListBox.ItemStyle', string.Empty).AsString; // do not localize
    GroupHeaderStyle := PropertiesService.GetValue('GroupHeaderStyle', string.Empty).AsString; // do not localize
    GroupFooterStyle := PropertiesService.GetValue('GroupFooterStyle', string.Empty).AsString; // do not localize
  end;
end;
