...
begin
  ...
  ADisableAlign := True;
  AlignList := TList<IInterface>.Create;
  try
    // Align
...
begin
  ...
  else if Supports(Value, IRoot, R) then
    R.AddObject(Self);
end;
...
var
  Content: IContent;
begin
  if Supports(FParent, IContent, Content) then
...
begin
  FreeAndNil(FUsedRectangles);
  FreeAndNil(FFreeRectangles);
  inherited;
end;
...
procedure RegisterShortCuts;
  ...
  const
    OtherKeys: array [0..3] of TShortCut = (vkInsert, vkDelete, vkReturn, vkEscape);
  var
    I: Integer;
  begin
