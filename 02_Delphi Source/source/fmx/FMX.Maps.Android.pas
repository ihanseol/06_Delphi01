...
type
  ...
  private
    ...
    FCameraIdleListener: TCameraIdleListener;
  ...
  TCameraIdleListener = class(TJavaLocal, JGoogleMap_OnCameraIdleListener)
  private
    [Weak] FMapView: TAndroidMapView;
  public
...
begin
  ...
  if FCameraIdleListener = nil then
  begin
    FCameraIdleListener := TCameraIdleListener.Create(Self);
...
begin
  InvokeOnMap(
    ...
    begin
      ...
      FBearing := CameraPosition.bearing;
      FZoom := CameraPosition.zoom;
      FTilt := CameraPosition.tilt;
      FLocation := TMapCoordinate.Create(Target.latitude, Target.longitude);
    end);
end;
    ...
    begin
      InvokeOnMap(FMapView, procedure (Map: JGoogleMap)
        begin
          ...
          Map.setOnCameraIdleListener(nil);
...
begin
  ...
  if (FCameraIdleListener <> nil) and (Result <> S_OK) then
    Result := FCameraIdleListener.QueryInterface(IID, Obj);
...
var
  ApiAvailability: JGoogleApiAvailability;
  ErrorCode: Integer;
  ErrorDialog: JDialog;
begin
  if FPlayServicesStatus = TServicesStatus.Unchecked then
  begin
    ApiAvailability := TJGoogleApiAvailability.JavaClass.getInstance;
    ErrorCode := ApiAvailability.isGooglePlayServicesAvailable(TAndroidHelper.Context);
    if ErrorCode <> TJConnectionResult.JavaClass.SUCCESS then
    begin
      FPlayServicesStatus := TServicesStatus.Error;
      try
        ErrorDialog := ApiAvailability.getErrorDialog(TAndroidHelper.Activity, ErrorCode, PLAY_SERVICES_RESOLUTION_REQUEST);
        ErrorDialog.show;
      except
        on Exception do
        begin
...
begin
  InvokeOnMap(procedure (Map: JGoogleMap)
    begin
      ...
      Map.setOnCameraIdleListener(FCameraIdleListener);
      Map.setOnMarkerClickListener(FMarkerClickListener);
      Map.setOnMarkerDragListener(FMarkerDragListener);
...
constructor TCameraIdleListener.Create(MapView: TAndroidMapView);
begin
  inherited Create;
  FMapView := MapView;
end;

procedure TCameraIdleListener.onCameraIdle;
var
  CameraPosition: JCameraPosition;
  Target: JLatLng;
begin
  CameraPosition := FMapView.FGoogleMap.getCameraPosition;
  Target := CameraPosition.target;
  FMapView.FBearing := CameraPosition.bearing;
  FMapView.FZoom := CameraPosition.zoom;
  FMapView.FTilt := CameraPosition.tilt;
  FMapView.FLocation := TMapCoordinate.Create(Target.latitude, Target.longitude);
