...
type
  ...
  private
    ...
    { Handlers }
    procedure CustomEditorExitHandler(Sender: TObject);
  protected
...
begin
  ClearCellClicks;
  FreeAndNil(FEditorLineFill);
  FreeAndNil(FEditorBackground);
  FreeAndNil(FOddFill);
...
procedure TStyledGrid.CustomEditorExitHandler(Sender: TObject);
begin
  if EditorMode and (FInplaceEdit <> nil) and (FInplaceEdit.TagObject is TColumn) then
    TColumn(FInplaceEdit.TagObject).Model.HideEditor(True);
end;
...
var
  ...
  LSize: TSizeF;
begin
  if [TGridPart.Content] * FUpdating = [] then
  begin
...
begin
  if not InRange(Index, 0, HeaderItemCount - 1) then
    ListIndexError(Index, HeaderItemCount - 1, Self);
...
procedure TStyledGrid.UpdateHeader;
  ...
  begin
    ...
    LItem.StyleLookup := LItem.Column.HeaderSettings.StyleLookup;
    LItem.TextSettings := LItem.Column.HeaderSettings.TextSettings;
    LItem.StyledSettings := LItem.Column.HeaderSettings.StyledSettings;
  ...
  begin
    ...
    try
      ...
      try
        ...
        begin
          LCustomEditor.OnExit := CustomEditorExitHandler;
          LCustomEditor.DisableDisappear := True;
          LCustomEditor.TagObject := Columns[Model.Col];
    ...
    try
      ...
      try
        ...
        begin
          LControl := nil;
          if FInplaceEdit is TControl then
            TControl(FInplaceEdit).OnExit := nil;
