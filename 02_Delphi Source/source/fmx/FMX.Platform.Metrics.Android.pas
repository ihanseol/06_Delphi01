...
uses
  System.Types, System.Rtti, FMX.Graphics, FMX.Platform, FMX.Platform.Metrics;

type
  /// <summary>This class represents all interfaces for getting metrics</summary>
  TAndroidMetricsServices = class(TDefaultPlatformPropertiesService, IFMXDefaultMetricsService, IFMXDefaultPropertyValueService,
                                  IFMXSystemInformationService, IFMXSystemFontService, IFMXLocaleService,
                                  IFMXListingService, IFMXPlatformPropertiesService)
  public const
...
uses
  System.SysUtils, Androidapi.JNI.JavaTypes, Androidapi.Helpers, FMX.Pickers, FMX.Text.TextEditor;
...
begin
  ...
  { Platform values }
  SetValue('DropDownKind', TValue.From<TDropDownKind>(TDropDownKind.Native));
  SetValue('NeedSetFocusAfterButtonClick', False);
  SetValue('SpinBox.CanFocusOnPlusMinus', False);
  SetValue('TextEditor.DisableCaretInsideWords', False);
  SetValue('TextEditor.ShouldDrawLeftAndRightSelectionSides', False);
  SetValue('TextEditor.ShouldPlaceLeftSelectionPointBelow', True);
  SetValue('TextEditor.UseLongTapForWordSelection', True);
  SetValue('IME.DisplayMode', TValue.From<TIMEDisplayMode>(TIMEDisplayMode.InlineText));
end;
...
begin
  ...
  if not TPlatformServices.Current.SupportsPlatformService(IFMXPlatformPropertiesService) then
    TPlatformServices.Current.AddPlatformService(IFMXPlatformPropertiesService, Self);
end;
