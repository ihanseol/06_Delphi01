...
procedure TWinWindowBorder.MouseLeaveCheckHandler(Sender: TObject);
var
  CursorPixel: TPoint;
begin
  GetCursorPos(&CursorPixel);
  if Form.IsHandleAllocated and (WindowFromPoint(CursorPixel) <> Wnd) then
  begin
...
var
  ...
  LMessage: TMessage;
begin
  // As soon as a window uses region, then it stops to receive WMNCMouseLeave messages, even if we are using TrackMousEvent.
  // So we are emulating WMNCMouseLeave message via timer.
