...
begin
  ...
  if AParent is TCommonCustomForm then
    LForm := TCommonCustomForm(AParent)
  else
  begin
    if (AParent <> nil) and (AParent.Root <> nil) then
    begin
      LComponent := AParent.Root.GetObject;
      if LComponent is TCommonCustomForm then
        LForm := TCommonCustomForm(LComponent);
    end;
  end;
  if LForm <> nil then
...
function WindowsStyleSelection(const APlatform: TOSPlatform): string;
  ...
  begin
    Result := TOSVersion.Check(10, 0) and (TOSVersion.Build >= 22000);
  end;
