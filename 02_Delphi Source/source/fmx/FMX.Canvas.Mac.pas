...
procedure SetupImageSaveParams(const AImgDestRef: CGImageDestinationRef; const AImgRef: CGImageRef; const ASaveParams: PBitmapCodecSaveParams);
var
  LCount: Integer;
...
begin
  ...
  begin
    FloatVal := ASaveParams.Quality / 100;
    Keys[LCount] := CFSTR('kCGImageDestinationLossyCompressionQuality');
    Value[LCount] := CFNumberCreate(nil, kCFNumberFloat32Type, @FloatVal);
    Inc(LCount);
  end;

  // The background color to use when the image has an alpha
  // component, but the destination format doesn’t support alpha.
  Keys[LCount] := CFSTR('kCGImageDestinationBackgroundColor');
  Value[LCount] := CGColorCreateGenericRGB(0, 0, 0, 1);
  Inc(LCount);

  if LCount > 0 then
  begin
    Dict := CFDictionaryCreate(nil, @Keys[0], @Value[0], LCount, nil, nil);
    CGImageDestinationSetProperties(AImgDestRef, Dict);
  end
  else
    Dict := nil;

  CGImageDestinationAddImage(AImgDestRef, AImgRef, Dict);
  CGImageDestinationFinalize(AImgDestRef);

  if Dict <> nil then
  begin
    CFRelease(Dict);
    for var LValue in Value do
    begin
      if LValue <> nil then
        CFRelease(LValue);
    end;
  ...
  try
    ...
    try
      ...
      try
        ...
        if ImgRef <> nil then
        try
          SetupImageSaveParams(ImgDestRef, ImgRef, SaveParams);
          Result := True;
        finally
          CGImageRelease(ImgRef);
        end;
  ...
  begin
    ...
    try
      ...
      try
        ...
        DoClearRect(TRectF.Create(0, 0, FWidth, FHeight), Color);
      finally
        RestoreState(State);
      end;
    finally
...
begin
  // The macOS Sonoma changed work with backingScaleFactor, so now Context automatically applies backingScaleFactor scale
  // and doesn't let to extract it from Context. So now we don't need to apply scale again.
  if TOSVersion.Check(14) and (Bitmap = nil) then
...
begin
  ...
  // The macOS Sonoma changed work with backingScaleFactor, so now Context automatically applies backingScaleFactor scale
  // and doesn't let to extract it from Context. So now we don't need to apply scale again.
  if (Bitmap = nil) and not TOSVersion.Check(14) then
