...
begin
  if FNeedChange then
  begin
    Converted := TryStrToDate(Model.Text, TempDateTime, FormatSettings);
    ...
    try
      if Converted then
        ...
        Model.Text := Model.GetFormattedDateTimeText;
    finally
      Model.EnableNotify;
    end;
    Repaint;
...
begin
  ...
  OldText := Model.Text;
  Model.Text := Model.GetFormattedDateTimeText;
  CaretPosition := Model.Text.Length;

  Change;

  { Live Binding }
  if Observers.IsObserving(TObserverMapping.EditLinkID) then
    if (OldText <> Model.Text) and TLinkObservers.EditLinkIsEditing(Observers) then
    begin
