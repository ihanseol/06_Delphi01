...
uses
  System.Types, System.Classes, System.UITypes, System.Character, System.Math, FMX.Types, FMX.Graphics, FMX.TextLayout;
...
function TCaretPosition.ToString: string;
begin
  Result := Format('TCaretPosition{Line=%d, Pos=%d}', [Line, Pos]);
end;
...
type
  ...
  private type
    TLexemeType = (Word, OneLetterWord, HalfFullWidth, CJKSymbolsAndPunctuation, Punctuation, None);
...
function TIMETextLineSourceProxy.CanEmbedIMEText: Boolean;
begin
  Result := FNeedEmbedIMEText and not FMarkedText.IsEmpty;
end;

constructor TIMETextLineSourceProxy.Create(const AOriginalLinseSource: ITextLinesSource);
begin
  FOriginalLinesSource := AOriginalLinseSource;
end;

function TIMETextLineSourceProxy.GetCount: Integer;
begin
  Result := FOriginalLinesSource.GetCount;
  // We are emulating single Marked Text line in case if there are no any lines
  if CanEmbedIMEText and (Result = 0) and not FMarkedText.IsEmpty then
    Result := 1;
end;

function TIMETextLineSourceProxy.GetLine(const ALineIndex: Integer): string;
var
  OriginalLinesCount: Integer;
begin
  OriginalLinesCount := FOriginalLinesSource.GetCount;
  if CanEmbedIMEText and (OriginalLinesCount = 0) and not FMarkedText.IsEmpty then
    Result := MarkedText
  else
  begin
    Result := FOriginalLinesSource.GetLine(ALineIndex);
    // IME
    if CanEmbedIMEText and (ALineIndex = FMarkedTextPosition.Line) then
      Result := Result.Insert(FMarkedTextPosition.Pos, FMarkedText);
  end;
end;

function TIMETextLineSourceProxy.GetLineBreak: string;
begin
  Result := FOriginalLinesSource.GetLineBreak;
end;

function TIMETextLineSourceProxy.GetOriginalCount: Integer;
begin
  Result := FOriginalLinesSource.GetCount;
end;

function TIMETextLineSourceProxy.GetOriginalLine(const ALineIndex: Integer): string;
begin
  Result := FOriginalLinesSource.GetLine(ALineIndex);
end;

function TIMETextLineSourceProxy.GetText: string;
var
  MarkedTextPos: Integer;
begin
  Result := FOriginalLinesSource.GetText;
  // IME
  if CanEmbedIMEText then
  begin
    MarkedTextPos := PosToTextPos(FMarkedTextPosition);
    Result := Result.Insert(MarkedTextPos, FMarkedText);
  end;
end;

function TIMETextLineSourceProxy.PosToTextPos(const APostion: TCaretPosition): Integer;
var
  EndMarkedTextPosition: TCaretPosition;
begin
  Result := FOriginalLinesSource.PosToTextPos(APostion);
  // IME
  if CanEmbedIMEText then
  begin
    EndMarkedTextPosition := TCaretPosition.Create(FMarkedTextPosition.Line, FMarkedTextPosition.Pos + FMarkedText.Length);
    if APostion > EndMarkedTextPosition then
      Result := Result + FMarkedText.Length;
  end;
end;

procedure TIMETextLineSourceProxy.SetMarkedTextPosition(const Value: TCaretPosition);
begin
  FMarkedTextPosition := Value;
end;

procedure TIMETextLineSourceProxy.SetNeedEmbedIMEText(const Value: Boolean);
begin
  if FNeedEmbedIMEText <> Value then
  begin
    FNeedEmbedIMEText := Value;
    if not FNeedEmbedIMEText then
    begin
...
procedure TIMETextLineSourceProxy.SetMarkedText(const Value: string);
begin
  FMarkedText := Value;
end;

function TIMETextLineSourceProxy.TextPosToPos(const APos: Integer): TCaretPosition;
begin
  Result := FOriginalLinesSource.TextPosToPos(APos);
  // IME
  if CanEmbedIMEText and (Result.Line = FMarkedTextPosition.Line) then
    Result.Pos := Result.Pos + FMarkedText.Length;
end;
