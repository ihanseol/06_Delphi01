...
const
{$IF defined(IOS)}
  MinimumSupportedFeatureSet = MTLFeatureSet_iOS_GPUFamily1_v1;
{$ELSE}
  MinimumSupportedFeatureSet = MTLFeatureSet_macOS_GPUFamily1_v1;
{$ENDIF}
...
type
  ...
  public
    ...
    class function IsMetalSupported(const AFamily: MTLFeatureSet = MinimumSupportedFeatureSet): Boolean; virtual;
  end;
...
class function TCustomContextMetal.IsMetalSupported(const AFamily: MTLFeatureSet): Boolean;
{$IF defined(MACOS64)}
var
  LDevice: MTLDevice;
{$ENDIF}
  ...
  {$ELSE}
    ...
    try
      Result := (LDevice <> nil) and
                (LDevice.supportsFeatureSet(AFamily));
    finally
      if LDevice <> nil then
      begin
        LDevice.release;
...
class function TContextMetal.CreatePipelineState(const AConfiguration: TPipelineStateConfiguration):
  ...
  begin
    ...
    try
      ...
      FreeAndNil(LMetalVertexDeclaration);
    end;
  end;
...
begin
  ...
  {$ELSE}
    Result := TMacWindowHandle(Parent).CurrentMetalDrawable;
  {$ENDIF}
  end
  else
    Result := nil;
...
{$IF DEFINED(IOS)}
  // macOS presents current drawable in TMacMetalView, So we don't need to use CommandBuffer.
