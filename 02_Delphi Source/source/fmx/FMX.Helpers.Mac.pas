...
function NativeViewToSurface(const ANativeView: NSView; const ASurface: TBitmapSurface; const AScale: Single = 1.0): Boolean;
...
function NativeViewToSurface(const ANativeView: NSView; const ASurface: TBitmapSurface; const AScale: Single): Boolean;
var
  Size: NSSize;
...
begin
  ...
  try
    ...
    ContRef := CGBitmapContextCreate(ASurface.Bits, ASurface.Width, ASurface.Height, 8, 4 * ASurface.Width, ColorSpace,
      kCGImageAlphaPremultipliedLast);
    try
      if not SameValue(AScale, 1) then
      begin
        LScaleMatrix := CGAffineTransformMakeScale(AScale, AScale);
        CGContextConcatCTM(ContRef, LScaleMatrix);
      end;
