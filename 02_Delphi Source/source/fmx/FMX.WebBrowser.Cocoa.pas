...
begin
  ...
  try
    ...
    if TNativeWebViewHelper.NativeViewToSurface(FWebView, Surface, Scale) then
    begin
...
begin
  if SameText(AboutBlank, URL) then
    NormalizedUri := URL
  else
  begin
  ...
  if NSNewURL = nil then
    NSNewURL := TNSUrl.Wrap(TNSUrl.OCClass.URLWithString(StrToNSStr(NormalizedUri)));
