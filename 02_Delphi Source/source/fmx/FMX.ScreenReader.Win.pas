...
type
  ...
  public
    FFmxForm: TAccForm;
    constructor Create(const AForm: TAccForm);
  ...
  // Main Acc message
    WM_GETOBJECT:
      begin
        ...
        begin
          LAccForm := TFormAccessibility.Create(LFmxAccForm);
...
var
  ...
  FormPoint: TPointF;
begin
  // xLeft, yTop - screen coordinates in px
  VariantInit(pvarChild);
  ScreenDp := PxToDp(TPoint.Create(xLeft, yTop));
  FormPoint := FFmxForm.ScreenToClient(ScreenDp);
  for I := FFmxForm.GetChildControlsCount - 1 downto 0 do
    if Supports(FFmxForm.GetChildControl(I), IControl, Child) and Child.GetVisible then
    begin
      ChildControl := Child.GetObject as TControl;

      if ChildControl.HitTest and ChildControl.PointInObject(FormPoint.X, FormPoint.Y) then
      begin
        TVarData(pvarChild).VType := VT_I4;
        TVarData(pvarChild).VInteger := I + 1;
        Result := S_OK;
        Exit;
      end;
    end;
...
var
  WndBounds: TRect;
begin
  WndBounds := WindowHandleToPlatform(FFmxForm.Handle).WndBounds;
  pxLeft := WndBounds.Left;
  pyTop := WndBounds.Top;
  pcxWidth := WndBounds.Width;
  pcyHeight := WndBounds.Height;
  Result := S_OK;
end;
...
constructor TFormAccessibility.Create(const AForm: TAccForm);
begin
  FFmxForm := AForm;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Obj := FFmxForm.GetChildControl(LChildID - 1);
...
begin
  ...
  begin
    ...
    begin
      ScreenBounds.TopLeft := DpToPx(FParent.ClientToScreen(LControl.AbsoluteRect.TopLeft));
      ScreenBounds.BottomRight := DpToPx(FParent.ClientToScreen(LControl.AbsoluteRect.BottomRight));
      pxLeft := ScreenBounds.Left;
      pyTop := ScreenBounds.Top;
      pcxWidth := ScreenBounds.Width;
      pcyHeight := ScreenBounds.Height;
      Result := S_OK;
    end
    else
      Result := S_FALSE;
...
begin
  ...
  begin
    ppdispParent := TFormAccessibility.Create(TAccForm(FParent));
    Result := S_OK;
  end;
end;
