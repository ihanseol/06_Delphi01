...
begin
  if (Idx < 0) or (Idx >= Count) then
    RangeIndexError(Index, Count - 1, Self);
...
begin
  ...
  if (TreeView <> nil) and (OldSize <> BoundsRect.Size) then
...
begin
  ...
  begin
    UserChanged := FIsChecked <> FCheck.IsChecked;
    FIsChecked := FCheck.IsChecked;
  end;
  LTreeView := TreeView;
  if (LTreeView <> nil) and UserChanged then
  begin
    LTreeView.SetFocus;
