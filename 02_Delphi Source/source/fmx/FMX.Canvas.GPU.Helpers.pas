...
type
  TDrawingMode = (Normal, WriteStencilInvert, ReadStencil, ClearStencil, ReadStencilInvert);
...
begin
  ...
  begin
    case DrawingMode of
      ...
      TDrawingMode.ReadStencilInvert:
        begin
          FContext.SetContextState(TContextState.csStencilOn);
          FContext.SetStencilFunc(TStencilFunc.Equal, 0, $FF);
          FContext.SetStencilOp(TStencilOp.Keep, TStencilOp.Keep, TStencilOp.Keep);
...
begin
  ...
  else if FCurrentMaterial <> FSolidMat then
  begin
    SetLength(SolidDecl, 3);
    SolidDecl[0].Format := TVertexFormat.Vertex;
    SolidDecl[0].Offset := 0;
