...
uses
  System.Types, System.UITypes, System.Classes, FMX.Presentation.Style, FMX.ComboEdit, FMX.Controls, FMX.Edit.Style.New,
...
uses
  ...
  FMX.Presentation.Factory, FMX.Forms, FMX.Text, FMX.Consts, FMX.Platform.Metrics;
...
begin
  ...
  if InRange(Model.ItemIndex, 0, Model.Count - 1) then
    Model.Text := Model.Items[Model.ItemIndex];
end;
...
begin
  inherited;
  if (Model.Count = 0) or Handled or Model.ReadOnly then
    Exit;
  if Observers.IsObserving(TObserverMapping.EditLinkID) then
    if TLinkObservers.EditLinkIsReadOnly(Observers) then
      Exit
  ...
  NewItemIndex := EnsureRange(Model.ItemIndex + DeltaItemIndex, 0, Model.Count - 1);

  if NewItemIndex <> Model.ItemIndex then
  begin
    TLinkObservers.PositionLinkPosChanging(Observers);
...
type
  TFixedStyledComboEdit= class(TStyledComboEdit)
  private
...
{ TFixedStyledComboEdit }

procedure TFixedStyledComboEdit.RecalculateAbsoluteMatrices;
begin
  inherited;
...
initialization
  TPresentationProxyFactory.Current.Register(TComboEdit, TControlType.Styled, TStyledPresentationProxy<TFixedStyledComboEdit>);
finalization
  TPresentationProxyFactory.Current.Unregister(TComboEdit, TControlType.Styled, TStyledPresentationProxy<TFixedStyledComboEdit>);
end.
