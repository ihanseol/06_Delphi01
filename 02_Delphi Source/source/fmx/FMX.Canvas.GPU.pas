...
begin
  ...
  begin
    ...
    FDrawingMode := LSource.FCanvasHelper.DrawingMode;
  end;
end;
...
begin
  ...
  begin
    ...
    LDest.FCanvasHelper.DrawingMode := FDrawingMode;
  end;
...
{$IFDEF MSWINDOWS}
  ...
  begin
    var WndClientSize := ParentHandle.WndClientSize; // px
    ParentHandle.CreateBuffer(WndClientSize.Width, WndClientSize.Height);
  end;
{$ENDIF}
end;
...
{$IFDEF MSWINDOWS}
  if (Parent <> nil) and WindowHandleToPlatform(Parent).Transparency then
    WindowHandleToPlatform(Parent).ResizeBuffer(Trunc(Width * Scale), Trunc(Height * Scale));
{$ENDIF}
end;
...
begin
  ...
  begin
    ...
    if (Parent <> nil) and WindowHandleToPlatform(Parent).Transparency then
    begin
      var WidthPx := Trunc(Width * Scale);
      var HeightPx := Trunc(Height * Scale);
      Context.CopyToBits(WindowHandleToPlatform(Parent).BufferBits, WidthPx * 4, Rect(0, 0, WidthPx, Heightpx));
    end;
    {$ENDIF}
  end;
  inherited;
...
begin
  ...
  begin
    ...
    FCanvasHelper.DrawingMode := TDrawingMode.ReadStencilInvert;
  end;
end;
