...
procedure TWinPrinterDevice.PrinterSettingsReadError(const AdditionalMsg: string);
begin
  raise EPrinterSettingsError.CreateFmt(SPrinterSettingsReadError, [AdditionalMsg]);
end;

procedure TWinPrinterDevice.PrinterSettingsWriteError(const AdditionalMsg: string);
begin
...
procedure TPrinterWin.GetPrinter(var ADevice, ADriver, APort: string; var ADeviceMode: THandle);
begin
  ADevice :=  PChar(ActivePrinter.Device);
  ADriver := PChar(ActivePrinter.Driver);
  APort := PChar(ActivePrinter.Port);
  ADeviceMode := FDeviceMode;
end;
...
procedure TPrinterWin.SetPrinter(ADevice, ADriver, APort: string; ADeviceMode: THandle);
var
  I, J: Integer;
begin
  CheckPrinting(False);
  ...
  begin
    ...
    begin
      FDeviceMode := GlobalAlloc(GHND,
        DocumentProperties(0, FPrinterHandle, PChar(ADevice), nil, nil, 0));
      ...
      begin
        ...
        if FDevMode = nil then
          RaiseLastOSError;
        if DocumentProperties(0, FPrinterHandle, PChar(ADevice), FDevMode^, FDevMode^, DM_OUT_BUFFER) < 0 then
        begin
          GlobalUnlock(FDeviceMode);
          GlobalFree(FDeviceMode);
          FDeviceMode := 0;
