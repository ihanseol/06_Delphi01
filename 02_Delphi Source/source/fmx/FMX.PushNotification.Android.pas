...
uses
  ...
  FMX.Platform.Android, FMX.Consts, System.Permissions, System.Types, Androidapi.JNI.Util;
...
procedure TFcmPushService.StartService;
  ...
  procedure RequestPostNotificationsPermission;
  begin
    ...
    begin
      PermissionsService.RequestPermissions([JStringToString(TJManifest_permission.JavaClass.POST_NOTIFICATIONS)],
        ...
        begin
          if (Length(GrantResults) < 1) or (GrantResults[0] = TPermissionStatus.Denied) then
            TJutil_Log.JavaClass.w(StringToJString('FcmPushService'), StringToJString('The app has not been allowed to post notifications'));
        end);
    end;
  end;
