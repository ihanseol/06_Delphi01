...
function TAndroidTextInputContextMenu.HasClipboardValue: Boolean;
  ...
  procedure ReleaseIfObject(const AValue: TValue);
  var
    ValueObject: TObject;
  begin
    if not AValue.IsObjectInstance then
      Exit;

    ValueObject := AValue.AsObject;
    ValueObject.Free;
  end;
...
begin
  ...
  begin
    ...
    try
      Result := not ClipboardValue.IsEmpty;
    finally
      ReleaseIfObject(ClipboardValue);
    end;
  end
  else
