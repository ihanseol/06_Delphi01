...
begin
  ...
  begin
    DeviceNames := GlobalAlloc(GHND, SizeOf(TDevNames) + (Length(Device) + Length(Driver) + Length(Port) + 3) *
      SizeOf(Char));
...
function TFMXDialogService.GetHWND: HWND;
  ...
  function TryGetTopMostFormWnd(out AFormHandle: HWND): Boolean;
  var
  ...
  begin
    ...
    begin
      Form := Screen.Forms[I];
      if Form.Visible and Form.IsHandleAllocated then
      begin
        AFormHandle := FormToHWND(Form);
        Exit(True);
      end;
    end;
    Result := False;
...
begin
  ...
  else if not TryGetTopMostFormWnd(Result) then
