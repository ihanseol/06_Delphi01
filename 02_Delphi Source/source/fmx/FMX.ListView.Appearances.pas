...
begin
  ...
  begin
    ...
    try
      ...
      DstAppearance.TextShadow := Self.TextShadow;
  ...
  begin
    ...
    try
      ...
      if TextShadow.Enabled then
      begin
        if Self.TextShadow.Color <> TAlphaColorRec.Null then
          DstDrawable.TextShadowColor := Self.TextShadow.Color;
      end
      else
        DstDrawable.TextShadowColor := TAlphaColorRec.Null;
...
begin
  ...
  FTextShadow := TTextShadowOptions.Create(Self);
  FTextShadow.OnChanged := TextShadowChanged;
  FFont := TFont.Create;
...
function TCustomTextObjectAppearance.IsTextShadowStored: Boolean;
begin
  if DefaultValues is TCustomTextObjectAppearance then
    Result := not Equals(TCustomTextObjectAppearance(DefaultValues).TextShadow)
  else
    Result := True;
end;
...
procedure TCustomTextObjectAppearance.SetTextShadow(const Value: TTextShadowOptions);
begin
  FTextShadow.Assign(Value);
end;
...
procedure TCustomTextObjectAppearance.TextShadowChanged(Sender: TObject);
begin
  DoChange;
end;
...
procedure TAppearanceListViewItems.DoEndUpdate;
begin
  if FIsDelayedChanged then
  begin
    FIsDelayedChanged := False;
    ResetIndexes;
    Changed;
  end;
...
procedure TTextShadowOptions.AssignTo(Dest: TPersistent);
var
  DestOptions: TTextShadowOptions;
begin
  if Dest is TTextShadowOptions then
  begin
    DestOptions := TTextShadowOptions(Dest);
...
procedure TTextShadowOptions.Changed;
begin
  if Assigned(FOnChanged) then
    FOnChanged(Self);
end;

constructor TTextShadowOptions.Create(AOwner: TPersistent);
begin
  inherited Create;
  FOwner := AOwner;
  FColor := DefaultColor;
...
destructor TTextShadowOptions.Destroy;
begin
  FOwner := nil;
  inherited;
end;

function TTextShadowOptions.Equals(Obj: TObject): Boolean;
begin
  if Obj is TTextShadowOptions then
  begin
    var Value := TTextShadowOptions(Obj);
    Result := (Value.Enabled = Enabled) and (Value.Color = Color);
  end
  else
    Result := False;
end;

function TTextShadowOptions.GetOwner: TPersistent;
begin
  Result := FOwner;
end;

procedure TTextShadowOptions.SetColor(const Value: TAlphaColor);
begin
...
procedure TTextShadowOptions.SetEnabled(const Value: Boolean);
begin
  if FEnabled <> Value then
  begin
    FEnabled := Value;
