...
type
  ...
  TTabControlChangingEvent = procedure(Sender: TObject; var AAllowChange: Boolean) of object;
  ...
  private
    ...
    FOnChanging: TTabControlChangingEvent;
  ...
  protected
    ...
    procedure DoChanging(var AAllowChange: Boolean); virtual;
    ...
    procedure SetTabIndexByUser(const ATabIndex: Integer); virtual;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
  ...
  published
    ...
    property OnChanging: TTabControlChangingEvent read FOnChanging write FOnChanging;
    property OnDragEnter;
    property OnDragLeave;
    property OnDragOver;
    property OnDragDrop;
...
begin
  ...
  if SupportsPlatformService(IFMXPlatformPropertiesService, PropertiesService) then
    Trimming := PropertiesService.GetValue('Trimming', TValue.From<TTextTrimming>(TTextTrimming.None)) // do not localize
...
begin
  ...
  try
    try
      // It's important to have non zero size by default. 
      // Because zero size leads to unloading style from TTabItem and it can not calculate its size.
      FInfo.Size := TSizeF.Create(1, 1);
...
begin
  ...
  begin
    ...
    begin
      ...
      if ChangeIndex then
        FTabControl.SetTabIndexByUser(Index);
      inherited;
{$IFDEF MSWINDOWS}
...
begin
  ...
  begin
    if Key = vkLeft then
    begin
      NewTabIndex := FTabControl.FindVisibleTab(TTabControl.TFindKind.Back);
      if NewTabIndex <> -1 then
      begin
        FTabControl.SetTabIndexByUser(NewTabIndex);
        Key := 0;
        TabControl.GoToActiveTab;
      end;
    end
    ...
    begin
      NewTabIndex := FTabControl.FindVisibleTab(TTabControl.TFindKind.Next);
      if NewTabIndex <> -1 then
      begin
        FTabControl.SetTabIndexByUser(NewTabIndex);
        Key := 0;
...
function TTabItem.DoSetSize(const ASize: TControlSize; const NewPlatformDefault: Boolean; ANewWidth, ANewHeight: Single;
...
begin
  ...
  begin
    Result := inherited DoSetSize(ASize, NewPlatformDefault, ASize.Width, ANewHeight, ALastWidth, ALastHeight);
    UpdateLayoutControl;
  end;
end;
...
function TTabItem.GetParentClassStyleLookupName: string;
begin
  Result := GenerateStyleName(TTabItem.ClassName);
end;
...
begin
  ...
  begin
    FTabBarControls[Button].Free;
    FTabBarControls[Button] := nil;
  end;
  FreeAndNil(FAniCalculations);
  FreeAndNil(FStoredTabSize);
  FreeAndNil(FTabBarControlPanel);
  inherited;
end;
    ...
    begin
      ...
      begin
        if TabIndex = FindVisibleTab(TFindKind.First) then
          NewTabIndex := FindVisibleTab(TFindKind.Last)
        else
          NewTabIndex := FindVisibleTab(TFindKind.Back);
      end
      else
      begin
        if TabIndex = FindVisibleTab(TFindKind.Last) then
          NewTabIndex := FindVisibleTab(TFindKind.First)
        else
          NewTabIndex := FindVisibleTab(TFindKind.Next);
      end;
      SetTabIndexByUser(NewTabIndex);
...
procedure TTabControl.DoChanging(var AAllowChange: Boolean);
begin
...
begin
  ...
  FDefaultFullSize := False;
  TabHeight := FSavedTabHeight;
end;
...
procedure TTabControl.SetTabIndexByUser(const ATabIndex: Integer);
var
  AllowChange: Boolean;
begin
  if FTabIndex <> ATabIndex then
  begin
    AllowChange := True;
    DoChanging(AllowChange);
    if AllowChange then
      TabIndex := ATabIndex;
  end;
end;
