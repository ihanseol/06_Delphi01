...
{ TSetProperty
  ...
  protected
    function CBRect(const ItemRect: TRect) : TRect;
    function GetOrdValue: NativeInt; override;
  public
    // ICustomPropertyDrawing
    procedure PropDrawName(ACanvas: TCanvas; const ARect: TRect;
      ASelected: Boolean);
...
var
  NewValue: TColor;
begin
  if TryStringToColor(Value, NewValue) then
    SetOrdValue(NewValue)
  else
...
function TColorProperty.PaintColorBox(const Value: string; ACanvas: TCanvas;
  ...
  begin
    if ASelected then
      Result := GetIDEStyleServices.GetSystemColor(clHighlightText)
    else
      Result := GetIDEStyleServices.GetSystemColor(clWindowText);
  end;

var
  Right: Integer;
begin
  Right := (ARect.Bottom - ARect.Top - TIDEThemeMetrics.AlignFactor) {* 2} + ARect.Left;
  var LCanvasRecall := TCanvasRecall.Create(ACanvas, [creBrush]);
  try
    ACanvas.FillRect(ARect);
    var LRect := TRect.Create(ARect.Left + TIDEThemeMetrics.AlignFactor div 2,
      ARect.Top + TIDEThemeMetrics.AlignFactor div 2,
      Right + TIDEThemeMetrics.AlignFactor div 2,
      ARect.Bottom - TIDEThemeMetrics.AlignFactor div 2);
    ACanvas.Brush.Color := StringToColor(Value);
    ACanvas.FillRect(LRect);
    ACanvas.Brush.Color := ColorToBorderColor(ACanvas.Brush.Color);
    ACanvas.FrameRect(LRect);
  finally
    LCanvasRecall.Free;
    Result := Rect(Right + (TIDEThemeMetrics.AlignFactor * 2), ARect.Top, ARect.Right, ARect.Bottom);
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if Brush.Style = bsClear then
      begin
        Brush.Color := GetIDEStyleServices.GetSystemColor(clWindow);
        Brush.Style := bsSolid;
      end
      else
        Brush.Color := GetIDEStyleServices.GetSystemColor(clWindowText);
      // ok on with the show
...
begin
  var LFormDPI := TIDEHighDPIService.GetFormDPI(nil);
...
procedure TCursorProperty.ListMeasureWidth(const Value: string;
  ACanvas: TCanvas; var AWidth: Integer);
begin
  AWidth := AWidth + TIDEHighDPIService.GetFormDPI(nil).ScaleValue(cCursorSize + TIDEThemeMetrics.AlignFactor);
end;
...
begin
  AHeight := TIDEThemeMetrics.AdjustSize(Max(ACanvas.TextHeight('Wg'),
    TIDEHighDPIService.GetFormDPI(nil).ScaleValue(cCursorSize + TIDEThemeMetrics.AlignFactor)));
end;
...
begin
  ...
  try
    ...
    if FontDialog.Execute then SetOrdValue(IntPtr(FontDialog.Font));
  finally
    FontDialog.Free;
  end;
end;
...
begin
  var LControl: TControl := nil;
  if GetComponent(0) is TControl then
  begin
    LControl := GetComponent(0) as TControl;
    LControl.ControlState := LControl.ControlState + [csReadingProps];
  end;
  try
...
function TSetElementProperty.GetOrdValue: NativeInt;
begin
  var LControl: TControl := nil;
  if GetComponent(0) is TControl then
  begin
