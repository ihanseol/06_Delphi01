...
type
  ...
  TStructureMenuItem = class(TInterfacedObject, IDispatch, ISupportErrorInfo,
    IOTAStructureNodeMenuItem, IOTAStructureNodeMenuItem290)
  private
    FItem: IMenuItem;
  protected
    { IDispatch }
...
begin
{$IFDEF CPUX64}
  L := LongRec(Int64Rec(ASprig.Item).Lo).Hi; // grab xxxxLLxx byte
{$ELSE}
...
begin
  Result := nil;
{$IFDEF CPUX64}
  L := LongRec(Int64Rec(AItem).Lo).Hi; // grab xxxxLLxx byte
{$ELSE}
...
function TSprigStructureNode.get_Data: IntPtr;
begin
  Result := FData;
end;
...
procedure TSprigStructureNode.set_Data(Value: IntPtr);
begin
  FData := Value;
end;
...
function TStructureMenuItem.GetAction: TBasicAction;
var
  MenuItem290: IMenuItem290;
begin
  if Supports(FItem, IMenuItem290, MenuItem290) then
    Result := MenuItem290.Action
  else
    Result := nil;
end;
