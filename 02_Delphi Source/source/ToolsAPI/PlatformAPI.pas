...
uses
  ...
  Vcl.Graphics, Vcl.Forms, Vcl.ImgList;
...
const
  ...
  cWin64xPlatform        = PlatformConst.cWin64xPlatform;
...
type
  ...
  TOTAPutFileFlags = set of TOTAPutFileFlag;
  ...
  IOTAPlatformSDKAndroid210 = interface(IOTAPlatformSDK)
    ...
    /// </summary>
    function GetNDKGdbServerPath: string; deprecated 'not in use anymore';
    /// <summary>
    /// Returns the zipalign.exe tool of the SDK 
    /// </summary>
    function GetSDKZipAlignPath: string; deprecated 'not in use anymore';
    /// <summary>
    ...
    /// </summary>
    function GetSDKAaptPath: string; deprecated 'not in use anymore';
    /// <summary>
    ...
    /// </summary>
    procedure SetNDKGdbServerPath(const Value: string); deprecated 'not in use anymore';
    /// <summary>
    /// Sets the zipalign.exe tool of the SDK 
    /// </summary>
    procedure SetSDKZipAlignPath(const Value: string); deprecated 'not in use anymore';
    /// <summary>
    ...
    /// </summary>
    procedure SetSDKAaptPath(const Value: string); deprecated 'not in use anymore';
    /// <summary>
  ...
  IOTAPlatformSDKAndroid = interface(IOTAPlatformSDKAndroid270)
    ...
    /// </summary>
    function GetSDKApkSignerPath: string; deprecated 'not in use anymore';
    /// <summary>
    /// Sets the apksigner.jar tool of the SDK 
    /// </summary>
    procedure SetSDKApkSignerPath(const Value: string); deprecated 'not in use anymore';
...
function PlatformDisplayTitle(const APlatformName: string): string;
function PlatformIDToName(const APlatformID: TPlatformIds): string;
function NameToPlatformID(const APlatformName: string): TPlatformIds;

var
  PlatformImageList: TCustomImageList = nil;
...
begin
  ...
  else if (BorlandIDEServices as IOTAPlatformServices).PlatformExists(APlatformName) then
    Result := (BorlandIDEServices as IOTAPlatformServices).Platforms[APlatformName].DisplayName + sPlatform
  else
    Result := APlatformName + sPlatform;
end;
...
begin
  ...
  if Assigned(LPlatformServices) then
    for var LPlatform in LPlatformServices.AllPlatforms do
      if SameText(APlatformName, LPlatformServices.Platforms[LPlatform].Name) then
        Exit(LPlatformServices.Platforms[LPlatform].GetId);
end;
...
begin
  ...
  if (BC.Platform <> '') and Abbreviate then
    Result := PlatformDisplayTitle(BC.Platform)
  else
    Result := Format('%s - %s', [Result, PlatformDisplayTitle(BC.Platform)]);
end;
