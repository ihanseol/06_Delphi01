...
uses
  ...
  FMX.Advertising.Firebase, FMX.BiometricAuth,
  {$IFDEF MSWINDOWS}
  FMX.WindowsStore,
  {$ENDIF}
  // Designers
  ...
  Winapi.D2D1, ToolsAPI, ToolsAPI.UI;
...
type
  ...
  TTakePhotoActionSelectionEditor = class(TSelectionEditor)
  public
    procedure RequiresUnits(Proc: TGetStrProc); override;
  end;
...
//  RegisterClass(TMenuItem);
  ...
  RegisterComponents(srAdditional, [TSpeedButton, TCornerButton, TArcDial,
    TAniIndicator, TMediaPlayer, TMediaPlayerControl, TCameraComponent, TBannerAd, TFirebaseBannerAd, TBiometricAuth]);
...
//  RegisterPropertyEditor(TypeInfo(TBounds), TFmxObject, '', TRectAnimationProperty);
  ...
  RegisterSelectionEditor(TCustomTakePhotoAction, TTakePhotoActionSelectionEditor);
  ...
  GroupDescendentsWith(TBiometricAuth, FMX.Controls.TControl);
...
{ TTakePhotoActionSelectionEditor }

procedure TTakePhotoActionSelectionEditor.RequiresUnits(Proc: TGetStrProc);
begin
  inherited;
  Proc('FMX.Graphics');
  Proc('FMX.MediaLibrary');
end;
