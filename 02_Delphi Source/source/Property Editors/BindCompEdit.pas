...
uses
  System.Win.Registry, System.TypInfo, Winapi.CommCtrl, StdConst, DsnConst,
  DesignEditors, VCLEditors, BindCompDrag, BindCompDsnResStrs, BindCompNewStd,
  Vcl.Dialogs, Generics.Collections, BindExpertsUIUnit, System.UITypes, BrandingAPI,
  IDETheme.Utils, ToolsAPI, ToolsAPI.UI;
...
{  var
  ...
  begin
    ...
    try
      Width := ScaleValue(ReadInteger(FDataBindingListClassName, 'Width', TIDEHighDPIService.ConvertToDefaultPPI(Self, Width)));
      Height := ScaleValue(ReadInteger(FDataBindingListClassName, 'Height', TIDEHighDPIService.ConvertToDefaultPPI(Self, Height)));
      ...
      for LIndex := 0 to ListView1.Columns.Count - 1 do
        with ListView1.Columns.Items[LIndex] do
          Width := ScaleValue(ReadInteger(FDataBindingListClassName, 'Width' + IntToStr(LIndex), TIDEHighDPIService.ConvertToDefaultPPI(Self, Width)));
{      Kind := ReadInteger(FActionListClassName, 'CategoryKind', Ord(ckNone));
      ...
      CategoryPanel.Width := ScaleValue(ReadInteger(FDataBindingListClassName, 'CategoryWidth', TIDEHighDPIService.ConvertToDefaultPPI(Self, CategoryPanel.Width)));
...
begin
  ...
  try
    ...
    WriteInteger(FDataBindingListClassName, 'Width', TIDEHighDPIService.ConvertToDefaultPPI(Self, Width));
    WriteInteger(FDataBindingListClassName, 'Height', TIDEHighDPIService.ConvertToDefaultPPI(Self, Height));
    for I := 0 to ListView1.Columns.Count - 1 do
      with ListView1.Columns.Items[I] do
        WriteInteger(FDataBindingListClassName, 'Width' + IntToStr(I), TIDEHighDPIService.ConvertToDefaultPPI(Self, Width));
    ...
    WriteInteger(FDataBindingListClassName, 'CategoryWidth', TIDEHighDPIService.ConvertToDefaultPPI(Self, CategoryPanel.Width));
      ...
      try
        ...
        for I := ListView1.Items.Count - 1 downto 0 do
          ...
          begin
            ...
            if (idYes <> (BorlandIDEServices as INTAIDEUIServices).MessageDlg(
...
begin
  ...
  try
    ScaleForPPI(TIDEHighDPIService.GetFormDPI(nil).CurrentPPI);
