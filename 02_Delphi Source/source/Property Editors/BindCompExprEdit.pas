  ...
  begin
    ...
    begin
      try
        ...
        on E: Exception do
        begin
          (BorlandIDEServices as INTAIDEUIServices).MessageDlg(E.Message, TMsgDlgType.mtError, [mbOK], 0);
        end;
      end;
    end;
  end;
...
begin
  ...
  try
    FVisualizer.Width := ScaleValue(ReadInteger(LSubKey, sVisWidth, TIDEHighDPIService.ConvertToDefaultPPI(Self, FVisualizer.Width)));
    FVisualizer.Height := ScaleValue(ReadInteger(LSubKey, sVisHeight, TIDEHighDPIService.ConvertToDefaultPPI(Self, FVisualizer.Height)));
    FVisualizer.PanelVisualizerList.Width := ScaleValue(ReadInteger(LSubKey, sVisListWidth,
      TIDEHighDPIService.ConvertToDefaultPPI(Self, FVisualizer.PanelVisualizerList.Width)));
  finally
    Free;
  end;
end;
...
begin
  ...
  with TRegIniFile.Create(GetRegKey) do
  try
    Width := ScaleValue(ReadInteger(LSubKey, sWidth, TIDEHighDPIService.ConvertToDefaultPPI(Self, Width)));
    Height := ScaleValue(ReadInteger(LSubKey, sHeight, TIDEHighDPIService.ConvertToDefaultPPI(Self, Height)));
    ...
    for LIndex := 0 to ListView1.Columns.Count - 1 do
      with ListView1.Columns.Items[LIndex] do
        Width := ScaleValue(ReadInteger(LSubKey, sWidth + IntToStr(LIndex), TIDEHighDPIService.ConvertToDefaultPPI(Self, Width)));
    PanelCollections.Width := ScaleValue(ReadInteger(LSubKey, sPanelCollectionsWidth, TIDEHighDPIService.ConvertToDefaultPPI(Self, PanelCollections.Width)));
...
begin
  ...
  try
    ...
    WriteInteger(LSubKey, sWidth, TIDEHighDPIService.ConvertToDefaultPPI(Self, Width));
    WriteInteger(LSubKey, sHeight, TIDEHighDPIService.ConvertToDefaultPPI(Self, Height));
    if FVisualizer <> nil then
    begin
      WriteInteger(LSubKey, sVisWidth, TIDEHighDPIService.ConvertToDefaultPPI(Self, FVisualizer.Width));
      WriteInteger(LSubKey, sVisHeight, TIDEHighDPIService.ConvertToDefaultPPI(Self, FVisualizer.Height));
      WriteInteger(LSubKey, sVisListWidth, TIDEHighDPIService.ConvertToDefaultPPI(Self, FVisualizer.PanelVisualizerList.Width));
    end;
    for LIndex := 0 to ListView1.Columns.Count - 1 do
      with ListView1.Columns.Items[LIndex] do
        WriteInteger(LSubKey, sWidth + IntToStr(LIndex), TIDEHighDPIService.ConvertToDefaultPPI(Self, Width));
    if PanelCollections.Visible then
      WriteInteger(LSubKey, sPanelCollectionsWidth, TIDEHighDPIService.ConvertToDefaultPPI(Self, PanelCollections.Width));
...
begin
  ...
  if AConfirmDelete and
    (idYes <> (BorlandIDEServices as INTAIDEUIServices).MessageDlg(
