...
uses
  ...
  StructureViewAPI, Character, BrandingAPI, IDETheme.Utils, ToolsAPI.UI;
...
begin
  ...
  if not Designer.IsComponentLinkable(SourceRoot) then
    if (BorlandIDEServices as INTAIDEUIServices).MessageDlg(Format(SDSLinkForms, [Designer.GetRoot.Name,
...
begin
  Inc(DesignerCount);
  FMinWidth := TIDEHighDPIService.ConvertToDefaultPPI(Self, Width);
  FMinHeight := TIDEHighDPIService.ConvertToDefaultPPI(Self, Height);
...
begin
  CheckFieldDelete;
  if (BorlandIDEServices as INTAIDEUIServices).MessageDlg(SDSConfirmDeleteAll, mtConfirmation, mbOKCancel, 0) <> idCancel then
  begin
    SelectAll;
    RemoveFields(GetActiveListbox);
  end;
