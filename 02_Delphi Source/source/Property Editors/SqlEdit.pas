...
uses Windows, Messages, ActiveX, SysUtils, Forms, Classes, Controls, Graphics,
  StdCtrls, ExtCtrls, ActnList, DesignIntf, ToolsAPI;
{$ENDIF}
{$IFDEF LINUX}
...
function EditSQL(AComponent: TComponent; APropName: string; ADesigner: IDesigner;
                 ASQL: TStrings;
                 AGetTableNames: TGetTableNamesProc;
                 AGetFieldNames: TGetFieldNamesProc;
                 AStartTblName : string = '';
...
function EditSQL(AComponent: TComponent; APropName: string; ADesigner: IDesigner;
  ASQL: TStrings; AGetTableNames: TGetTableNamesProc; AGetFieldNames: TGetFieldNamesProc;
  AStartTblName : string = ''; AQuoteChar : string = ''; ANeedsQuoteCharFunc:
  TRequiresQuoteCharProc = nil ): Boolean; overload;
begin
  Result := mrOk = EditStringsWithCodeEditor(
    AComponent, APropName, ASQL, ADesigner,
    ...
    begin
      AReleaseObject := TSQLEditForm.Create(nil);
      with TSQLEditForm(AReleaseObject) do
      begin
        GetTableNamesForSchema := nil;
        GetTableNames := AGetTableNames;
        GetFieldNamesForSchema := nil;
        GetFieldNames := AGetFieldNames;
        QuoteChar := AQuoteChar;
        StartTable := AStartTblName;
        FSchemaName := '';
        NameRequiresQuoteChar := ANeedsQuoteCharFunc;
        SQLMemo.Lines.Assign(AInStrings);
        SQLMemo.Modified := False;
        CodeWndBtn.Enabled := ACodeEditorSupported;
        Result := ShowModal;
        AOutStrings := SQLMemo.Lines;
        AModified := SQLMemo.Modified;
      end;
    end,
    procedure (AOutStrings: TStrings)
    begin
      ASQL.Assign(AOutStrings);
    end,
    shSQL)
end;
...
procedure TSQLEditForm.SQLMemoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
