...
function IsBindScopeComponent(AComponent: TComponent): Boolean;
function DataSourceComponentOfBindScope(AComponent: TComponent): TComponent;
...
uses 
  ...
  ToolsAPI, ToolsAPI.UI, PropInspApi, BindCompProperties, BindMethodsFormU,
...
begin
  ...
  Result := (BorlandIDEServices as INTAIDEUIServices).MessageDlg(LMessage, mtConfirmation, mbYesNoCancel, 0) = idYes;
end;
...
function IsBindScopeComponent(AComponent: TComponent): Boolean;
begin
  if AComponent is TCustomBindSourceDB then
    Result := True
  else if AComponent is TBaseObjectBindSource then
    Result := True
  else
    Result := False;
end;
...
begin
  if AComponent is TCustomBindSourceDB then
    Result := TCustomBindSourceDB(AComponent).DataComponent
  else if AComponent is TBaseObjectBindSource then
    Result := TBaseObjectBindSource(AComponent).InternalAdapter
  else
    Result := nil;
end;
  ...
  case Stage of
    cdPostPaint:
      begin
        ...
        begin
          Canvas.Pen.Color := TIDEStyleUtilsService.GetGrayBlendColor(LStyle);
...
begin
  ...
  begin
    ...
    if FTempStringList.Count > 0 then
      Height := (Min(FTempStringList.Count, 11) + 3) * TIDEHighDPIService.GetFormDPI(nil).ScaleValue(FDefItemHeight)
    else
      Height := TIDEHighDPIService.GetFormDPI(nil).ScaleValue(FDefItemHeight);
...
begin
  ...
  begin
    ...
    begin
      AValid := False;
      (BorlandIDEServices as INTAIDEUIServices).MessageDlg(sEnterAFieldName, mtError, [mbOK], 0);
    end;
    if FieldExists(LFieldName) then
    begin
      AValid := False;
      (BorlandIDEServices as INTAIDEUIServices).MessageDlg(sDuplicateFieldName, mtError, [mbOK], 0);
    end;
  end;
end;
