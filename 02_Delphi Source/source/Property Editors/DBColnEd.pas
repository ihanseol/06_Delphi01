...
{$IFDEF MSWINDOWS}
uses TypInfo, DBGrids, DsnDBCst, Winapi.CommCtrl, BrandingAPI, ToolsAPI, ToolsAPI.UI;
{$ENDIF}
...
begin
  ...
  if (Collection.Count > 0) and
    ...
    Result := (BorlandIDEServices as INTAIDEUIServices).MessageDlg(sDeleteColumnsQuestion, mtConfirmation, mbYesNoCancel, 0);
...
procedure TDBGridColumnsEditor.FormCreate(Sender: TObject);
begin
  inherited;
  if IDEThemeAvailable then
  begin
    IDEThemeManager.RegisterFormClass(TDBGridColumnsEditor);
