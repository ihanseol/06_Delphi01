...
function TCustomBackendAuth.ExportLogin(const APassword: string): string;
var
  I, J: Integer;
  s: string;
  LLoginBytes: TBytes;
...
begin
  CheckLoggedIn;
  Result := Self.GetAuthApi.ExportLogin(LoggedInValue);
  if (APassword <> '') and (Result <> '') then
  begin
    LPasswordBytes := TEncoding.UTF8.GetBytes(APassword);
    LLoginBytes := TEncoding.UTF8.GetBytes(Result);
    J := Low(LPasswordBytes);
    for I := Low(LLoginBytes) to High(LLoginBytes) do
    begin
      LLoginBytes[I] := LLoginBytes[I] xor LPasswordBytes[J];
      Inc(J);
      if J > High(LPasswordBytes) then
        J := Low(LPasswordBytes);
    end;
    SetLength(s, Length(LLoginBytes) * 2);
    BinToHex(@LLoginBytes[0], PChar(s), Length(LLoginBytes));
    Result := s;
  end;
end;

procedure TCustomBackendAuth.ImportLogin(const ALogin, APassword: string);
var
  I, J: Integer;
  s: string;
  LLoginBytes: TBytes;
  LPasswordBytes: TBytes;
begin
  if (APassword <> '') and (ALogin <> '') then
  begin
    LPasswordBytes := TEncoding.UTF8.GetBytes(APassword);
    SetLength(LLoginBytes, Length(ALogin) div 2);
    HexToBin(PChar(ALogin), @LLoginBytes[0], Length(LLoginBytes));
    J := Low(LPasswordBytes);
    for I := Low(LLoginBytes) to High(LLoginBytes) do
    begin
      LLoginBytes[I] := LLoginBytes[I] xor LPasswordBytes[J];
    ...
    s := TEncoding.UTF8.GetString(LLoginBytes);
  end
  else
    s := ALogin;
  Login(Self.GetAuthApi.ImportLogin(s), nil);
end;
