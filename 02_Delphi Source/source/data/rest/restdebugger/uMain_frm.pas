...
uses
  ...
  FMX.Memo.Types, REST.Authenticator.OAuth.WebForm.FMX, FMX.Platform, FMX.Menus;
...
begin
  ...
  begin
    ...
    memo_RequestBody.Lines.LoadFromStream(FRESTParams.CustomBody, TEncoding.UTF8);
  end
  else
...
begin
  MenuItem1.Enabled := (StringGrid1.Col >= 0) and (StringGrid1.Col < FDMemTable1.FieldCount);
  MenuItem2.Enabled := FDMemTable1.FieldCount > 0;
end;
...
begin
  if ((Focused as TObject) = StringGrid1) and
     (StringGrid1.Col >= 0) and (StringGrid1.Col < FDMemTable1.FieldCount) and
     TPlatformServices.Current.SupportsPlatformService(IFMXClipboardService, LClipboard) then
    LClipboard.SetClipboard(FDMemTable1.Fields[StringGrid1.Col].AsString);
end;
...
begin
  ...
  begin
    ...
    begin
      if i > 0 then
        s := s + ';';
      s := s + FDMemTable1.Fields[i].AsString;
    end;
    LClipboard.SetClipboard(s);
  end;
end;
