  ...
  /// </summary>
   ...
   public type
    ...
    public
      ...
      OnNeedClientCertificate: TNeedClientCertificateEvent;
      TenantId: string;
      TenantSecret: string;
      ConnectTimeout: Integer;
      ReadTimeout: Integer;
      SqidsAlphabet: string;
      SqidsMinLength: Integer;
    ...
    private
      FAuthToken: string;
      FAuthTokenExpiry: string;
      FUser: TUser;
    public
      constructor Create(const AAuthToken, AAuthTokenExpiry: string; const AUser: TUser);
      property AuthToken: string read FAuthToken;
      property AuthTokenExpiry: string read FAuthTokenExpiry;
...
begin
  ...
  AClient.OnNeedClientCertificate := FConnectionInfo.OnNeedClientCertificate;
  AClient.ConnectTimeout := FConnectionInfo.ConnectTimeout;
  AClient.ReadTimeout := FConnectionInfo.ReadTimeout;
  AClient.SqidsAlphabet := FConnectionInfo.SqidsAlphabet;
  AClient.SqidsMinLength := FConnectionInfo.SqidsMinLength;
end;
...
begin
  ...
  AJSONObject.TryGetValue<string>(TJSONNames.SessionTokenExpiry, LSessionTokenExpiry);
  Result := TLogin.Create(LSessionToken, LSessionTokenExpiry, LUser);
end;
...
begin
  ...
  Dest.SqidsMinLength := 0;
  Dest.OnValidateCertificate := nil;
  Dest.OnNeedClientCertificate := nil;
end;
...
constructor TEMSClientAPI.TLogin.Create(const AAuthToken, AAuthTokenExpiry: string;
  const AUser: TUser);
begin
  FAuthToken := AAuthToken;
  FAuthTokenExpiry := AAuthTokenExpiry;
  FUser := AUser;
end;
