...
uses
  ...
  System.NetEncoding, System.NetEncoding.Sqids,
...
type
  ...
  protected
    ...
    /// <summary>
    /// OnBeforeExecute dispatch method. Called internally.
    /// </summary>
  ...
  public
    ...
    *)
    ...
    procedure Cancel;
    /// <summary>
    ///  Scpecifies when the request was cancelled using Cancel call.
    /// </summary>
    property IsCancelled: Boolean read GetIsCancelled;

    /// <summary>
    /// <para>
    /// Executes a request asynchronously, i.e. run it in its own thread. There is no automatic serialization of
    ...
    /// <summary>
    /// OnBeforeExecute gets fired before TRESTRequest.Execute has started.
    /// </summary>
    property OnBeforeExecute: TCustomRESTRequestNotifyEvent read FOnBeforeExecute write FOnBeforeExecute;
    /// <summary>
    /// OnAfterExecute gets fired after TRESTRequest.Execute has finished and the Response has been processed. Any
  ...
  private
    ...
    FPosting: Boolean;
    FSqidsEncoding: TSqidsEncoding;
    FSqidsAlphabet: string;
    FSqidsMinLength: Integer;
    ...
    function GetClientCertificatePassword: string;
    function GetClientCertificatePath: string;
    function GetClientCertificateStream: TStream;
    procedure SetClientCertificatePath(const AValue: string);
    procedure SetClientCertificateStream(const AValue: TStream);
    procedure SetClientCertificatePassword(const AValue: string);
    ...
    procedure SetReceiveDataEvent(const AValue: TReceiveDataEvent);
    procedure SetSendDataEvent(const AValue: TSendDataEvent);
    procedure SetSqidsAlphabet(const AValue: string);
    procedure SetSqidsMinLength(const AValue: Integer);
    function GetReceiveDataExEvent: TReceiveDataExEvent;
    procedure SetReceiveDataExEvent(const AValue: TReceiveDataExEvent);
  ...
  public
    ...
    /// <summary>
    /// Returns Sqids encoder initialized using SqidsAlphabet and SqidsMinLength.
    /// </summary>
    function GetSqidsEncoding: TSqidsEncoding;
    ...
    /// <summary>
    /// Specifies alphabet for Sqids encoder. Empty string is default value.
    /// </summary>
    property SqidsAlphabet: string read FSqidsAlphabet write SetSqidsAlphabet;
    /// <summary>
    /// Specifies minimal length of hash for Sqids encoder. 0 is default value.
    /// </summary>
    property SqidsMinLength: Integer read FSqidsMinLength write SetSqidsMinLength default 0;
    ...
    /// <summary>Event fired when client is sending request body data</summary>
    property OnSendData: TSendDataEvent read GetSendDataEvent write SetSendDataEvent;
    /// <summary>Event fired when client is receiving response body data</summary>
    property OnReceiveData: TReceiveDataEvent read GetReceiveDataEvent write SetReceiveDataEvent;
    /// <summary>Event fired when client is receiving response body data, allowing access to that data</summary>
...
uses
  System.Rtti, System.StrUtils, System.TypInfo, System.JSON.Types,
  System.Net.Mime, System.NetConsts,
...
var
  ...
  LParam: TRESTRequestParameter;
  i: Integer;
  LOpts: TRESTRequestParameterOptions;
begin
  Result := False;
  LNewParams := nil;
  BeginUpdate;
  try
    ...
    begin
      ...
      begin
        if (LNewParams.IndexOf(LParam.Name) < 0) and
           (LNewParams.IndexOf(CSqidsParamPrefix + LParam.Name) < 0) then
        begin
          Delete(LParam);
    ...
    begin
      LName := LNewParams[i];
      LOpts := [TRESTRequestParameterOption.poAutoCreated];
      if LName.StartsWith(CSqidsParamPrefix) then
      begin
        LName := LName.Substring(Length(CSqidsParamPrefix));
        LOpts := LOpts + [TRESTRequestParameterOption.poSqids];
      end;
      ...
      begin
        ...
        LParam.Options := LParam.Options + LOpts;
        // return True to indicate changes made
        Result := True;
      end;
    end;
...
var
  ...
  procedure ReplaceValue(const AName, AValue: string; var AURL: string);
  var
    LReplace: string;
  begin
    LReplace := '{' + AName + '}';
    // Trailing '/' needed to pass blank value
    if (AValue = '') and (AURL.Length > LReplace.Length) and
       SameText(AURL.Substring(AURL.Length - LReplace.Length), LReplace) then
      AURL := AURL + '/';
    AURL := StringReplace(AURL, LReplace, AValue, [rfReplaceAll, rfIgnoreCase]);
  end;

begin
  for LParameter in AParamList do
    if LParameter.Kind = TRESTRequestParameterKind.pkURLSEGMENT then
      if poSqids in LParameter.Options then
      begin
        LValue := FClient.GetSqidsEncoding.Encode(LParameter.Value);
        ReplaceValue(LParameter.Name, LValue, AURL);
        ReplaceValue(CSqidsParamPrefix + LParameter.Name, LValue, AURL);
      end
      else
        ReplaceValue(LParameter.Name, LParameter.Value, AURL);
end;
      ...
      begin
        ...
        begin
          ...
          if LParam.Stream <> nil then
            LMultipartFormData.AddStream(LParam.Name, LParam.Stream, False, LParam.Value,
...
var
  ...
  LExt: string;
  LTryFallback: Boolean;
begin
  FExecutionPerformance.Start;

  HandleEvent(DoBeforeExecute);
  ...
  try
    ...
    try
      ...
      try
        ...
        LTryFallback := True;
        try
          LContentType := Client.HTTPClient.Response.ContentType;
          if LContentType <> '' then
            TMimeTypes.Default.GetTypeInfo(LContentType, LExt, LMimeKind)
          else
          ...
          if LContentIsString then
            try
              ...
              on E: EEncodingError do
                // LMimeKind=Text, then something is broken
                // LMimeKind=Undefined, then looks like it is not a text
...
begin
  try
    Params.AddItem(AName, AJsonObject.ToString, TRESTRequestParameterKind.pkGETorPOST,
      [TRESTRequestParameterOption.poDoNotEncode], TRESTContentType.ctAPPLICATION_JSON);
  finally
    if AFreeJson then
      AJsonObject.Free;
  end;
...
function TCustomRESTRequest.GetBodyEncoding: TEncoding;
var
  LPage: Integer;
begin
  LPage := BodyCodePage;
...
var
  ...
  begin
    ...
    begin
      ...
      LValue := LValues[i];
      if not (TRESTRequestParameterOption.poDoNotEncode in LParam.Options) then
        LValue := TNetEncoding.URL.EncodeQuery(LValue);
...
begin
  ...
  for LParam in AParamList do
    ...
    begin
      ...
      begin
        if not (TRESTRequestParameterOption.poDoNotEncode in LParam.Options) then
          LValue := TNetEncoding.URL.EncodeQuery(LParam.Value);
        LValue := LName + '=' + LValue;
      end;
...
begin
  ...
  begin
    ...
    begin
      if AName.Contains('.') or AName.Contains('[') then
        Result := LJSONObj.TryGetValue(AName, AValue)
      else
      begin
...
procedure TCustomRESTClient.SetSqidsMinLength(const AValue: Integer);
begin
  if FSqidsMinLength <> AValue then
  begin
    FreeAndNil(FSqidsEncoding);
    FSqidsMinLength := AValue;
  end;
end;

procedure TCustomRESTClient.SetSqidsAlphabet(const AValue: string);
begin
  if FSqidsAlphabet <> AValue then
  begin
    FreeAndNil(FSqidsEncoding);
    FSqidsAlphabet := AValue;
  end;
end;

function TCustomRESTClient.GetSqidsEncoding: TSqidsEncoding;
begin
  if FSqidsEncoding = nil then
    FSqidsEncoding := TSqidsEncoding.Create(SqidsAlphabet, SqidsMinLength);
  Result := FSqidsEncoding;
end;

function TCustomRESTClient.GetMaxRedirects: Integer;
begin
  if HTTPClient <> nil then
    Result := HTTPClient.MaxRedirects
  else
    Result := CHTTPDefMaxRedirects;
end;
...
var
  ...
  LPrevSend: TSendDataEvent;
  LPrevReceive: TReceiveDataEvent;
  LPrevReceiveEx: TReceiveDataExEvent;
begin
  ...
  begin
    ...
    LPrevSend := OnSendData;
    LPrevReceive := OnReceiveData;
    LPrevReceiveEx := OnReceiveDataEx;
    CreateHttpClient;
    OnValidateCertificate := LPrevValidCert;
    OnNeedClientCertificate := LPrevNeedCert;
    OnAuthEvent := LPrevAuth;
    OnSendData := LPrevSend;
    OnReceiveData := LPrevReceive;
    OnReceiveDataEx := LPrevReceiveEx;
  end
  else
    CreateHttpClient;
  BaseURL := '';
...
procedure TCustomRESTClient.SetMaxRedirects(const AValue: Integer);
begin
  if HTTPClient <> nil then
    HTTPClient.MaxRedirects := AValue;
end;
...
function TCustomRESTClient.GetClientCertificatePassword: string;
begin
  Result := HTTPClient.ClientCertificatePassword;
end;

function TCustomRESTClient.GetClientCertificatePath: string;
begin
  Result := HTTPClient.ClientCertificatePath;
end;

function TCustomRESTClient.GetClientCertificateStream: TStream;
begin
  Result := HTTPClient.ClientCertificateStream;
end;

procedure TCustomRESTClient.SetClientCertificatePassword(const AValue: string);
begin
  HTTPClient.ClientCertificatePassword := AValue;
end;

procedure TCustomRESTClient.SetClientCertificatePath(const AValue: string);
begin
  HTTPClient.ClientCertificatePath := AValue;
end;

procedure TCustomRESTClient.SetClientCertificateStream(const AValue: TStream);
begin
  HTTPClient.ClientCertificateStream := AValue;
end;

function TCustomRESTClient.GetSendDataEvent: TSendDataEvent;
begin
  Result := HTTPClient.OnSendData;
end;

procedure TCustomRESTClient.SetSendDataEvent(const AValue: TSendDataEvent);
begin
  HTTPClient.OnSendData := AValue;
end;

function TCustomRESTClient.GetReceiveDataEvent: TReceiveDataEvent;
begin
  Result := HTTPClient.OnReceiveData;
end;

function TCustomRESTClient.GetReceiveDataExEvent: TReceiveDataExEvent;
begin
  Result := HTTPClient.OnReceiveDataEx;
end;

procedure TCustomRESTClient.SetReceiveDataEvent(const AValue: TReceiveDataEvent);
begin
  HTTPClient.OnReceiveData := AValue;
end;

procedure TCustomRESTClient.SetReceiveDataExEvent(
  const AValue: TReceiveDataExEvent);
begin
  HTTPClient.OnReceiveDataEx := AValue;
end;
