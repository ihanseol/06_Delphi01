...
begin
  ...
  Result := FieldInfo.OriginalFieldName <> '';
end;
...
begin
  ...
  if not (Result.FieldKind in Result.DataSet.StoredFieldKinds) then
...
begin
  if VarType(Value) = varString then
    C := StrToCurr(string(MarshaledAString(TVarData(Value).VString)))
  else
    C := Value;
...
begin
  if VarType(Value) = varString then
    BCD := StrToBcd(string(MarshaledAString(TVarData(Value).VString)))
  else
    BCD := VarToBcd(Value);
  Result := PutConstNode(ftBCD, @BCD, 18);
end;
...
begin
  if VarType(Value) = varString then
  begin
    if not TryStrToDate(string(MarshaledAString(TVarData(Value).VString)), DateTime) then
...
begin
  if VarType(Value) = varString then
    TimeStamp := StrToSQLTimeStamp(string(MarshaledAString(TVarData(Value).VString)))
  else
...
begin
  if VarType(Value) = varString then
    TimeStampOffset := StrToSQLTimeStampOffset(string(MarshaledAString(TVarData(Value).VString)))
  else
...
begin
  if VarType(Value) = varString then
    F := StrToFloat(string(MarshaledAString(TVarData(Value).VString)))
  else
    F := Value;
...
begin
  if VarType(Value) = varString then
    I := StrToInt(string(MarshaledAString(TVarData(Value).VString)))
  else
    I := Value;
  Size := 2;
  case DataType of
...
begin
  if VarType(Value) = varString then
    DateTime := StrToTime(string(MarshaledAString(TVarData(Value).VString)))
  else
    DateTime := VarToDateTime(Value);
  TimeStamp := DateTimeToTimeStamp(DateTime);
