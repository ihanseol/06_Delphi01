...
type
  ...
  TSQLBlobStreamEx = class(TSQLBlobStream)
  public
{$IF Sizeof(LongInt) <> Sizeof(NativeInt)}
    function Read(var Buffer; Count: NativeInt): NativeInt; override;
    function Read(Buffer: TBytes; Offset, Count: NativeInt): NativeInt; override;
{$ENDIF Sizeof(LongInt) <> Sizeof(NativeInt)}
  end;

{$IF Sizeof(LongInt) <> Sizeof(NativeInt)}
function TSQLBlobStreamEx.Read(var Buffer; Count: NativeInt): NativeInt;
begin
  if not FHasData then
    ReadBlobData;
  Result := inherited Read(Buffer, Count);
...
function TSQLBlobStreamEx.Read(Buffer: TBytes; Offset, Count: NativeInt): NativeInt;
begin
  if not FHasData then
    ReadBlobData;
  Result := inherited Read(Buffer, Offset, Count);
...
begin
  Result := TSQLBlobStreamEx.Create(Field as TBlobField, Mode);
end;
...
begin
  ...
  try
    ...
    List.Free;
    Command.Free;  // Force command to be removed from connection's command list
  end;
end;
