...
{$IFDEF MSWINDOWS}
var
  OSFormatSettings: TFormatSettings;
{$ENDIF}
...
{$IFDEF MSWINDOWS}
  else if VarType(AValue) = VT_DECIMAL then
    Result := StrToBcd(AValue, OSFormatSettings)
{$ENDIF}
  else
...
{$IFDEF MSWINDOWS}
  OSFormatSettings := TFormatSettings.Create;
{$ENDIF}
finalization
