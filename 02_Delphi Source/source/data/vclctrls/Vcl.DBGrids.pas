...
type
  TColumnValue = (cvColor, cvWidth, cvFont, cvAlignment, cvReadOnly, cvTitleColor,
    cvTitleCaption, cvTitleAlignment, cvTitleFont, cvImeMode, cvImeName, cvBiDiMode);
...
type
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
    function  DefaultAlignment: TAlignment;
    function  DefaultBiDiMode: TBiDiMode;
    function  DefaultColor: TColor;
    function  DefaultFont: TFont;
  ...
  private
    ...
    class procedure InitImageCollection; static;
...
begin
  ...
  begin
    ...
    try
      ...
      if cvBiDiMode in TColumn(Source).AssignedValues then
        BiDiMode := TColumn(Source).BiDiMode;
...
function TColumn.DefaultBiDiMode: TBiDiMode;
var
  Grid: TCustomDBGrid;
begin
  Grid := GetGrid;
  ...
  begin
    if Assigned(Field) then
    begin
      if DBUseRightToLeftBidiMode(Grid, FField) then
...
begin
  Grid := GetGrid;
  if Assigned(Grid) then
    if Grid.FInternalColor = clNone then
      Result := Grid.Color
    else
...
function TColumn.GetBiDiMode: TBiDiMode;
begin
  if cvBiDiMode in FAssignedValues then
    Result := FBiDiMode
  else
    Result := DefaultBiDiMode;
end;
...
function TColumn.IsBiDiModeStored: Boolean;
begin
  Result := (cvBiDiMode in FAssignedValues) and (FBiDiMode <> DefaultBiDiMode);
end;
...
procedure TColumn.SetBiDiMode(const Value: TBiDiMode);
begin
...
begin
  if (Index < 0) or (Index > Count) then
    ListIndexError(Index, Count, Self);
  SetLength(FList, Count + 1);
  if Index < Count - 1 then
  begin
...
class procedure TCustomDBGrid.InitImageCollection;
...
begin
  if FIndicatorImageCollection <> nil then
    Exit;
...
begin
  ...
  WriteText(Canvas, Rect, 3, 2, Value, Column.Alignment,
    (Column.BidiMode = bdRightToLeft) and
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if dgRowLines in Options then Inc(RowHeight, ActualGridLineWidth);
        if dgIndicator in Options then
        begin
  ...
  if FDatalink.Active then
    with Message, FDataLink.DataSet do
      case ScrollCode of
        ...
        SB_THUMBPOSITION,
        SB_THUMBTRACK:
          if (dgThumbTracking in Options) or (ScrollCode = SB_THUMBPOSITION) then
          begin
            if IsSequenced then
            begin
              SI.cbSize := sizeof(SI);
