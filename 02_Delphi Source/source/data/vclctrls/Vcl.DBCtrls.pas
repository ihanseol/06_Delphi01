...
uses System.Variants, Winapi.Windows, System.SysUtils, Winapi.Messages, Vcl.Controls, Vcl.Forms, System.Classes,
     ...
     Data.DB, Winapi.RichEdit, System.Generics.Collections, Vcl.ImageCollection, Vcl.ImgList;
type
...
{ TDBNavigator }
  ...
  private
    ...
    function GetKind: TDBNavigatorKind;
    ...
    function GetButton(Index: TNavigateBtn): TNavButton;
  protected
    property Buttons[Index: TNavigateBtn]: TNavButton read GetButton;
    procedure ActiveChanged;
    procedure DataChanged;
    procedure EditingChanged;
    procedure Notification(AComponent: TComponent;
      Operation: TOperation); override;
    function GetButtonImageList: TCustomImageList; override;
  ...
  published
    ...
    property Kind: TDBNavigatorKind read GetKind write SetKind default dbnHorizontal;
    property ParentCtl3D;
    property ParentShowHint;
    property PopupMenu;
    [Default(True)]
...
uses Vcl.DBLogDlg, Vcl.DBPWDlg, Vcl.Clipbrd, Data.DBConsts, Vcl.VDBConsts, Vcl.Dialogs,
  System.Math, Vcl.Themes, System.Types, System.UITypes, Winapi.ActiveX, Vcl.VirtualImageList;
...
function TDBNavigator.GetButtonImageList: TCustomImageList;
var
  I: TNavigateBtn;
  LVImageList: TVirtualImageList;
begin
  if FButtonsImageCollection = nil then
  begin
    FButtonsImageCollection := TImageCollection.Create(nil);
    for I := Low(BtnTypeName) to High(BtnTypeName) do
      FButtonsImageCollection.Add(BtnTypeName[I], HInstance,
        'DBN_' + BtnTypeName[I], ['', '_20X']);
  end;
  LVImageList := TVirtualImageList.Create(Self);
  LVImageList.SetSize(DefButtonImageSize, DefButtonImageSize);
  LVImageList.AutoFill := True;
  LVImageList.ImageCollection := FButtonsImageCollection;
  Result := LVImageList;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      LDescription.ImageIndex := Integer(I);
      LDescription.GlyphResInstance := 0;
      LDescription.GlyphResName := '';
...
begin
  ...
  begin
    ...
    for J := Low(TNavigateBtn) to High(TNavigateBtn) do
  ...
  begin
    ...
    if J = High(TNavigateBtn) then Exit;
    Inc(J);
  end;
end;
...
begin
  if ([csWriting, csReading] * ComponentState = []) and (FHints.Count = 0) then
  begin
    if not Assigned(FDefHints) then
      InitHints;
    if csDesigning in ComponentState then
...
function TDBNavigator.GetKind: TDBNavigatorKind;
begin
  Result := TDBNavigatorKind(inherited Orientation);
end;

procedure TDBNavigator.SetKind(Value: TDBNavigatorKind);
begin
  inherited Orientation := TOrientation(Value);
end;
...
begin
  if (DataSource <> nil) and Supports(DataSource.DataSet, IDataSetCommandSupport, Intf) then
...
function TDBNavigator.GetButton(Index: TNavigateBtn): TNavButton;
begin
...
procedure TDBNavigator.BtnIDClick(Index: TNavBtnID);
begin
  BtnClick(TNavigateBtn(Index));
end;
...
begin
  ...
  Buttons[nbApplyUpdates].Enabled := CanModify and (nbApplyUpdates in VisibleButtons) and CanApplyUpdates;
  Buttons[nbCancelUpdates].Enabled := CanModify and (nbCancelUpdates in VisibleButtons) and CanCancelUpdates;
end;
...
procedure TDBNavigator_EnabledChanged(Self: TBaseNavigator);
begin
  if Self is TDBNavigator then
    TDBNavigator(Self).ActiveChanged;
end;
...
var
  ...
  S: string;
begin
  if (FListField <> nil) and (FListField.FieldKind in FListField.DataSet.StoredFieldKinds) and
...
begin
  ...
  begin
    ...
    Invalidate;
    SearchText := '';
    if Accept and FChangeFocus and CanModify then SelectKeyValue(ListValue);
...
begin
  inherited;
  FChangeFocus := (Message.FocusedWnd = Handle) or (Message.FocusedWnd = FDataList.Handle);
  try
    CloseUp(not FChangeFocus);
  finally
    FChangeFocus := True;
  end;
  if (IsCustomStyleActive) and (seBorder in StyleElements) then
...
type
  IRichEditOleCallback = interface(IUnknown)
    ...
    function GetContextMenu(seltype: Word; oleobj: IOleObject; const chrg: TCharRange;
      var menu: HMENU): HRESULT; stdcall;
  end;

  TRichEditOleCallback = class(TInterfacedObject, IRichEditOleCallback)
  private
    FRichEdit: TDBRichEdit;
  protected
    function GetNewStorage(out stg: IStorage): HRESULT; stdcall;
    function GetInPlaceContext(out Frame: IOleInPlaceFrame; out Doc: IOleInPlaceUIWindow;
      lpFrameInfo: POleInPlaceFrameInfo): HRESULT; stdcall;
    ...
    function GetContextMenu(seltype: Word; oleobj: IOleObject; const chrg: TCharRange;
      var menu: HMENU): HRESULT; stdcall;
  public
    constructor Create(ARichEdit: TDBRichEdit);
  end;

constructor TRichEditOleCallback.Create(ARichEdit: TDBRichEdit);
begin
  inherited Create;
  FRichEdit := ARichEdit;
end;
...
begin
  Result := E_NOTIMPL;
  try
    if (reco = RECO_DROP) and fReally then
      if FRichEdit.BeginEditing then
        Result := S_OK
      else
        Result := E_ABORT;
  except
...
begin
  P := Mouse.CursorPos;
  if (FRichEdit.GetPopupMenu <> nil) and not FRichEdit.SpellChecking and
     (Win32MajorVersion >= 5) and FRichEdit.ClientRect.Contains(FRichEdit.ScreenToClient(P)) then
    FRichEdit.Perform(WM_CONTEXTMENU, FRichEdit.Handle, MakeLParam(P.X, P.Y));
  Result := E_NOTIMPL;
end;
...
begin
  ...
  Intf := TRichEditOleCallback.Create(Self);
  SendMessage(Handle, EM_SETOLECALLBACK, 0, LPARAM(Intf));
end;
...
initialization
  ...
  FNavigatorEnableChanged := @TDBNavigator_EnabledChanged;

finalization
  Data.DB.DBScreen := nil;
  Data.DB.DBApplication := nil;
  FNavigatorEnableChanged := nil;
