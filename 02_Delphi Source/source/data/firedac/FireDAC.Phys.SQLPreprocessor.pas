...
type
  ...
  private
    ...
    FInComment1, FInComment2,
      FInStr1, FInStr2, FInStr3, FInStr4,
...
begin
  ...
  if FDInSet(cCh, ['=', ' ', #13, #10, #9, #0]) or
    ...
    if ConnMetadata.Kind = TFDRDBMSKinds.Oracle then begin
      ...
      // - Oracle regular expression class, eg :DIGIT:
      if FDInSet(GetChar, ['.', ':']) then begin
        UnCommit(2);
        Commit(0);
        Exit;
      end
  ...
  repeat
    ...
    '}':
      if (piExpandEscapes in Instrs) and
         not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr4 and (FInNames = []) then
        Break;
    '(':
      if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr4 and (FInNames = []) then
        Inc(FBraceLevel);
    ')':
      if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr4 and (FInNames = []) then begin
    ...
    '\':
      ...
      if (FInStr1 or FInStr2 or FInStr3 or FInStr4 or (FInNames <> [])) and
         (ConnMetadata <> nil) and (ConnMetadata.BackslashEscSupported) then
        GetChar;
    '/':
      begin
        GetChar;
        if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr3 and not FInStr4 and (FInNames = []) and (FCh = '*') then
    ...
    '*':
      begin
        GetChar;
        if not FInComment2 and not FInStr1 and not FInStr2 and not FInStr3 and not FInStr4 and (FInNames = []) and (FCh = '/') then
    ...
    '-':
      begin
        GetChar;
        if not FInComment1 and not FInStr1 and not FInStr2 and not FInStr3 and not FInStr4 and (FInNames = []) and (FCh = '-') then
          FInComment2 := True
        else
          PutBack;
      end;
    '''':
      if not FInComment1 and not FInComment2 and (FInNames = []) and not FInStr2 and not FInStr3 and not FInStr4 and
    ...
    #13, #10:
      begin
        if not FInComment1 and FInComment2 then
          FInComment2 := False;
        if not FInStr1 and not FInStr2 and not FInStr4 and (piTransformEOLs in Instrs) then
    ...
    ':':
      if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr3 and not FInStr4 and (FInNames = []) and
    ...
    '?':
      if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr3 and not FInStr4 and (FInNames = []) and
        ...
        ProcessQuestion;
    '{':
      if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr4 and (FInNames = []) and
    ...
    '!', '&':
      if Instrs * [piExpandMacros, piCreateMacros] <> [] then
        ProcessMacro(FCh);
    '$':
      if not FInStr1 and not FInStr2 and not FInStr4 and (FInNames = []) and
         (ConnMetadata <> nil) and (ConnMetadata.Kind = TFDRDBMSKinds.PostgreSQL) then
        ProcessQuoteTag;
    'a'..'z', 'A'..'Z':
      if ConnMetadata <> nil then
        if ConnMetadata.Kind = TFDRDBMSKinds.Oracle then begin
          if (FCh = 'q') and
             not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr3 and not FInStr4 and (FInNames = []) then begin
            if GetChar = '''' then
              if GetChar = '[' then
                FInStr4 := True
              else begin
        ...
        else if ConnMetadata.Kind in [TFDRDBMSKinds.Interbase, TFDRDBMSKinds.Firebird] then begin
          if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr3 and not FInStr4 and (FInNames = []) and
    ...
    ';':
      ...
      if (FCh = '"') and not (Byte('"') in FNameDelims1) then begin
        if not FInComment1 and not FInComment2 and (FInNames = []) and not FInStr1 and not FInStr3 and not FInStr4 and
          ...
          FInStr2 := not FInStr2;
              ...
              not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr3 and not FInStr4 then begin
        for eQuote := Low(TFDPhysNameQuoteLevel) to High(TFDPhysNameQuoteLevel) do begin
      ...
      else if (FCh = ']') and (ConnMetadata <> nil) and (ConnMetadata.Kind = TFDRDBMSKinds.Oracle) and
           not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr3 and FInStr4 and (FInNames = []) then begin
        if GetChar = '''' then
          FInStr4 := False
        else
          PutBack;
...
begin
  ...
  while iPos <= iLen do begin
    ...
    '/':
      begin
        Inc(iPos);
        if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr4 and (FInNames = []) and (iPos <= iLen) and (FDestination[iPos] = '*') then begin
    ...
    '*':
      begin
        Inc(iPos);
        if not FInComment2 and not FInStr1 and not FInStr2 and not FInStr4 and (FInNames = []) and (iPos <= iLen) and (FDestination[iPos] = '/') then
          if FInMySQLConditional then
            FInMySQLConditional := False
          else
            FInComment1 := False
    ...
    '-':
      begin
        Inc(iPos);
        if not FInComment1 and not FInStr1 and not FInStr2 and not FInStr4 and (FInNames = []) and (iPos <= iLen) and (FDestination[iPos] = '-') then begin
          if (FBraceLevel = 0) and (iFromStartedPos <> 0) and (iFromEndedPos = 0) then
            iFromEndedPos := iPos - 2;
          FInComment2 := True;
        end
    ...
    '''':
      if not FInComment1 and not FInComment2 and (FInNames = []) and not FInStr2 and not FInStr4 then
        FInStr1 := not FInStr1;
    #13, #10, ' ', #9, ',', ';':
      begin
    ...
    'A' .. 'Z', 'a' .. 'z':
      if (ConnMetadata.Kind = TFDRDBMSKinds.Oracle) and (ch = 'q') and
         not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr3 and not FInStr4 and (FInNames = []) and
         (iPos + 2 <= Length(FDestination)) and (FDestination[iPos + 1] = '''') and (FDestination[iPos + 2] = '[') then begin
        FInStr4 := True;
        Inc(iPos, 2);
      end
      else if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr4 and (FInNames = []) and
         (FBraceLevel = 0) and
        ...
        if not lResolved then begin
          ...
          if CheckKeyword('INTO') then begin
            ...
            Inc(iPos, 7);
                ...
                (ConnMetadata.Kind in [TFDRDBMSKinds.Oracle, TFDRDBMSKinds.Firebird, TFDRDBMSKinds.PostgreSQL]) and
          ...
          FSQLCommandKind := ConnMetadata.GetSQLCommandKind(FTokens);
                ...
                (ConnMetadata.Kind = TFDRDBMSKinds.MSSQL) and
                CheckKeyword('OUTPUT') then begin
          // Special handling of INSERT RETURNING without {INTO}, which is more
          // SELECT FOR UPDATE, than INSERT. Because returns a resultset and
          // inserts the new records (modifies DB).
          FTokens.Add('OUTPUT');
      ...
      if (ch = '"') and not (Byte('"') in FNameDelims1) then begin
        if not FInComment1 and not FInComment2 and (FInNames = []) and not FInStr1 and not FInStr4 then
          FInStr2 := not FInStr2;
      end
      ...
      else if (ch = ']') and (ConnMetadata.Kind = TFDRDBMSKinds.Oracle) and
              not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and not FInStr3 and FInStr4 and (FInNames = []) and
              (iPos + 1 <= Length(FDestination)) and (FDestination[iPos + 1] = '''') then begin
        FInStr4 := False;
        Inc(iPos);
      end;
    end;
