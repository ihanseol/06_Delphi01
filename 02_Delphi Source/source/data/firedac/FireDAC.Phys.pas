...
type
  ...
  TFDPhysRecoveryFlags = set of TFDPhysRecoveryFlag;
  ...
  protected
    ...
    function GetConnectionDefStat(const AConnectionDef: IFDStanConnectionDef): IFDPhysConnectionDefStat;
  ...
  TFDPhysConnectionHost = class(TInterfacedObject, IUnknown,
    IFDStanObjectHost, IFDPhysConnectionDefStat)
  private
    [Weak] FDriverObj: TFDPhysDriver;
    FConnectionDef: IFDStanConnectionDef;
    FCreatedConnections: Cardinal;
    FAliveConnections: Cardinal;
...
{$ENDIF}
  ...
  protected
    ...
    // IFDPhysConnectionDefStat
    function GetCreatedConnections: Cardinal;
    function GetAliveConnections: Cardinal;
    function GetBusyConnections: Cardinal;
    // other
  public
...
{$ENDIF}
    ...
    function GetCommandsExecuted: UInt64;
...
{$IFDEF FireDAC_MONITOR}
    procedure UpdateMonitor;
    procedure DumpParams(AKind: TFDMoniEventKind; ATypes: TParamTypes);
{$ENDIF}
...
uses
  System.Variants, System.Generics.Defaults, System.Generics.Collections,
    System.Types, System.Diagnostics, System.TypInfo,
...
{ ----------------------------------------------------------------------------- }
function TFDPhysManager.GetConnectionDefStat(const AConnectionDef: IFDStanConnectionDef): IFDPhysConnectionDefStat;
var
  oDrv: TFDPhysDriver;
begin
  ASSERT(AConnectionDef <> nil);
  ...
  if oDrv <> nil then
    try
      Result := oDrv.GetConnectionDefStat(AConnectionDef);
    finally
      oDrv.FLock.Leave;
    end;
end;
...
{ ----------------------------------------------------------------------------- }
function TFDPhysDriver.GetConnectionDefStat(const AConnectionDef: IFDStanConnectionDef): IFDPhysConnectionDefStat;
var
  oConnHost: TFDPhysConnectionHost;
begin
  ASSERT(AConnectionDef <> nil);
  ...
  try
    oConnHost := FindConnectionHost(AConnectionDef);
    Supports(oConnHost, IFDPhysConnectionDefStat, Result);
  finally
    FLock.Leave;
  end;
end;
...
begin
  AtomicIncrement(FCreatedConnections);
  if AtomicIncrement(FAliveConnections) = 1 then
    if FConnectionDef.Style <> atTemporary then
      FConnectionDef.ToggleUpdates(FPassCode, True, True);
end;
...
begin
  if AtomicDecrement(FAliveConnections) = 0 then
    if FConnectionDef.Style <> atTemporary then
      FConnectionDef.ToggleUpdates(FPassCode, True, False)
...
{ ----------------------------------------------------------------------------- }
function TFDPhysConnectionHost.GetCreatedConnections: Cardinal;
begin
  Result := FCreatedConnections;
end;

{ ----------------------------------------------------------------------------- }
function TFDPhysConnectionHost.GetAliveConnections: Cardinal;
begin
  Result := FAliveConnections;
end;

{ ----------------------------------------------------------------------------- }
function TFDPhysConnectionHost.GetBusyConnections: Cardinal;
begin
  if FConnectionDef.Params.Pooled and not FDIsDesignTime then
    Result := FPool.BusyObjects
  else
    Result := FAliveConnections;
end;
...
{$ENDIF}
    ...
    ConnectionDef.ReadOptions(FOptions.FormatOptions, FOptions.UpdateOptions,
      FOptions.FetchOptions, FOptions.ResourceOptions, FOptions.SecurityOptions);
    CheckTransaction;
  finally
    FDriverObj.FLock.Leave;
  end;
...
{-------------------------------------------------------------------------------}
function TFDPhysConnection.GetCommandsExecuted: UInt64;
begin
  Result := FCommandsExecuted;
end;
...
{$ENDIF}
    ...
    if (GetMetaInfoKind = mkNone) and
       not (GetCommandKind in FOptions.SecurityOptions.AllowedCommandKinds) then
      FDException(Self, [S_FD_LPhys, DriverID], er_FD_AccSecCommandKindNotAllowed,
        [GetEnumName(TypeInfo(TFDPhysCommandKind), Integer(GetCommandKind)), GetName]);
...
{-------------------------------------------------------------------------------}
{$IFDEF FireDAC_MONITOR}
procedure TFDPhysCommand.DumpParams(AKind: TFDMoniEventKind; ATypes: TParamTypes);
var
  i: Integer;
  oPars: TFDParams;
  s: String;
...
{$IFDEF FireDAC_MONITOR}
  ...
  begin
    ...
    if ekSQLVarIn in FConnectionObj.GetMonitor.EventKinds then
      DumpParams(ekSQLVarIn, [ptUnknown, ptInput, ptInputOutput]);
  ...
  begin
    ...
    if ekSQLVarOut in FConnectionObj.GetMonitor.EventKinds then
      DumpParams(ekSQLVarOut, [ptOutput, ptInputOutput, ptResult]);
...
{$ENDIF}
  try
    ...
    if not GetNextRecordSet then begin
      Inc(FConnectionObj.FCommandsExecuted);
...
{$ENDIF}
  ...
  procedure SetSingleMeta;
  var
    oConnMeta: IFDPhysConnectionMetadata;
  begin
    FConnection.CreateMetadata(oConnMeta);
    lSavepoint := FTransactionObj.GetActive and (oConnMeta.TxNested or oConnMeta.TxSavepoints) or
      not FTransactionObj.GetActive and oConnMeta.TxSupported;
  end;

  procedure SetArrayMeta;
  var
    oConnMeta: IFDPhysConnectionMetadata;
  begin
    FConnection.CreateMetadata(oConnMeta);
...
{$ENDIF}
  try
    ...
    try
      ...
      try
        FTransactionObj.Notify(cpBeforeCmdExecute, Self);
        if (ATimes = 1) and (AOffset = 0) then begin
          if FOptions.SecurityOptions.ExactUpdatedRecsNum >= 0 then
            SetSingleMeta;
          Process_SingleRow;
        end
        else begin
