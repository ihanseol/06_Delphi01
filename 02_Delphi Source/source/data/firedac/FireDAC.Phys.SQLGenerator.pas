...
type
  ...
  protected
    ...
    function GetCreatePrimaryKey(const AName, ACols: String; AInline: Boolean): String; virtual;
...
begin
  ...
  if lBeautify then begin
    ...
    if (FConnMeta.Kind = TFDRDBMSKinds.SQLite) and ((FParams = nil) or (FParams.BindMode = pbByName)) then
      Result := ':' + Result
    else
...
var
  ...
  begin
    if ATableParams.FFilterTable = nil then
      Exit(AFlt);
    FDCreateInterface(IFDStanExpressionParser, oParser);
    eOpts := [];
    if ATableParams.FFilterNoCase then
      Include(eOpts, ekNoCase);
    if ATableParams.FFilterPartial then
      Include(eOpts, ekPartial);
    Result := oParser.Prepare(TFDDatSTableExpressionDS.Create(ATableParams.FFilterTable),
      AFlt, eOpts, [], '').ToSQL(
      function (const AFieldName: string): string
      begin
        Result := sTabAlias + NormalizeColName(AFieldName);
      end);
  end;
...
var
  ...
  i: Integer;
  oCol: TFDDatSColumn;
  sFrom, sDef, sCols, sGen, sKey, sName, sPKey: String;
  oGens, oInds: TFDStringList;
  oView: TFDDatSView;
  oSort: TFDDatSMechSort;
  rName: TFDPhysParsedName;
  ePrevFlags: TFDPhysCommandGeneratorFlags;
begin
  FCommandKind := skCreate;
  FFlags := [];
  FTabAlias := '';
  oCols := FTable.Columns;
  oGens := TFDStringList.Create(dupIgnore, True, False);
  oInds := TFDStringList.Create(dupIgnore, True, False);
  ...
  try
    ...
    if tpPrimaryKey in AParts then begin
      ...
      if (sKey <> '') and (oInds.IndexOf(sKey) = -1) then begin
        oInds.Add(sKey);
        oView := FTable.Views.FindSortedView(sKey, [soPrimary], []);
        if oView <> nil then
          sName := oView.Name
        else
          sName := '';
        sPKey := GetCreatePrimaryKey(sName, sKey, ctNoAlterTableConstraint in FConnMeta.CreateTableOptions);
      end;
    end;
    // Create table
    if tpTable in AParts then begin
      ...
      for i := 0 to oCols.Count - 1 do begin
        ...
        if ColumnStorable(oCol) then begin
          ...
          if not (gfInlinePK in ePrevFlags) and (gfInlinePK in FFlags) then begin
            sPKey := '';
            sKey := oCol.Name;
            if oInds.IndexOf(sKey) = -1 then
              oInds.Add(sKey);
          end;
          if sDef <> '' then begin
            if sCols <> '' then
      ...
      if sCols = '' then
        Exit;
      if (ctNoAlterTableConstraint in FConnMeta.CreateTableOptions) and (sPKey <> '') then
        sCols := sCols + ',' + BRK + IND + sPKey;
        ...
        for i := 0 to oCols.Count - 1 do begin
          ...
          if ColumnStorable(oCol) and (caAutoInc in oCol.ActualAttributes) then begin
            ...
            if sGen = '' then begin
              ...
              if oCol.SourceGenerator = '' then
                oCol.SourceGenerator := sGen;
            end;
            if (sGen <> '') and (oGens.IndexOf(sGen) = -1) then begin
              oGens.Add(sGen);
    ...
    // Indexes
    if not (ctNoAlterTableConstraint in FConnMeta.CreateTableOptions) and (sPKey <> '') then
      Result.Add(sPKey);
    if tpIndexes in AParts then begin
      for i := 0 to FTable.Views.Count - 1 do begin
        ...
        if oView.Creator = vcDataSetIndex then begin
          ...
          if (sKey <> '') and (oInds.IndexOf(sKey) = -1) then begin
            oInds.Add(sKey);
            Result.Add(GetCreateIndex(oView.Name, sKey, [soUnique, soPrimary] * oSort.SortOptions <> []));
          end;
        end;
      end;
    end;
...
{-------------------------------------------------------------------------------}
function TFDPhysCommandGenerator.GetCreatePrimaryKey(const AName, ACols: String; AInline: Boolean): String;
var
  sTab, sCons: String;
  rName: TFDPhysParsedName;
  i: Integer;
begin
  ...
  if (AName = '') or (CompareText(Copy(AName, 1, 4), 'View') = 0) then begin
    ...
    sCons := FConnMeta.EncodeObjName(rName, nil, [eoQuote, eoNormalize]);
  end
  else
    sCons := AName;
  if AInline then
    ...
    Result := 'ALTER TABLE ' + sTab + ' ADD ';
  Result := Result + 'CONSTRAINT ' + sCons + ' PRIMARY KEY (';
  i := 1;
