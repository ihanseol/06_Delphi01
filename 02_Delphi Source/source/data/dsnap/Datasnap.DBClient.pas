...
{$ENDIF !NEXTGEN}
    ...
    procedure InternalCheck(Status: DBResult; const ADSBase: IDSBase);
...
begin
  if Status <> 0 then InternalCheck(Status, FDSBase);
end;

function TCustomClientDataSet.CreateDSBase: IDSBase;
begin
  CreateDbClientObject(CLSID_DSBase, IDSBase, Result);
  InternalCheck(Result.SetProp(dspropANSICODEPAGE, DefaultSystemCodePage), Result);
  InternalCheck(Result.SetProp(dspropUTF8METADATA, NativeUInt(True)), Result);
  InternalCheck(Result.SetProp(dspropUTF8ERRORMSG, NativeUInt(True)), Result);
end;
...
procedure TCustomClientDataSet.InternalCheck(Status: DBResult; const ADSBase: IDSBase);
var
...
begin
  ...
  begin
    ...
    ADSBase.GetErrorString(Status, Pointer(@ErrMsgBuffer[0]));
    ADSBase.GetProp(dspropUTF8ERRORMSG, @UTF8ErrorMessage);
    if LongBool(UTF8ErrorMessage) then
      ErrMsg := UTF8ToUnicodeString(MarshaledAString(@ErrMsgBuffer[0]))
    else
      ErrMsg := string(MarshaledAString(@ErrMsgBuffer[0]));
    raise EDBClient.Create(ErrMsg.Trim, Status);
  end;
end;
...
begin
  ...
  begin
    ...
    Result := TEncoding.UTF8.GetString(VariantArrayToBytes(VarPacket)).Trim;
  end;
end;
...
begin
  ...
  try
    ...
    E := EReconcileError.Create(string(MarshaledAString(pErrMessage)), string(MarshaledAString(pErrContext)), iErrCode, iRslt);
    try
...
begin
  ...
  LName := UTF8ToUnicodeString(MarshaledAString(@FieldDesc.szName[0]));
  ...
  begin
    Inc(I);
    LName := string.Format('%s_%d', [UTF8ToUnicodeString(MarshaledAString(@FieldDesc.szName[0])), I]);
  end;
...
begin
  FieldDefList.Update;
  Name := UTF8ToUnicodeString(MarshaledAString(@IndexDesc.szName[0]));
  Fields := '';
  DescFields := '';
  CaseInsFields := '';
...
begin
  case Agg.DataType of
    ...
    ftString, ftGUID:
      begin
        var s: string := string(MarshaledAString(Buffer));
        Result := s;
      end;
    ftWideString:
      begin
...
begin
  ...
  if Agg.Active and
     ((Agg.GroupingLevel = 0) or
      (Agg.IndexName = '') and (IndexFieldCount > 0) or
      (FIndexName <> '') and AnsiSameText(Agg.IndexName, FIndexName)) then
  begin
...
procedure TCustomClientDataSet.ResetAllAggs(Value: Boolean);
  ...
  begin
    ...
    begin
      Field := AggFields[I] as TAggregateField;
      if (Field.GroupingLevel = 0) or
         (Field.IndexName = '') and (IndexFieldCount > 0) or
         (FIndexName <> '') and AnsiSameText(Field.IndexName, FIndexName) then
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if Field.Handle <> nil then
      begin
        if not ((Field.GroupingLevel = 0) or
                (Field.IndexName = '') and (IndexFieldCount > 0) or
                (FIndexName <> '') and AnsiSameText(Field.IndexName, FIndexName)) then
