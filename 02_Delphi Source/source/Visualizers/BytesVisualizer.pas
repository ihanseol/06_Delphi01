...
const
  ...
  BytesVisualizerTypes: array[0..3] of TBytesVisualizerType =
  (
    (TypeName: 'TArray<Byte>';               AllDesc: False; IsGen: False; PtrExp: 'Pointer(%s)'; LenExp: 'Length(%s)'; PosExp: '';),
...
function TBytesViewerFrame.GetBytesExps(const ATypeName, AExpression: string;
  ...
  function TrimSystem(const ATypeName: string): string;
  begin
    Result := StringReplace(ATypeName, 'system.', '', [rfReplaceAll, rfIgnoreCase]);
  end;
...
begin
  ...
  APosExp := '';
  Result := False;
  LCurType := TrimSystem(ATypeName);
  LCurExp := '(' + AExpression + ')';
  repeat
    for I := Low(BytesVisualizerTypes) to High(BytesVisualizerTypes) do
      if SameText(BytesVisualizerTypes[I].TypeName, LCurType) then
      begin
    ...
    LCurType := TrimSystem(LCurType.Substring(1, LCurType.Length - 2));
