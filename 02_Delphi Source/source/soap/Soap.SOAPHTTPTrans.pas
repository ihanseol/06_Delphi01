...
{$ENDIF !AUTOREFCOUNT}
    ...
    procedure SetupHttpReq(AHttpRequest: IHTTPRequest);
...
procedure TClientCert.Assign(ASource: TPersistent);
begin
  if ASource is TClientCert then
  begin
    FCert := TClientCert(ASource).FCert;
    FileName := TClientCert(ASource).FileName;
    Stream := TClientCert(ASource).Stream;
    Password := TClientCert(ASource).Password;
  end
  else
...
procedure THTTPReqResp.SetupHttpReq(AHttpRequest: IHTTPRequest);
begin
  if ClientCertificate.FileName <> '' then
    AHttpRequest.SetClientCertificate(ClientCertificate.FileName, ClientCertificate.Password)
  else if ClientCertificate.Stream <> nil then
    AHttpRequest.SetClientCertificate(ClientCertificate.Stream, ClientCertificate.Password);
end;
