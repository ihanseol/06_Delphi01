...
procedure SetMonitorHook(NewHook : IIBSQLMonitorHook);
procedure RemoveMonitorHook(OldHook : IIBSQLMonitorHook);
...
type
  ...
  TMonitorList = TList<IIBSQLMonitorHook>;

  { TIBSQLMonitorHook Windows based MMF/Event driven}
...
begin
  if not (SQLMonitor is TIBSQLMonitor) then
    exit;
{$IFDEF MSWINDOWS}
  if not FEventsCreated then
  try
...
begin
{$IFDEF MSWINDOWS}
  if Arg is TIBSQLMonitor then
  begin
    if Assigned(FWriterThread) then
      FWriterThread.ReleaseMonitor(TIBSQLMonitor(Arg).FHWnd)
    else
      PostMessage(TIBSQLMonitor(Arg).FHWnd, CM_RELEASE, 0, 0);
  end;
{$ENDIF}
end;
...
{$IFDEF MSWINDOWS}
  ...
  begin
    ...
    begin
      ...
      begin
        if FMonitors[i] is TIBSQLMonitor then
        begin
          FTemp := TTraceObject.Create(st);
          PostMessage(TIBSQLMonitor(FMonitors[i]).Handle,
...
function MonitorHook: IIBSQLMonitorHook;
begin
  if (MonitorList.Count = 0) and (not bDone) then
  begin
    CS.BeginWrite;
      if (MonitorList.Count = 0) and (not bDone) then
      begin
        MonitorList.Add(TIBSQLMonitorHook.Create);
      end;
    CS.EndWrite;
  end;
  result := MonitorList.Last;
end;
...
var
  idx : Integer;
begin
  idx := MonitorList.IndexOf(OldHook);
  if idx >= 0 then
  begin
...
// If hook is in the list delete it and readd it at the end to make active (Last item is the active one);
procedure SetMonitorHook(NewHook : IIBSQLMonitorHook);
begin
  RemoveMonitorHook(NewHook);
  MonitorList.Add(NewHook);
end;
...
constructor TIBSQLMonitor.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TIBSQLMonitor.Destroy;
begin
{$IFDEF MSWINDOWS}
  if not (csDesigning in ComponentState) then
  begin
...
function TIBSQLMonitor.GetEnabled: Boolean;
begin
  Result := FEnabled;
end;

{$IFDEF MSWINDOWS}
procedure TIBSQLMonitor.MonitorWndProc(var Message: TMessage);
var
  st : TTraceObject;
begin
  case Message.Msg of
...
procedure TIBSQLMonitor.Release;
begin
  MonitorHook.ReleaseMonitor(self);
end;

procedure TIBSQLMonitor.SetEnabled(const Value: Boolean);
begin
...
initialization
  ...
  MonitorList := TMonitorList.Create;
  FWriterThread := nil;
  FReaderThread := nil;
  bDone := False;
