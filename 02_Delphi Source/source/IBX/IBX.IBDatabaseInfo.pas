...
unit IBX.IBDatabaseInfo;

interface

uses
  System.Classes, IBX.IBExternals, IBX.IB, IBX.IBDatabase, IBX.IBIntf;
...
type
  ...
  protected
    ...
    function GetSepEncrypted: TIBSEP;
    function GetPageBuffers: long;
  public
    ...
    property PageBuffers : Long read GetPageBuffers;
    ...
    property PreAllocated : Long read GetPreAllocated;
  published
...
begin
  ...
  result := StringOf(TEncoding.Convert(TEncoding.ANSI, TEncoding.Default, local_buffer,
     6 + Int(local_buffer[4]), Integer(local_buffer[5 + Int(local_buffer[4])])));
end;
...
function TIBDatabaseInfo.GetPreAllocated: Long;
begin
   Result := GetLongDatabaseInfo(isc_info_db_preallocate);
end;
...
begin
  Result :=  GetLongDatabaseInfo(isc_info_db_encrypted) = 1;
end;

function TIBDatabaseInfo.GetEUAActive: Boolean;
begin
  Result := GetLongDatabaseInfo(isc_info_db_eua_active) = 1;
end;
...
begin
  ...
  result := GetGDSLibrary.isc_portable_integer(@local_buffer[3], length);
end;

function TIBDatabaseInfo.GetSepEncrypted: TIBSEP;
var
  sep : Integer;
begin
  sep :=  GetLongDatabaseInfo(isc_info_db_sep_external);
  if sep = 1 then
    Result := sepExternal
  else if sep = 2 then
    Result := sepInternal
...
function TIBDatabaseInfo.GetPageBuffers: Long;
begin
  Result := GetLongDatabaseInfo(isc_info_set_page_buffers);
end;
