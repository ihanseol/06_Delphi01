...
uses
  ...
  System.Generics.Collections, IBX.IBInputOutput;
...
type
  ...
  public
    ...
    function GetLiveModes : TLiveModes; virtual; abstract;
  published
  ...
  // This data is consistant for all data in that row.  No need to store
  //    it with each field for each row
  ...
  // Common row information.
  TRowData = packed Record
    rdFieldCount: Short;
    rdColumns : Array[1..1] of TColumnData;
  end;
  PRowData = ^TRowData;
...
{$ENDIF NEXTGEN}
    ...
    procedure SetBufferChunks(Value: Int64);
    ...
    function AdjustPosition(FCache: TRecBuf; Offset: NativeInt;
                            Origin: Integer): NativeInt;
    procedure ReadCache(FCache: TRecBuf; Offset: NativeInt; Origin: Integer;
                       Buffer: TRecBuf);
    procedure ReadRecordCache(RecordNumber: NativeInt; Buffer: TRecBuf;
                              ReadOldBuffer: Boolean);
    procedure WriteCache(FCache: TRecBuf; Offset: NativeInt; Origin: Integer;
                        Buffer: TRecBuf);
    procedure WriteRecordCache(RecordNumber: NativeInt; Buffer: TRecBuf);
    ...
    procedure SetPrecommittedReads(const Value: Boolean);

  protected
    function GetPrepared: Boolean;
    procedure ActivateConnection;
    function ActivateTransaction(ForEdits : Boolean = false): Boolean;
...
{$ENDIF !NEXTGEN}
  ...
  protected
    ...
    [default (false)]
    property PrecommittedReads : Boolean read FPrecommittedReads write SetPrecommittedReads;
...
{$ENDIF !NEXTGEN}
  ...
  function ChangeState(Field : TField) : TIBChangeState; inline;
...
const
  ...
  SQL_BYTES = 1; // representing SQL_VARYING or SQL_TEXT but charset of OCTETS

function ChangeState(Field : TField) : TIBChangeState;
begin
  if Field.DataSet is TIBCustomDataset then
    Result := TIBCustomDataset(Field.DataSet).ChangeState(Field)
  else
    Result := csUnknown;
end;
...
begin
  ...
  FPreBase.BeforeTransactionEnd := DoBeforeTransactionEnd;
...
begin
  InputObject.Database := Database;
  InputObject.Transaction := Transaction;
  InputObject.SQL := FQSelect.SQL;
  InputObject.BatchInput;
end;

procedure TIBCustomDataSet.InternalBatchOutput(OutputObject: TIBBatchOutput);
begin
  OutputObject.Database := Database;
  OutputObject.Transaction := Transaction;
  OutputObject.SQL := FQSelect.SQL;
  OutputObject.BatchOutput;
end;
...
begin
  ...
  begin
    Database.Open;
    if FPrecommittedReads then
      // The act of pulling it starts the transaction if needed
      Database.PrecommittedTransaction;
  end;
end;

function TIBCustomDataSet.ActivateTransaction(ForEdits : Boolean): Boolean;
begin
  Result := False;
  if not Assigned(Transaction) then
    IBError(ibxeTransactionNotAssigned, [nil]);
  if not FPrecommittedReads then
  begin
  ...
  begin
    // Ensure that the read only is still active (it never should not be)
    if not FQSelect.Transaction.InTransaction then
      FQSelect.Transaction.AutoStartTransaction;
    if ForEdits then
      if not Transaction.Active then
      begin
        Result := True;
...
/// <summary> Returns the change state for the current record for InterBase subscriptions
/// </summary>
/// <param Field="Field">The field whose change state you want to look at
/// </param>
/// <remarks>
...
/// </returns>
function TIBCustomDataSet.ChangeState(Field: TField): TIBChangeState;
var
  CurrentRecord: PRecordData;
begin
  CurrentRecord := PRecordData(GetActiveBuf);
  if (Field.FieldNo < 0) then
    Result := csSame
  else
  if (FMappedFieldPosition[Field.FieldNo - 1] > 0) and
     (FMappedFieldPosition[Field.FieldNo - 1] <= FRowData^.rdFieldCount) then
    result := CurrentRecord^.rdFields[FMappedFieldPosition[Field.FieldNo - 1]].fdChangeStatus
  else
    Result := csSame;
end;
...
/// <summary> Returns the change state for the current record for InterBase subscriptions
/// </summary>
/// <param Index="Index">The field index you want to look at the change state for.
/// </param>
/// <remarks>
/// This is the same as calling The TField version passing the result of Fields[i].
/// Exceptions raised will be the same as invalid indexes passed to Fields[i].
/// Caluculated fields and fields not part of the dataset always return csSame.
/// </remarks>
/// <returns>The IBX.TIBChangeState for the associated TField.
/// </returns>
function TIBCustomDataSet.ChangeState(Index: Integer): TIBChangeState;
begin
  Result := ChangeState(Fields[Index]);
end;

/// <summary> Returns the change state for the current record for InterBase subscriptions
/// </summary>
/// <param FieldName="String">The field name you want to look at the change state for.
/// </param>
/// <remarks>
/// This is the same as calling The TField version passing the result of FieldByName.
/// Exceptions raised will be the same as invalid indexes passed to FieldByName.
/// Caluculated fields and fields not part of the dataset always return csSame.
/// </remarks>
/// <returns>The IBX.TIBChangeState for the associated TField.
/// </returns>
function TIBCustomDataSet.ChangeState(FieldName: String): TIBChangeState;
begin
  Result := ChangeState(FieldByName(FieldName));
end;
...
begin
  if (not FPrecommittedReads) or
     (Sender = FPreBase) then
  begin
    if Active then
      Active := False;
...
{ Read the record from FQSelect.Current into the record buffer
  Then write the buffer to in memory cache.  RecordNumber of -1
  indicates setting up the model record.  During this setup the
  FRowData information because it is the same for all rows. }
{$IFDEF NEXTGEN}
...
begin
  ...
  if RecordNumber < 0 then
    FRowData^.rdFieldCount := Qry.Current.Count;
  ...
  begin
    ...
    begin
      if RecordNumber < 0 then
      begin
        FRowData^.rdColumns[j].fdDataType := Qry.Current[i].Data.SqlDef;
        FRowData^.rdColumns[j].fdDataScale := Qry.Current[i].Data.sqlscale;
        FRowData^.rdColumns[j].fdNullable := (Qry.Current[i].Data.sqltype and 1 = 1);
        FRowData^.rdColumns[j].fdCharSetSize := Qry.Current[i].CharSetSize;
      end;

      p^.rdFields[j].fdIsNull :=
           (FRowData^.rdColumns[j].fdNullable and (Qry.Current[i].Data.sqlind^ < 0));
      p^.rdFields[j].fdChangeStatus := Qry.Current[i].ChangeState;
      // Only the Model needs this set, otherwise set in AllocatBuffer
      //   when the model is copied to the record.
      if RecordNumber < 0 then
        FRowData^.rdColumns[j].fdPersistedFloatField := false;
      LocalData := Qry.Current[i].Data.sqldata;
      // Look ar the original.  Because of Octets using a specially defined type
      //   FRowData^.rdColumns[j].fdDataType will not necessarily match the
      //   true SqlDef
      case Qry.Current[i].Data.SqlDef of
        SQL_TIMESTAMP:
        begin
        ...
        SQL_SHORT, SQL_LONG:
        begin
          if (FRowData^.rdColumns[j].fdDataScale = 0) then
          begin
            ...
            if (FRowData^.rdColumns[j].fdDataScale >= (-4)) then
            begin
            ...
            begin
              ...
              begin
                ...
                if Assigned(aField) and
                  ...
                  FRowData^.rdColumns[j].fdPersistedFloatField := true;
              end;
              if FRowData^.rdColumns[j].fdPersistedFloatField then
              begin
        ...
        begin
          if (Qry.Current[i].SQLVar.SQLSubtype and $FF) = 1 then
          begin
            p^.rdFields[j].fdDataSize := Qry.Current[i].Data.sqllen;
            p^.rdFields[j].fdDataLength := Qry.Current[i].Data.SqlLen;
            FRowData^.rdColumns[j].fdDataType := SQL_BYTES;
            SetLength(LocalBytes, p^.rdFields[j].fdDataSize);
            if RecordNumber >= 0 then
              LocalBytes := Qry.Current[i].AsBytes;
            p^.rdFields[j].fdDataLength := Length(LocalBytes);
            if (p^.rdFields[j].fdDataLength = 0) then
              LocalData := nil
            else
              LocalData := Pointer(LocalBytes);
          end
          else
          begin
            ...
            if Qry.Current[i].Data.RelName = '' then
              ...
              p^.rdFields[j].fdDataSize := Qry.Current[i].Data.sqllen div FRowData^.rdColumns[j].fdCharSetSize * 2;
            ...
            begin
              ...
              begin
                ...
                //   can not be used.
                if (FRowData^.rdColumns[j].fdDataType = SQL_TEXT) then
                  // computed Char fields return un encoded (IB Bug)
                  if Qry.Current[i].Data.RelName = '' then
                  begin
                    p^.rdFields[j].fdDataSize := Qry.Current[i].Data.sqllen * SizeOf(Char);
      ...
      begin
        ...
        FRowData^.rdColumns[j].fdDataOfs := FRecordSize;
        Inc(FRecordSize, p^.rdFields[j].fdDataSize);
      end
      else
      begin
        if (FRowData^.rdColumns[j].fdDataType = SQL_VARYING) or
           (FRowData^.rdColumns[j].fdDataType = SQL_TEXT) or
           (FRowData^.rdColumns[j].fdDataType = SQL_BYTES) then
        begin
          if LocalData <> nil then
            Move(LocalData^, PByte(Buffer)[FRowData^.rdColumns[j].fdDataOfs], p^.rdFields[j].fdDataLength)
        end
        else
          Move(LocalData^, PByte(Buffer)[FRowData^.rdColumns[j].fdDataOfs], p^.rdFields[j].fdDataSize)
      end;
    end;
  end;
...
begin
  case State of
    ...
    dsNewValue:
      result := ActiveBuffer;
    dsOldValue:
      if (PRecordData(ActiveBuffer)^.rdSavedOffset <> -1) then
      begin
        ...
        if (PRecordData(ActiveBuffer)^.rdRecordNumber = PRecordData(FOldBuffer)^.rdRecordNumber) then
          result := FOldBuffer
        else
          result := ActiveBuffer;
  else
  ...
  for i := 0 to FieldCount - 1 do
    ...
    begin
      ...
      begin
        pbd^[j].Finalize;
        PISC_QUAD(
          PByte(Buff) + FRowData^.rdColumns[k].fdDataOfs)^ :=
          pbd^[j].BlobID;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Qry := FQRefresh;
      if (not Transaction.InTransaction) and PrecommittedReads then
      begin
        if Qry.Transaction <> QSelect.Transaction then
          Qry.Transaction := QSelect.Transaction;
      end
      else
      begin
        if Qry.Transaction <> Transaction then
          Qry.Transaction := Transaction;
      end;
      SetInternalSQLParams(Qry, Buff);
      Qry.ExecQuery;
      try
        ...
        begin
          ...
          FetchCurrentRecordToBuffer(Qry,
                                     ...
                                     Buff);
          if FCachedUpdates and (ofs <> -1) then
          begin
...
var
  ...
  procedure PrepareSQL(iSQL : TIBSQL; lm : TLiveMode);
  begin
    try
...
begin
  ...
  try
    ActivateConnection;
    DidActivate := ActivateTransaction(true);
    FBase.CheckDatabase;

    //In the case that precommittedtransaction is true, and
    //  it thought it needed to activate the primary transaction
    //  but because autostart was off on the transaction start it
    //  long enough to prepare the queries then deactivate it at the end.
    //  I am takign advantage of the fact that IBSQL does not unprepare
    //  itself when its transaction closes.
    if FPrecommittedReads and DidActivate and
    ...
    begin
      ...
      FLiveMode := [];
      if Assigned(FUpdateObject) then
        FLiveMode := FUpdateObject.GetLiveModes
      else
      begin
        PrepareSQL(FQDelete, lmDelete);
        PrepareSQL(FQInsert, lmInsert);
        PrepareSQL(FQModify, lmModify);
        PrepareSQL(FQRefresh, lmRefresh);
      end;
...
{$ENDIF NEXTGEN}
    try
      if (PRecordData(Buffer)^.rdSavedOffset = -1) then
      begin
...
procedure TIBCustomDataSet.SetBufferChunks(Value: Int64);
begin
  if (Value <= 0) then
    FBufferChunks := BufferCacheSize
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FQSelect.Transaction := Value.PrecommittedTransaction;
        FPreBase.Transaction := Value.PrecommittedTransaction;
      end
      else
      begin
        FQSelect.Transaction := nil;
        FPreBase.Transaction := nil;
      end;
    end
    else
      FPreBase.Transaction := nil;
...
{$ENDIF NEXTGEN}
      ...
      begin
        ...
        begin
          ...
          data := cr + FRowData^.rdColumns[j].fdDataOfs;
          case FRowData^.rdColumns[j].fdDataType of
            ...
            SQL_BYTES :
            begin
              SetLength(b, PRecordData(cr)^.rdFields[j].fdDataLength);
              move(Data[0], b[0], PRecordData(cr)^.rdFields[j].fdDataLength);
              Qry.Params[i].AsBytes := b;
            end;
...
procedure TIBCustomDataSet.SetPrecommittedReads(const Value: Boolean);
begin
  if FPrecommittedReads <> Value then
  begin
    FPrecommittedReads := Value;
    if Value then
    begin
      if Assigned(Database) then
      begin
        FQSelect.Transaction := Database.PrecommittedTransaction;
        FPreBase.Transaction := Database.PrecommittedTransaction;
      end
      else
      begin
        FQSelect.Transaction := nil;
...
function TIBCustomDataSet.AdjustPosition(FCache: TRecBuf; Offset: NativeInt;
                                        Origin: Integer): NativeInt;
var
  OldCacheSize: NativeInt;
begin
  ...
  begin
    case Origin of
      ...
      BUFFER_END:      FBPos := FBEnd + Offset;
    end;
    OldCacheSize := FCacheSize;
    while (FBPos >= FCacheSize) do
    ...
    case Origin of
      ...
      BUFFER_END:      FOBPos := FOBEnd + Offset;
    end;
    OldCacheSize := FOldCacheSize;
    while (FBPos >= FOldCacheSize) do
...
procedure TIBCustomDataSet.ReadCache(FCache: TRecBuf; Offset: NativeInt; Origin: Integer;
                                    Buffer: TRecBuf);
var
  pCache: PByte;
  bOld: Boolean;
begin
  ...
  if not bOld then
    pCache := PByte(FBufferCache) + NativeInt(pCache)
  else
    pCache := PByte(FOldBufferCache) + NativeInt(pCache);
  Move(pCache^, PByte(Buffer)[0], DWORD(FRecordBufferSize));
  AdjustPosition(FCache, FRecordBufferSize, BUFFER_CURRENT);
end;

procedure TIBCustomDataSet.ReadRecordCache(RecordNumber: NativeInt; Buffer: TRecBuf;
                                          ReadOldBuffer: Boolean);
begin
  ...
  begin
    ...
    if FCachedUpdates and
      (PRecordData(Buffer)^.rdSavedOffset <> -1) then
...
procedure TIBCustomDataSet.WriteCache(FCache: TRecBuf; Offset: NativeInt; Origin: Integer;
                                     Buffer: TRecBuf);
var
  pCacheOffset: NativeInt;
  pCache : TRecBuf;
  bOld: Boolean;
  dwEnd: NativeInt;
begin
...
procedure TIBCustomDataSet.WriteRecordCache(RecordNumber: NativeInt; Buffer: TRecBuf);
begin
  if RecordNumber >= 0 then
  begin
    if FUniDirectional then
...
begin
  ...
  begin
    ...
    fs.Mode := bmReadWrite;
    SplitToRelCol(Database.SQLDialect, Field.Origin, rel, col);
    fs.RelationName := rel;
    fs.ColumnName := col;
    fs.Database := Database;
    fs.Transaction := FQSelect.Transaction;
    fs.WriteTransaction := Transaction;
...
//    fs.Transaction := Transaction;
    fs.BlobID :=
      PISC_QUAD(@(PByte(Buff)[FRowData^.rdColumns[FMappedFieldPosition[Field.FieldNo - 1]].fdDataOfs]))^;
...
begin
  ...
  begin
    ...
    begin
      ...
      cColData := FRowData^.rdColumns[FMappedFieldPosition[Field.FieldNo - 1]];
      Data := PByte(Buff) + cColData.fdDataOfs;
      if (cColData.fdDataType = SQL_VARYING) or (cColData.fdDataType = SQL_TEXT) then
      begin
        ...
        begin
          ...
          if (cColData.fdDataType = SQL_TEXT) and (not TStringField(Field).FixedChar) then
        ...
        if (cColData.fdDataType = SQL_BYTES) and (Field is TVarBytesField) then
        begin
          // TVarByesField expects first 2 bytes to be the length
          Move(cFieldData.fdDataLength, PByte(Buffer)[0], SizeOf(Word));
          Move(Data^, PByte(Buffer)[SizeOf(Word)], cFieldData.fdDataLength);
        end
        else
...
begin
  ...
  begin
    ...
    if Result then
    begin
      CurrToBCD(System.Currency(Pointer(@Buffer[0])^), lBcd, 32, Field.Size);
...
begin
  if Assigned(Transaction) then
    Transaction.CheckAutoStop;
  If PreCommittedReads then
    QSelect.Transaction.CheckAutoStop;
...
{$ENDIF NEXTGEN}
  if Assigned(FRowData) then
    ReallocMem(FRowData, 0);
  FCurrentRecord := -1;
  FOpen := False;
  FRecordCount := 0;
  FDeletedRecords := 0;
  ...
  try
    ...
    begin
      ...
      case FQSelect.Current[i].Data.sqltype and not 1 of
        ...
        SQL_VARYING, SQL_TEXT:
        begin
          // Is this the special case of an OCTET and there is not a persisted stringfield
          if ((FQSelect.Current[i].SQLVar.SQLSubtype and $FF) = 1) and
             not ((FieldCount > 0) and
                  Assigned(Fields.FindField(FieldAliasName)) and
                  (Fields.FieldByName(FieldAliasName) is TIBStringField)) then
          begin
            if (FQSelect.Current[i].Data.sqltype and not 1) = SQL_VARYING then
...
procedure TIBCustomDataSet.InternalOpen;
  ...
  procedure SetRowData(n: Integer);
  begin
    IBAlloc(FRowData, 0, Sizeof(TRowData) + ((n - 1) * SizeOf(TColumnData)));
  end;
...
begin
  ...
  begin
    ...
    SetRowData(FQSelect.Current.Count);
    {Step 2, 3}
    ...
    if UniDirectional then
      ...
      FBufferChunkSize := FRecordBufferSize * NativeInt(BufferChunks);
    ...
    for i := 0 to FieldCount - 1 do
      ...
      begin
        ...
        begin
          pbd^[j].Finalize;
          PISC_QUAD(
            PByte(Buff) + FRowData^.rdColumns[k].fdDataOfs)^ := pbd^[j].BlobID;
          PRecordData(Buff)^.rdFields[k].fdIsNull := pbd^[j].Size = 0;
        end;
        Inc(j);
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Move(buffer[0], PRecordData(Buff)^.rdFields[FMappedFieldPosition[Field.FieldNo - 1]].fdDataLength, SizeOf(Word));
          Move(Buffer[SizeOf(Word)], Buff[FRowData^.rdColumns[FMappedFieldPosition[Field.FieldNo - 1]].fdDataOfs],
                 PRecordData(Buff)^.rdFields[FMappedFieldPosition[Field.FieldNo - 1]].fdDataSize);
        end
        else
          Move(Buffer[0], Buff[FRowData^.rdColumns[FMappedFieldPosition[Field.FieldNo - 1]].fdDataOfs],
                 PRecordData(Buff)^.rdFields[FMappedFieldPosition[Field.FieldNo - 1]].fdDataSize);
        if (FRowData^.rdColumns[FMappedFieldPosition[Field.FieldNo - 1]].fdDataType = SQL_TEXT) or
           (FRowData^.rdColumns[FMappedFieldPosition[Field.FieldNo - 1]].fdDataType = SQL_VARYING) then
...
begin
    ...
    try
      ...
      FQuery.ExecSQL;
      if FQuery.SQLType in [SQLSelect, SQLSelectForUpdate] then
      begin
        FQuery.FetchAll;
...
begin
  ...
  if TIBDataSet(ResultSet).SQLType in [SQLSelect, SQLSelectForUpdate] then
  begin
...
// ExecSQL does not work on Select SQL.  Therefor temporally switch PrecommittedReads
//   off, execute the SQL, then revert to the original value
procedure TIBDataSet.ExecSQL;
var
  FOldPrecommitted : Boolean;
begin
  FOldPrecommitted := FPrecommittedReads;
  if FOldPrecommitted then
    PrecommittedReads := not FOldPrecommitted;
  try
    InternalExecQuery;
    FRowsAffected := FQSelect.RowsAffected;
  finally
...
function TIBCustomDataSet.GetPrepared: Boolean;
begin
  Result := InternalPrepared or
           (PrecommittedReads and QSelect.Prepared);
end;
...
begin
  ...
  begin
    ...
    RelationName := String(FQSelect.Current[i].Data.relname);
    FieldName := String(FQSelect.Current[i].Data.sqlname);
    // If precommitted reads are on, but a global temp table is in the query,
    //    turn it off
    if PrecommittedReads then
      PrecommittedReads := not Database.IsTempTable(RelationName);
    if not PrecommittedReads then
      ActivateTransaction;

    f := FindField(FieldAliasname);
    if Assigned(f) then
    begin
      if (FieldOptions.UpdatePersistent and (f.LifeCycle = lcPersistent)) or
         (f.LifeCycle <> lcPersistent) then
      begin
...
begin
  OutputObject.OutputXML(QSelect);
end;
