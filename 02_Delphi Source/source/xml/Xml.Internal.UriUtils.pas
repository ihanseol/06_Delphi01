...
type
  TUtilsFilenameToUriOption = (fuSetLocalhost, fuPlainColon);
  TUtilsFilenameToUriOptions = set of TUtilsFilenameToUriOption;
...
{$ENDIF NEXTGEN}
  Xml.Internal.AbnfUtils, System.Classes;
...
begin
  ...
  begin
    ...
    begin
      ...
      Inc(I);
      if not IsUriHexWideChar(S[I-AdjustIndex]) then
      begin
        Result := False;
        Exit;
      end;
    ...
    else if not (IsUriUnreservedWideChar(S[I-AdjustIndex])
      or (S[I-AdjustIndex] = ';') or (S[I-AdjustIndex] = '@') or (S[I-AdjustIndex] = '&')
      or (S[I-AdjustIndex] = '=') or (S[I-AdjustIndex] = '+') or (S[I-AdjustIndex] = '$')
      or (S[I-AdjustIndex] = ',')) then
    begin
      Result := False;
      Exit;
    end;
...
begin
  ...
  if not IsAbnfALPHAWideChar(S[FirstIndex]) then
  begin
    Result := False;
    Exit;
  end;
  Result := True;
  for I := 1+FirstIndex to L-AdjustIndex do
...
begin
  ...
  if not (IsUriAlphanumWideChar(S[FirstIndex]) and IsUriAlphanumWideChar(S[L-AdjustIndex])) then
  begin
    Result := False;
    Exit;
  end;
...
begin
  ...
  if not (IsUriAlphaWideChar(S[FirstIndex]) and IsUriAlphanumWideChar(S[L-AdjustIndex])) then
  begin
    Result := False;
    Exit;
  end;
...
begin
  ...
  if not isAbnfALPHAChar(S[FirstIndex]) then
  begin
    Result := False;
    Exit;
  end;
...
begin
  ...
  if S[L] = '.' then
    Dec(L);
  I := L;
  while I >= FirstIndex do
  begin
    if S[I] = '.' then
      break;
    Dec(I);
  end;
  if not IsUriToplabelStr(Copy(S, I + 1+FirstIndex, L - I)) then
  begin
    Result := False;
    Exit;
  end;
  ...
  begin
    ...
    if not IsUriDomainlabelStr(Copy(S, I + 1+FirstIndex, L - I)) then
    begin
      Result := False;
      Exit;
    end;
...
begin
  ...
  L := L - AdjustIndex;
  if not (IsUriAlphanumChar(S[FirstIndex]) and IsUriAlphanumChar(S[L])) then
  begin
    Result := False;
    Exit;
  end;
...
begin
  ...
  L := L - AdjustIndex;
  if not (IsUriAlphaChar(S[FirstIndex]) and IsUriAlphanumChar(S[L])) then
  begin
    Result := False;
    Exit;
  end;
...
function ResolveRelativeUriStr(const BaseUri, RelUri: string;
  var ResultUri: string): Boolean;
var
...
begin
  ...
  try
    ...
    if (RelUriAnalyzer.UriPath = '') and not (RelUriAnalyzer.HasUriScheme or RelUriAnalyzer.HasUriAuthority or RelUriAnalyzer.HasUriQuery)
      then
    begin
      // Same document reference detected
      BaseUriAnalyzer.SetUriFragment(RelUriAnalyzer.UriFragment, RelUriAnalyzer.HasUriFragment);
      ResultUri := BaseUriAnalyzer.UriReference;
      Exit;
    end;
    if RelUriAnalyzer.HasUriScheme then
    begin
    ...
    // inherit scheme:
    RelUriAnalyzer.SetUriScheme(BaseUriAnalyzer.UriScheme, BaseUriAnalyzer.HasUriScheme);
    if not RelUriAnalyzer.HasUriAuthority then
    begin
      // inherit Authority:
      RelUriAnalyzer.SetUriAuthority(BaseUriAnalyzer.UriAuthority,
        BaseUriAnalyzer.HasUriAuthority);

      //if not (Copy(RelUriAnalyzer.UriPath, 1, 1) = '/') then
      if not RelUriAnalyzer.UriPath.StartsWith('/') then
      begin
        // analyze Paths:
        Segments := TStringList.Create;
        try
          //SlashPos := LastDelimiter('/', BaseUriAnalyzer.UriPath);
          SlashPos := BaseUriAnalyzer.UriPath.LastIndexOf('/');
          if SlashPos > 0 then
            //PathBuffer := Copy(BaseUriAnalyzer.UriPath, 2, SlashPos - 1)
            PathBuffer := BaseUriAnalyzer.UriPath.Substring( 1, SlashPos)
              // Copy Path without last segment and first Character which is always '/'
          else
            PathBuffer := '';
          PathBuffer := Concat(PathBuffer, RelUriAnalyzer.UriPath);
          // cut PathBuffer into Segments:
          //SlashPos := Pos('/', PathBuffer);
          SlashPos := PathBuffer.IndexOf('/');
          while SlashPos >= 0 do
          begin
            //Segments.Add(Copy(PathBuffer, 1, SlashPos - 1));
            Segments.Add(PathBuffer.Substring(0, SlashPos));
            //PathBuffer := Copy(PathBuffer, SlashPos + 1, Length(PathBuffer)
            //  - SlashPos);
            PathBuffer := PathBuffer.Substring(SlashPos);
            //SlashPos := Pos('/', PathBuffer);
            SlashPos := PathBuffer.IndexOf('/');
          end; {while ...}
          Segments.Add(PathBuffer);
          if (PathBuffer = '..') or (PathBuffer = '.') then
            Segments.Add('');
              // Necessary to preserve ending '/' under some circumstances
          // remove '.' Segments:
          QueryIndex := Segments.IndexOf('.');
          while QueryIndex > -1 do
          begin
            Segments.Delete(QueryIndex);
            QueryIndex := Segments.IndexOf('.');
          end;
          ...
          begin
            Segments.Delete(QueryIndex);
            Segments.Delete(Pred(QueryIndex));
            QueryIndex := Segments.IndexOf('..');
          end;
          // test for malformed Path:
          if Segments.Count > 0 then
            if Segments.Strings[0] = '..' then
            begin
              Result := False;
              Exit;
            end;
          ...
          for I := 0 to Pred(Segments.Count) do
            PathBuffer := Concat(PathBuffer, '/', Segments.Strings[I]);
          RelUriAnalyzer.SetUriPath(PathBuffer);
        finally
          Segments.Free;
        end;
      end; {if not (Copy(UriPath,1,1) = '/') ...}
    end; {if not HasAuthorityScheme ...}
    ResultUri := RelUriAnalyzer.UriReference;
  finally
    ...
    begin
      ...
      begin
        ...
        try
          ...
          // Test for malformed Path:
          if Segments.Count > 0 then
            if Segments.WideStrings[0] = '..' then
            begin
              Result := False;
              Exit;
            end;
          PathBuffer := '';
          for I := 0 to Pred(Segments.Count) do
            PathBuffer := Concat(PathBuffer, '/', Segments.WideStrings[I]);
          RelUriAnalyzer.SetUriPath(PathBuffer);
        finally
          Segments.Free;
        end;
...
begin
  ...
  try
    if UriAnalyzer.SetUriReference(uri) then
    begin
      if CompareText(UriAnalyzer.UriScheme, 'file') = 0 then
      begin
        Result := True;
        PathBuffer := UriAnalyzer.UriPath;
        L := PathBuffer.Length;
        if L > 0 then
        begin
          // remove leading '/':
          Dec(L);
          PathBuffer := PathBuffer.Substring(1, L);
          I := 0;
          while I < L do
          begin
            if PathBuffer.Chars[I] = '%' then
            begin
              // Resolve escape sequence:
              Path := Concat(Path, Chr(StrToInt(Concat('x', PathBuffer.Chars[I + 1],
                PathBuffer.Chars[I + 2]))));
              I := I + 2;
            end
{$IFNDEF LINUX}
            // translate '/' to '\' for Windows filenames:
            else if PathBuffer.Chars[I] = '/' then
            begin
              Path := Concat(Path, '\');
            end
{$ENDIF}
        ...
        Authority := UriAnalyzer.UriAuthority;
        if UriAnalyzer.HasUriQuery then
          Query := Concat('?', UriAnalyzer.UriQuery);
        if UriAnalyzer.HasUriFragment then
          Fragment := Concat('#', UriAnalyzer.UriFragment);
      end; {if ...}
    end; {if ...}
  finally
    UriAnalyzer.Free;
