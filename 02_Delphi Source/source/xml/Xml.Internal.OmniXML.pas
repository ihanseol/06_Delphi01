...
begin
  ...
  if not LStrEncoding.IsEmpty and (string.CompareText(LStrEncoding, 'utf-8') <> 0) then
  ...
  try
    ...
    if (LEncoding <> XTS.Encoding) and not TEncoding.IsStandardEncoding(LEncoding) then
      LEncoding.Free;
    FreeAndNil(XTS);
    FURL := '';
  end;
