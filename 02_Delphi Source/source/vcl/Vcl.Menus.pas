...
{$ENDIF}
  ...
  public
    ...
    property MenuHandle: TMenuHandle read FHandle;
{$IF DEFINED(CLR)}
...
begin
  ...
  begin
    ...
    if MenuItem.Default then
      Font.Style := Font.Style + [fsBold];
    TStyleManager.ActiveStyle.DrawText(ACanvas.Handle,
      TStyleManager.ActiveStyle.GetElementDetails(MenuStates[TopLevel, Selected, MenuItem.Enabled]), LCaption, Rect, Flags);
  end;
end;
        ...
        begin
          ...
          begin
            if Checked and not TopLevel then
              LStyle.DrawElement(ACanvas.Handle, LStyle.GetElementDetails(CheckMarkBkgStates[Enabled]), GlyphRect);
            with GlyphRect do
...
{$ENDIF}
  ...
  begin
    ...
    var LStyle := TStyleManager.ActiveStyle;
...
function TPopupMenu.GetMenuHandle: TMenuHandle;
begin
  if Assigned(FItems) then
    Result := FItems.MenuHandle
  else
    Result := 0;
end;

function TPopupMenu.HandleAllocated: Boolean;
begin
  Result := Assigned(FItems) and FItems.HandleAllocated;
end;
