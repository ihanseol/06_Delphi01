...
uses
  ...
  Vcl.Buttons, Vcl.ActnList, System.UITypes, System.Generics.Collections;
...
type
  ...
  private
    ...
    FSmoothMouseWheelScrolling: Boolean;
    ...
    procedure SetSelected(Index: Integer; const AValue: Boolean);
    procedure InitSelection;
    function IsFirstConseqSel(AIndex: Integer): Boolean;
    function IsLastConseqSel(AIndex: Integer): Boolean;
    procedure LimitMaximumItemCount(AUpdate: Boolean);
  ...
  public
    ...
    /// <summary>
    /// Deselects all list items, including current item.
    /// </summary>
    ...
    procedure SelectAll;
    /// <summary>
    /// Returns an enumerator of selected item indexes.
    /// </summary>
    ...
    /// <summary>
    /// Specifies whether the user can select more than one list item.
    /// </summary>
    ...
    /// <summary>
    /// Specifies whether scrolling by mouse wheel is performed smoothly by pixels
    /// instead of by full item height or width.
    /// </summary>
    property SmoothMouseWheelScrolling: Boolean read FSmoothMouseWheelScrolling
      write FSmoothMouseWheelScrolling default False;
    /// <summary>
  ...
  published
    ...
    /// </summary>
    property LinkHotColor: TColor read FLinkHotColor write FLinkHotColor default clHighLight;
    /// <summary>
  ...
  private
    ...
    procedure SetCheckState(Value: TCheckBoxState);
  protected
    function CanObserve(const ID: Integer): Boolean; override;
  ...
  published
    ...
    /// If AllowGrayed is set to True, the check box has three possible states: selected, cleared, and dimmed.
    /// If AllowGrayed is set to False, the check box has only two possible states: selected and cleared.
    /// </summary>
    ...
    /// Use CheckState to determine whether the check box is selected (cbChecked), cleared (cbUnchecked), or dimmed (cbGrayed).
    /// </summary>
  ...
  /// </summary>
  [ObservableMember('ItemIndex', True)]
  ...
  published
    ...
    /// <summary>
    /// Returns index of a checked radio button in the group. When no radio button is checked
    /// then returns -1. When assigned value is equal to the index of this radio button in the
    /// group, then Checked is set to True, otherwise to False.
    /// </summary>
...
{ TControlListItemSelectionOptions }
...
begin
  ...
  begin
    if MultiSelect and Selected[AIndex] or
       not MultiSelect and (AIndex = FItemIndex) and (FItemIndex <> -1) then
    begin
...
procedure TCustomControlList.LimitMaximumItemCount(AUpdate: Boolean);
var
  LRows, LHeight, LCount: Integer;
begin
  LRows := GetRows;
  LHeight := GetItemHeight(True);
  if Int64(LRows) * Int64(LHeight) > MaxInt then
  begin
    LCount := MaxInt div LHeight;
    if AUpdate then
      ItemCount := LCount
    else
      FItemCount := LCount;
...
function TCustomControlList.IsLastConseqSel(AIndex: Integer): Boolean;
var
  I, LFirst: Integer;
begin
  for I := AIndex + 1 to ItemCount - 1 do
    if Selected[I] then
      Exit(False);
  LFirst := -1;
  ...
  if LFirst = -1 then
    Exit(False);
  for I := LFirst - 1 downto 0 do
    if Selected[I] then
      Exit(False);
  Result := True;
end;

function TCustomControlList.IsFirstConseqSel(AIndex: Integer): Boolean;
var
  I, LLast: Integer;
begin
  for I := AIndex - 1 downto 0 do
    if Selected[I] then
      Exit(False);
  LLast := -1;
  for I := AIndex to ItemCount - 1 do
    if not Selected[I] then
      Break
    else
      LLast := I;
  if LLast = -1 then
    Exit(False);
  for I := LLast + 1 to ItemCount - 1 do
    if Selected[I] then
      Exit(False);
  Result := True;
end;
...
var
  ...
  function GetControlAtPos(AX, AY: Integer): TControl;
  var
    LItemIndex: Integer;
    LItemRect: TRect;
  begin
    LItemIndex := ItemAtPos(AX, AY, LItemRect);
    if LItemIndex >= 0 then
    begin
      Dec(AX, LItemRect.Left);
      Dec(AY, LItemRect.Top);
      Result := ItemControlAtPos(LItemIndex, AX, AY);
    end
    else
      Result := nil;
    if Result = nil then
  ...
  case Message.Msg of
    ...
    WM_LBUTTONDOWN, WM_LBUTTONDBLCLK:
      ...
      begin
        ...
        LControl := GetControlAtPos(TWMMouse(Message).XPos, TWMMouse(Message).YPos);
        if (TControlHookClass(LControl).DragMode = dmAutomatic) or
           (LControl <> Self) and (DragMode = dmAutomatic) then
        begin
          ControlState := ControlState + [csLButtonDown];
          Dispatch(Message);

          if TControlHookClass(LControl).DragMode = dmAutomatic then
            TControlHookClass(LControl).BeginAutoDrag;
    ...
    CM_HINTSHOW:
      begin
        ...
        LControl := GetControlAtPos(LpHintInfo^.CursorPos.X, LpHintInfo^.CursorPos.Y);
    ...
    WM_SETCURSOR:
      if (Word(Message.LParam) = HTCLIENT) and not (csDesigning in ComponentState) then
      begin
        GetCursorPos(P);
        P := ScreenToClient(P);
        LControl := GetControlAtPos(P.X, P.Y);
        if LControl.Cursor <> crDefault then
        begin
          Winapi.Windows.SetCursor(Screen.Cursors[LControl.Cursor]);
          Message.Result := 1;
          Exit;
        end;
      end;
...
var
  ...
  LScrollCount: Integer;
begin
  inherited;
  LOldScrollPos := FScrollPos;

  if SmoothMouseWheelScrolling then
  begin
    LScrollCount := (Mouse.WheelScrollLines * Message.WheelDelta * 4) div (WHEEL_DELTA);
    Dec(FScrollPos, LScrollCount);
  end
  else if InternalColumnLayout = cltMultiLeftToRight then
  begin
...
var
  LItemIndex,
  LPrevItemIndex, I: Integer;
  LItemRect: TRect;
  LControl: TControl;
  LPrevSelected: Boolean;
...
begin
  ...
  begin
    ...
    LMultiSelect := MultiSelect and (Button = mbLeft) and not (ssDouble in Shift);
    LPrevSelected := Selected[LItemIndex];
    LPrevItemIndex := ItemIndex;
    if LMultiSelect and ([ssCtrl, ssShift] * Shift = []) then
      InitSelection;
    ...
    begin
      ...
      if LMultiSelect then
        if (ssShift in Shift) and (LPrevItemIndex <> - 1) then
        begin
          if LPrevItemIndex < ItemIndex then
            for I := LPrevItemIndex to ItemIndex do
              Selected[I] := True
          else
            for I := ItemIndex to LPrevItemIndex do
              Selected[I] := True;
        end
        else if ssCtrl in Shift then
        begin
          if LPrevItemIndex <> -1 then
            Selected[LPrevItemIndex] := True;
          Selected[ItemIndex] := not LPrevSelected;
        end;

      DoItemClicked;
    end
    else if LMultiSelect and (ssCtrl in Shift) then
    begin
      Selected[ItemIndex] := not LPrevSelected;
      DoItemClicked;
    end;
...
begin
  ...
  begin
    ...
    if MultiSelect then
    begin
      LShift := KeyDataToShiftState(0);
      if not (ssShift in LShift) then
        InitSelection
      else if IsLastConseqSel(LPrevItemIndex) then
        LSelMode := 1
      else if IsFirstConseqSel(LPrevItemIndex) then
        LSelMode := -1
      else
      begin
        InitSelection;
    ...
    if LSelMode <> 0 then
      for I := LPrevItemIndex to ItemIndex - 1 do
        Selected[I] := LSelMode = 1;
...
begin
  ...
  begin
    ...
    if LSelMode <> 0 then
      for I := LPrevItemIndex downto ItemIndex + 1 do
        Selected[I] := LSelMode = 1;
...
begin
  ...
  case Key of
    ...
    VK_END:   FindLastItem;
    Ord('A'):
      if (Shift = [ssCtrl]) and MultiSelect then
      begin
        SelectAll;
        DoItemClicked;
      end;
...
procedure TCustomControlList.SetMultiSelect(const AValue: Boolean);
begin
...
function TCustomControlList.GetSelected(Index: Integer): Boolean;
var
  LSel: Boolean;
begin
  if MultiSelect then
    Result := FSelected.TryGetValue(Index, LSel) and LSel or (Index = ItemIndex)
  else
    Result := Index = ItemIndex;
end;

procedure TCustomControlList.SetSelected(Index: Integer; const AValue: Boolean);
var
  LSel: Boolean;
begin
  if Selected[Index] <> AValue then
  begin
    if MultiSelect then
    begin
      if AValue then
      begin
        if not (FSelected.TryGetValue(Index, LSel) and LSel) then
          FSelected.AddOrSetValue(Index, True);
      end
      else
      begin
        FSelected.Remove(Index);
...
procedure TCustomControlList.InitSelection;
begin
  FSelected.Clear;
  Invalidate;
end;

procedure TCustomControlList.ClearSelection;
begin
  InitSelection;
  ItemIndex := -1;
end;

procedure TCustomControlList.SelectAll;
var
  I: Integer;
  LSel: Boolean;
begin
  ...
  begin
    for I := 0 to ItemCount - 1 do
      if not FSelected.TryGetValue(I, LSel) then
        FSelected.Add(I, True)
      else if not LSel then
        FSelected.AddOrSetValue(I, True);
    Invalidate;
  end;
end;

function TCustomControlList.GetSelectedCount: Integer;
var
  LSelItem: TPair<Integer, Boolean>;
begin
  Result := 0;
  if MultiSelect then
    for LSelItem in FSelected do
      if LSelItem.Value and (LSelItem.Key <> ItemIndex) then
        Inc(Result);
  if ItemIndex >= 0 then
    Inc(Result);
end;
...
type
  TSelectedEnumerator = class(TInterfacedObject, IEnumerator, IEnumerator<Integer>,
    IEnumerable<Integer>)
  private
    FControlList: TCustomControlList;
    FSelectedList: TList<Integer>;
    FEnumerator: TEnumerator<Integer>;
  protected
    procedure Reset;
    function MoveNext: Boolean;
    function GetCurrent: TObject;
    function GetCurrentInt: Integer;
    function IEnumerator<Integer>.GetCurrent = GetCurrentInt;
    function GetEnumerator: IEnumerator;
    function GetEnumeratorInt: IEnumerator<Integer>;
    function IEnumerable<Integer>.GetEnumerator = GetEnumeratorInt;
  public
    constructor Create(const AControlList: TCustomControlList);
    destructor Destroy; override;
  end;

constructor TSelectedEnumerator.Create(const AControlList: TCustomControlList);
begin
  inherited Create;
  FControlList := AControlList;
  FSelectedList := TList<Integer>.Create;
  Reset;
end;

destructor TSelectedEnumerator.Destroy;
begin
...
procedure TSelectedEnumerator.Reset;
var
  LSelItem: TPair<Integer, Boolean>;
begin
  FreeAndNil(FEnumerator);
  FSelectedList.Clear;
  if FControlList.MultiSelect then
    for LSelItem in FControlList.FSelected do
      if LSelItem.Value and (LSelItem.Key <> FControlList.ItemIndex) then
        FSelectedList.Add(LSelItem.Key);
  if (FControlList.ItemIndex >= 0) and not FSelectedList.Contains(FControlList.ItemIndex) then
    FSelectedList.Add(FControlList.ItemIndex);
  FSelectedList.Sort;
  FEnumerator := FSelectedList.GetEnumerator;
end;

function TSelectedEnumerator.MoveNext: Boolean;
begin
  Result := FEnumerator.MoveNext;
end;

function TSelectedEnumerator.GetCurrent: TObject;
begin
  Result := nil;
end;

function TSelectedEnumerator.GetEnumerator: IEnumerator;
begin
  Result := nil;
end;

function TSelectedEnumerator.GetCurrentInt: Integer;
begin
  Result := FEnumerator.Current;
end;

function TSelectedEnumerator.GetEnumeratorInt: IEnumerator<Integer>;
begin
  Result := Self;
end;

function TCustomControlList.GetSelectedEnumerator: IEnumerable<Integer>;
begin
  Result := TSelectedEnumerator.Create(Self);
end;
...
procedure TControlListControl.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  FMouseDown := True;
  InvalidateWithItem;
  inherited;
...
procedure TControlListControl.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  FMouseDown := False;
  InvalidateWithItem;
  inherited;
...
begin
  inherited Create(AOwner);
  FCheckState := TCheckBoxState.cbUnchecked;
  Width := DefaultCheckboxSize;
  Height := DefaultCheckboxSize;
  ControlStyle := ControlStyle + [csNeedsDesignDisabledState] - [csDoubleClicks, csSetCaption];
end;

function TControlListCheckBox.GetChecked: Boolean;
begin
  Result := CheckState = cbChecked;
end;

procedure TControlListCheckBox.SetChecked(Value: Boolean);
begin
  if Value then
...
procedure TControlListCheckBox.SetCheckState(Value: TCheckBoxState);
begin
...
function TControlListCheckBox.CanObserve(const ID: Integer): Boolean;
begin
  Result := False;
  if ID = TObserverMapping.EditLinkID then
    Result := True
...
procedure TControlListCheckBox.ObserverAdded(const ID: Integer;
...
procedure TControlListCheckBox.ObserverToggle(const AObserver: IObserver;
  const Value: Boolean);
var
...
procedure TControlListCheckBox.Toggle;
begin
  ...
  case CheckState of
    cbUnchecked:
      if AllowGrayed then CheckState := cbGrayed else CheckState := cbChecked;
    cbChecked: CheckState := cbUnchecked;
    cbGrayed: CheckState := cbChecked;
  end;
...
procedure TControlListCheckBox.Click;
begin
  Toggle;
  inherited Changed;
  inherited Click;
...
const
  C_GRAYED_MARGIN = 4;
var
  LVertMargin, LHorzMargin: Integer;
  LStyle: TCustomStyleServices;
  LDetails: TThemedElementDetails;
  LPaintRect: TRect;
  LDrawFlags: Integer;
begin
  ...
  LVertMargin := (Height - ScaleValue(DefaultCheckboxSize)) div 2;
  LHorzMargin := (Width - ScaleValue(DefaultCheckboxSize)) div 2;
  LPaintRect := Rect(LHorzMargin, LVertMargin,
    LHorzMargin + ScaleValue(DefaultCheckboxSize),
    LVertMargin + ScaleValue(DefaultCheckboxSize));
  LStyle := StyleServices(Self);
  if LStyle.Enabled then
  begin
    case CheckState of
      cbUnchecked:
        case State of
          clstNormal:   LDetails := LStyle.GetElementDetails(tbCheckBoxUncheckedNormal);
          clstHot:      LDetails := LStyle.GetElementDetails(tbCheckBoxUncheckedHot);
          clstPressed:  LDetails := LStyle.GetElementDetails(tbCheckBoxUncheckedPressed);
          clstDisabled: LDetails := LStyle.GetElementDetails(tbCheckBoxUncheckedDisabled);
        end;
      cbChecked:
        case State of
          clstNormal:   LDetails := LStyle.GetElementDetails(tbCheckBoxCheckedNormal);
          clstHot:      LDetails := LStyle.GetElementDetails(tbCheckBoxCheckedHot);
          clstPressed:  LDetails := LStyle.GetElementDetails(tbCheckBoxCheckedPressed);
          clstDisabled: LDetails := LStyle.GetElementDetails(tbCheckBoxCheckedDisabled);
        end;
      cbGrayed:
        case State of
          clstNormal:   LDetails := LStyle.GetElementDetails(tbCheckBoxMixedNormal);
          clstHot:      LDetails := LStyle.GetElementDetails(tbCheckBoxMixedHot);
          clstPressed:  LDetails := LStyle.GetElementDetails(tbCheckBoxMixedPressed);
          clstDisabled: LDetails := LStyle.GetElementDetails(tbCheckBoxMixedDisabled);
        end;
    end;
    LStyle.DrawElement(Canvas.Handle, LDetails, LPaintRect, nil, CurrentPPI);
  end
  else
  begin
    LDrawFlags := DFCS_BUTTONCHECK or DFCS_ADJUSTRECT;
    if CheckState = cbChecked then
      LDrawFlags := LDrawFlags or DFCS_CHECKED;
    if not Enabled then
      LDrawFlags := LDrawFlags or DFCS_INACTIVE;
    DrawFrameControl(Canvas.Handle, LPaintRect, DFC_BUTTON, LDrawFlags);

    if CheckState = cbGrayed then
    begin
      if Enabled then
      ...
      Canvas.FillRect(Rect(
        LVertMargin + ScaleValue(C_GRAYED_MARGIN),
        LVertMargin + ScaleValue(C_GRAYED_MARGIN),
        LPaintRect.Right - LHorzMargin - ScaleValue(C_GRAYED_MARGIN),
        LPaintRect.Bottom - LVertMargin - ScaleValue(C_GRAYED_MARGIN)));
    end;
  end;
end;

{ TControlListRadioButton }

constructor TControlListRadioButton.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Width := DefaultCheckboxSize;
  Height := DefaultCheckboxSize;
...
procedure TControlListRadioButton.SetChecked(Value: Boolean);
var
  LCtrl: TControl;
begin
  if (FChecked <> Value) and Value then
  begin
    FChecked := Value;
    for LCtrl in Parent.GetControls do
      if (LCtrl <> Self) and (LCtrl is TControlListRadioButton) and
         TControlListRadioButton(LCtrl).Checked then
      begin
        TControlListRadioButton(LCtrl).FChecked := False;
        TControlListRadioButton(LCtrl).Invalidate;
      end;
    Invalidate;
  end;
end;

function TControlListRadioButton.GetItemIndex: Integer;
var
  LCtrl: TControl;
begin
  Result := 0;
  for LCtrl in Parent.GetControls do
    if LCtrl is TControlListRadioButton then
      if TControlListRadioButton(LCtrl).Checked then
        Exit
      else
        Inc(Result);
  Result := -1;
...
procedure TControlListRadioButton.SetItemIndex(const Value: Integer);
var
  LCtrl: TControl;
  I: Integer;
begin
...
function TControlListRadioButton.CanObserve(const ID: Integer): Boolean;
begin
  Result := False;
  if ID = TObserverMapping.EditLinkID then
    Result := True
...
procedure TControlListRadioButton.Toggle;
begin
  if Observers.IsObserving(TObserverMapping.EditLinkID) then
  begin
    if TLinkObservers.EditLinkIsReadOnly(Observers) then
...
procedure TControlListRadioButton.Click;
begin
  Toggle;
  inherited Changed;
  inherited Click;
...
procedure TControlListRadioButton.Paint;
const
  C_GRAYED_MARGIN = 4;
var
  LVertMargin, LHorzMargin: Integer;
...
begin
  ...
  begin
    if Checked then
      case State of
        clstNormal:   LDetails := LStyle.GetElementDetails(tbRadioButtonCheckedNormal);
        clstHot:      LDetails := LStyle.GetElementDetails(tbRadioButtonCheckedHot);
        clstPressed:  LDetails := LStyle.GetElementDetails(tbRadioButtonCheckedPressed);
        clstDisabled: LDetails := LStyle.GetElementDetails(tbRadioButtonCheckedDisabled);
      end
    else
      case State of
        clstNormal:   LDetails := LStyle.GetElementDetails(tbRadioButtonUncheckedNormal);
        clstHot:      LDetails := LStyle.GetElementDetails(tbRadioButtonUncheckedHot);
        clstPressed:  LDetails := LStyle.GetElementDetails(tbRadioButtonUncheckedPressed);
        clstDisabled: LDetails := LStyle.GetElementDetails(tbRadioButtonUncheckedDisabled);
      end;
    LStyle.DrawElement(Canvas.Handle, LDetails, LPaintRect, nil, CurrentPPI);
  end
  else
  begin
    LDrawFlags := DFCS_BUTTONRADIO or DFCS_ADJUSTRECT;
    if Checked then
      LDrawFlags := LDrawFlags or DFCS_CHECKED;
    if not Enabled then
      LDrawFlags := LDrawFlags or DFCS_INACTIVE;
