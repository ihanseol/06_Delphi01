...
{$ENDIF}
  ...
  Winapi.ShellScaling, Winapi.WTSApi32;
...
{ TVCLDesignerHighDPIMode }
  ...
  TVCLDesignerHighDPIMode = (
    ...
    /// This PPI value is preserved when moving the designer to another monitor with a different PPI.
    /// </summary>
    hdmUserEditable, hdmSystemUnaware
  );
...
{$ENDIF}
    ...
    /// <summary>
    /// Returns the TStyleElements to be ignored in design-time.
    /// </summary>
...
{$ENDIF}
  ...
  TFormStateItem = (fsCreating, fsVisible, fsShowing, fsModal, fsCreatedMDIChild, fsActivated);
  TFormState = set of TFormStateItem;
...
{$ENDIF}
  private
    ...
    FChildStoredNormalRect,
    FChildStoredBoundsRect: TRect;
    FChildStoredMinPosition: TPoint;
    FMDIChildClientOffsetCX,
    FMDIChildClientOffsetCY: Integer;
    FVisualManager: IFormVisualManager;
    FVisualManagerInitialized: Boolean;
...
{$ENDIF}
    ...
    procedure SetVisualManager(Value: IFormVisualManager);
    ...
    function VisualManager_AcceptMessage(Message: TMessage): Boolean;
    procedure VisualManager_AddForm;
    procedure VisualManager_DeleteForm;
    procedure VisualManager_WndProc(Message: TMessage);
...
{$ENDIF}
  protected
    ...
    function CanShowInTaskBar: Boolean; virtual;
    procedure ChangeScale(M, D: Integer; isDpiChange: Boolean); override;
    ...
    function GetVisualManager(AInitialization: Boolean): IFormVisualManager;
...
{$ENDIF}
    property VisualManager: IFormVisualManager read FVisualManager write SetVisualManager;
  ...
  public
    ...
    /// <summary>
    /// Creates and initializes a new form for specific DPI using ScaleForPPI method.
    /// If DPI is not defined then form is scaled using ScaleForCurrentDPI method.
    /// Use this method instead of CreateNew to get form, for which you do not need to call scale methods after creation.
    /// </summary>
    ...
    /// </summary>
    procedure ScaleForCurrentDPI(ForceScaling: Boolean = False); virtual;
    procedure ScaleForPPI(NewPPI: Integer); override;
    procedure Close;
    ...
    /// <summary>
    /// On Windows 11 forms default to light mode regardless of the system setting.
    /// With this method you can have this form's frame drawn in dark mode colors
    /// when the dark mode system setting is enabled.
    /// </summary>
    procedure EnableImmersiveDarkMode(Enable: Boolean);
...
{$ENDIF}
    ...
    /// <summary>Controls rounded form corners on Windows 11</summary>
...
{$ENDIF}
  ...
  public
    ...
    /// <summary> Return number of scanlines between the scanline containing cursor hotspot and the last scanline
    /// included in the cursor mask for the current cursor. </summary>
...
{$ENDIF}
    ...
    FDefaultRoundedCorners: TRoundedCornerType;
...
{$ENDIF}
    ...
    /// <summary>The default value for RoundedCorners on all subsequent form instances</summary>
...
type
  ...
  strict private
    ...
    FNeedChangeSize: Boolean;
    procedure AdjustMDIScrollBars;
    procedure ChangeSize;
    function IsStyleBorder: Boolean;
    function GetBorderSize(UseActiveStyle: Boolean = True): TRect;
...
{TChildFormMessageHandler}

type
  TChildFormMessageHandler = class(TWinControlMessageHandler)
  strict private type
    TSysButtonType = (sbtClose, sbtMaximize, sbtMinimize);
    TSysButton = record
      MouseIn,
      MouseDown: Boolean;
      Bounds: TRect;
  ...
  private
    FDefaultBorderSize: TRect;
    FActive: Boolean;
    FHScrollBarRect,
    FVScrollBarRect: TRect;
    FScrollBoxSize: Integer;
    FIcon: TIcon;
    FIconRect,
    FCaptionRect: TRect;
    FIconDPI: Integer;
    FRgn: HRGN;
    FCaptured: Boolean;
    FCapturedButton: TSysButtonType;
    FCheckingNCMouseLeave: Boolean;
    FButtons: array[TSysButtonType] of TSysButton;
    FSavedAnchors: TAnchors;
    FAnchorsIsSaved: Boolean;
    function GetForm: TCustomForm;
    function CustomBorderEnabled: Boolean;
    function IsSystemBorder: Boolean;
    property Form: TCustomForm read GetForm;
  protected
    function HandleMessage(var Message: TMessage): Boolean; override;
    procedure PaintBorder;
    procedure RestoreAll;
    procedure Tile(AVertical: Boolean);
    procedure Cascade;
  public
    constructor Create(AControl: TWinControl); override;
    destructor Destroy; override;
...
constructor TChildFormMessageHandler.Create(AControl: TWinControl);
begin
  inherited Create(AControl);
  FIconDPI := Screen.PixelsPerInch;
end;

destructor TChildFormMessageHandler.Destroy;
begin
  FreeAndNil(FIcon);
  inherited;
end;

procedure TChildFormMessageHandler.RestoreAll;
var
  I: Integer;
begin
  for I := 0 to Application.MainForm.MDIChildCount - 1 do
    if IsIconic(Application.MainForm.MDIChildren[I].Handle) or IsZoomed(Application.MainForm.MDIChildren[I].Handle) then
      SendMessage(Application.MainForm.MDIChildren[I].Handle, WM_SYSCOMMAND, SC_RESTORE, 0);
end;

function TChildFormMessageHandler.GetForm: TCustomForm;
begin
  Result := TCustomForm(Control);
end;

function TChildFormMessageHandler.IsSystemBorder: Boolean;
begin
  Result := not (Form.IsCustomStyleActive and (seBorder in Form.StyleElements) and
    not (TStyleManager.FormBorderStyle = fbsSystemStyle));
end;

function TChildFormMessageHandler.CustomBorderEnabled: Boolean;
begin
  Result := TStyleManager.ChangeChildFormSystemBorder and IsSystemBorder and TOSVersion.Check(10) and (Form.BorderStyle <> bsNone);
end;

function TChildFormMessageHandler.HandleMessage(var Message: TMessage): Boolean;
const
  cDefaultMinWidth = 200;
  cButtonsHitTest: array[TSysButtonType] of LongInt = (HTCLOSE, HTMAXBUTTON, HTMINBUTTON);
  cCheckNCMouseLeaveTimerID = 99;

var
  LRect: TRect;
  LOldBorderSize: TRect;
  LRgn, LHRgn, LVRgn: HRgn;
  LMinMaxInfo: PMinMaxInfo;
  LMinWidth: Integer;
  R: TRect;
  LWindowPlacement: TWindowPlacement;

  function CanUpdateScrollBars: Boolean;
  begin
    Result := not FVScrollBarRect.IsEmpty or not FHScrollBarRect.IsEmpty;
  end;
  ...
  procedure CallSystemMenu(X, Y: Integer; AFromIcon: Boolean);
  var
    R: TRect;
  begin
    if AFromIcon then
    begin
      ...
      X := R.Left + FDefaultBorderSize.Left;
      Y := R.Top + FDefaultBorderSize.Top;
    end;
    SendMessage(Handle, WM_POPUPSYSTEMMENU, 0, MakeLong(X, Y));
  end;
  ...
  function GetHitTest(P: TPoint; ADefaultResult: LongInt): LongInt;
  begin
    Result := ADefaultResult;
    case Result of
      HTSYSMENU, HTCAPTION, HTCLOSE,
      HTMAXBUTTON, HTMINBUTTON:
        Result := HTNOWHERE;
    end;
    P := NormalizePoint(P);
    if FIconRect.Contains(P) then
      Result := HTSYSMENU
    else
    if FCaptionRect.Contains(P) then
      Result := HTCAPTION
    else
    if FButtons[sbtClose].Bounds.Contains(P) then
      Result := HTCLOSE
    else
    if FButtons[sbtMaximize].Bounds.Contains(P) then
      Result := HTMAXBUTTON
    else
    if FButtons[sbtMinimize].Bounds.Contains(P) then
      Result := HTMINBUTTON;
  end;

  procedure StartCheckNCMouseLeave;
  begin
    if not FCheckingNCMouseLeave then
    begin
      FCheckingNCMouseLeave := True;
      SetTimer(Handle, cCheckNCMouseLeaveTimerID, 100, nil);
    end;
  end;

  function CheckMouseMove(P: TPoint): Boolean;
  var
    LNeedUpdate: Boolean;
    LMouseIn: Boolean;
    I: TSysButtonType;
  begin
    ...
    for I := Low(FButtons) to High(FButtons) do
      if not FCaptured or (I = FCapturedButton) then
      begin
        LMouseIn :=  FButtons[I].Bounds.Contains(P);
        if LMouseIn then
        begin
          Result := True;
          StartCheckNCMouseLeave;
        end;
        if (LMouseIn and not FButtons[I].MouseIn) or
           (not LMouseIn and FButtons[I].MouseIn) then
        begin
          FButtons[I].MouseIn := LMouseIn;
          LNeedUpdate := True;
        end;
      end;
    if LNeedUpdate then
  ...
  procedure StopCheckNCMouseLeave(AUpdate: Boolean);
  begin
    if FCheckingNCMouseLeave then
    begin
      FCheckingNCMouseLeave := False;
      KillTimer(Handle, cCheckNCMouseLeaveTimerID);
      if AUpdate then
        CheckMouseMove(TPoint.Create(-1, -1));
    end;
  end;

  function CheckMouseDown(P: TPoint): Boolean;
  var
    LNeedUpdate: Boolean;
    I: TSysButtonType;
  begin
    ...
    for I := Low(FButtons) to High(FButtons) do
    begin
      if FButtons[I].Bounds.Contains(P) then
      begin
        FButtons[I].MouseIn := True;
        FButtons[I].MouseDown := True;
        Result := True;
        LNeedUpdate := True;
        FCaptured := True;
        FCapturedButton := I;
        StopCheckNCMouseLeave(False);
        SetCapture(Handle);
      end;
    end;
    if LNeedUpdate then
  ...
  function CheckMouseUp(P: TPoint): Boolean;
  var
    LNeedUpdate: Boolean;
    I: TSysButtonType;
    LButtonHitTest: LongInt;
    LOldMouseIn: Boolean;
  begin
    Result := False;
    LNeedUpdate := False;
    LButtonHitTest := HTNOWHERE;
    P := NormalizePoint(P);
    for I := Low(FButtons) to High(FButtons) do
    begin
      LOldMouseIn := FButtons[I].MouseIn;
      FButtons[I].MouseIn := FButtons[I].Bounds.Contains(P);
      if FButtons[I].MouseDown and FButtons[I].MouseIn then
      begin
        FButtons[I].MouseDown := False;
        LButtonHitTest := cButtonsHitTest[I];
        Result := True;
        LNeedUpdate := True;
      end
      else
      begin
        FButtons[I].MouseDown := False;
        if LOldMouseIn <> FButtons[I].MouseIn then
        begin
          LNeedUpdate := True;
          Result := True;
        end;
    ...
    if LButtonHitTest <> HTNOWHERE then
      for I := Low(FButtons) to High(FButtons) do
        FButtons[I].MouseIn := False;

    case LButtonHitTest of
      HTCLOSE:
        DoClose;
      HTMAXBUTTON:
        DoMaximize;
  ...
  procedure CheckNCMouseLeave;
  begin
    if WindowFromPoint(TPoint.Create(Mouse.CursorPos.X, Mouse.CursorPos.Y)) <> Handle then
      StopCheckNCMouseLeave(True);
  end;

  procedure ActivateFormOnParentControl;
  var
    I: Integer;
    LForm: TCustomForm;
  begin
    ...
    for I := 0 to Form.Parent.ControlCount - 1 do
      if Form.Parent.Controls[I].Visible and (Form.Parent.Controls[I] is TCustomForm) then
      begin
        LForm := TCustomForm(Form.Parent.Controls[I]);
        if (LForm.BorderStyle <> bsNone) and Assigned(LForm.MessageHandler) and
           (LForm.MessageHandler is TChildFormMessageHandler) and TChildFormMessageHandler(LForm.MessageHandler).FActive then
        begin
          SendMessage(LForm.Handle, WM_NCACTIVATE, 0, 0);
          LForm.Deactivate;
        end;
      end;
  ...
  procedure GetScrollBarRects(ACalcDefaultBorderSize: Boolean);
  var
    LBarInfo: TScrollBarInfo;
  begin
    FScrollBoxSize := 0;
    LBarInfo.cbSize := SizeOf(LBarInfo);
    GetScrollBarInfo(Handle, Integer(OBJID_HSCROLL), LBarInfo);
    if STATE_SYSTEM_INVISIBLE and LBarInfo.rgstate[0] = 0 then
    begin
       FHScrollBarRect := LBarInfo.rcScrollBar;
       if (FHScrollBarRect.Height <> Form.GetSystemMetrics(SM_CYHSCROLL)) and (FHScrollBarRect.Height > 0)  then
         FHScrollBarRect.Bottom := FHScrollBarRect.Top + Form.GetSystemMetrics(SM_CYHSCROLL);
      FScrollBoxSize := FHScrollBarRect.Height;
      if ACalcDefaultBorderSize then
        Dec(FDefaultBorderSize.Bottom, FHScrollBarRect.Height);
      Inc(FHScrollBarRect.Bottom, Form.BorderWidth);
    end
    else
      FHScrollBarRect := TRect.Empty;

    GetScrollBarInfo(Handle, Integer(OBJID_VSCROLL), LBarInfo);
    if STATE_SYSTEM_INVISIBLE and LBarInfo.rgstate[0] = 0 then
    begin
      FVScrollBarRect := LBarInfo.rcScrollBar;
      if (FVScrollBarRect.Width <> Form.GetSystemMetrics(SM_CXVSCROLL)) and (FVScrollBarRect.Width > 0)  then
         FVScrollBarRect.Right := FVScrollBarRect.Left + Form.GetSystemMetrics(SM_CXVSCROLL);
      FScrollBoxSize := FVScrollBarRect.Width;
      if GetWindowLong(Handle, GWL_EXSTYLE) and WS_EX_RTLREADING <> 0 then
      begin
        if ACalcDefaultBorderSize then
          Dec(FDefaultBorderSize.Left, FVScrollBarRect.Width);
        Dec(FVScrollBarRect.Left, Form.BorderWidth);
      end
      else
      begin
        if ACalcDefaultBorderSize then
          Dec(FDefaultBorderSize.Right, FVScrollBarRect.Width);
        Inc(FVScrollBarRect.Right, Form.BorderWidth);
      end;
    end
    else
     FVScrollBarRect := TRect.Empty;
  end;
...
begin
  ...
  case Message.Msg of
    ...
    WM_SYSCOMMAND:
    begin
      if (TWMSysCommand(Message).CmdType = SC_MOVE) and IsIconic(Handle) and not IsZoomed(Handle) and not IsWindowVisible(Handle) then
        Result := True
      else
      if TWMSysCommand(Message).CmdType = SC_RESTORE then
    ...
    CM_STYLECHANGED:
    begin
      if (Form.Parent <> nil) and IsIconic(Handle) then
        FActive := False;

      if (Form.Parent <> nil) and (IsIconic(Handle) or IsZoomed(Handle)) then
      begin
        LWindowPlacement.Length := SizeOf(LWindowPlacement);
        GetWindowPlacement(Handle, LWindowPlacement);
        LWindowPlacement.rcNormalPosition := Form.FChildStoredNormalRect;
        LWindowPlacement.ptMinPosition := Form.FChildStoredMinPosition;
        SetWindowPlacement(Handle, LWindowPlacement);
      end;
    end;

    WM_NCACTIVATE:
    begin
      FActive := TWMNCActivate(Message).Active;
    end;

    WM_SHOWWINDOW:
    begin
      if (Form.Parent <> nil) and TWMShowWindow(Message).Show then
      begin
        DefaultWinProc(Message);
        if (csRecreating in Form.Parent.ControlState) then
        begin
          if FActive then
            SendMessage(Handle, WM_NCACTIVATE, 1, 0);
        end
        else
          ActivateFormOnParentControl;
        Result := True;
      end;
    end;
    ...
    CM_FOCUSCHANGED:
    begin
      if (Form.FormStyle <> fsMDIChild) and (Form.Parent <> nil) and
         (GetParentForm(TCMFocusChanged(Message).Sender, False) = Form) then
       ActivateFormOnParentControl;
    end;

    WM_MOUSEACTIVATE:
    begin
      if (Form.FormStyle <> fsMDIChild) and (Form.Parent <> nil) then
        ActivateFormOnParentControl
      else
      if not FActive then
        Form.Show;
    ...
    begin
      if Form.FormStyle = fsMDIChild then
      begin
        case TWMSIZE(Message).SizeType of
          SIZE_MAXIMIZED, SIZE_MINIMIZED:
          begin
            if Form.FChildStoredNormalRect.IsEmpty then
            begin
              LWindowPlacement.Length := SizeOf(LWindowPlacement);
              GetWindowPlacement(Handle, LWindowPlacement);
              Form.FChildStoredNormalRect := LWindowPlacement.rcNormalPosition;
              Form.FChildStoredMinPosition := LWindowPlacement.ptMinPosition;
            end;
          end;
          SIZE_RESTORED:
          begin
            if not Form.FChildStoredNormalRect.IsEmpty and not (csRecreating in Application.MainForm.ControlState) then
              Form.FChildStoredNormalRect := TRect.Empty;
          end;
        end;
      end
      else
      if (Form.Parent <> nil) and not (csRecreating in Form.Parent.ControlState) then
      begin
        case TWMSIZE(Message).SizeType of
          SIZE_MAXIMIZED, SIZE_MINIMIZED:
          begin
    ...
    WM_NCCALCSIZE:
    begin
      if IsSystemBorder and TWMNCCalcSize(Message).CalcValidRects then
      begin
        LRect := TWMNCCalcSize(Message).CalcSize_Params^.rgrc[0];
        LOldBorderSize := FDefaultBorderSize;
        Form.DefaultHandler(Message);
        FDefaultBorderSize := TWMNCCalcSize(Message).CalcSize_Params^.rgrc[0];
        TWMNCCalcSize(Message).CalcSize_Params^.rgrc[0] := LRect;

        FDefaultBorderSize.Left := FDefaultBorderSize.Left - LRect.Left;
        FDefaultBorderSize.Top := FDefaultBorderSize.Top - LRect.Top;
        FDefaultBorderSize.Bottom := LRect.Bottom - FDefaultBorderSize.Bottom;
        FDefaultBorderSize.Right := LRect.Right - FDefaultBorderSize.Right;
        GetScrollBarRects(True);

        if (LOldBorderSize.Top <> 0) and (Form.FormStyle = fsMDIChild) then
        begin
          if (LOldBorderSize.Top <> FDefaultBorderSize.Top) and CheckPerMonitorV2SupportForWindow(Handle) and (Form.CurrentPPI <> Form.FCurrentPPI) then
          begin
            Form.FMDIChildClientOffsetCY := (FDefaultBorderSize.Top + FDefaultBorderSize.Bottom) - (LOldBorderSize.Top + LOldBorderSize.Bottom);
            Form.FMDIChildClientOffsetCX := (FDefaultBorderSize.Left + FDefaultBorderSize.Right) - (LOldBorderSize.Left + LOldBorderSize.Right);
          end;
        end;
      end;
    end;
    ...
    begin
      if IsSystemBorder and IsZoomed(Handle) and (Form.FCurrentPPI <> Screen.PixelsPerInch) and CheckPerMonitorV2SupportForWindow(Handle) and (Form.FormStyle = fsMDIChild) then
      begin
        if ((TWMWindowPosChanging(Message).WindowPos^.flags and SWP_NOSIZE = 0)  or
           (TWMWindowPosChanging(Message).WindowPos^.flags and SWP_NOMOVE = 0)) and
           (Application.MainForm <> nil) and (Application.MainForm.ClientHandle <> 0) then
        begin
          Form.DefaultHandler(Message);
          GetWindowRect(Application.MainForm.ClientHandle , R);
          TWMWindowPosChanging(Message).WindowPos^.y := -FDefaultBorderSize.Top;
          TWMWindowPosChanging(Message).WindowPos^.x := -FDefaultBorderSize.Left;
          TWMWindowPosChanging(Message).WindowPos^.cx := R.Width + FDefaultBorderSize.Left + FDefaultBorderSize.Right;
          TWMWindowPosChanging(Message).WindowPos^.cy := R.Height + FDefaultBorderSize.Top + FDefaultBorderSize.Bottom;
          Result := True;
        end;
      end
      else
      if IsZoomed(Handle) and (Form.Parent <> nil) then
      begin
        if IsSystemBorder then
        begin
          Form.DefaultHandler(Message);
          GetClientRect(Form.Parent.Handle, R);
          TWMWindowPosChanging(Message).WindowPos^.y := 0;
          TWMWindowPosChanging(Message).WindowPos^.x := -FDefaultBorderSize.Left;
          TWMWindowPosChanging(Message).WindowPos^.cx := R.Width + FDefaultBorderSize.Left + FDefaultBorderSize.Right;
          TWMWindowPosChanging(Message).WindowPos^.cy := R.Height + FDefaultBorderSize.Bottom;
          Result := True;
        end;
        if not FAnchorsIsSaved then
        begin
          FSavedAnchors := Form.Anchors;
          FAnchorsIsSaved := True;
          Form.Anchors := [akLeft, akTop, akRight, akBottom];
        end;
      end;
      if FAnchorsIsSaved and not IsZoomed(Handle) then
      begin
        Form.Anchors := FSavedAnchors;
        FAnchorsIsSaved := False;
      end;
    end;
  end;

  if not CustomBorderEnabled then
    Exit;
  ...
  case Message.Msg of
    ...
    WM_TIMER:
    begin
     if TWMTimer(Message).TimerID = cCheckNCMouseLeaveTimerID then
       CheckNCMouseLeave;
    end;
    ...
    begin
      Form.DefaultHandler(Message);
      Message.Result := GetHitTest(TPoint.Create(TWMNCHitTest(Message).XPos, TWMNCHitTest(Message).YPos), Message.Result);
      Result := True;
    end;

    WM_GETMINMAXINFO:
    begin
      if not Form.IScaling then
      begin
        Form.DefaultHandler(Message);
        LMinWidth := MulDiv(cDefaultMinWidth, Form.FCurrentPPI, Screen.DefaultPixelsPerInch);
        if Form.Constraints.MinWidth < LMinWidth then
        begin
          LMinMaxInfo := TWMGetMinMaxInfo(Message).MinMaxInfo;
          LMinMaxInfo^.ptMinTrackSize.x := LMinWidth;
        end;
        Result := True;
      end;
    end;
    ...
    WM_NCMOUSEMOVE:
    begin
      Result := CheckMouseMove(TPoint.Create(TWMNCHitMessage(Message).XCursor, TWMNCHitMessage(Message).YCursor));
    end;
    ...
    begin
      if FCaptured then
      begin
        CheckMouseMove(TPoint.Create(Mouse.CursorPos.X, Mouse.CursorPos.Y));
        Result := True;
      end
      else
        StopCheckNCMouseLeave(True);
    ...
    begin
      ...
      begin
        FCaptured := False;
        ReleaseCapture;
        CheckMouseUp(TPoint.Create(Mouse.CursorPos.X, Mouse.CursorPos.Y));
        Result := True;
      end;
    end;
    ...
    begin
      if TWMNCHitMessage(Message).HitTest = HTSYSMENU then
        CallSystemMenu(0, 0, True)
      else
        Result := CheckMouseDown(TPoint.Create(TWMNCHitMessage(Message).XCursor, TWMNCHitMessage(Message).YCursor));
    end;
    ...
    begin
      if TWMNCHitMessage(Message).HitTest = HTSYSMENU then
        ...
        Result := CheckMouseUp(TPoint.Create(TWMNCHitMessage(Message).XCursor, TWMNCHitMessage(Message).YCursor));
    end;
    ...
    begin
      ...
      begin
        GetWindowRect(Handle, R);
        FRgn := CreateRectRgn(0, 0, R.Width, R.Height);
    ...
    WM_SHOWWINDOW:
    begin
      if TWMShowWindow(Message).Show and not ((Form.Parent <> nil) and (csRecreating in Form.Parent.ControlState)) then
        FActive := True;
    end;
    ...
    WM_NCPAINT, WM_NCUAHDRAWCAPTION:
    begin
      GetScrollBarRects(False);

      if (not FVScrollBarRect.IsEmpty or not FHScrollBarRect.IsEmpty) and not IsIconic(Handle) then
      begin
        if not FVScrollBarRect.IsEmpty then
          LVRgn := CreateRectRgn(FVScrollBarRect.Left, FVScrollBarRect.Top, FVScrollBarRect.Right, FVScrollBarRect.Bottom)
        else
          LVRgn := 0;
        if not FHScrollBarRect.IsEmpty then
        begin
          if GetWindowLong(Handle, GWL_EXSTYLE) and WS_EX_RTLREADING <> 0 then
           LHRgn := CreateRectRgn(FHScrollBarRect.Left - FVScrollBarRect.Width,
             FHScrollBarRect.Top, FHScrollBarRect.Right, FHScrollBarRect.Bottom)
         else
           LHRgn := CreateRectRgn(FHScrollBarRect.Left, FHScrollBarRect.Top,
             FHScrollBarRect.Right + FVScrollBarRect.Width, FHScrollBarRect.Bottom);
        end
        else
          LHRgn := 0;

        if (LVRgn <> 0) and (LHRgn <> 0) then
        begin
          CombineRgn(LVRgn, LVRgn, LHRgn, RGN_OR);
          DeleteObject(LHRgn);
        end;
    ...
    WM_WINDOWPOSCHANGING:
    begin
      if (Form.BorderStyle <> bsNone) and (GetWindowLong(Handle, GWL_EXSTYLE) and WS_EX_TOOLWINDOW = 0)  then
        SetWindowLong(Handle, GWL_EXSTYLE, GetWindowLong(Handle, GWL_EXSTYLE) or WS_EX_TOOLWINDOW);
    end;
  end;
end;

procedure TChildFormMessageHandler.PaintBorder;
type
  TButtonDrawState = (bdsNormal, bdsHot, bdsPressed, bdsInActive);

  const
  cSpaceIcon = 5;
  cSpaceIconTextAllDPI = 5;
  cSpaceCaption = 3;
  cDefaultButtonWidth = 47;
  cToolButtonIconSize = 7;
  cButtonIconSize: array[TSysButtonType] of Integer = (9, 10, 10);


  cBorderColor = clWhite;
  cFrameColor: array[Boolean] of TColor = ($00AAAAAA, $00707070);
  cCaptionTextColor: array[Boolean] of TColor = ($00999999, clBlack);

  cButtonFillColor: array[TSysButtonType, TButtonDrawState] of TColor =
    ((cBorderColor, $002311E8, $007A70F1, cBorderColor),
     (cBorderColor, $00E5E5E5, $00CCCCCC, cBorderColor),
     (cBorderColor, $00E5E5E5, $00CCCCCC, cBorderColor));

  cButtonIconColor: array[TSysButtonType, TButtonDrawState] of TColor =
    ((clBlack, clWhite, clWhite, $00999999),
     (clBlack, clBlack, clBlack, $00999999),
     (clBlack, clBlack, clBlack, $00999999));

  cCloseToolButtonFillColor: array[TButtonDrawState] of TColor =
    ($005050C7, $004343E0, $003D3D99, $00A1A1A1);
  cCloseToolButtonIconColor: array[TButtonDrawState] of TColor =
    (clWhite, clWhite, clWhite, $00F8F8F8);
...
var
  ...
  function GetButtonDrawState(AButtonType: TSysButtonType): TButtonDrawState;
  begin
    Result := bdsNormal;
    if FButtons[AButtonType].MouseDown and FButtons[AButtonType].MouseIn then
      Result := bdsPressed
    else
    if FButtons[AButtonType].MouseIn then
      Result := bdsHot
    else
    if not FActive then
      Result := bdsInActive;
  ...
  procedure DrawButton(ACanvas: TCanvas; AButtonType: TSysButtonType; ARect: TRect);
  var
    LState: TButtonDrawState;
  begin
    FButtons[AButtonType].Bounds := ARect;
    if FButtons[AButtonType].Bounds.IsEmpty then
    begin
      FButtons[AButtonType].MouseIn := False;
      FButtons[AButtonType].MouseDown := False;
      Exit;
    end;

    LState := GetButtonDrawState(AButtonType);
    ACanvas.Brush.Style := bsSolid;
    ACanvas.Brush.Color := cButtonFillColor[AButtonType, LState];
    ACanvas.FillRect(FButtons[AButtonType].Bounds);
    case AButtonType of
      sbtClose:
        DrawCloseIcon(ACanvas, FButtons[AButtonType].Bounds,
          cButtonIconColor[AButtonType, LState], cButtonIconSize[AButtonType], LDPI);
      sbtMaximize:
      begin
        if IsZoomed(Handle) then
          DrawRestoreIcon(ACanvas, FButtons[AButtonType].Bounds,
           cButtonIconColor[AButtonType, LState], cButtonIconSize[AButtonType], LDPI)
        else
         DrawMaximizeIcon(ACanvas, FButtons[AButtonType].Bounds,
           cButtonIconColor[AButtonType, LState], cButtonIconSize[AButtonType], LDPI);
      end;
      sbtMinimize:
        if IsIconic(Handle) then
          ...
          DrawMinimizeIcon(ACanvas, FButtons[AButtonType].Bounds,
           cButtonIconColor[AButtonType, LState], cButtonIconSize[AButtonType], LDPI);
    end;
  end;

  procedure DrawToolCloseButton(ACanvas: TCanvas; ARect: TRect);
  var
    LState: TButtonDrawState;
  begin
    FButtons[sbtClose].Bounds := ARect;
    if FButtons[sbtClose].Bounds.IsEmpty then
      Exit;

    LState := GetButtonDrawState(sbtClose);
    ACanvas.Brush.Style := bsSolid;
    ACanvas.Brush.Color := cCloseToolButtonFillColor[LState];
    ACanvas.FillRect(FButtons[sbtClose].Bounds);
    DrawCloseIcon(ACanvas, FButtons[sbtClose].Bounds,
    cCloseToolButtonIconColor[LState], cToolButtonIconSize, LDPI);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        IconX := Form.GetSystemMetrics(SM_CXSMICON);
        if IconX = 0 then
          IconX := Form.GetSystemMetrics(SM_CXSIZE);
        IconY := Form.GetSystemMetrics(SM_CYSMICON);
        if IconY = 0 then
         IconY := Form.GetSystemMetrics(SM_CYSIZE);
      end;
  ...
  procedure DrawFrame(ACanvas: TCanvas; ARect: TRect;
    ALeft, ATop, ARight, ABottom: Boolean);
  begin
    ACanvas.Pen.Color := cFrameColor[FActive];
    if ALeft then
    begin
  ...
  procedure DrawFrameWithBorder(ARect: TRect; ALeft, ATop, ARight, ABottom: Boolean);
  begin
    ...
    try
      R := TRect.Create(0, 0, LBuffer.Width, LBuffer.Height);
      LBuffer.Canvas.Brush.Color := cBorderColor;
      LBuffer.Canvas.FillRect(R);
      DrawFrame(LBuffer.Canvas, R, ALeft, ATop, ARight, ABottom);
      LCanvas.Draw(ARect.Left, ARect.Top, LBuffer);
    finally
      LBuffer.Free;
    end;
  end;

  procedure DrawCaptionObjects(ACanvas: TCanvas; ARect: TRect);
  var
    R: TRect;
    X, Y: Integer;
    LTextRect: TRect;
    LFlags: Longint;
    LIsRightToLeft: Boolean;
    LSpace, LIconSpace: Integer;
  begin
    if IsIconic(Handle) then
      LDPI := Screen.PixelsPerInch
    else
      LDPI := Form.CurrentPPI;

    LIsRightToLeft := GetWindowLong(Handle, GWL_EXSTYLE) and WS_EX_RTLREADING <> 0;
    LSpace := MulDiv(cSpaceCaption, LDPI, Screen.DefaultPixelsPerInch);
    LIconSpace := MulDiv(cSpaceIcon, LDPI, Screen.DefaultPixelsPerInch);

    FCaptionRect := ARect;
    InflateRect(FCaptionRect, -LSpace, -LSpace);
    Inc(FCaptionRect.Bottom);
    if IsZoomed(Handle) then
    begin
      FCaptionRect.Right := ARect.Right - FDefaultBorderSize.Right;
      FCaptionRect.Left := ARect.Left + FDefaultBorderSize.Left;
    end;

    if biSystemMenu in Form.BorderIcons then
    begin
      if Form.BorderStyle in [bsSingle, bsSizeAble] then
      begin
        if FIconDPI <> LDPI then
        begin
          FIconDPI := LDPI;
          FreeAndNil(FIcon);
        end;
        LIcon := GetIcon;
        R.Left := ARect.Left + LIconSpace;
        R.Bottom := ARect.Bottom;
        R.Top := ARect.Top;
        R.Right := R.Left + R.Height;
        X := (R.Left + R.Width - LIcon.Width) div 2;
        Y := (R.Top + R.Height - LIcon.Height) div 2;
        if X < R.Left then
          X := R.Left;
        DrawIconEx(ACanvas.Handle, X, Y, LIcon.Handle, LIcon.Width, LIcon.Height, 0, 0, DI_NORMAL);
        FIconRect := TRect.Create(X, Y, X + LIcon.Width, Y + LIcon.Height);
        FCaptionRect.Left := FIconRect.Right;
      end
      else
        FIconRect := TRect.Empty;

      if Form.BorderStyle in [bsToolWindow, bsSizeToolWin, bsDialog] then
      begin
        R := FCaptionRect;
        R.Left := R.Right - FCaptionRect.Height;
        if Form.BorderStyle = bsDialog then
          DrawButton(ACanvas, sbtClose, R)
        else
        begin
          InflateRect(R, -LSpace, -LSpace);
          DrawToolCloseButton(ACanvas, R);
        end;
        FCaptionRect.Right := R.Left;
        DrawButton(ACanvas, sbtMinimize, TRect.Empty);
        DrawButton(ACanvas, sbtMaximize, TRect.Empty);
      end
      else
      begin
        LButtonWidth := MulDiv(cDefaultButtonWidth, LDPI, Screen.DefaultPixelsPerInch);
        R := FCaptionRect;
        R.Left := R.Right - LButtonWidth;
        DrawButton(ACanvas, sbtClose, R);
        if biMaximize  in Form.BorderIcons then
        begin
          OffsetRect(R, -LButtonWidth, 0);
          DrawButton(ACanvas, sbtMaximize, R);
        end
        else
          DrawButton(ACanvas, sbtMaximize, TRect.Empty);
        ...
        begin
          OffsetRect(R, -LButtonWidth, 0);
          DrawButton(ACanvas, sbtMinimize, R);
        end
        else
          DrawButton(ACanvas, sbtMinimize, TRect.Empty);
        FCaptionRect.Right := R.Left;
    ...
    LTextRect := FCaptionRect;
    LTextRect.Left := FCaptionRect.Left + cSpaceIconTextAllDPI;
    LFlags := DT_SINGLELINE or DT_VCENTER or DT_END_ELLIPSIS or DT_NOPREFIX;
    if LIsRightToLeft then
      LFlags := LFlags or DT_RIGHT or DT_RTLREADING
    else
      LFlags := LFlags or DT_LEFT;
    ACanvas.Font.Assign(Screen.CaptionFont);
    ACanvas.Font.ScaleForDPI(LDPI);
    ACanvas.Font.Color := cCaptionTextColor[FActive];
    ACanvas.Brush.Style := bsClear;
    DrawText(ACanvas.Handle, Form.Caption, Length(Form.Caption), LTextRect, LFlags);
...
begin
  ...
  try
    try
      ...
      LIsIconic := IsIconic(Handle);
      if ((FDefaultBorderSize.Top > 0) and (Form.Width > 0)) or LIsIconic then
      begin
        if LIsIconic then
        begin
          GetWindowRect(Handle, R);
          LBuffer := TBitmap.Create(R.Width, R.Height);
        end
        else
          LBuffer := TBitmap.Create(Form.Width, FDefaultBorderSize.Top);
        try
          R := TRect.Create(0, 0, LBuffer.Width, LBuffer.Height);
          LBuffer.Canvas.Brush.Color := cBorderColor;
          LBuffer.Canvas.FillRect(R);
          DrawFrame(LBuffer.Canvas, R, True, not IsZoomed(Handle), True, LIsIconic);
          DrawCaptionObjects(LBuffer.Canvas, R);
      ...
      begin
        DrawFrameWithBorder(Rect(0, FDefaultBorderSize.Top, FDefaultBorderSize.Left, Form.Height),
         True, FDefaultBorderSize.Top = 0, False, True);
        DrawFrameWithBorder(Rect(Form.Width - FDefaultBorderSize.Right, FDefaultBorderSize.Top, Form.Width, Form.Height),
          False, FDefaultBorderSize.Top = 0, True, True);
        DrawFrameWithBorder(Rect(FDefaultBorderSize.Left, Form.Height - FDefaultBorderSize.Bottom, Form.Width - FDefaultBorderSize.Right, Form.Height),
         False, False, False, True);

        if Form.BorderWidth > 0 then
        begin
          LIsRightToLeft := GetWindowLong(Handle, GWL_EXSTYLE) and WS_EX_RTLREADING <> 0;
          LCanvas.Brush.Color := Form.Color;
          if not FVScrollBarRect.IsEmpty and not FHScrollBarRect.IsEmpty then
            LBoxSize := FScrollBoxSize
          else
            LBoxSize := 0;
          LSaveDC := SaveDC(LCanvas.Handle);
          try
            if LBoxSize > 0 then
            begin
              if LIsRightToLeft then
                R := TRect.Create(FDefaultBorderSize.Left + Form.BorderWidth, Form.Height - FDefaultBorderSize.Bottom - Form.BorderWidth - LBoxSize,
                  FDefaultBorderSize.Left + Form.BorderWidth + LBoxSize, Form.Height - FDefaultBorderSize.Bottom - Form.BorderWidth)
              else
                R := TRect.Create(Form.Width - FDefaultBorderSize.Right - Form.BorderWidth - LBoxSize,  Form.Height - FDefaultBorderSize.Bottom - Form.BorderWidth - LBoxSize,
                  Form.Width - FDefaultBorderSize.Right - Form.BorderWidth, Form.Height - FDefaultBorderSize.Bottom - Form.BorderWidth);
              LCanvas.FillRect(R);
              if LIsRightToLeft then
                R := TRect.Create(FDefaultBorderSize.Left, Form.Height - FDefaultBorderSize.Bottom - LBoxSize,
                 FDefaultBorderSize.Left + LBoxSize, Form.Height - FDefaultBorderSize.Bottom)
              else
                R := TRect.Create(Form.Width - FDefaultBorderSize.Right - LBoxSize, Form.Height - FDefaultBorderSize.Bottom - LBoxSize,
                 Form.Width - FDefaultBorderSize.Right, Form.Height - FDefaultBorderSize.Bottom);
              ExcludeClipRect(LCanvas.Handle, R.Left, R.Top, R.Right, R.Bottom);
            end;
            LCanvas.FillRect(TRect.Create(FDefaultBorderSize.Left, FDefaultBorderSize.Top,
              FDefaultBorderSize.Left + Form.BorderWidth, Form.Height - FDefaultBorderSize.Bottom));
            LCanvas.FillRect(TRect.Create(Form.Width - FDefaultBorderSize.Right - Form.BorderWidth, FDefaultBorderSize.Top,
              Form.Width - FDefaultBorderSize.Right, Form.Height - FDefaultBorderSize.Bottom));
            LCanvas.FillRect(TRect.Create(FDefaultBorderSize.Left, Form.Height - FDefaultBorderSize.Bottom - Form.BorderWidth,
              Form.Width - FDefaultBorderSize.Right, Form.Height - FDefaultBorderSize.Bottom));
            LCanvas.FillRect(TRect.Create(FDefaultBorderSize.Left, FDefaultBorderSize.Top,
              Form.Width - FDefaultBorderSize.Right, FDefaultBorderSize.Top + Form.BorderWidth));
          finally
            RestoreDC(LCanvas.Handle, LSaveDC);
          end;
        end;
      end;
    finally
...
procedure TChildFormMessageHandler.Cascade;
const
  cCaptionMargin = 3;
...
var
  ...
  LClientRect: TRect;
  LOffset, LOffset2: Integer;
  I, J, W, H: Integer;
begin
  RestoreAll;
  LMainForm := Application.MainForm;
  GetWindowRect(LMainForm.ClientHandle, LClientRect);
  LOffset := FDefaultBorderSize.Top - MulDiv(cCaptionMargin, Form.CurrentPPI, Screen.DefaultPixelsPerInch);
  LOffset2 := Round(LClientRect.Width * 0.2) div LOffset * LOffset;
  W := LClientRect.Width - LOffset2;
  H := LClientRect.Height - LOffset2;
  J := 0;
  for I := LMainForm.MDIChildCount - 1 downto 0 do
  begin
    LChildForm := LMainForm.MDIChildren[I];
    if (J + W > LClientRect.Width) or (J + H > LClientRect.Height) then
      J := 0;
    if LChildForm.BorderStyle in [bsSizeable, bsSizeToolWin] then
      LChildForm.SetBounds(J, J, W, H)
    else
      LChildForm.SetBounds(J, J, LChildForm.Width, LChildForm.Height);
    Inc(J, LOffset);
  end;
end;

procedure TChildFormMessageHandler.Tile(AVertical: Boolean);
var
  LMainForm, LChildForm: TForm;
  LColumnCount: Integer;
  LInColumnCount: Integer;
  LClientRect: TRect;
  I, J, X, Y, L, T, W, H: Integer;
begin
  RestoreAll;
  LMainForm := Application.MainForm;
  if AVertical then
  begin
    LInColumnCount := Trunc(Sqrt(LMainForm.MDIChildCount));
    LColumnCount := LMainForm.MDIChildCount div LInColumnCount;
  end
  else
  begin
    LColumnCount := Trunc(Sqrt(LMainForm.MDIChildCount));
    LInColumnCount := LMainForm.MDIChildCount div LColumnCount;
  end;
  if LInColumnCount * LColumnCount < LMainForm.MDIChildCount then
    Inc(LInColumnCount);
  GetWindowRect(LMainForm.ClientHandle, LClientRect);
  W := LClientRect.Width div LColumnCount;
  H := LClientRect.Height div LInColumnCount;
  X := LClientRect.Width;
  Y := LClientRect.Height;
  J := LColumnCount;
  for I := LMainForm.MDIChildCount - 1 downto 0 do
  begin
    LChildForm := LMainForm.MDIChildren[I];
    ...
    T := Max(Y - H, 0);
    if LChildForm.BorderStyle in [bsSizeable, bsSizeToolWin] then
      LChildForm.SetBounds(L, T, W, H)
    else
      LChildForm.SetBounds(L, T, LChildForm.Width, LChildForm.Height);
    Dec(Y, H);
    if (Y - H < 0) and (I > 0) then
    begin
      Y := LClientRect.Height;
      Dec(X, W);
      J := Max(J - 1, 1);
      LInColumnCount := I div J;
      if LInColumnCount * J < I then
        Inc(LInColumnCount);
      H := LClientRect.Height div Max(LInColumnCount, 1);
    end;
  end;
end;
...
constructor TCustomForm.CreateScaledNew(AOwner: TComponent; ADPI: Integer = 0; Dummy: Integer = 0);
begin
  inherited Create(AOwner);
  InitializeNewForm;
  if Font.PixelsPerInch <> PixelsPerInch then
  begin
    Font.ScaleForDPI(PixelsPerInch);
    Font.PixelsPerInch := PixelsPerInch;
  end;
  if ADPI = 0 then
    ScaleForCurrentDPI
  else
    ScaleForPPI(Max(ADPI, Screen.DefaultPixelsPerInch));
end;
...
begin
  ...
  begin
    if not (csDesigning in ComponentState) and (Parent <> nil) then
      Font.Assign(TWinControlClassHook(Parent).Font)
    else
    begin
  ...
  case Operation of
    ...
    opRemove:
      begin
        ...
        if (FVisualManager is TComponent) and (TComponent(FVisualManager) = AComponent) then
          FVisualManager := nil;
...
var
  ...
  I, LPriorWidth, LPriorHeight: Integer;
  LOffsetCX, LOffsetCY: Integer;
  LRect: TRect;
begin
  ...
  begin
    ...
    ScaleMargins(NewPPI, LCurrentPixelsPerInch);
    ...
    begin
      ...
      begin
        LOffsetCX := FMDIChildClientOffsetCX;
        LOffsetCY := FMDIChildClientOffsetCY;
        FMDIChildClientOffsetCX := 0;
        FMDIChildClientOffsetCY := 0;
      end
      else
      begin
        LOffsetCX := 0;
      ...
      begin
        if not (csDesigning in ComponentState) and ((FormStyle = fsMDIChild) and
          AutoScroll and (FClientWidth = 0) and (FClientHeight = 0)) then
        begin
          WinAPI.Windows.GetClientRect(Handle, LRect);
          FClientWidth := LRect.Width;
          FClientHeight := LRect.Height;
        end;
        ...
        if sfWidth in ScalingFlags then
          FClientWidth := MulDiv(FClientWidth + LOffsetCX, NewPPI, LCurrentPixelsPerInch);
        if sfHeight in ScalingFlags then
          FClientHeight := MulDiv(FClientHeight + LOffsetCY, NewPPI, LCurrentPixelsPerInch);
        if FClientWidth > 0 then inherited ClientWidth := FClientWidth;
        if FClientHeight > 0 then inherited ClientHeight := FClientHeight;
      end
      else if IsClientSizeStored or (not (csDesigning in ComponentState) and (FormStyle = fsMDIChild) and AutoScroll)  then
      begin
        LPriorHeight := ClientHeight;
        if LPriorHeight > 0 then
          Inc(LPriorHeight, LOffsetCY);
        LPriorWidth := ClientWidth;
        if LPriorWidth > 0 then
          Inc(LPriorWidth, LOffsetCX);

        ClientWidth := MulDiv(LPriorWidth, NewPPI, LCurrentPixelsPerInch);
        ClientHeight := MulDiv(LPriorHeight, NewPPI, LCurrentPixelsPerInch);
      end;
    ...
    begin
      if csDesigning in ComponentState then
        Font.ChangeScale(FFontHeightFor96DPI, NewPPI, Screen.DefaultPixelsPerInch, True)
      else
        Font.ChangeScale(NewPPI, LCurrentPixelsPerInch, True);
    end
    else
...
procedure TCustomForm.ScaleForCurrentDPI(ForceScaling: Boolean = False);
var
  LCurrentPixelsPerInch: Integer;
begin
  DisableAlign;
  try
    LCurrentPixelsPerInch := FCurrentPPI;
    if (csLoading in ComponentState) or ForceScaling then
    begin
      ...
      begin
        ...
        for var I := 0 to ComponentCount - 1 do
          ...
          begin
            ...
            if TControlClass(Components[I]).Font.IsDPIRelated then
              TControlClass(Components[I]).Font.PixelsPerInch := FCurrentPPI;
          end;
      end;
    end;
...
procedure TCustomForm.EnableImmersiveDarkMode(Enable: Boolean);
var
  UseImmersiveDarkMode: DWord;
begin
  if TOSVersion.Check(10) and (TOSVersion.Build >= 20348) then // Windows Server 2022 and Windows 11
  begin
    if Enable then
      UseImmersiveDarkMode := 1
    else
      UseImmersiveDarkMode := 0;
    Winapi.Dwmapi.DwmSetWindowAttribute(Handle, DWMWA_USE_IMMERSIVE_DARK_MODE, @UseImmersiveDarkMode, SizeOf(DWord));
  end;
end;
...
function TCustomForm.GetVisualManager(AInitialization: Boolean): IFormVisualManager;
begin
  if not AInitialization and not FVisualManagerInitialized then
    Exit(nil);

  if (csDesigning in ComponentState) or not (Self is TForm) or (Self = Application.MainForm) then
  begin
    FVisualManagerInitialized := False;
    Exit(nil);
  end;

  Result := FVisualManager;
  if Result = nil then
  begin
    if (Application.MainForm <> nil) and ((FormStyle = fsMDIChild) or (Owner = Application)) then
      Result := Application.MainForm.FVisualManager
    else
    if (FormStyle <> fsMDIChild) and (Owner is TCustomForm) then
      Result := TCustomForm(Owner).VisualManager;
  end;

  if AInitialization then
  begin
    if (Result <> nil) and ((Result is TControl) and (GetParentForm(TControl(Result)) = Self) or not Result.AcceptForm(TForm(Self))) then
      Result := nil;
    FVisualManagerInitialized := Result <> nil;
  end;
end;

procedure TCustomForm.SetVisualManager(Value: IFormVisualManager);
begin
  if FVisualManager <> Value then
  begin
    if (FVisualManager <> nil) and (FVisualManager is TComponent) then
      TComponent(FVisualManager).RemoveFreeNotification(Self);
    if not (csDesigning in ComponentState) and
       not (csLoading in ComponentState) then
      VisualManager_DeleteForm;
    FVisualManager := Value;
    if (FVisualManager <> nil) and (FVisualManager is TComponent) then
      TComponent(FVisualManager).FreeNotification(Self);
    if not (csDesigning in ComponentState) and
       not (csLoading in ComponentState) then
      VisualManager_AddForm;
  end;
end;
  ...
  begin
    ...
    begin
      if Application.MainForm = Self then
        Application.MainFormOnTaskBar := FShowInTaskBar
      else
      if CanShowInTaskBar then
        RecreateWnd;
    end; 
  end;
end;
...
begin
  ...
  if not ParentFont or ((Parent = nil) and not (csDesigning in ComponentState)) then
  begin
    LParentFont := ParentFont;
    Font.ChangeScale(M, D, isDpiChange);
    ParentFont := LParentFont;
  end;
end;
...
var
  ...
  LActiveMDIChild: TForm;
  LActiveMDIChildState: TWindowState;
  ParentWnd: HWND;
...
{$ENDIF}
      ...
      CM_CUSTOMSTYLECHANGED:
        begin
          ...
          LActiveMDIChildState  := wsNormal;
          if FormStyle = fsMDIForm then
          begin
            LActiveMDIChild := ActiveMDIChild;
            if LActiveMDIChild <> nil then
              LActiveMDIChildState := ActiveMDIChild.WindowState;
            for I := 0 to MDIChildCount - 1 do
            begin
              LWindowPlacement.Length := SizeOf(LWindowPlacement);
              GetWindowPlacement(MDIChildren[I].Handle, LWindowPlacement);
              MDIChildren[I].FChildStoredBoundsRect := LWindowPlacement.rcNormalPosition;
              MDIChildren[I].FChildStoredMinPosition := LWindowPlacement.ptMinPosition;
            end;
          end
          else
            LActiveMDIChild := nil;
          ...
          if  LActiveMDIChild <> nil then
          begin
            if LActiveMDIChild.WindowState <> LActiveMDIChildState then
              LActiveMDIChild.WindowState := LActiveMDIChildState;
            LActiveMDIChild.Show;
          end;
          ...
          begin
            for I := 0 to MDIChildCount - 1 do
            begin
              if (MDIChildren[I].WindowState = wsNormal) or (MDIChildren[I].WindowState = wsMinimized) then
                MDIChildren[I].BoundsRect := MDIChildren[I].FChildStoredBoundsRect;

              if MDIChildren[I].WindowState <> wsNormal then
              begin
                LWindowPlacement.Length := SizeOf(LWindowPlacement);
                GetWindowPlacement(MDIChildren[I].Handle, LWindowPlacement);
                if (WindowState = wsMaximized) and not MDIChildren[I].FChildStoredNormalRect.IsEmpty then
                  LWindowPlacement.rcNormalPosition := MDIChildren[I].FChildStoredNormalRect
                else
                if (MDIChildren[I].WindowState = wsMinimized) then
                begin
                  LWindowPlacement.ptMinPosition := MDIChildren[I].FChildStoredMinPosition;
                  LWindowPlacement.flags := WPF_SETMINPOSITION;
                end;
                SetWindowPlacement(MDIChildren[I].Handle, LWindowPlacement);
              end;
            end;
          end;
...
begin
  with Message do
    case Msg of
      WM_NCACTIVATE:
      begin
        if not (csDestroying in ComponentState) and TOSVersion.Check(10) then
        begin
          if (FormStyle = fsMDIForm) and (ActiveMDIChild <> nil) then
            SendMessage(ActiveMDIChild.Handle, Message.Msg, Message.wParam, Message.LParam);
        end
        else
          Default;
      end;
      WM_NCCALCSIZE:
      begin
        if TOSVersion.Check(10) and (FormStyle = fsMDIForm) and TWMNCCalcSize(Message).CalcValidRects and
          not ((ActiveMDIChild <> nil) and IsZoomed(ActiveMDIChild.Handle)) then
        begin
          Default;
          LParams := TWMNCCalcSize(Message).CalcSize_Params;
          with LParams^.rgrc[0] do
          begin
            Dec(Top, 2);
            Dec(Left, 2);
            Inc(Right, 2);
...
function TCustomForm.GetDefaultCornerPreference: Cardinal;
begin
  case Application.DefaultRoundedCorners of
...
function TCustomForm.GetFormCornerPreference: Cardinal;
begin
...
procedure TCustomForm.SetRoundedCorners(const Value: TRoundedCornerType);
begin
...
procedure TCustomForm.SetWindowCorners;
var
  LCornerPreference: DWord;
begin
  if not (csDesigning in ComponentState) and HandleAllocated then
  begin
    LCornerPreference := GetFormCornerPreference;
    if TOSVersion.Check(10) and (TOSVersion.Build >= 22000) then // Windows 11
      DwmSetWindowAttribute(Handle, DWMWA_WINDOW_CORNER_PREFERENCE, @LCornerPreference, sizeof(DWord));
  end;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Application.FInitialMainFormState := wsNormal;
        end;
        LStyle := GetWindowLong(Handle, GWL_STYLE);
        case FWindowState of
          wsMaximized: LStyle := LStyle or WS_MAXIMIZE;
          wsMinimized: LStyle := LStyle or WS_MINIMIZE;
        else           LStyle := LStyle and not (WS_MAXIMIZE or WS_MINIMIZE);
        end;
        SetWindowLong(Handle, GWL_STYLE, LStyle);
      end
    end;
...
function TCustomForm.CanShowInTaskBar: Boolean;
begin
  Result := Application.MainFormOnTaskBar and (Parent = nil) and (FormStyle <> fsMDIChild);
end;
          ...
          pmExplicit:
            begin
              ...
              begin
                ...
                if (csRecreating in ControlState) and (Application.MainForm = Self) then
                  WndParent := 0
                else
                  WndParent := Application.MainFormHandle;
...
{$ENDIF}
      ...
      if (Application.MainFormOnTaskBar or FShowInTaskBar) and (FormStyle <> fsMDIChild) and
         ((FCreatingMainForm or (((csDesigning in ComponentState) or (csRecreating in ControlState)) and (Self = Application.MainForm)))
         or
         (not (csDesigning in ComponentState) and FShowInTaskBar and CanShowInTaskBar)) then
      begin
        if (Self = Application.MainForm) or FCreatingMainForm then
        begin
          Application.FMainFormOnTaskBar := True;
          FShowInTaskBar := True;
        end
        else
          WndParent := 0;
...
procedure TCustomForm.VisualManager_AddForm;
var
  LVisualManager: IFormVisualManager;
begin
  LVisualManager := GetVisualManager(True);
  if LVisualManager <> nil then
    LVisualManager.AddForm(TForm(Self));
end;

procedure TCustomForm.VisualManager_DeleteForm;
var
  LVisualManager: IFormVisualManager;
begin
  if not (csRecreating in ControlState) then
  begin
    LVisualManager := GetVisualManager(False);
    if LVisualManager <> nil then
      LVisualManager.DeleteForm(TForm(Self));
    FVisualManagerInitialized := False;
  end;
end;

function TCustomForm.VisualManager_AcceptMessage(Message: TMessage): Boolean;
begin
  case Message.Msg of
    WM_SHOWWINDOW,
    WM_MOUSEACTIVATE,
    ...
    CM_SYSCOMMAND:
      Result := True;
    WM_SIZE:
      Result := (TWMSize(Message).SizeType = SIZE_MINIMIZED);
    else
      // When a message is not managed by VisualManager, then exit immediately.
      // The form may be destroyed at this moment, for example after CM_RELEASE.
      // Then reading FVisualManagerInitialized will lead to AV (RSP-43547).
      Exit(False);
  end;
  if not FVisualManagerInitialized then
    Result := False;
end;

procedure TCustomForm.VisualManager_WndProc(Message: TMessage);
var
  LVisualManager: IFormVisualManager;
begin
  ...
  LVisualManager := GetVisualManager(False);
  if LVisualManager <> nil then
    LVisualManager.FormWndProc(TForm(Self), Message);
end;
...
begin
  ...
  begin
    ...
    InitMessageHandler(TChildFormMessageHandler);
  end
  else
  begin
    if not (csDesigning in ComponentState) and (Parent <> nil) and (FBorderStyle <> bsNone) and (FBorderStyle <> bsDialog) then
      InitMessageHandler(TChildFormMessageHandler)
    else
    if Assigned(MessageHandler) and (MessageHandler is TChildFormMessageHandler) then
...
procedure TCustomForm.WMNCRButtonUp(var Message: TWMNCLButtonUp);
begin
  inherited;
  if not (csDesigning in ComponentState) and (Message.HitTest = HTCAPTION) and Assigned(FCustomTitleBar) and FCustomTitleBar.Enabled and
     (biSystemMenu in FBorderIcons) and ((FBorderStyle = bsSingle) or (FBorderStyle = bsSizeable)) then
    SendMessage(Handle, WM_POPUPSYSTEMMENU, 0, MakeLong(Message.XCursor, Message.YCursor));
end;
...
begin
  if not (fsShowing in FormState) then LockDrawing;
...
begin
  if not (fsShowing in FormState) then UnlockDrawing;
...
var
  ...
  procedure ScaleWindowPlacemnt;
  begin
    LWindowPlacement.Length := SizeOf(LWindowPlacement);
    GetWindowPlacement(LChild.Handle, LWindowPlacement);
    LWindowPlacement.ptMinPosition.X := MulDiv(LWindowPlacement.ptMinPosition.X, FCurrentPPI, OldPPI);
    LWindowPlacement.ptMinPosition.Y := MulDiv(LWindowPlacement.ptMinPosition.Y, FCurrentPPI, OldPPI);
    LWindowPlacement.flags := WPF_SETMINPOSITION;
    SetWindowPlacement(LChild.Handle, LWindowPlacement);
    LChild.FChildStoredNormalRect.Left := MulDiv(LChild.FChildStoredNormalRect.Left, FCurrentPPI, OldPPI);
    LChild.FChildStoredNormalRect.Top := MulDiv(LChild.FChildStoredNormalRect.Top, FCurrentPPI, OldPPI);
    LChild.FChildStoredNormalRect.Right := MulDiv(LChild.FChildStoredNormalRect.Right, FCurrentPPI, OldPPI);
    LChild.FChildStoredNormalRect.Bottom := MulDiv(LChild.FChildStoredNormalRect.Bottom, FCurrentPPI, OldPPI);
    LWindowPlacement.rcNormalPosition := LChild.FChildStoredNormalRect;
    LWindowPlacement.flags := 0;
    SetWindowPlacement(LChild.Handle, LWindowPlacement);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FormStyle = fsMDIForm) and TOSVersion.Check(10) and CheckPerMonitorV2SupportForWindow(Handle) then
      begin
        for I := 0 to MDIChildCount - 1 do
        begin
          LChild := MDIChildren[I];
          if IsZoomed(LChild.Handle) then
          begin
            if LChild.Visible and not IsWindowVisible(LChild.Handle) then
              ShowWindow(LChild.Handle, SW_SHOW);
          end
          else
          begin
            if not LChild.FChildStoredNormalRect.IsEmpty and IsIconic(LChild.Handle) then
              ScaleWindowPlacemnt
            else
            begin
              L := MulDiv(LChild.Left, FCurrentPPI, OldPPI);
              T := MulDiv(LChild.Top, FCurrentPPI, OldPPI);
              LChild.SetBounds(L, T, LChild.Width, LChild.Height);
              LChild.FChildStoredNormalRect := LChild.BoundsRect;
            end;
          end;

          if LChild.IsCustomStyleActive then
            LChild.Perform(TMessage(Message).Msg, TMessage(Message).WParam, TMessage(Message).LParam);

          SetWindowPos(LChild.Handle, 0, 0, 0, 0, 0, SWP_FRAMECHANGED or SWP_NOACTIVATE or
            SWP_NOMOVE or SWP_NOSIZE or SWP_NOZORDER);
        end;
        if FClientHandle <> 0 then
        begin
          SetWindowPos(FClientHandle, 0, 0, 0, ClientWidth - 1, ClientHeight, SWP_NOACTIVATE or SWP_NOMOVE or SWP_NOZORDER);
          SetWindowPos(FClientHandle, 0, 0, 0, ClientWidth + 1, ClientHeight, SWP_NOACTIVATE or SWP_NOMOVE or SWP_NOZORDER);
          ReAlign;
        end;
      end;
      if (FormStyle <> fsMDIForm) and TOSVersion.Check(10) and CheckPerMonitorV2SupportForWindow(Handle) and (Parent = nil) then
      begin
        for I := 0 to Screen.FormCount - 1 do
        begin
          if (Screen.Forms[I].Parent <> nil) and (Screen.Forms[I].BorderStyle <> bsNone) and (GetParentForm(Screen.Forms[I]) = Self) then
          begin
            LChild := Screen.Forms[I];
            if IsZoomed(LChild.Handle) then
            begin
              if LChild.Visible then
                ShowWindow(LChild.Handle, SW_SHOWMAXIMIZED);
            end
            else
            if IsIconic(LChild.Handle) then
              ScaleWindowPlacemnt
            else
            begin
              L := MulDiv(LChild.Left, FCurrentPPI, OldPPI);
              T := MulDiv(LChild.Top, FCurrentPPI, OldPPI);
              if LChild.AutoScroll then
              begin
                W := MulDiv(LChild.Width, FCurrentPPI, OldPPI);
                H := MulDiv(LChild.Height, FCurrentPPI, OldPPI);
              end
              else
              begin
                W := LChild.Width;
...
var
  LNeedsBringToFront: Boolean;
begin
  Visible := True;
  LNeedsBringToFront := True;
  if FShowInTaskBar and CanShowInTaskBar and (Self <> Application.MainForm) and HandleAllocated and IsIconic(Handle) then
  begin
    if not FFirstShowing then
      SendMessage(Handle, WM_SYSCOMMAND, SC_RESTORE, 0);
    LNeedsBringToFront := not FFirstShowing;
  end;
  if LNeedsBringToFront then
    BringToFront;
  FFirstShowing := False;
end;
...
begin
  if HandleAllocated and
    not ((FormStyle = fsMDIChild) and (Application.MainForm <> nil) and (csRecreating in Application.MainForm.ControlState)) then
  begin
...
begin
  if (FFormStyle = fsMDIForm) and (ClientHandle <> 0) then
    if TStyleManager.ChangeChildFormSystemBorder and TOSVersion.Check(10) and not IsCustomStyleActive and (ActiveMDIChild <> nil) and
       (ActiveMDIChild.MessageHandler <> nil) and (ActiveMDIChild.MessageHandler is TChildFormMessageHandler) then
      SendMessage(ActiveMDIChild.Handle, WM_MDITILE, TileParams[FTileMode], 0)
    else
    begin
      if (ActiveMDIChild <> nil) and (ActiveMDIChild.MessageHandler <> nil) and (ActiveMDIChild.MessageHandler is TChildFormMessageHandler) then
        TChildFormMessageHandler(ActiveMDIChild.MessageHandler).RestoreAll;
...
begin
  if (FFormStyle = fsMDIForm) and (ClientHandle <> 0) then
    if TStyleManager.ChangeChildFormSystemBorder and TOSVersion.Check(10) and not IsCustomStyleActive and (ActiveMDIChild <> nil) and
       (ActiveMDIChild.MessageHandler <> nil) and (ActiveMDIChild.MessageHandler is TChildFormMessageHandler) then
      SendMessage(ActiveMDIChild.Handle, WM_MDICASCADE, 0, 0)
    else
    begin
      if (ActiveMDIChild <> nil) and (ActiveMDIChild.MessageHandler <> nil) and (ActiveMDIChild.MessageHandler is TChildFormMessageHandler) then
        TChildFormMessageHandler(ActiveMDIChild.MessageHandler).RestoreAll;
...
{$ENDIF}
  ...
  FIconFont.IsScreenFont := True;
  FMenuFont := TFont.Create;
  FMenuFont.IsScreenFont := True;
  FHintFont := TFont.Create;
  FHintFont.IsScreenFont := True;
  FMessageFont := TFont.Create;
  FMessageFont.IsScreenFont := True;
  FCaptionFont := TFont.Create;
  FCaptionFont.IsScreenFont := True;
  GetMetricSettings;
...
function TScreen.GetCursorHeightMargin: Integer;
{$IF DEFINED(CLR)}
begin
...
function FindHintControl(Control: TControl): TControl;
begin
  Result := Control;
...
begin
  ...
  FDefaultFont.IsScreenFont := True;
...
function TApplication.GetHintControl: TControl;
begin
  Result := FHintControl;
end;
...
begin
  ...
  NewHintControl := FindHintControl(FindDragTarget(Control.ClientToScreen(SmallPointToPoint(MouseMsg.Pos)), True));
...
begin
  ...
  if FShowHint and (FHintControl <> nil) and ForegroundTaskCheck(EnumAllWindowsOnActivateHint) and
    (FHintControl = FindHintControl(FindDragTarget(CursorPos, True))) and
...
begin
  ...
  begin
    ...
    begin
      ...
      FMainForm.FShowInTaskBar := Value;
      // Recreate other forms to ensure correct style
      for I := 0 to Screen.FormCount - 1 do
      begin
        LForm := Screen.Forms[I];
        if (LForm <> FMainForm) and LForm.ShowInTaskBar and LForm.CanShowInTaskBar then
          LForm.RecreateWnd;
      end;
    end;
...
begin
  ...
  try
    if (FStyleHook <> nil) and
       ...
       (FStyleHook.Control.Visible or
        FStyleHook.Control.HandleAllocated and IsWindowVisible(FStyleHook.Control.Handle)) then
    begin
      DC := GetWindowDC(Handle);
      try
        if FVertical then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LHandlePrev := GetWindow(Control.Handle, GW_HWNDPREV);
        SetWindowPos(FVertScrollWnd.Handle, LHandlePrev, Control.Left + LRect.Left,
         Control.Top + LRect.Top, LRect.Right - LRect.Left, LRect.Bottom - LRect.Top, SWP_SHOWWINDOW);
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LHandlePrev := GetWindow(Control.Handle, GW_HWNDPREV);
        SetWindowPos(FHorzScrollWnd.Handle, LHandlePrev, Control.Left + LRect.Left,
          Control.Top + LRect.Top, LRect.Right - LRect.Left, LRect.Bottom - LRect.Top, SWP_SHOWWINDOW);
      end;
...
procedure TFormStyleHook.TMainMenuBarStyleHook.ReleaseIcon;
begin
  FreeAndNil(FIcon);
end;
...
begin
  ...
  begin
    ...
    if FShowMDIButtons then
      FShowMDIButtons := (FFormHook.Form.ActiveMDIChild <> nil) and (FFormHook.Form.ActiveMDIChild.BorderIcons <> []);
    FHotMDIButton := -1;
    FPressedMDIButton := -1;
    FOldMDIHotButton := -1;
    if not FShowMDIButtons then
      FreeAndNil(FIcon);
    Invalidate;
  end;
end;
...
begin
    ...
    if PeekMessage(Msg, 0, 0, 0, PM_REMOVE) then
      case Msg.message of
        ...
        CM_RELEASE: 
          begin 
            FInMenuLoop := False;
            FDispatchMessage := Msg.hwnd <> FFormHook.Form.Handle;
          end;
        WM_CLOSE: 
          begin
            FInMenuLoop := False;
  ...
  if FDispatchMessage then
    DispatchMessage(Msg)
  else if Msg.Message = CM_RELEASE then
...
begin
  if (FFormHook.Form.ActiveMDIChild <> nil) and (biMaximize in FFormHook.Form.ActiveMDIChild.BorderIcons) then 
    SendMessage(FFormHook.Form.ActiveMDIChild.Handle,
...
begin
  if (FFormHook.Form.ActiveMDIChild <> nil) and (biMinimize in FFormHook.Form.ActiveMDIChild.BorderIcons) then
    SendMessage(FFormHook.Form.ActiveMDIChild.Handle,
...
begin
  ...
  try
    ...
    if CheckPerMonitorV2SupportForWindow(FFormHook.Form.Handle) then
      Buffer.Canvas.Font.ScaleForDPI(FFormHook.Form.CurrentPPI);
...
begin
  ...
  HookMenus;
  SendMessage(Child.Handle, WM_POPUPSYSTEMMENU, 0, MakeLong(X, Y));
  UnHookMenus;
...
begin
  ...
  HookMenus;
  SendMessage(FFormHook.Handle, WM_POPUPSYSTEMMENU, 0, MakeLong(X, Y));
  UnHookMenus;
...
begin
  ...
  if not Result and IsStyleBorder then
    case Message.Msg of
      ...
      WM_SETTEXT, WM_NCUAHDRAWCAPTION:
        if Form.FormStyle = fsMDIChild then
        begin
          WndProc(Message);
          Result := Handled;
...
begin
  ...
  then
    begin
      FMDIHorzScrollBar.Height := GetSystemMetrics(SM_CYHSCROLL);
  ...
  if (FMDIVertScrollBar <> nil) and IsWindowVisible(FMDIVertScrollBar.Handle) then
  begin
    FMDIVertScrollBar.Width := GetSystemMetrics(SM_CYHSCROLL);
    if (FMDIHorzScrollBar <> nil) and IsWindowVisible(FMDIHorzScrollBar.Handle)
    then
      SetWindowPos(FMDIVertScrollBar.Handle, HWND_TOP,
...
begin
  ...
  case Message.Msg of
    WM_MDIDESTROY:
      PostMessage(Handle, WM_MDICHILDCLOSE, Message.wParam, 0);
    WM_NCACTIVATE:
      begin
...
function TFormStyleHook.GetBorderSize(UseActiveStyle: Boolean = True): TRect;
var
  Size: TSize;
  Details: TThemedElementDetails;
  ...
  // We can't use AdjustWindowRectExForWindow because we need to pass a custom PPI to AdjustWindowRectExForDPI
  function AdjustWindowRectExForPPI(var lpRect: TRect; dwStyle: DWORD; bMenu: BOOL; dwExStyle: DWORD): Boolean;
  begin
    if CheckPerMonitorV2SupportForWindow(Form.Handle) then
      Result := AdjustWindowRectExForDpi(lpRect, dwStyle,  bMenu, dwExStyle, Form.PixelsPerInch)
    else
...
begin
  ...
  if UseActiveStyle and (csDesigning in Form.ComponentState) then
  begin
    AdjustWindowRectExForPPI(Result, GetWindowLong(Form.Handle, GWL_STYLE),
      False, GetWindowLong(Form.Handle, GWL_EXSTYLE));
    Result.Left := Abs(Result.Left);
    Result.Top := Abs(Result.Top);
    FCaptionHeight := Result.Top;
    Exit;
  end;
  ...
  if not (csDesigning in Form.ComponentState) and
     ...
     (((Form.WindowState = wsMaximized) and (Form.Parent = nil)) or
  ...
  begin
    ...
    if TOSVersion.Check(10) and IsZoomed(Handle) and (Form.FormStyle = fsMDIChild) then
    begin
      FW := WinApi.Windows.GetSystemMetrics(SM_CXFRAME) +
        MulDiv(4, Screen.PixelsPerInch, Screen.DefaultPixelsPerInch);
      FH := FW;
    end
    else
...
var
  ...
  Menu: HMENU;
begin
  if ((Form.FormStyle = fsMDIChild) or (Form.Parent <> nil)) and (ID = SC_CLOSE) then
  begin
...
var
  ...
  Detail: TThemedWindow;
  LStyledBorderRect, R, R1, R2, DrawRect, ButtonRect, TextRect: TRect;
...
begin
  if ((Form = Application.MainForm) and IsIconic(Form.Handle)) or
     ((Form.FormStyle = fsMDIChild) and IsZoomed(Form.Handle)) then
    Exit;
  ...
  R := GetBorderSize;
  LStyledBorderRect := GetBorderSize(False);
  {draw caption}
  ...
  if (csDesigning in Control.ComponentState) and (TempRect.Height > LStyledBorderRect.Top) then
  begin
    var LBitmap := TBitmap.Create;
    try
      LBitmap.SetSize(TempRect.Width, TempRect.Height);
      LStyle.DrawElement(LBitmap.Canvas.Handle, Details, TempRect, nil, LPPI);
      // cut image
      LBitmap.SetSize(TempRect.Width, LStyledBorderRect.Top);
      SetStretchBltMode(CaptionBuffer.Handle, HALFTONE);
      CaptionBuffer.Canvas.StretchDraw(TempRect, LBitmap);
    finally
      LBitmap.Free;
    end;
  end
  ...
  begin
    ...
    if not LStyle.HasElementFixedPosition(Details) then
     ...
     if IsZoomed(Handle) and (Form.Parent <> nil) then
       OffsetRect(ButtonRect, R.Left, 0);

    if (csDesigning in Control.ComponentState) and
      (Control.PixelsPerInch <> Screen.PixelsPerInch) then
    begin
    ...
    begin
      if IsIconic(Form.Handle) and (Form <> Application.MainForm) then
      begin
        IX := WinApi.Windows.GetSystemMetrics(SM_CXSMICON);
        IY := WinApi.Windows.GetSystemMetrics(SM_CYSMICON);
      end
      else
      begin
  ...
  begin
    ...
    begin
      ...
      var XOffset := R.Right - LStyledBorderRect.Right;
      OffsetRect(DrawRect, XOffset, 0);
    end;
    Details := LStyle.GetElementDetails(Detail);
    LStyle.DrawElement(Canvas.Handle, Details, DrawRect);
  end;
  ...
  begin
    ...
    begin
      Details := LStyle.GetElementDetails(Detail);
      if (csDesigning in Control.ComponentState) and (DrawRect.Height > LStyledBorderRect.Bottom) then
      begin
        var LBitmap := TBitmap.Create;
        try
          LBitmap.SetSize(DrawRect.Width, DrawRect.Height);
          LStyle.DrawElement(LBitmap.Canvas.Handle, Details, Rect(0, 0, DrawRect.Width, DrawRect.Height), nil, LPPI);
          // cut image
          LBitmap.SetSize(DrawRect.Width, LStyledBorderRect.Bottom);
          SetStretchBltMode(Canvas.Handle, HALFTONE);
          Canvas.StretchDraw(DrawRect, LBitmap);
        finally
          LBitmap.Free;
        end;
      end
...
procedure TFormStyleHook.WMNCActivate(var Message: TMessage);
  ...
  function IsChildForm: Boolean;
  begin
    Result := (Form.FormStyle = fsMDIChild) or ((Form.Parent <> nil) and (Form.BorderStyle <> bsNone));
  end;
...
begin
  ...
  if IsChildForm  then
  begin
    if IsChildForm and (Win32MajorVersion >=6) then
      SetRedraw(False);
    ...
    begin
      ...
      if not (csDestroying in Control.ComponentState) and
        ...
        RedrawWindow(Handle, nil, 0, RDW_ERASE + RDW_INVALIDATE + RDW_ALLCHILDREN + RDW_UPDATENOW);
    end;
  end;
...
begin
  ...
  if FChangeSizeCalled and not (IsIconic(Handle) and (Form <> Application.MainForm) and (Form.FormStyle <> fsMDIChild)) then
    Exit;
  ...
  begin
    GetClientRect(Form.Parent.Handle, R);
    R1 := GetBorderSize;
    Message.WindowPos^.y := -1;
    Message.WindowPos^.x := -R1.Left;
    Message.WindowPos^.cx := R.Width + R1.Left + R1.Right;
    Message.WindowPos^.cy := R.Height + R1.Bottom + 1;
  end;
  ...
  if Changed then
  begin
    if IsIconic(Handle) and (Form.Parent <> nil) then
      FNeedChangeSize := True
    else
    begin
      ChangeSize;
      if Form.BorderStyle <> bsNone then
...
begin
  ...
  case Message.Msg of
    ...
    WM_SIZE:
    begin
      if FNeedChangeSize then
      begin
        FNeedChangeSize := False;
        ChangeSize;
        if Form.BorderStyle <> bsNone then
          InvalidateNC;
      end;
...
procedure TFormStyleHook.ReleaseIcon;
begin
  FreeAndNil(FIcon);
  if FMainMenuBarHook <> nil then
    FMainMenuBarHook.ReleaseIcon;
end;
