...
begin
  case MDIAction of
    maActivate:
      SendMessage(GetParent(Form.Handle), WM_MDIACTIVATE, Form.Handle, 0);
    maClose:
...
{$ENDIF}
    WM_ERASEBKGND:
    begin
      if DoubleBuffered and (LPARAM(TMessage(Message).wParam) <> TMessage(Message).lParam) then
      begin
        Message.Result := 1;
        exit;
      end;
...
begin
  ...
  if (LControl <> nil) and (csDesigning in LControl.ComponentState) then
...
var
  LStyle: TCustomStyleServices;
  LState: TThemedWindow;
  LMenuState: TThemedMenu;
...
begin
  ...
  begin
    if (FState = bsDown) or MouseInControl then
    begin
      if FState = bsDown then
        LDetails := LStyle.GetElementDetails(ttbButtonHot)
      else
        LDetails := LStyle.GetElementDetails(ttbButtonPressed);
      LStyle.DrawElement(Canvas.Handle, LDetails,
        R, nil, FCurrentPPI);
    end;

    LMenuState := tmSystemRestoreNormal;
    case ButtonStyle of
      mbMinimize:
      begin
        LMenuState := tmSystemMinimizeNormal;
        Dec(R.Bottom, ScaleValue(2));
      end;
      mbClose:
      begin
        LMenuState := tmSystemCloseNormal;
        Inc(R.Left, ScaleValue(2));
        Dec(R.Bottom, ScaleValue(2));
      end;
    end;
    LDetails := LStyle.GetElementDetails(LMenuState);
    LStyle.DrawElement(Canvas.Handle, LDetails,
      R, nil, FCurrentPPI);
  end
  else
  begin
    LState := twMDICloseButtonNormal;
    case ButtonStyle of
      mbMinimize:
        begin
          if FState = bsDown then
            LState := twMDIMinButtonPushed
          else
          if MouseInControl then
            LState := twMDIMinButtonHot
          else
            LState := twMDIMinButtonNormal;
        end;
      mbRestore:
        begin
          if FState = bsDown then
            LState := twMDIRestoreButtonPushed
          else
          if MouseInControl then
            LState := twMDIRestoreButtonHot
          else
            LState := twMDIRestoreButtonNormal;
        end;
      mbClose:
        begin
          if FState = bsDown then
            LState := twMDICloseButtonPushed
          else
          if MouseInControl then
            LState := twMDICloseButtonHot;
        end;
    end;
    LDetails := LStyle.GetElementDetails(LState);
    LStyle.DrawElement(Canvas.Handle, LDetails,
      R, nil, FCurrentPPI);
  end;
end;
...
procedure TCustomActionMainMenuBar.MDIChildMaximized(ChildForm: TCustomForm);
  ...
  begin
    ...
    begin
      ...
      FScaleFactor := Self.ScaleFactor;
      FCurrentPPI := Self.CurrentPPI;
      Visible := True;
    end;
  end;
