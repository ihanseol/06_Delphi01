...
procedure TVirtualImageList.ResetOverlays;
var
  I: TOverlay;
begin
  for I := Low(FOverlays) to High(FOverlays) do
    FOverlays[I] := -1;
end;

procedure TVirtualImageList.UpdateOverlays;
var
  I: TOverlay;
begin
  if HandleAllocated then
    for I := Low(FOverlays) to High(FOverlays) do
      if FOverlays[I] <> -1 then
        ImageList_SetOverlayImage(Handle, FOverlays[I], I + 1);
end;

function TVirtualImageList.Overlay(ImageIndex: Integer; Overlay: TOverlay): Boolean;
begin
  Result := inherited Overlay(ImageIndex, Overlay);
  if Result then
    FOverlays[Overlay] := ImageIndex;
end;
...
function TVirtualImageList.Merge(AVirtualImageList: TVirtualImageList): Integer;
begin
  if (AVirtualImageList <> nil) and (ImageCollection = AVirtualImageList.ImageCollection) then
    Result := FImages.Merge(AVirtualImageList.Images)
  else
    Result := -1;
end;

procedure TVirtualImageList.AddImages(AVirtualImageList: TVirtualImageList);
begin
  Merge(AVirtualImageList);
end;
