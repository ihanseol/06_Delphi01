...
type
  ...
  private
    ...
    FDesignMode: Boolean;
    FTrapHitTest: Boolean;
    FTimerDelay: Cardinal;
    FTimer: TTimer;
    ...
    procedure SetTrapHitTest(const Value: Boolean);
  protected
    ...
    property TrapHitTest: Boolean read FTrapHitTest write SetTrapHitTest;
  public
    property Canvas;
...
procedure TSystemTitlebarButton.TimerEvent(Sender: TObject);
begin
  if FTrapHitTest then
  begin
    var LPoint: TPoint;
    GetCursorPos(LPoint);
...
begin
  ...
  begin
    ...
    begin
      ...
      if MouseInControl or FTrapHitTest then
      begin
        if FButtonType = sbClose then
        begin
...
procedure TSystemTitlebarButton.SetTrapHitTest(const Value: Boolean);
begin
  if FTrapHitTest <> Value then
  begin
    FTrapHitTest := Value;
    if FTrapHitTest then
      FTimer.Enabled := True;
...
procedure TSystemTitlebarButton.WndProc(var Message: TMessage);
begin
  if FTrapHitTest and (Message.Msg = CM_HITTEST) then
    Message.Result := 0
  else
    inherited;
end;
...
begin
  ...
  FIsWin11 := TOSVersion.Check(10) and (TOSVersion.Build >= 22000);
  FAlphaValue := 255;
  ...
  ControlStyle := ControlStyle + [csAcceptsControls, csOverrideStylePaint] - [csOpaque];
...
const
  cDefaultCustomButtonSize = 23; // TCustomSpeedButton.Create
var
...
begin
  ...
  begin
    if (FCustomButtons[i].Button.Width > 0) and (FCustomButtons[i].Button.Width <> cDefaultCustomButtonSize) then
      W := FCustomButtons[i].Button.Width;
    if csDesigning in ComponentState then
...
begin
  ...
  LClientTop := -Round(5 * FClient.CurrentPPI / Screen.DefaultPixelsPerInch + 0.01) - 3;
  ...
  begin
    ...
    begin
      if not Client.CustomTitleBar.SystemHeight or (Client.BoundsRect.Top >= 0) then
        ...
        LPoint.Y := LClientTop div 2; // LClientTop returns the pixel offset when the form is maximized
...
procedure TCustomTitleBarPanel.WndProc(var Message: TMessage);
begin
  inherited;
  if FIsWin11 and (Message.Msg = WM_NCHITTEST) then
  begin
    var LPoint := ScreenToClient(Point(TWMNCHitTest(Message).XPos, TWMNCHitTest(Message).YPos));
    if LPoint.Y > Client.GlassFrame.Top then
      exit;
    if FTitleButtonRestore.BoundsRect.Contains(LPoint) and not FTitleButtonRestore.TrapHitTest then
    begin
      FTitleButtonRestore.TrapHitTest := True;
      FTitleButtonRestore.Invalidate;
    end else if not FTitleButtonRestore.BoundsRect.Contains(LPoint) and FTitleButtonRestore.TrapHitTest then
    begin
      FTitleButtonRestore.TrapHitTest := False;
      FTitleButtonRestore.Invalidate;
    end;
  end;
end;
