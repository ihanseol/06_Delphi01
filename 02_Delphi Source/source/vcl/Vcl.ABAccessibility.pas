...
begin
  ...
  begin
    ...
    if Assigned(Control) and Assigned(TCustomActionBar(Control).ActionClient) then
    begin
      pvarChild := TCustomActionBar(Control).ActionClient.Index + 1;
      Result := S_OK;
    end;
  end
  else
  if (FActionClient is TActionClientItem) and Assigned(TActionClientItem(FActionClient).Control) and Assigned(FActionClient.ChildActionBar) then
  begin
