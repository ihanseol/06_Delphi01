...
type
  ...
  protected
    ...
    function GetSourceImageIndex(AIndex: Integer; AWidth, AHeight: Integer; AProportional: Boolean): Integer;
...
procedure TImageCollectionItemSources.Dormant;
begin
  for var I := 0 to Count - 1 do
    GetItem(I).Image.Dormant;
end;
...
procedure TImageCollectionItems.Dormant;
begin
  for var I := 0 to Count - 1 do
    GetItem(I).FSourceImages.Dormant;
end;
...
function TImageCollection.GetSourceImageIndex(AIndex: Integer; AWidth, AHeight: Integer; AProportional: Boolean): Integer;
var
...
function TImageCollection.GetSourceImage(AIndex: Integer; AWidth, AHeight: Integer; AProportional: Boolean): TWICImage;
var
  FIndex: Integer;
begin
  if AIndex < 0 then
  ...
  begin
    FIndex := GetSourceImageIndex(AIndex, AWidth, AHeight, AProportional);
    if FIndex >= 0 then
...
begin
  ...
  if Assigned(SourceImage.Bitmap) and not SourceImage.Bitmap.HandleAllocated then
    SourceImage.Bitmap.HandleNeeded;
  try
    ...
    if FUseDormantBitmaps and Assigned(SourceImage.Bitmap) and SourceImage.Bitmap.HandleAllocated then
      SourceImage.Dormant;
  end;
end;
...
begin
  ...
  SourceImage := GetSourceImage(AIndex, ARect.Width, ARect.Height, AProportional);
  if SourceImage <> nil then
  begin
    if AProportional then
      ARect := UpdateRectForProportionalSize(ARect, SourceImage.Width, SourceImage.Height, True);
...
procedure TImageCollection.Dormant;
begin
...
begin
  if Assigned(FOnDraw) then
    FOnDraw(GetSourceImage(AIndex, ARect.Width, ARect.Height, AProportional), ACanvas, ARect, AProportional)
  else
