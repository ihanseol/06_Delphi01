...
uses
  ...
  Winapi.CommCtrl, Winapi.Imm, Vcl.ImgList, Vcl.ActnList, System.SyncObjs, System.Types, System.UITypes, System.Generics.Collections
{$IF DEFINED(LINUX)}
  , WinUtils;
{$ELSE}
  ;
...
{$ENDIF}
  ...
  CM_STYLEELEMENTSCHANGED        = CM_BASE + 94;
...
{ TControl }
  ...
  TControlStateItem = (csLButtonDown, csClicked, csPalette,
    ...
    csPrintClient, csReadingProps, csDPIIsolated);
  ...
  TControlStyleItem = (csAcceptsControls, csCaptureMouse,
  ...
  TControlStyle = set of TControlStyleItem;
...
{$ENDIF}
    ...
    procedure CMStyleElementsChanged(var Message: TMessage); message CM_STYLEELEMENTSCHANGED;
...
{$ENDIF}
  protected
    ...
    procedure DoStyleElementsChanged; virtual;
    ...
    function GetStyleElements: TStyleElements; virtual;
...
{$ENDIF}
    ...
    function GetInternalStyleElements: TStyleElements;
...
{$ENDIF}
    property StyleElements: TStyleElements read GetStyleElements write SetStyleElements default [seFont, seClient, seBorder];
...
{ TWinControl }
  ...
  TWinControlMessageHandler = class
  private
    FControl: TWinControl;
    FDefaultWinProcCalling: Boolean;
    function GetHandle: HWND;
  protected
    procedure DefaultWinProc(var Message: TMessage);
    function HandleMessage(var Message: TMessage): Boolean; virtual;
    property IsDefaultWinProcCalling: Boolean read FDefaultWinProcCalling;
  public
  ...
  TWinControlMessageHandlerClass = class of TWinControlMessageHandler;

  TControlEnumeratorFilterType = (ceftAll, ceftEnabled, ceftDisabled, ceftVisible, ceftInvisible, ceftCustom);
  TControlEnumeratorFilter = set of TControlEnumeratorFilterType;
  ...
  private
    ...
    FIndex: Integer;
  protected
    procedure EnumControls(AControl: TWinControl; AFilter: TControlEnumeratorFilter);
  public
    constructor Create(AControl: TWinControl; AFilter: TControlEnumeratorFilter);
    destructor Destroy; override;
  ...
  TDoubleBufferedMode = (dbmDefault, dbmRequested);
...
{$ENDIF}
    ...
    FMessageHandler: TWinControlMessageHandler;
    FOnEnumerateControl: TEnumerateControlEvent;
    ...
    procedure SetDoubleBufferedMode(Value: TDoubleBufferedMode);
    procedure SetDPIScaling(Value: Boolean);
...
{$ENDIF}
  protected
    ...
    FDoubleBufferedMode: TDoubleBufferedMode;
...
{$ENDIF}
    ...
    /// <summary>
    /// Defines that double buffering can be enabled for control.
    /// </summary>
    function CanUseDoubleBuffering: Boolean; virtual;
    ...
    procedure CreateMessageHandler(AMessageHandlerClass: TWinControlMessageHandlerClass);
    ...
    procedure DoStyleElementsChanged; override;
    ...
    function IsDPIScalingRequired: Boolean; virtual;
    procedure RequestDPIScaling; overload;
    procedure RequestDPIScaling(Value: Boolean); overload;
...
{$ENDIF}
    ...
    property MessageHandler: TWinControlMessageHandler read FMessageHandler;
...
{$ENDIF}
    ...
    procedure DefaultHandler(var Message); override;
    procedure InitMessageHandler(AMessageHandlerClass: TWinControlMessageHandlerClass); virtual;
    procedure ReleaseMessageHandler;
    ...
    property DoubleBufferedMode: TDoubleBufferedMode read FDoubleBufferedMode write SetDoubleBufferedMode default dbmDefault;
    ...
    /// <summary>
    /// Use this method to force creating handles of child controls.
    /// It helps to do correct adjusting of the control or form before showing.
    /// </summary>
    ...
    function CanEnumerateControl(AControl: TControl): Boolean; virtual;
    ...
    property OnEnumerateControl: TEnumerateControlEvent read FOnEnumerateControl write FOnEnumerateControl;
  end;
...
{ Mouse support }
  ...
  private
    ...
    FWheelRouting: TWheelRouting;
    procedure GetMouseData;
    procedure GetNativeData;
    procedure GetWheelData;
  ...
  protected
    ...
    procedure InternalPositionAt(const Rect: TRect; AAdjustCursor: Boolean); virtual;
  public
    constructor Create(AOwner: TComponent); override;
    procedure AutoSize;
  ...
  protected
    ...
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
    function GetHintPosition(const ARect: TRect): THintPosition; virtual;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property CurrentPPI: Integer read GetCurrentPPI;
  ...
  published
    ...
    property Position: THintPosition read FPosition write FPosition default hpAutoFromBottom;
  end;
  ...
  TDPIContextService = class
    class function IsControlDPIIsolated(AControl: TWinControl): Boolean;
    ...
    class function GetClientRect(hWnd: HWND; var lpRect: TRect): Boolean; overload;
    class function GetClientRect(AControl: TWinControl; var lpRect: TRect): Boolean;  overload;
    ...
    class function GetWindowPlacement(hWnd: HWND; var WindowPlacement: TWindowPlacement): Boolean; overload;
    class function GetWindowPlacement(AControl: TWinControl; var WindowPlacement: TWindowPlacement): Boolean; overload;
    class function SetWindowPlacement(hWnd: HWND; const WindowPlacement: TWindowPlacement): Boolean;
    class function SetWindowPos(hWnd: HWND; hWndInsertAfter: HWND; X, Y, cx, cy: Integer; uFlags: UINT): Boolean;
    class function ClientToScreen(hWnd: HWND; var lpPoint: TPoint): Boolean;
    class function ScreenToClient(hWnd: HWND; var lpPoint: TPoint): Boolean;
  end;
...
var UpdatingDPIContext: Boolean = False;
var UpdatingDPIContextCount: Integer = 0;

class function TDPIContextService.ScaleValue(AControl: TControl; const Value: Integer): Integer;
begin
  if AControl.FRootControl = nil then
    AControl.UpdateRootControl;
  Result := MulDiv(Value, Screen.DefaultPixelsPerInch, AControl.FRootControl.PixelsPerInch);
end;

class function TDPIContextService.ScaleValue(AControl: TControl; const Value: Double): Double;
begin
  if AControl.FRootControl = nil then
    AControl.UpdateRootControl;
  Result := Value * AControl.FScaleFactor;
end;

class function TDPIContextService.ScaleValue(AControl: TControl; const Value: TPoint): TPoint;
begin
  Result.X := TDPIContextService.ScaleValue(AControl, Value.X);
  Result.Y := TDPIContextService.ScaleValue(AControl, Value.Y);
end;

class function TDPIContextService.ScaleValue(AControl: TControl; const Value: TRect): TRect;
begin
  Result.TopLeft := TDPIContextService.ScaleValue(AControl, Value.TopLeft);
  Result.BottomRight := TDPIContextService.ScaleValue(AControl, Value.BottomRight);
end;

class function TDPIContextService.IsControlDPIIsolated(AControl: TWinControl): Boolean;
begin
  Result := (AControl <> nil) and
    ((csDPIIsolated in AControl.ControlState) or ((AControl.Parent <> nil) and (csDPIIsolated in AControl.Parent.ControlState)));
end;

Procedure TrySetUpdatingDPIContext(AValue: Boolean);
begin
  if AValue then
  begin
    if UpdatingDPIContextCount = 0 then
      UpdatingDPIContext := AValue;
    Inc(UpdatingDPIContextCount);
  end
  else
  begin
    if UpdatingDPIContextCount = 0 then
...
function TrySetThreadDpiAwarenessContext(DPIContext: DPI_AWARENESS_CONTEXT; out ContextUpdated: Boolean): DPI_AWARENESS_CONTEXT;
begin
  Result := GetThreadDpiAwarenessContext;
  var LDPIAwareness := GetAwarenessFromDpiAwarenessContext(Result);
  if LDPIAwareness <> GetAwarenessFromDpiAwarenessContext(DPIContext) then
  begin
    Result := SetThreadDpiAwarenessContext(DPIContext);
    ContextUpdated := True;
  end
  else
...
class function TDPIContextService.GetClientRect(hWnd: HWND; var lpRect: TRect): Boolean;
begin
  var LControl := FindControl(hWnd);
  if (LControl <> nil) and TDPIContextService.IsControlDPIIsolated(LControl) then
  begin
    var LUpdated := False;
    TrySetUpdatingDPIContext(True);
    var LPreviousDPIContext := TrySetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_UNAWARE, LUpdated);
    try
      Result := Winapi.Windows.GetClientRect(hWnd, lpRect);
    finally
      if LUpdated then
        SetThreadDpiAwarenessContext(LPreviousDPIContext);
      TrySetUpdatingDPIContext(False);
    end;
  end
  else
    Result := Winapi.Windows.GetClientRect(hWnd, lpRect);
...
class function TDPIContextService.GetClientRect(AControl: TWinControl; var lpRect: TRect): Boolean;
begin
  if TDPIContextService.IsControlDPIIsolated(AControl) then
    Result := TDPIContextService.GetClientRect(AControl.Handle, lpRect)
  else
    Result := Winapi.Windows.GetClientRect(AControl.Handle, lpRect);
end;

class function TDPIContextService.GetWindowRect(hWnd: HWND; var lpRect: TRect): Boolean;
begin
  var LControl := FindControl(hWnd);
  if (LControl <> nil) and TDPIContextService.IsControlDPIIsolated(LControl) then
  begin
    ...
    var LPreviousDPIContext := TrySetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_UNAWARE, LUpdated);
    try
      Result := Winapi.Windows.GetWindowRect(hWnd, lpRect);
    finally
      if LUpdated then
        SetThreadDpiAwarenessContext(LPreviousDPIContext);
      TrySetUpdatingDPIContext(False);
...
class function TDPIContextService.GetWindowRect(AControl: TWinControl; var lpRect: TRect): Boolean;
begin
  if TDPIContextService.IsControlDPIIsolated(AControl) then
    Result := TDPIContextService.GetWindowRect(AControl.Handle, lpRect)
  else
    Result := Winapi.Windows.GetWindowRect(AControl.Handle, lpRect);
end;

class function TDPIContextService.GetWindowPlacement(hWnd: HWND; var WindowPlacement: TWindowPlacement): Boolean;
begin
  var LControl := FindControl(hWnd);
  if (LControl <> nil) and TDPIContextService.IsControlDPIIsolated(LControl) then
  begin
    ...
    var LPreviousDPIContext := TrySetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_UNAWARE, LUpdated);
    try
      Result := Winapi.Windows.GetWindowPlacement(hWnd, WindowPlacement);
    finally
      if LUpdated then
        SetThreadDpiAwarenessContext(LPreviousDPIContext);
      TrySetUpdatingDPIContext(False);
...
class function TDPIContextService.GetWindowPlacement(AControl: TWinControl; var WindowPlacement: TWindowPlacement): Boolean;
begin
  if TDPIContextService.IsControlDPIIsolated(AControl) then
    Result := TDPIContextService.GetWindowPlacement(AControl.Handle, WindowPlacement)
  else
    Result := Winapi.Windows.GetWindowPlacement(AControl.Handle, WindowPlacement);
end;

class function TDPIContextService.SetWindowPlacement(hWnd: HWND; const WindowPlacement: TWindowPlacement): Boolean;
begin
  var LControl := FindControl(hWnd);
  if (LControl <> nil) and TDPIContextService.IsControlDPIIsolated(LControl) then
  begin
    ...
    var LPreviousDPIContext := TrySetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_UNAWARE, LUpdated);
    try
      Result := Winapi.Windows.SetWindowPlacement(hWnd, WindowPlacement);
    finally
      if LUpdated then
        SetThreadDpiAwarenessContext(LPreviousDPIContext);
      TrySetUpdatingDPIContext(False);
...
class function TDPIContextService.SetWindowPos(hWnd: HWND; hWndInsertAfter: HWND; X, Y, cx, cy: Integer; uFlags: UINT): Boolean;
begin
  var LControl := FindControl(hWnd);
  if (LControl <> nil) and TDPIContextService.IsControlDPIIsolated(LControl) then
  begin
    ...
    var LPreviousDPIContext := TrySetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_UNAWARE, LUpdated);
    try
      Result := Winapi.Windows.SetWindowPos(hWnd, hWndInsertAfter, X, Y, cx, cy, uFlags);
    finally
      if LUpdated then
        SetThreadDpiAwarenessContext(LPreviousDPIContext);
      TrySetUpdatingDPIContext(False);
...
class function TDPIContextService.ClientToScreen(hWnd: HWND; var lpPoint: TPoint): Boolean;
begin
  Result := Winapi.Windows.ClientToScreen(hWnd, lpPoint);
end;

class function TDPIContextService.ScreenToClient(hWnd: HWND; var lpPoint: TPoint): Boolean;
begin
  Result := Winapi.Windows.ScreenToClient(hWnd, lpPoint);
end;
...
function GetDesignerForControl(AControl: TControl): IDesignerHook;
begin
  if not (csDesigning in AControl.ComponentState) then Exit(nil);
  var LForm := GetParentForm(AControl);
  if Assigned(LForm) then
    Result := LForm.Designer
  else
    Result := nil;
end;
...
procedure TControl.UpdateRootControl;
begin
  FRootControl := GetRealParentForm(Self);
  if (FRootControl = nil) or TDPIContextService.IsControlDPIIsolated(FRootControl) or ((FRootControl <> nil) and (csDesigning in FRootControl.ComponentState)) then
    FRootControl := Application.MainForm;
  if (FRootControl <> nil) then
    FScaleFactorRoot := Screen.DefaultPixelsPerInch / FRootControl.PixelsPerInch;
end;
...
procedure TControl.CMStyleElementsChanged(var Message: TMessage);
begin
  UpdateStyleElements;
end;
...
begin
  ...
  if AutoSize and (ClassNameIs('TPanel') or ClassNameIs('TStackPanel')) then
    AdjustSize;
end;
...
function TControl.GetInternalStyleElements: TStyleElements;
begin
  Result := FStyleElements;
end;

function TControl.GetStyleElements: TStyleElements;
begin
  if not (csReadingProps in ControlState) and (csDesigning in ComponentState) and
   not (csWriting in ComponentState) and IsCustomStyleActive then
  begin
    var LDesigner := GetDesignerForControl(Self);
    if Assigned(LDesigner) then
       Exit(FStyleElements - LDesigner.GetExcludedStyleElements(Self));
  end;
  Result := FStyleElements;
end;

procedure TControl.DoStyleElementsChanged;
begin
  Perform(CM_STYLEELEMENTSCHANGED, 0, 0);
end;

procedure TControl.SetStyleElements(const Value: TStyleElements);
var
  LPrevStyleActive: Boolean;
begin
  if FStyleElements <> Value then
  begin
    LPrevStyleActive := IsCustomStyleActive;
    FStyleElements := Value;
    if LPrevStyleActive or IsCustomStyleActive then
      DoStyleElementsChanged;
  end;
end;
...
begin
  ...
  begin
    ...
    Result := (FStyleElements <> []) and not SameText(LStyleName, TStyleManager.SystemStyleName) and
...
constructor TControlEnumerator.Create(AControl: TWinControl; AFilter: TControlEnumeratorFilter);
begin
  FControl := AControl;
  FControls := TList<TControl>.Create;
  EnumControls(FControl, AFilter);
  FIndex := -1;
end;

destructor TControlEnumerator.Destroy;
begin
  FControls.Free;
  inherited;
end;

function TControlEnumerator.GetEnumerator: TControlEnumerator;
begin
  Result := Self;
end;

procedure TControlEnumerator.EnumControls(AControl: TWinControl; AFilter: TControlEnumeratorFilter);
var
  LCanAdd: Boolean;
  LControl: TControl;
begin
  ...
  begin
    ...
    if (ceftAll in AFilter) and (LControl is TWinControl) then
      EnumControls(TWinControl(LControl), AFilter);

    LCanAdd := True;

    if ceftCustom in AFilter then
      LCanAdd := FControl.CanEnumerateControl(LControl);

    if LCanAdd then
    begin
      if ceftEnabled in AFilter then
        LCanAdd := LControl.Enabled or (ceftDisabled in AFilter)
      else
      if ceftDisabled in AFilter then
        LCanAdd := not LControl.Enabled;
    end;

    if LCanAdd then
    begin
      if ceftVisible in AFilter then
        LCanAdd := LControl.Visible or (ceftInVisible in AFilter)
      else
      if ceftInVisible in AFilter then
        LCanAdd := not LControl.Visible;
    end;

    if LCanAdd then
      FControls.Add(LControl);
  end;
end;

function TControlEnumerator.GetCurrent: TControl;
begin
  Result := FControls.Items[FIndex];
end;
...
constructor TWinControlMessageHandler.Create(AControl: TWinControl);
begin
  FControl := AControl;
end;

function TWinControlMessageHandler.GetHandle: HWND;
begin
  Result := FControl.Handle;
end;

function TWinControlMessageHandler.HandleMessage(var Message: TMessage): Boolean;
begin
  Result := False;
end;

procedure TWinControlMessageHandler.DefaultWinProc(var Message: TMessage);
begin
  FDefaultWinProcCalling := True;
  try
    FControl.WindowProc(Message);
  finally
    FDefaultWinProcCalling := False;
  end;
end;
...
function TWinControl.GetControls(AFilter: TControlEnumeratorFilter): TControlEnumerator;
begin
  Result := TControlEnumerator.Create(Self, AFilter);
end;

function TWinControl.CanEnumerateControl(AControl: TControl): Boolean;
begin
  Result := True;
  if Assigned(FOnEnumerateControl) then
    FOnEnumerateControl(Self, AControl, Result);
end;

procedure TWinControl.InitMessageHandler(AMessageHandlerClass: TWinControlMessageHandlerClass);
begin
  CreateMessageHandler(AMessageHandlerClass);
end;

procedure TWinControl.CreateMessageHandler(AMessageHandlerClass: TWinControlMessageHandlerClass);
begin
  if AMessageHandlerClass = nil then
    raise EArgumentNilException.CreateRes(@SArgumentNil);

  if not Assigned(FMessageHandler) or (FMessageHandler.ClassType <> AMessageHandlerClass) then
  begin
    ReleaseMessageHandler;
    FMessageHandler := AMessageHandlerClass.Create(Self);
  end;
end;

procedure TWinControl.ReleaseMessageHandler;
begin
...
function TWinControl.IsDPIScalingRequired: Boolean;
begin
  Result := False;
end;

procedure TWinControl.RequestDPIScaling;
begin
  SetDPIScaling(IsDPIScalingRequired);
end;

procedure TWinControl.RequestDPIScaling(Value: Boolean);
begin
  if IsDPIScalingRequired then
    SetDPIScaling(Value);
end;

procedure TWinControl.SetDPIScaling(Value: Boolean);
begin
  if not (csDesigning in ComponentState) and not (csLoading in ComponentState) and HandleAllocated and
    CheckPerMonitorV2SupportForWindow(Handle) then
      SendMessage(Handle, CCM_DPISCALE, WParam(Value), 0);
end;
...
{$ENDIF}
begin
   if Assigned(FMessageHandler) and
     (FHandle <> 0) and
     not (csDestroying in ComponentState) and
     not (csDestroyingHandle in ControlState) and
     not FMessageHandler.IsDefaultWinProcCalling and
     FMessageHandler.HandleMessage(Message) then
    Exit;
...
{$ENDIF}
    begin
      ...
      begin
        var LPoint := ScreenToClient(TPoint.Create(LoWord(LParam), HiWord(LParam)));
        var R: TRect;
        WinApi.Windows.GetClientRect(Handle, R);
        Result := Parent.Perform(Msg, WParam, LParam);
        if (Result <> 0) or R.Contains(LPoint) then
          Exit;
      end;
      case Msg of
...
begin
  ...
  begin
    ...
    if CanUseDoubleBuffering then
      Perform(CM_DOUBLEBUFFEREDCHANGED, 0, 0);
  end;
end;

procedure TWinControl.SetDoubleBufferedMode(Value: TDoubleBufferedMode);
begin
  if Value <> FDoubleBufferedMode then
  begin
    FDoubleBufferedMode := Value;
    Perform(CM_DOUBLEBUFFEREDCHANGED, 0, 0);
  end;
end;
...
begin
  ...
  if (NewPPI <> FCurrentPPI) or ((Parent <> nil) and (Parent.PixelsPerInch <> PixelsPerInch)) then
  begin
...
const
  WindowPos: array[Boolean] of HWND = (HWND_BOTTOM, HWND_TOP);
var
  N, M: Integer;
  Pos: HWND;
begin
  ...
  begin
    ...
    if FParent.FControls <> nil then M := FParent.FControls.Count else M := 0;
    SetZOrderPosition(M + N);
  end
  else if WindowHandle <> 0 then
  begin
    if TopMost and
       (GetWindowLong(GetParent(WindowHandle), GWL_EXSTYLE) and WS_EX_TOPMOST <> 0) then
      Pos := HWND_TOPMOST
    else
      Pos := WindowPos[TopMost];
    SetWindowPos(WindowHandle, Pos, 0, 0, 0, 0, SWP_NOMOVE + SWP_NOSIZE);
  end;
end;
...
procedure TWinControl.ChildHandlesNeeded;
begin
  for var LCtrl in GetControls([ceftAll, ceftVisible]) do
    if LCtrl is TWinControl then
      TWinControl(LCtrl).HandleNeeded;
end;
...
procedure TWinControl.DoStyleElementsChanged;
begin
  if HandleAllocated then
   if IsWindowVisible(Handle) then
     PostMessage(Handle, CM_STYLEELEMENTSCHANGED, 0, 0)
   else
     inherited;
end;
...
begin
  ...
  begin
    Canvas.Font := Screen.HintFont;
    Canvas.Font.ChangeScale(Screen.ActiveCustomForm.CurrentPPI, Screen.PixelsPerInch, True);
  end;
...
procedure TCustomHintWindow.InternalPositionAt(const Rect: TRect; AAdjustCursor: Boolean);
var
  LPPI: Integer;
begin
  AutoSize;
  ...
  FPopAbove := HintParent.GetHintPosition(Rect) = hpAbove;
  if FPopAbove then
    Top := Rect.Top - Height
  else
  begin
    Top := Rect.Bottom;
    if AAdjustCursor then
...
begin
  GetCursorPos(Pos);
  InternalPositionAt(Rect(Pos.X, Pos.Y, Pos.X, Pos.Y), True);
end;

procedure TCustomHintWindow.PositionAt(Rect: TRect);
begin
  InternalPositionAt(Rect, False);
end;

procedure TCustomHintWindow.PositionAt(Point: TPoint);
begin
  InternalPositionAt(Rect(Point.X, Point.Y, Point.X, Point.Y), False);
end;
...
begin
  ...
  begin
    ...
    TextHeight := 0;
    Canvas.Font := Screen.HintFont;
    Canvas.Font.ChangeScale(Self.CurrentPPI, Screen.PixelsPerInch, True);
...
function TCustomHint.GetHintPosition(const ARect: TRect): THintPosition;
var
  LPos: THintPosition;
  LAbove: Boolean;
begin
  ...
  case LPos of
    hpAutoFromTop:
      LAbove := ARect.Top > Screen.Height div 2;
    hpAutoFromCenter:
      LAbove := ARect.Top + ARect.Height div 2 > Screen.Height div 2;
    hpAutoFromBottom:
      LAbove := ARect.Bottom > Screen.Height div 2;
    hpBelow:
      LAbove := False;
    hpAbove:
      LAbove := True;
...
begin
  ...
  Pos := Control.ClientToScreen(Rect(Control.Width div 2, 0, Control.Width div 2, Control.Height));
  ShowHint(Pos);
end;
...
begin
  if Screen.ActiveCustomForm <> nil then
    if Screen.ActiveCustomForm.ActiveControl <> nil then
      ShowHint(Screen.ActiveCustomForm.ActiveControl)
    else
      ShowHint(Screen.ActiveCustomForm);
end;
              ...
              begin
                Synchronize(
                  ...
                  var
                    ...
                    LControl : TControl;
                  begin
                     LControl := LHintWindow.HintParent.FLatestHintControl;
                     if Assigned(LControl) and (not LControl.Hint.IsEmpty) then
                     begin
                       LCPos := LControl.ScreenToClient(Mouse.CursorPos);
                       if LControl.ClientRect.Contains(LControl.ScreenToClient(Mouse.CursorPos)) then
...
begin
  ...
  begin
    ...
    Canvas.Font := Screen.HintFont;
    Canvas.Font.ChangeScale(LPPI, Screen.PixelsPerInch, True);
