...
var
  G: TGraphic;
  D: TRect;
  LIsAnimating, LIsOpaque: Boolean;
begin
  LIsAnimating := (Picture.Graphic <> nil) and Picture.Graphic.IsAnimating;
  if not LIsAnimating then
  ...
  begin
    LIsOpaque := csOpaque in ControlStyle;
    if LIsAnimating and not LIsOpaque then
      ControlStyle := ControlStyle + [csOpaque];
    Invalidate;
    if LIsAnimating and not LIsOpaque then
      ControlStyle := ControlStyle - [csOpaque];
...
procedure TCustomRadioGroup.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
...
var
  I: Integer;
  LNeedLockDrawing: Boolean;
begin
  inherited;
  LNeedLockDrawing := IsCustomStyleActive;
  if LNeedLockDrawing then
    LockDrawing;
  try
    for I := 0 to FButtons.Count - 1 do
...
procedure TSplitter.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
  FMinSize := MulDiv(FMinSize, M, D);
end;
...
begin
  inherited DropDown;
  if AutoDropDownWidth then
    DropDownWidth := DropDownWidth + ItemHeight + 5;
end;
...
begin
  ...
  begin
    ...
    else if TStyleManager.IsCustomStyleActive and
      (seFont in StyleElements) then
      Brush.Color := StyleServices(Self).GetSystemColor(clWindowText)
    else
...
procedure TCustomLinkLabel.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited ChangeScale(M, D, isDpiChange);
  if (M <> D) and UseVisualStyle then
    RecreateWnd;
end;
