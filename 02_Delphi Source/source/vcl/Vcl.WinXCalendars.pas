...
type
  ...
  {$SCOPEDENUMS OFF}
  TViewInfoProperty = (vpToday, vpHighlighted, vpSelected, vpCurrent, vpFocused, vpHighlightToday,
    vpFirstOfGroup);
  TViewInfoProperties = set of TViewInfoProperty;
  {$SCOPEDENUMS ON}
...
begin
  ...
  ControlStyle := ControlStyle - [csCaptureMouse] + [csOverrideStylePaint];
...
begin
  inherited;
  FHeaderInfo.Font.ChangeScale(M, D, isDpiChange);
  FHeaderInfo.DaysOfWeekFont.ChangeScale(M, D, isDpiChange);
...
begin
  ...
  ControlStyle := [csCaptureMouse, csNeedsBorderPaint, csClickEvents, csOpaque, csDoubleClicks, csFixedHeight,
    csReflector, csPannable, csOverrideStylePaint];
  TabStop := True;
