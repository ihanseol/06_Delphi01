...
type
  ...
  TNumberBoxMode = (nbmInteger, nbmFloat, nbmCurrency, nbmInt64);
  TNumberBoxSpinButtonPlacement = (nbspNone, nbspCompact, nbspInline);
  TNumberBoxCurrencyFormat = (nbcfPrefix, nbcfPostfix, nbcfPrefixSpace, nbcfPostfixSpace, nbcfNone);
  ...
  private
    ...
    function IsMinMaxActive: Boolean;
    ...
    function GetValueInt64: Int64;
    procedure SetValueInt64(const AValue: Int64);
  protected
...
function TCustomNumberBox.IsMinMaxActive: Boolean;
begin
  Result := not ((FMinValue = FMaxValue) and (FMinValue = 0.0))
end;
...
begin
  ...
  begin
    FMinValue := AValue;
    if IsMinMaxActive and (FValue < FMinValue) then
      Value := FMinValue;
  end;
end;
...
begin
  ...
  begin
    FMaxValue := AValue;
    if IsMinMaxActive and (FValue > FMaxValue) then
      Value := FMaxValue;
  end;
end;
...
procedure TCustomNumberBox.ApplyConstraints;
begin
  if IsMinMaxActive or (FMode in [nbmInteger, nbmCurrency, nbmInt64]) then
    SetValue(FValue);
end;
...
begin
  ...
  begin
    FTextChanging := True;
    try
      if FUseNaNValue and not (FMode in [nbmInteger, nbmInt64]) then
        Value := NaN
      else
        Value := 0;
    finally
...
begin
  ...
  begin
    if not (FTextChanging and Focused) then
    begin
      if IsMinMaxActive then
      ...
      nbmInt64:
        begin
          if AValue > Int64.MaxValue then
            AValue := Int64.MaxValue
          else
          if AValue < Int64.MinValue then
            AValue := Int64.MinValue;
        end;
      end;
    end;
  end;
...
function TCustomNumberBox.GetValueInt64: Int64;
begin
  if not FValue.IsNaN then
    Result := Round(FValue)
  else
...
procedure TCustomNumberBox.SetValueInt64(const AValue: Int64);
begin
  Value := AValue;
end;
...
begin
  ...
  begin
    if Mode in [nbmInteger, nbmInt64] then
      Result := IntToStr(ValueInt64)
    else if FDecimal > 0 then
...
begin
  Result := ['0'..'9', '+'];
  if not (Mode in [nbmInteger, nbmInt64]) and (FDecimal > 0) then
  ...
  if not IsMinMaxActive or (FMinValue < 0) then
    Result := Result + ['-'];
end;
...
var
  ...
  LECount: Integer;
begin
  if (AText = '') or (AText = '-') or (AText = '+') or (AText = FormatSettings.DecimalSeparator) or
     (AText = 'e') or (AText = 'E') then
    Exit(False);
  ...
  begin
    ...
    if CharInSet(AText[I], ['-', '+']) and
       not ((I = 1) or CharInSet(AText[I - 1], ['e', 'E']) and (I < AText.Length)) then
      Exit(False)
    else
    ...
    if CharInSet(AText[I], ['e', 'E']) then
    begin
      Inc(LECount);
      if (LECount > 1) or (I = 1) or (I = AText.Length) then
        Exit(False);
    end;
  end;
end;
...
begin
  ...
  begin
    LText := Text;
    if LText.IndexOfAny(['e', 'E']) >= 0 then
    begin
      if CharInSet(AKey, ['e', 'E']) then
        Result := False
    end
    else
    ...
    if (AKey = '-') and ((SelStart <> 0) or (LText.Contains('-') and (SelStart = 0) and (SelLength = 0))) then
      Result := False
    else
