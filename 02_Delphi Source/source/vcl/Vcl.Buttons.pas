...
{$ENDIF}
  ...
  published
    ...
    property DoubleBufferedMode default dbmRequested;
    property DragCursor;
    property DragKind;
    property DragMode;
    property Enabled;
  ...
  private
    ...
    function UseGlyphs: Boolean;
  protected
    procedure BtnIDClick(Index: TNavBtnID); virtual;
    procedure DefineButtons(const ADescriptions: TArray<TButtonDescription>);
    function GetButton(Index: TNavBtnID): TNavButton;
    procedure SetVisible(const Value: TArray<TNavBtnID>);
    ...
    function GetButtonImageList: TCustomImageList; virtual;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
...
{ TNavButton }
  TNavButtonStyleItem = (nsAllowTimer);
...
var
  // Do not use this variable. It will be removed in next major release.
  FNavigatorEnableChanged: procedure (Self: TBaseNavigator) = nil;
...
{$ENDIF}
  ...
  FDoubleBufferedMode := dbmRequested;
  DoubleBuffered := True;
end;
...
function TBaseNavigator.GetButtonImageList: TCustomImageList;
begin
  Result := nil;
end;
...
begin
  ...
  FConfirmDelete := True;
  FullRepaint := False;
  FButtonImages := GetButtonImageList();
end;

function TBaseNavigator.UseGlyphs: Boolean;
var
  LStyle: TCustomStyleServices;
begin
  LStyle := StyleServices(Self);
  Result := (csDesigning in ComponentState) or
    not LStyle.Enabled or LStyle.IsSystemStyle;
end;
...
begin
  inherited;
  LUseGlyphs := UseGlyphs;
  for I in FBtnIDs do
  begin
    LNavButton := FButtonInstances[I].Instance;
    if LNavButton <> nil then
      if LUseGlyphs and
         ((FButtonImages <> nil) and (LNavButton.ImageIndex = -1) or
          (FButtonImages = nil) and LNavButton.Glyph.Empty) then
        SetButtonGlyph(I)
      else if not LUseGlyphs then
        // With custom styles, glyphs are painted in the Paint method
        if FButtonImages <> nil then
          LNavButton.ImageIndex := -1
        else
...
procedure TBaseNavigator.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
  if FButtonImages <> nil then
...
var
  LStyle: TCustomStyleServices;
begin
  if not Flat and not UseGlyphs then
    with Canvas do
    begin
      Brush.Style := bsSolid;
      LStyle := StyleServices(Self);
...
procedure TBaseNavigator.DefineButtons(const ADescriptions: TArray<TButtonDescription>);
var
...
procedure TBaseNavigator.SetVisible(const Value: TArray<TNavBtnID>);
var
  I: TNavBtnID;
  W, H: Integer;
...
var
  I: TNavBtnID;
begin
  if Assigned(FNavigatorEnableChanged) then
    FNavigatorEnableChanged(Self)
  else
  for I in FBtnIDs do
    if FButtonInstances[I].Instance <> nil then
...
begin
  if FButtonImages <> nil then
  begin
    if FButtonInstances[Index].Description.ImageIndex <> -1 then
    begin
      FButtonInstances[Index].Instance.Images := FButtonImages;
      FButtonInstances[Index].Instance.ImageIndex := Ord(Index);
    end;
  end
  else
  begin
...
begin
  inherited Paint;
  LStyle := StyleServices(Self);
  if not TBaseNavigator(Parent).UseGlyphs then
  begin
    if not Enabled then
