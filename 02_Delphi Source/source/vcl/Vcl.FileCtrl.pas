...
function SelectDirectory(const StartDirectory: string; out Directories: TArray<string>; Options: TSelectDirFileDlgOpts = [];
  const Title: string = ''; const FolderNameLabel: string = ''; const OkButtonLabel: string = ''; ParentWnd: HWND = 0): Boolean; overload;
...
procedure TDriveComboBox.DropDown;
begin
  inherited DropDown;
  if AutoDropDownWidth then
    DropDownWidth := DropDownWidth + 22;
end;
...
begin
  ...
  Font.ChangeScale(LPPIActiveForm, Screen.PixelsPerInch, True);
...
begin
  ...
  try
    ...
    if ParentWnd = 0 then
      ...
      Result := LFileDialog.Execute(ParentWnd);
    if Result then
    begin
      if sdAllowMultiselect in Options then
      begin
