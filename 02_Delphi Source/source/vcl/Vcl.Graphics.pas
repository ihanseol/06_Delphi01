...
{$IF DEFINED(LINUX)}
  WinUtils, Libc,
{$ENDIF}
  Winapi.Windows, System.UITypes, System.SysUtils, System.Classes, System.Generics.Collections
{$IF NOT DEFINED(CLR)}
  , Winapi.Wincodec
{$ENDIF}
  ;
...
{$ENDIF}
  ...
  private
    ...
    FIsDPIRelated: Boolean;
    FIsScreenFont: Boolean;
{$IF DEFINED(CLR)}
...
{$ENDIF}
  ...
  TCanvasRecallElements = (creBrush, creFont, crePen);
  TCanvasRecallElement = set of TCanvasRecallElements;
  ...
  private
    ...
    FPen: TPenRecall;
    FFont: TFontRecall;
    FRecallElement: TCanvasRecallElement;
  public
    constructor Create(const Canvas: TCanvas; const RecallElement: TCanvasRecallElement);
    destructor Destroy; override;
  end;
  ...
  protected
    ...
    function GetIsAnimating: Boolean; virtual;
  public
...
{$ENDIF}
    ...
    function GetIsBottomTop: Boolean;
  protected
    FImage: TBitmapImage;
...
{$ENDIF}
    ...
    property IsBottomTop: Boolean read GetIsBottomTop;
  end;
...
{$ENDIF}
    ...
    property Images: TList<TIconRec> read FImages;
    property IconImage: TIconImage read FImage;
  end;
...
function TryStringToColor(const S: string; var Color: TColor): Boolean;// inline;
...
{$IF NOT DEFINED(CLR)}
  System.Win.Registry,
{$ENDIF}
  System.RTLConsts, System.Types, System.UIConsts, Vcl.Consts, Winapi.ActiveX, System.TypInfo;
...
begin
  Result := System.UIConsts.TryStringToColor(S, Color);
end;
...
procedure TFont.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  if isDPIChange and FIsDPIRelated then
    ScaleForDPI(M)
  else
  begin
    if isDpiChange then
...
procedure TFont.ChangeScale(AHeight: Integer; M, D: Integer; isDpiChange: Boolean);
begin
  if isDPIChange then
  begin
    FPixelsPerInch := M;
  ...
  Height := MulDiv(AHeight, M, D);
end;

procedure TFont.ScaleForDPI(const Value: Integer);
var
  LOldDPI: Integer;
begin
  if Value <> FPixelsPerInch then
  begin
    if FIsDPIRelated then
    begin
      LOldDPI := PixelsPerInch;
      FPixelsPerInch := Value;
      Height := MulDiv(Height, FPixelsPerInch, LOldDPI);
    end
    else
      Height := MulDiv(Height, Value, FPixelsPerInch);
  end;
end;
...
{$ELSE}
      ...
      try
        ...
        if FPixelsPerInch <> TFont(Source).PixelsPerInch then
        begin
          if TFont(Source).IsDPIRelated then
            FIsDPIRelated := True;

          if not FIsDPIRelated or TFont(Source).IsScreenFont then
            Size := TFont(Source).Size
          else
            FPixelsPerInch := TFont(Source).PixelsPerInch;
        end;
      finally
        TFont(Source).Unlock;
      end;
...
constructor TCanvasRecall.Create(const Canvas: TCanvas;
  const RecallElement: TCanvasRecallElement);
begin
  FRecallElement := RecallElement;
  if creBrush in FRecallElement then
    FBrush := TBrushRecall.Create(Canvas.Brush);
  if crePen in FRecallElement then
    FPen := TPenRecall.Create(Canvas.Pen);
  if creFont in FRecallElement then
    FFont := TFontRecall.Create(Canvas.Font);
end;
...
procedure ReadIconByIndex(Stream: TStream; var Icon: HICON; ImageCount: Integer;
  StartOffset, IconIndex: Integer; var IconSize: TPoint);
var
  List: PIconRecArray;
...
begin
  Icon := 0;
  HeaderLen := SizeOf(TIconRec) * ImageCount;
  var LStreamPosition := Stream.Position;
  List := AllocMem(HeaderLen);
  try
    Stream.Read(List^, HeaderLen);
  ...
  ReadIconByIndex(Stream, Icon, ImageCount, StartOffset, Index, IconSize);
...
function TGraphic.GetIsAnimating: Boolean;
begin
  Result := False;
end;
...
{$ELSE}
  Add('tiff', SVTIFFImages, 0, TWICImageEx);
  Add('tif', SVTIFFImages, 0, TWICImageEx);
...
{$ENDIF}
        ...
        if (Handle <> 0) and (SrcDIB.dsbm.bmWidth = biWidth) and
          (SrcDIB.dsbm.bmHeight = Abs(biHeight)) and (biBitCount > 8) then
...
{$ENDIF}
    ...
    if (Result <> 0) then
    begin
      PrevHeight := DIB.dsBmih.biHeight;
{$IF DEFINED(CLR)}
      GetObject(Result, Marshal.SizeOf(TypeOf(DIB)), DIB);
{$ELSE}
      GetObject(Result, SizeOf(DIB), @DIB);
{$ENDIF}
      if Abs(DIB.dsBmih.biHeight) = Abs(PrevHeight) then
        DIB.dsBmih.biHeight := PrevHeight;
    end;
  end;
end;
...
function TBitmap.GetIsBottomTop: Boolean;
begin
  Result := FImage.FDIB.dsBmih.biHeight >= 0;
end;
...
{$ENDIF}
       ...
       16: case biCompression of
             ...
             BI_BITFIELDS:
               if dsBitFields[1] = $7E0 then Result := pf16Bit
...
{$ENDIF}
      ...
      if Abs(DIB.dsBmih.biHeight) = Abs(bmiHeader.biHeight) then
        DIB.dsBmih.biHeight := bmiHeader.biHeight;
...
{$ENDIF}
    ...
    begin
      ...
      biWidth := bmWidth;
      if bmHeight = Abs(FImage.FDIB.dsBmih.biHeight) then
        biHeight := FImage.FDIB.dsBmih.biHeight
      else
        biHeight := bmHeight;
      biPlanes := 1;
...
begin
  ...
  begin
    ...
    begin
      ...
      FImageCount := TIcon(Source).ImageCount;
      FImages.Clear;
      FImages.AddRange(TIcon(Source).Images);
    end
    else
{$IF DEFINED(CLR)}
      NewImage(nil, nil);
...
{$ELSE}
    ...
    FImageCount := CI.Count;
    FImages.Clear;
    HeaderLen := SizeOf(TIconRec) * FImageCount;
    List := AllocMem(HeaderLen);
    try
      Image.Read(List^, HeaderLen);
      for var I := 0 to FImageCount - 1 do
        FImages.Add(List^[I]);
    finally
      FreeMem(List, HeaderLen);
    end;
...
procedure TWICImage.Dormant;
begin
  if Assigned(FBitmap) and (FBitmap.HandleAllocated) then
    FBitmap.Dormant;
  if Assigned(FScaledBuffer) then
    FScaledBuffer.Dormant;
end;
...
begin
  ...
  if (AInterpolationMode = wipmNone) or Empty then
  begin
    Result.Assign(Self);
...
class function TWICImageEx.CanLoadFromStream(Stream: TStream): Boolean;
var
  LImage: TWICImage;
  ...
  begin
    ...
    try
      ...
      try
        LData.CopyFrom(Stream, Stream.Size - Stream.Position);
        LData.Position := 0;
        LStream := TStreamAdapter.Create(LData);
        Result := not Failed(ImagingFactory.CreateDecoderFromStream(LStream, guid_null, WICDecodeMetadataCacheOnDemand, BitmapDecoder));
      finally
        LData.Free;
      end;
    finally
...
var
  ...
  begin
    if Failed(Result) then
      raise EInvalidGraphic.CreateFmt(SInvalidImage + ' (0x%.8x)', [Result]);
  end;
