...
{$ENDIF}
    FLastEditMousePos: TSmallPoint;
    FLastComboMousePos: TSmallPoint;
...
procedure TCustomGroupBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
var
  Flags: TScalingFlags;
begin
  ...
  if not DefaultHeaderFont and (sfFont in Flags) then
    HeaderFont.ChangeScale(M, D, isDpiChange);
  inherited;
end;
...
begin
  ...
  if (LItemData = CB_ERR) and ((Count = 0) or (Index < 0) or (Index > Count)) then
    ListIndexError(Index, Count - 1, Self);
{$IF DEFINED(CLR)}
  if LItemData <> 0 then
...
begin
  ...
  begin
    ...
    if (Parent <> nil) and TWinControlClass(Parent).AutoSize and
       (Parent.ClassNameIs('TPanel') or Parent.ClassNameIs('TStackPanel')) then
    begin
      LStyle := GetWindowLong(Handle, GWL_STYLE);
      if ((LStyle and CBS_OWNERDRAWFIXED <> 0) or (LStyle and CBS_OWNERDRAWVARIABLE <> 0)) then
        RequestAlign;
    end;
  end;
end;
...
begin
  ...
  case Message.Msg of
    ...
    WM_MOUSEMOVE:
    begin
      if FLastEditMousePos <> TWMMouseMove(Message).Pos then
      begin
        FLastEditMousePos := TWMMouseMove(Message).Pos;
    ...
    begin
      case Msg of
        ...
        begin
          if PopupMenu <> nil then
          begin
            Perform(WM_CONTEXTMENU, Message.WParam, Message.LParam);
            Exit;
          end;
          if Assigned(OnContextPopup) then
          begin
            LHandled := False;
            LPoint := ScreenToClient(TPoint.Create(LoWord(Message.LParam), HiWord(Message.LParam)));
            OnContextPopup(Self, LPoint, LHandled);
            if LHandled then
              Exit;
          end;
        end;
...
{$ENDIF}
        ...
        WM_MOUSEMOVE:
          if FLastComboMousePos <> TWMMouseMove(Message).Pos then
          begin
            FLastComboMousePos := TWMMouseMove(Message).Pos;
...
begin
  ...
  begin
    ...
    if Index >= 0 then
    begin
      var s: string := Items[Index];
      if UseRightToLeftReading then
        Rect.Left := Rect.Right - FCanvas.TextWidth(s) - ScaleValue(2);
      if odComboBoxEdit in State then
        FCanvas.TextOut(Rect.Left + 1, Rect.Top + 1, s) //Visual state of the text in the edit control
      else
        FCanvas.TextOut(Rect.Left + 2, Rect.Top, s); //Visual state of the text(items) in the deployed list
    end;
  end;
end;
...
begin
  ...
  begin
    if (DropDownWidth > 0) and (Style <> csSimple) then
    begin
      ...
      if ComboRect.Left + ListRect.Width > Screen.MonitorFromWindow(Handle).WorkAreaRect.Right then
...
begin
  ...
  if not LNeedAdjustHeight and (Parent <> nil) and TWinControlClass(Parent).AutoSize and
     (Parent.ClassNameIs('TPanel') or Parent.ClassNameIs('TStackPanel')) then
    RequestAlign;
end;
...
function TButtonControl.CanUseDoubleBuffering: Boolean;
begin
  Result := (DoubleBufferedMode = dbmRequested) and not (Application.InRemoteSession and Application.SingleBufferingInRemoteSessions);
end;
...
procedure TImageMargins.ChangeScale(M, D: Integer);
begin
  if (M <> D) and ((FLeft <> 0) or (FTop <> 0) or (FRight <> 0) or (FBottom <> 0)) then
  begin
...
procedure TCustomButton.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
  FImageMargins.ChangeScale(M, D);
end;
...
{$ELSE}
    Result := TObject(ListBox.GetItemData(Index));
    if TListBoxItemData(Result) = LB_ERR then
      ListIndexError(Index, Count - 1, Self);
{$ENDIF}
  end;
end;
...
begin
  inherited;
  if (FStyle = lbOwnerDrawVariable) and Assigned(FOnMeasureItem) and (Count > 0) then
    RecreateWnd;
end;
...
begin
  if Assigned(FOnMeasureItem) and (Style = lbOwnerDrawVariable) then
  begin
    Canvas.Font := Font;
...
begin
  inherited Create(AOwner);
  Width := 121;
  Height := MulDiv(GetSystemMetrics(SM_CYHSCROLL), 96, Screen.PixelsPerInch);
  TabStop := True;
...
begin
  ...
  begin
    ...
    LThumbTooSmall := False;

    if Info.nPage = 0 then
    begin
      Result.Top := LVertTrackRect.Top + Round(((Info.nPos - Info.nMin) / (Info.nMax - Info.nMin)) * (LVertTrackRect.Height - LThumbSize));
      Result.Bottom := Result.Top + LThumbSize;
    end
    else
    begin
      Result.Top := LVertTrackRect.Top + Round(((Info.nPos - Info.nMin) / (Info.nMax - Info.nMin + 1)) * LVertTrackRect.Height);
      Result.Bottom := LVertTrackRect.Top + Round(((Info.nPos + Integer(Info.nPage) - Info.nMin) / (Info.nMax - Info.nMin + 1)) * LVertTrackRect.Height) - 1;
      if Result.Bottom - Result.Top < LThumbSize then
      begin
        LThumbTooSmall := True;
        if Info.nMax - Info.nMin - Integer(Info.nPage) = 0 then
          Result.Top := LVertTrackRect.Top
        else
          Result.Top := LVertTrackRect.Top + Round(((Info.nPos - Info.nMin) / (Info.nMax - Info.nMin - Integer(Info.nPage))) * (LVertTrackRect.Height - LThumbSize));
        Result.Bottom := Result.Top + LThumbSize;
      end;
    end;
    ...
    begin
      ...
      if LThumbTooSmall then
        Result.Top := Result.Bottom - LThumbSize;
    end;
    if LVertDownButtonRect.Top - VertUpButtonRect.Bottom < LThumbSize then
        Result := TRect.Empty;
  end;
end;
...
begin
  ...
  begin
    LThumbSize := GetSystemMetrics(SM_CYVTHUMB);
    Result.Top := Result.Top + LThumbSize;
    Result.Bottom := Result.Bottom - LThumbSize;
  end
  else
    Result := TRect.Empty;
end;
...
begin
  ...
  begin
    ...
    Dec(Result.Bottom, (LThumbSize * 2 - VertScrollRect.Height) div 2);
    if Result.Bottom - Result.Top < LThumbSize div 2 then
      Result.Bottom := Result.Top + LThumbSize div 2;
  end
  else
    Result := TRect.Empty;
end;
...
begin
  ...
  begin
    ...
    begin
      Result.Left := LHorzTrackRect.Left + Round(((Info.nPos - Info.nMin) / (Info.nMax - Info.nMin + 1)) * LHorzTrackRect.Width);
      Result.Right := LHorzTrackRect.Left + Round(((Info.nPos + Integer(Info.nPage) - Info.nMin) / (Info.nMax - Info.nMin + 1)) * LHorzTrackRect.Width) - 1;
...
begin
  ...
  if (seFont in Control.StyleElements) or not Control.Enabled then
    FontColor := LStyle.GetStyleFontColor(FontColorStates[Control.Enabled])
  else
    FontColor := TWinControlClass(Control).Font.Color;
end;
...
begin
  ...
  if (Control is TCustomComboBox) and (TCustomComboBox(Control).Style = csOwnerDrawFixed) then
...
var
  ...
  M: Integer;
begin
  M := Control.ScaleValue(CCaptionMargin);
  ...
  if Control.BiDiMode <> bdRightToLeft then
    Result := Rect(M, 0,
      M + Canvas.TextWidth(S), Canvas.TextHeight(S))
  else
    Result := Rect(Control.Width - Canvas.TextWidth(S) - M, 0,
      Control.Width - M, Canvas.TextHeight(S));
end;
...
begin
  ...
  begin
    ...
    if Control is TCustomStaticText then
      ...
      S := Text;
    LFlags := Alignments[TCustomStaticText(Control).Alignment] or DT_WORDBREAK;
    if seFont in Control.StyleElements then
      DrawControlText(Canvas, Details, S, R, LFlags)
    else
    begin
      Canvas.Font := TWinControlClass(Control).Font;
      DrawText(Canvas.Handle, S, Length(S), R, LFlags);
    end;
  end;
end;
