...
(*$HPPEMIT '#pragma alias "@Vcl@Printers@TPrinter@GetPrinterA$qqrr20System@UnicodeStringt1t1rui"="@Vcl@Printers@TPrinter@GetPrinter$qqrr20System@UnicodeStringt1t1rui"' *)
(*$HPPEMIT '#pragma alias "@Vcl@Printers@TPrinter@SetPrinterA$qqr20System@UnicodeStringt1t1ui"="@Vcl@Printers@TPrinter@SetPrinter$qqr20System@UnicodeStringt1t1ui"' *)
(*$HPPEMIT '#else' *)
...
(*$HPPEMIT '#pragma alias "@Vcl@Printers@TPrinter@GetPrinterW$qqrr20System@UnicodeStringt1t1rui"="@Vcl@Printers@TPrinter@GetPrinter$qqrr20System@UnicodeStringt1t1rui"' *)
(*$HPPEMIT '#pragma alias "@Vcl@Printers@TPrinter@SetPrinterW$qqr20System@UnicodeStringt1t1ui"="@Vcl@Printers@TPrinter@SetPrinter$qqr20System@UnicodeStringt1t1ui"' *)
(*$HPPEMIT '#endif' *)
(*$HPPEMIT '#endif' *)
...
procedure TPrinter.GetPrinter(var ADevice, ADriver, APort: string; var ADeviceMode: THandle);
begin
  with TPrinterDevice(Printers.Objects[PrinterIndex]) do
  begin
    ADevice := Device;
...
procedure TPrinter.SetPrinter(ADevice, ADriver, APort: string; ADeviceMode: THandle);
var
  I, J: Integer;
begin
  CheckPrinting(False);
  ...
  with Printers do   // <- this rebuilds the FPrinters list
    for I := 0 to Count - 1 do
    begin
      if TPrinterDevice(Objects[I]).IsEqual(PChar(ADriver), PChar(ADevice), PChar(APort)) then
      begin
        TPrinterDevice(Objects[I]).Port := APort;
        J := I;
        Break;
  ...
  begin
    J := FPrinters.Count;
    FPrinters.AddObject(Format(SDeviceOnPort, [ADevice, APort]),
      TPrinterDevice.Create(PChar(ADriver), PChar(ADevice), PChar(APort)));
  end;
  FPrinterIndex := J;
  ...
  begin
    if FDeviceMode = 0 then  // alloc new device mode block if one was not passed in
    begin
      FDeviceMode := GlobalAlloc(GHND, DocumentProperties(0, FPrinterHandle, PChar(ADevice), nil, nil, 0));
      ...
      begin
        ...
        if FDevMode = nil then
          RaiseLastOSError;
        if DocumentProperties(0, FPrinterHandle, PChar(ADevice), FDevMode, nil, DM_OUT_BUFFER) < 0 then
        begin
          GlobalUnlock(FDeviceMode);
          GlobalFree(FDeviceMode);
          FDeviceMode := 0;
...
{$ELSE}
  ...
  try
    ...
    begin
      GetDefaultPrinter(nil, @I);
      SetLength(DefaultPrinter, I);
      if GetDefaultPrinter(PChar(DefaultPrinter), @I) then SetLength(DefaultPrinter, I-1);
      Cur := PChar(DefaultPrinter);
      Device := FetchStr(Cur);
    end;
    with Printers do
      for I := 0 to Count-1 do
