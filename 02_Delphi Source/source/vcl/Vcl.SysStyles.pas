...
type
  ...
  TSysPopupItemStateItem = (isHot, isDisabled, isChecked, isDefault);
  TSysPopupItemState = set of TSysPopupItemStateItem;
...
type
  TSysToolbarButtonStateItem = (bsEnabled, bsPressed, bsChecked, bsHidden);
  TSysToolbarButtonState = set of TSysToolbarButtonStateItem;
  TSysToolbarButtonStyleItem = (bsBtn, bsSep, bsCheck, bsGroup, bsCheckGroup, bsDropDown);
  TSysToolbarButtonStyle = set of TSysToolbarButtonStyleItem;
...
var
  ...
  procedure DrawModernSubMenu(const ARect: TRect);
  var
    I: Integer;
    X, Y: Integer;
    LSaveIndex: Integer;
    ...
    LColor: TColor;
  begin
    LScaleFactor := LPPI / Screen.DefaultPixelsPerInch;
    LSaveIndex := SaveDC(DC);
    try
      LCanvas := TCanvas.Create;
      LCanvas.Handle := DC;
      LOffset := Round(4 * LScaleFactor);
      try
        if not LStyle.GetElementColor(LDetails, ecTextColor, LColor) then
          LColor := clBtnText;
        LOldPen := SelectObject(LCanvas.Handle, GetStockObject(DC_PEN));
        SetDCPenColor(LCanvas.Handle, ColorToRGB(LColor));
        Y := ARect.CenterPoint.Y - 1;
        if not RightToLeft then
        begin
          X := ARect.CenterPoint.X + Round(2 * LScaleFactor) - 2;
          for I := 1 to Trunc(LScaleFactor + 1) do
          begin
            LCanvas.MoveTo(X, Y);
            LCanvas.LineTo(X - LOffset, Y - LOffset);
            LCanvas.MoveTo(X, Y);
            LCanvas.LineTo(X - LOffset, Y + LOffset);
            Dec(X);
          end;
        ...
        SelectObject(LCanvas.Handle, LOldPen);
      finally
        LCanvas.Handle := 0;
        LCanvas.Free;
      end;
  ...
  begin
    ...
    begin
      LSubMenuRect.Top := ItemRect.Top;
      LSubMenuRect.Bottom := ItemRect.Bottom;
      DrawModernSubMenu(LSubMenuRect);
      Exit;
    end;
...
begin
  ...
  try
    ...
    Canvas.Font.ScaleForDPI(CurrentPPI);
...
var
  ...
  AText: PChar;
begin
  var LControl := Screen.ActiveControl;
  if LControl = nil then
    LControl := Screen.ActiveCustomForm;

  if LControl <> nil then
    Canvas.Font := TControlClass(LControl).Font
  else
    Canvas.Font := SysControl.Font;
