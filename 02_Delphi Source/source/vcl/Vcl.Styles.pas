...
uses
  ...
  Vcl.ButtonGroup, Vcl.ExtCtrls, Vcl.Consts, Vcl.SysStyles, Winapi.D2D1, Vcl.Direct2D, Vcl.StyleAPI, Vcl.StyleBitmap;
...
begin
  case Font of
    ...
    sfCategoryButtonsCategoryNormal: Result := TseStyle(FSource).Fonts[ktfCategoryButtonsCategoryNormal].Color;
    sfCategoryButtonsCategorySelected: Result := TseStyle(FSource).Fonts[ktfCategoryButtonsCategorySelected].Color;
    sfCategoryButtonsHot: Result := TseStyle(FSource).Fonts[ktfCategoryButtonsHot].Color;
    sfCategoryButtonsNormal: Result := TseStyle(FSource).Fonts[ktfCategoryButtonsNormal].Color;
    sfCategoryButtonsSelected: Result := TseStyle(FSource).Fonts[ktfCategoryButtonsSelected].Color;
         ...
         teWindow:
          begin
            ...
            begin
              ...
              Buffer := TBitmap.Create;
              try
                Buffer.Canvas.Brush.Color := GetSystemColor(clBtnFace);
...
var
  I: Integer;
  LPair: TPair<TComponent, TStyleHook>;
  LControl: TControl;
begin
  if FControls = nil then
    Exit;
  LPair := FControls.ExtractPair(Control);
...
var
  ...
  LList: TList<TWinControl>;
  LControl: TWinControl;
begin
  LList := TList<TWinControl>.Create;
  try
    for LItem in FControls do
      if (LItem.Key is ControlClass) and (LItem.Value is StyleHookClass) then
      begin
        if LItem.Key is TWinControl then
          LList.Add(TWinControl(LItem.Key));
      end;
    for LControl in LList do
      DoRemoveControl(LControl);
  finally
    LList.Free;
  end;
end;
...
var
  ...
  LClass: TClass;
  LStyleHookList: TStyleHookList;
begin
  Result := False;
  LStyleHook := nil;
  if not FControls.TryGetValue(Control, LStyleHook) then
  begin
    LClass := Control.ClassType;
    while (LClass <> TWinControl) and (LClass <> TCustomControl) do
    begin
      if RegisteredStyleHooks.TryGetValue(LClass, LStyleHookList) then
      begin
        LStyleHook := CreateStyleHook(LStyleHookList);
        Break;
      end;
...
begin
  case Operation of
    ...
    snReleaseAllStyleHooks:
      TThread.ForceQueue(nil, FreeControlHooks, 0);
  end;
end;
...
var
  LStyleObject: TSeStyleObject;
  LBitmapLink: TseBitmapLink;
  LRequiredElement: Boolean;
begin
  LBitmapLink := nil;
  LStyleObject := nil;
  LRequiredElement := False;
  case TThemedCategoryPanelGroup(State) of
    tcpChevronOpenedNormal..tcpChevronOpenedPressed,
    tcpThemedChevronOpenedNormal..tcpThemedChevronOpenedPressed:
    begin
      LStyleObject := GetElementObject(TCustomStyle(Style), CControlName, CChevronOpenedElement);
      LRequiredElement := True;
    end;
    tcpChevronClosedNormal..tcpChevronClosedPressed,
    tcpThemedChevronClosedNormal..tcpThemedChevronClosedPressed:
    begin
      LStyleObject := GetElementObject(TCustomStyle(Style), CControlName, CChevronClosedElement);
      LRequiredElement := True;
    end;
  end;
  if LRequiredElement then
  begin
    Result := True;
    if LStyleObject <> nil then
      LBitmapLink := TSeBitmapObject(LStyleObject).Bitmap;
    if LBitmapLink <> nil then
      Size := LBitmapLink.Rect.Size
    else
    ...
    Result := inherited GetElementSize(Style, DC, Part, State, Rect, ElementSize, Size, DPI);
end;
...
class function TCustomStyleCategoryButtonElements.GetElementSize(Style: TCustomStyleServices; DC: HDC; Part, State: Integer;
   Rect: PRect; ElementSize: TElementSize; out Size: TSize; DPI: Integer = 0): Boolean;
var
  LStyleObject: TSeStyleObject;
  LBitmapLink: TseBitmapLink;
  ...
  case TThemedCategoryButtons(State) of
    ...
    tcbCategoryNormal:
      case ElementColor of
        ...
        ecTextColor:
          Color := TseStyle(TCustomStyle(Style).FSource).Fonts[ktfCategoryButtonsCategoryNormal].Color;
        ecBorderColor:
          Color := TseStyle(TCustomStyle(Style).FSource).SysColors[clBtnHighlight];
        ecEdgeShadowColor:
          Color := TseStyle(TCustomStyle(Style).FSource).Colors[ktcCategoryButtonsGradientEnd];
    ...
    tcbCategorySelected:
      case ElementColor of
        ...
        ecGradientColor2:
          Color := TseStyle(TCustomStyle(Style).FSource).Colors[ktcGenericGradientBase];
        ecTextColor:
          Color := TseStyle(TCustomStyle(Style).FSource).Fonts[ktfCategoryButtonsCategorySelected].Color;
        ecBorderColor:
          Color := TseStyle(TCustomStyle(Style).FSource).SysColors[clBtnHighlight];
        ecEdgeShadowColor:
          Color := TseStyle(TCustomStyle(Style).FSource).Colors[ktcGenericGradientEnd];
    ...
    tcbButtonNormal:
      case ElementColor of
        ecTextColor:
          Color := TseStyle(TCustomStyle(Style).FSource).Fonts[ktfCategoryButtonsNormal].Color;
        ecBorderColor:
          Color := TseStyle(TCustomStyle(Style).FSource).SysColors[clBtnHighlight];
      else
        Result := inherited GetElementColor(Style, Part, State, ElementColor, Color);
    ...
    tcbButtonHot:
      case ElementColor of
        ecTextColor:
          Color := TseStyle(TCustomStyle(Style).FSource).Fonts[ktfCategoryButtonsHot].Color;
        ecBorderColor:
          Color := TseStyle(TCustomStyle(Style).FSource).SysColors[clBtnHighlight];
      else
        Result := inherited GetElementColor(Style, Part, State, ElementColor, Color);
    ...
    tcbButtonSelected:
      case ElementColor of
        ecTextColor:
          Color := TseStyle(TCustomStyle(Style).FSource).Fonts[ktfCategoryButtonsSelected].Color;
        ecBorderColor:
          Color := TseStyle(TCustomStyle(Style).FSource).SysColors[clBtnHighlight];
      else
        Result := inherited GetElementColor(Style, Part, State, ElementColor, Color);
...
begin
  ...
  try
    case TThemedMenu(State) of
      ...
      tmPopupItemHot: DrawMenuItem(kmscPopupMenu, kmicNormal, kmidsSelected);
      tmPopupItemDisabled,
