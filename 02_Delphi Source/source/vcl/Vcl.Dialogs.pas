...
{$ENDIF}
    procedure DoExecuteDialog(Dialog: IFileDialog); virtual;
    procedure DoFileOkClickDialog(Dialog: IFileDialog); virtual;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
...
var
  ...
  LContext: DPI_AWARENESS_CONTEXT;
...
{$ENDIF}
    try
      ...
      if TOSVersion.Check(10) and (TOSVersion.Build >= 14393) then
        LContext := SetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_SYSTEM_AWARE)
      else
        LContext := nil;
      try
        ...
        if Assigned(LContext) then
          SetThreadDpiAwarenessContext(LContext);
      end;
    finally
{$IFDEF CPUX86}
      asm
...
begin
  FOpenDialog.DoFileOkClickDialog(FFileDialog.Dialog);
...
var
  LOptions: Cardinal;
begin
  FOpenDialog.DoExecuteDialog(FFileDialog.Dialog);
...
procedure TOpenDialog.DoExecuteDialog(Dialog: IFileDialog);
begin
end;

procedure TOpenDialog.DoFileOkClickDialog(Dialog: IFileDialog);
begin
end;
...
begin
  ...
  begin
    DeviceNames := GlobalAlloc(GHND, SizeOf(TDevNames) +
     (Length(Device) + Length(Driver) + Length(Port) + 3) * SizeOf(Char));
...
var
  ...
  begin
    if TOSVersion.Check(6, 2) then
      Result := GetMsgIconResourceName <> ''
    else
{$IF DEFINED(CLR)}
      Result := IconIDs[DlgType] <> -1;
{$ELSE}
      Result := IconIDs[DlgType] <> nil;
{$ENDIF}
  end;
...
{$ENDIF}
    if HasIcon then
      ...
      begin
        ...
        begin
          Picture.Icon.Handle := LoadIcon(0, IconIDs[DlgType]);
          AutoSize := True;
        end;
...
const
  IconMap: array[TMsgDlgIcon] of TTaskDialogIcon = (tdiNone, tdiWarning,
    tdiError, tdiInformation, tdiShield);
...
{$ENDIF}
      ...
      HelpContext := HelpCtx;
      HelpFile := HelpFileName;
      MainIcon := IconMap[MsgDlgIcons[DlgType]];
      Position := Point(X, Y);
      Text := Msg;
      Title := Instruction;
    end;
