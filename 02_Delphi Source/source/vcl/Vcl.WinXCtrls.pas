...
type
  ...
  TActivityIndicatorColor = (aicBlack, aicWhite, aicCustom);
  TActivityIndicatorSize = (aisSmall, aisMedium, aisLarge, aisXLarge, aisCustom);
  TActivityIndicatorType = (aitMomentumDots, aitRotatingSector, aitSectorRing, aitRotatingLines, aitRefresh);
  ...
  strict private const
    cIndicatorSizes: array[TActivityIndicatorSize] of Integer = (24, 32, 48, 64, 0);
  private
    FAnimate: Boolean;
    FIndicatorColor: TActivityIndicatorColor;
    FIndicatorCustomColor: TColor;
    FIndicatorSize: TActivityIndicatorSize;
    FIndicatorType: TActivityIndicatorType;
    FFrameDelay: Word;
    FFrameIndex: Integer;
    FTimer: TTimer;
    FFrameCollection: TImageCollection;
    FFrameCount: Integer;
    FFrameBitmap: TBitmap;
    FLoadedFrames: Boolean;
    FLoadedIndicatorColor: TColor;
    ...
    procedure SetIndicatorCustomColor(Value: TColor);

    // Message Handling Methods
    procedure WMEraseBkgnd(var Msg: TWMEraseBkgnd); message WM_ERASEBKGND;
    procedure ChangeFrames;
  protected
    ...
    /// aicCustom - The color of elements is defined with IndicatorCustomColor property.
    /// if VCL Style is active then color can be from the style if seClient value in StyleElements is enabled</summary>
    property IndicatorColor: TActivityIndicatorColor read FIndicatorColor write SetIndicatorColor default aicBlack;
    /// <summary>Specifies custom color for activity indicator.</summary>
    property IndicatorCustomColor: TColor read FIndicatorCustomColor write SetIndicatorCustomColor default clBtnText;
    ...
    /// aisCustom - custom size, which you can set with Width and Height properties</summary></summary>
    ...
    /// aitSectorRing - Sectors of a segmented ring are highlighted in sequence until the ring is completely highlighted. On the next pass, each sector is displayed in dimmed state.
    /// aitRotatingLines - Rotating lines with different transparency in a ring.
    /// aitRefresh - Rotating refresh icon.</summary>
...
procedure DrawParentImage(Control: TWinControl; DC: HDC);
var
  P: TPoint;
  LSaveIndex: Integer;
  LStyle: TCustomStyleServices;
  LSaveState: TControlState;
begin
  if Control.Parent = nil then
    Exit;
  LStyle := StyleServices(Control.Parent);
  if not LStyle.IsSystemStyle then
    LStyle.DrawParentBackground(Control.Handle, DC, nil, False)
  else
  begin
    LSaveState := Control.ControlState;
    Control.ControlState := Control.ControlState + [csPaintCopy];
    try
      LSaveIndex := SaveDC(DC);
      try
        ...
        RestoreDC(DC, LSaveIndex);
      end;
    finally
      Control.ControlState := LSaveState;
    end;
  end;
end;
...
begin
  ...
  FFrameCollection := TImageCollection.Create(Self);
  ...
  FIndicatorCustomColor := clBtnText;
  FLoadedIndicatorColor := clBlack;

  Height := cIndicatorSizes[FIndicatorSize];
  Width := cIndicatorSizes[FIndicatorSize];
end;
...
function TCustomActivityIndicator.IsSizeStored: Boolean;
begin
  Result := FIndicatorSize = aisCustom;
end;

function TCustomActivityIndicator.GetIndicatorColor: TColor;
var
  LStyle: TCustomStyleServices;
begin
  Result := clBlack;
  case FIndicatorColor of
    ...
    aicCustom:
    begin
      Result := FIndicatorCustomColor;
      if seClient in StyleElements then
      begin
        LStyle := StyleServices(Self);
        if not LStyle.IsSystemStyle then
          Result := LStyle.GetSystemColor(Result);
      end;
    end;
  end;
...
const
  TypeName: array [TActivityIndicatorType] of string =
    ('MOMENTUMDOTS_', 'ROTATINGSECTOR_', 'SECTORRING_', 'ROTATINGLINES_', 'REFRESH_');

var
  LPng: TPngImage;
  LBmp, LFrameBmp: TBitmap;
...
begin
  FLoadedFrames := False;
  try
    LResourceName := 'WINXCTRLS_' + TypeName[FIndicatorType] + 'BLACK_64';
    LPng := TPngImage.Create;
    try
      LBmp := TBitmap.Create;
      try
        LPng.LoadFromResourceName(HInstance, LResourceName);
        LFrameSize := LPng.Height;
        FFrameCount := LPng.Width div LFrameSize;
        LBmp.Assign(LPng);
        FLoadedIndicatorColor := GetIndicatorColor;
        if FIndicatorColor <> aicBlack then
          SetPreMutipliedColor(LBmp, FLoadedIndicatorColor);
        FFrameCollection.Images.Clear;
        R1 := TRect.Create(0, 0, LBmp.Height, LBmp.Height);
        R2 := R1;
        for I := 0 to FFrameCount - 1 do
        begin
          LFrameBmp := TBitmap.Create(R1.Width, R1.Height);
          try
            LFrameBmp.AlphaFormat := afDefined;
            LFrameBmp.Canvas.CopyRect(R1, LBmp.Canvas, R2);
            LSourceItem := FFrameCollection.Images.Add.SourceImages.Add;
            LSourceItem.Image.Assign(LFrameBmp);
            OffsetRect(R2, R1.Width, 0);
          finally
            LFrameBmp.Free;
          end;
        end;
...
var
  LDrawRect: TRect;
begin
  if (Width = 0) or (Height = 0) or (csLoading in ComponentState) then
    Exit;

  FFrameBitmap.SetSize(Width, Height);

  if (Parent <> nil) and Parent.DoubleBuffered then
    PerformEraseBackground(Self, FFrameBitmap.Canvas.Handle);
  DrawParentImage(Self, FFrameBitmap.Canvas.Handle);
  LDrawRect := TRect.Create(0, 0, FFrameBitmap.Width, FFrameBitmap.Height);

  if FLoadedFrames then
  begin
    if FLoadedIndicatorColor <> GetIndicatorColor then
      ReloadFrames;
    if FAnimate then
      FFrameCollection.Draw(FFrameBitmap.Canvas, LDrawRect, FFrameIndex, True);
  end;

  if csDesigning in ComponentState then
  begin
    FFrameBitmap.Canvas.Pen.Style := psDot;
    FFrameBitmap.Canvas.Brush.Style := bsClear;
    FFrameBitmap.Canvas.Rectangle(LDrawRect);
  end;
...
procedure TCustomActivityIndicator.ChangeFrames;
var
  LSaveAnimate: Boolean;
begin
  LSaveAnimate := Animate;
  Animate := False;
  ReloadFrames;
  Animate := LSaveAnimate;
end;

procedure TCustomActivityIndicator.SetIndicatorCustomColor(Value: TColor);
begin
  if FIndicatorCustomColor <> Value then
  begin
    FIndicatorCustomColor := Value;
    if FIndicatorColor = aicCustom then
      ChangeFrames;
  end;
end;
...
procedure TCustomActivityIndicator.Loaded;
begin
  inherited;
  Resize;
end;
...
begin
  inherited;
  if FIndicatorSize <> aisCustom then
    SetBounds(Left, Top,
      ScaleValue(cIndicatorSizes[FIndicatorSize]),
      ScaleValue(cIndicatorSizes[FIndicatorSize]));
end;
...
begin
  ...
  TabStop := True;
  ControlStyle := [csCaptureMouse, csParentBackground, csOverrideStylePaint];
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if not LStyle.GetElementColor(LStyle.GetElementDetails(LDetail),
        ...
        LStyle.DrawText(MemImage.Canvas.Handle, LStyle.GetElementDetails(LDetail),
...
class procedure TSearchBox.InitButtonImageCollection;
