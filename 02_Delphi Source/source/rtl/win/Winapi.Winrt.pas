...
type
  ...
  {$EXTERNALSYM DUMMYSTRUCTNAME}
  PACKAGE_VERSION = record 
    case Integer of
       0: (Version: UINT64);
       1: (VersionStruct: DUMMYSTRUCTNAME);
  end;
  {$EXTERNALSYM PACKAGE_VERSION}
...
function MddBootstrapInitialize(majorMinorVersion: UINT32; versionTag: PCWSTR; 
  minVersion: PACKAGE_VERSION): HRESULT; stdcall;
...
function MddBootstrapShutdown: HRESULT; stdcall;
...
const
  ...
  mddbootstrap          = 'Microsoft.WindowsAppRuntime.Bootstrap.dll';
...
type
  TMddBootstrapInitialize = function(majorMinorVersion: UINT32; versionTag: PCWSTR; 
    minVersion: PACKAGE_VERSION): HRESULT; stdcall;
  TMddBootstrapShutdown = function: HRESULT; stdcall;
  
var
  hWinAppBootstrap: THandle;
  _MddBootstrapInitialize: TMddBootstrapInitialize;
  _MddBootstrapShutdown: TMddBootstrapShutdown;
  
function CheckWinAppBootstrapLoaded: Boolean;
begin
  if hWinAppBootstrap = 0 then
  begin
    hWinAppBootstrap := LoadLibrary(mddbootstrap);
    if hWinAppBootstrap = 0 then
      Exit(False);

    @_MddBootstrapInitialize := GetProcAddress(hWinAppBootstrap, 'MddBootstrapInitialize');
    @_MddBootstrapShutdown := GetProcAddress(hWinAppBootstrap, 'MddBootstrapShutdown');
  end;
  Result := True;
end;
...
function MddBootstrapInitialize(majorMinorVersion: UINT32; versionTag: PCWSTR; 
  minVersion: PACKAGE_VERSION): HRESULT; stdcall;
begin
  if CheckWinAppBootstrapLoaded then
    Result := _MddBootstrapInitialize(majorMinorVersion, versionTag, minVersion)
  else
    Result := E_FAIL;
end;
