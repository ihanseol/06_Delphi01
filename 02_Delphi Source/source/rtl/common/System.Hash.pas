...
type
  ...
  /// <summary> Record to generate FNV1a 64-bit Hash values from data. Stores internal state of the process</summary>
  THashFNV1a64 = record
  public const
    FNV_PRIME = $00000100000001B3; //        1099511628211
    FNV_SEED  = $CBF29CE484222325; // 14695981039346656037
  private
    FHash: UInt64;
    function GetDigest: TBytes;

    class function Hash(const Data; Len: Cardinal; InitVal: UInt64): UInt64; static;
  public
    /// <summary>Initialize the Record used to calculate the FNV1a Hash</summary>
    class function Create: THashFNV1a64; static;
    ...
    /// <summary> Returns the hash value as a TBytes</summary>
    function HashAsBytes: TBytes;
    /// <summary> Returns the hash value as Int64</summary>
    ...
    class function GetHashString(const AString: string): string; overload; static;
    class function GetHashString(const AString: RawByteString): string; overload; static;
    /// <summary> Hash the given string and returns it's hash value as Int64</summary>
    class function GetHashValue(const AData: string): Int64; overload; static; inline;
    class function GetHashValue(const AData: RawByteString): Int64; overload; static; inline;
    /// <summary> Hash the given Data and returns it's hash value as Int64</summary>
    class function GetHashValue(const AData; ALength: Cardinal; AInitialValue: UInt64 = FNV_SEED): Int64; overload; static; inline;
  end;
...
class function THashFNV1a64.Hash(const Data; Len: Cardinal; InitVal: UInt64): UInt64;
var
  P, PEnd: PByte;
begin
  Result := InitVal;
  ...
  while P < PEnd do
  begin
    Result := (Result xor UInt64(P^)) * UInt64(FNV_PRIME);
    Inc(P);
  end;
end;

class function THashFNV1a64.Create: THashFNV1a64;
begin
  Result.FHash := FNV_SEED;
end;

function THashFNV1a64.GetDigest: TBytes;
begin
  SetLength(Result, 8);
  PUInt64(@Result[0])^ := FHash;
end;

class function THashFNV1a64.GetHashString(const AString: string): string;
begin
  Result := GetHashValue(AString).ToHexString(16);
end;

class function THashFNV1a64.GetHashValue(const AData: string): Int64;
begin
  Result := Int64(Hash(Pointer(AData)^, Length(AData) * SizeOf(Char), FNV_SEED));
end;

class function THashFNV1a64.GetHashString(const AString: RawByteString): string;
begin
  Result := GetHashValue(AString).ToHexString(16);
end;

class function THashFNV1a64.GetHashValue(const AData: RawByteString): Int64;
begin
  Result := Int64(Hash(Pointer(AData)^, Length(AData), FNV_SEED));
end;

class function THashFNV1a64.GetHashValue(const AData; ALength: Cardinal; AInitialValue: UInt64): Int64;
begin
  Result := Int64(Hash(AData, ALength, AInitialValue));
end;

function THashFNV1a64.HashAsBytes: TBytes;
begin
  Result := GetDigest;
end;

function THashFNV1a64.HashAsInteger: Int64;
begin
  Result := Int64(FHash);
end;

function THashFNV1a64.HashAsString: string;
begin
  Result := FHash.ToHexString(16);
end;

class function THashFNV1a64.GetHashBytes(const AData: string): TBytes;
var
  LHash: UInt64;
begin
  SetLength(Result, 8);
  LHash := Hash(Pointer(AData)^, Length(AData) * SizeOf(Char), FNV_SEED);
  PUInt64(@Result[0])^ := LHash;
end;

procedure THashFNV1a64.Reset(AInitialValue: UInt64);
begin
  FHash := AInitialValue;
end;

procedure THashFNV1a64.Update(const Input: string);
begin
  FHash := Hash(Pointer(Input)^, Length(Input) * SizeOf(Char), FHash);
end;

procedure THashFNV1a64.Update(const AData; ALength: Cardinal);
begin
  FHash := Hash(AData, ALength, FHash);
end;

procedure THashFNV1a64.Update(const AData: TBytes; ALength: Cardinal);
begin
  if ALength = 0 then
    ALength := Length(AData);
  FHash := Hash(PByte(AData)^, ALength, FHash);
