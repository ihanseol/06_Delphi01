...
function Min(const A, B: ShortInt): ShortInt; overload; inline;
function Min(const A, B: Byte): Byte; overload; inline;
function Min(const A, B: SmallInt): SmallInt; overload; inline;
function Min(const A, B: Word): Word; overload; inline;
...
function Min(const A, B: Currency): Currency; overload; inline;
{$ENDIF}
...
function Max(const A, B: ShortInt): ShortInt; overload; inline;
function Max(const A, B: Byte): Byte; overload; inline;
function Max(const A, B: SmallInt): SmallInt; overload; inline;
function Max(const A, B: Word): Word; overload; inline;
...
{$IF SizeOf(Extended) > SizeOf(Double)}
function Max(const A, B: Currency): Currency; overload; inline;
{$ENDIF}
...
{$IFDEF WIN64}
function SetCurrentSSERoundMode(const RoundMode: TSSERoundingMode): TSSERoundingMode;
asm
        .NOFRAME
        PUSH    RAX
...
begin
  ...
  begin
    originalRoundMode := {$IFDEF WIN64}SetCurrentSSERoundMode{$ELSE}SetRoundMode{$ENDIF}(TRoundingMode.rmNearest);
    try
      T := AValue * LFactorArray[ADigit][2];
      if Abs(T) < ROUNDMAX then T := Round(T);
      Result := T * LFactorArray[ADigit][1];
    finally
      {$IFDEF WIN64}SetCurrentSSERoundMode{$ELSE}SetRoundMode{$ENDIF}(originalRoundMode);
    end;
  end
  else
  begin
...
function Max(const A, B: ShortInt): ShortInt;
begin
  if A > B then
    Result := A
  else
...
function Max(const A, B: SmallInt): SmallInt;
begin
  if A > B then
    Result := A
  else
...
{$IF SizeOf(Extended) > SizeOf(Double)}
function Max(const A, B: Currency): Currency;
begin
  if A > B then
    Result := A
  else
