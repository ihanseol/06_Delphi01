...
type
  ...
  private
    ...
    function ReadDecimal: TJsonDecimal128;
...
procedure TBsonWriter.WriteValue(const Value: TJsonDecimal128);
begin
  inherited WriteValue(Value);
  WriteType(TBsonType.Decimal);
  WriteKey;
  WriteInt64(PInt64(@Value.lo)^);
  WriteInt64(PInt64(@Value.hi)^);
end;
...
procedure TBsonReader.ReadByType(AType: TBsonType);
  ...
  procedure Error;
  begin
    raise EJsonReaderException.Create(Self, Format(SUnsupportedBSONType, [Integer(AType)]));
  end;
...
//    TBsonType.BsonDocument: ;
    ...
    TBsonType.Decimal:
      SetToken(TJsonToken.Decimal, TValue.From<TJsonDecimal128>(ReadDecimal));
    TBsonType.MinKey:
...
function TBsonReader.ReadDecimal: TJsonDecimal128;
begin
  Result.lo := FReader.ReadUInt64;
  Result.hi := FReader.ReadUInt64;
  MovePosition(16);
end;
