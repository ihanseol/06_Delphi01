...
type
  ...
  private
    ...
    function GetItem(const Index: Integer): TGroup; overload;
    function GetItem(const Index: string): TGroup; overload;
    function GetNamedGroupIndex(const Index: string; out IntIndex: Integer): Boolean;
  public
    ...
    property Item[const Index: Integer]: TGroup read GetItem; default;
    [HPPGEN('TGroup operator[](const System::String Index) { return GetItem(Index); }')]
    property Item[const Index: string]: TGroup read GetItem; default;
  end;
  ...
  public
    ...
    [HPPGEN('System::DynamicArray<TMatch>::iterator begin() {return FList.begin();}' + sLineBreak +
            'System::DynamicArray<TMatch>::iterator end()   {return FList.end();}'   + sLineBreak +
...
function TGroupCollection.GetItem(const Index: Integer): TGroup;
begin
...
function TGroupCollection.GetNamedGroupIndex(const Index: string; out IntIndex: Integer): Boolean;
begin
  IntIndex := GetRegEx(FNotifier).NamedGroup(Index);
  Result := (IntIndex >= 0) and (IntIndex < Length(FList));
end;

function TGroupCollection.GetItem(const Index: string): TGroup;
var
  LIndex: Integer;
begin
  if GetNamedGroupIndex(Index, LIndex) then
    Result := FList[LIndex]
  else if LIndex = PCRE_ERROR_NOSUBSTRING then
    raise ERegularExpressionError.CreateResFmt(@SRegExInvalidGroupName, [Index])
  else
    raise ERegularExpressionError.CreateResFmt(@SRegExIndexOutOfBounds, [LIndex]);
end;

function TGroupCollection.TryGetNamedGroup(const Index: string; out Group: TGroup): Boolean;
var
  LIndex: Integer;
begin
  Result := GetNamedGroupIndex(Index, LIndex);
  if Result then
    Group := FList[LIndex];
end;

function TGroupCollection.ContainsNamedGroup(const Index: string): Boolean;
var
  LIndex: Integer;
begin
  Result := GetNamedGroupIndex(Index, LIndex);
...
procedure TRegEx.AddRawOptions(const Value: Integer);
begin
  FRegEx.AddRawOptions(Value);
end;

procedure TRegEx.Study(Options: TRegExStudyOptions);
begin
