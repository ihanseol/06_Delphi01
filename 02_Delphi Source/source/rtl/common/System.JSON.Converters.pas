...
type
  ...
  TJsonCustomObjectConverter<T: class, constructor> = class(TJsonConverter)
  protected
...
{$HINTS ON}
  ...
  TJsonTypedListConverter<V> = class(TJsonListConverter<V>)
  public
    function ReadJson(const AReader: TJsonReader; ATypeInf: PTypeInfo; const AExistingValue: TValue;
      const ASerializer: TJsonSerializer): TValue; override;
  end;
...
function TJsonCustomObjectConverter<T>.CreateInstance(ATypeInf: PTypeInfo;
  const ASerializer: TJsonSerializer): T;
var
  LObjectContract: TJsonObjectContract;
  LCreateInstanceP: function: T of object;
begin
  LCreateInstanceP := CreateInstance;
  if (TMethod(LCreateInstanceP).Code <> @TJsonCustomObjectConverter<T>.CreateInstance) or
     (ATypeInf = nil) or not CanConvert(ATypeInf) then
    Result := CreateInstance
  else
  begin
    LObjectContract := ASerializer.ContractResolver.ResolveContract(ATypeInf) as TJsonObjectContract;
    Result := LObjectContract.DefaultCreator.Invoke([]).AsType<T>;
  end;
end;

function TJsonCustomObjectConverter<T>.CanConvert(ATypeInf: PTypeInfo): Boolean;
begin
  Result := TJsonTypeUtils.InheritsFrom(ATypeInf, T);
end;
...
{ TJsonTypedListConverter<V> }

function TJsonTypedListConverter<V>.ReadJson(const AReader: TJsonReader; ATypeInf: PTypeInfo; const AExistingValue: TValue;
  const ASerializer: TJsonSerializer): TValue;
var
  LObjectContract: TJsonObjectContract;
  LList: TList<V>;
...
begin
  ...
  begin
    LObjectContract := ASerializer.ContractResolver.ResolveContract(ATypeInf) as TJsonObjectContract;
    LList := LObjectContract.DefaultCreator.Invoke([]).AsType<TList<V>>;
    LExistingValue := LList;
  end
  else
  begin
    LList := nil;
...
function TJsonCustomCreationConverter<T>.ReadJson(const AReader: TJsonReader; ATypeInf: PTypeInfo;
  const AExistingValue: TValue; const ASerializer: TJsonSerializer): TValue;
begin
  if not AExistingValue.IsEmpty and (AExistingValue.TypeInfo = ATypeInf) then
    Result := AExistingValue
  else if AReader.TokenType = TJsonToken.Null then
    Result := nil
  else
  begin
...
begin
  Result := ATypeInf = TypeInfo(System.TGUID);
end;
...
begin
  AWriter.WriteValue(AValue.AsType<TGUID>.ToString);
end;

{ TJsonHashSetConverter<V> }

function TJsonHashSetConverter<V>.ReadJson(const AReader: TJsonReader;
  ATypeInf: PTypeInfo; const AExistingValue: TValue;
  const ASerializer: TJsonSerializer): TValue;
var
  HashSet: THashSet<V>;
...
begin
  ...
  begin
    ASerializer.Populate(AReader, Arr);
    if AExistingValue.IsEmpty then
      HashSet := CreateInstance(ATypeInf, ASerializer)
    else
      HashSet := AExistingValue.AsType<THashSet<V>>;
    HashSet.AddRange(Arr);
    Result := TValue.From(HashSet);
  end;
end;

procedure TJsonHashSetConverter<V>.WriteJson(const AWriter: TJsonWriter;
  const AValue: TValue; const ASerializer: TJsonSerializer);
var
  HashSet: THashSet<V>;
begin
  if AValue.TryAsType(HashSet) then
    ASerializer.Serialize(AWriter, HashSet.ToArray)
  else
    raise EJsonException.Create(Format(SConverterNotMatchingType, [AValue.TypeInfo^.Name, THashSet<V>.ClassName]));
end;
