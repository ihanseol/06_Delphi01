...
{ TParser special tokens }
  ...
  pfidWindows     = pidWin32 or pidWin64 or pidWinArm64 or pidWin64x;
  ...
  pfidLinux       = pidLinux64 or pidLinuxArm64;
...
{ To be moved to System.UITypes in a future release }
  ...
  TShiftStateItem = (ssShift, ssAlt, ssCtrl,
...
{ TList class }
  ...
  protected
    ...
    procedure Put(Index: NativeInt; Item: Pointer);
  ...
  public
    ...
    function Add(Item: Pointer): NativeInt;
    ...
    function IndexOf(Item: Pointer): NativeInt;
    function IndexOfItem(Item: Pointer; Direction: TDirection): NativeInt;
    procedure Insert(Index: NativeInt; Item: Pointer);
    function Last: Pointer;
    procedure Move(CurIndex, NewIndex: NativeInt);
    function Remove(Item: Pointer): NativeInt; inline;
    function RemoveItem(Item: Pointer; Direction: TDirection): NativeInt;
    procedure Pack;
    ...
    property Items[Index: NativeInt]: Pointer read Get write Put; default;
...
{ TStrings class }
  ...
  TStringsDefinedItem = (sdDelimiter, sdQuoteChar, sdNameValueSeparator,
  ...
  public
    ...
    [HPPGEN('void __fastcall AddStrings(const System::UnicodeString *Strings, const System::NativeInt Strings_High);'+ #13#10 +
    ...
    [HPPGEN('void __fastcall AddStrings(const System::UnicodeString *Strings, const System::NativeInt Strings_High,' + #13#10 +
            '                 System::TObject* const *Objects, const System::NativeInt Objects_High);'               + #13#10 +
            'void __fastcall AddStrings(const System::DynamicArray<System::UnicodeString> Strings,'                  + #13#10 +
...
{ TStringList class }
  ...
  private type
    ...
    TOverridden = set of TOverriddenItem;
  private
...
{$ENDIF Sizeof(LongInt) <> Sizeof(NativeInt)}
  ...
  protected
    ...
    function GetCapabilities: TStreamCapabilities; virtual;
...
{$IF Sizeof(LongInt) <> Sizeof(NativeInt)}
    function Read(Buffer: TBytes; Offset, Count: NativeInt): NativeInt; overload; virtual;
    function Write(const Buffer: TBytes; Offset, Count: NativeInt): NativeInt; overload; virtual;
{$ENDIF Sizeof(LongInt) <> Sizeof(NativeInt)}
...
{$IF Sizeof(LongInt) <> Sizeof(NativeInt)}
    function Read(var Buffer: TBytes; Count: NativeInt): NativeInt; overload; inline;
    function Write(const Buffer: TBytes; Count: NativeInt): NativeInt; overload; inline;
{$ENDIF Sizeof(LongInt) <> Sizeof(NativeInt)}
...
{$IFDEF NEXTGEN}
    function ReadData(var Buffer: System.UTF8Char): NativeInt; overload; inline;
...
{$IFDEF NEXTGEN}
    function WriteData(const Buffer: UTF8Char): NativeInt; overload; inline;
...
{$ENDIF NEXTGEN}
    ...
    property Capabilities: TStreamCapabilities read GetCapabilities;
  end;
...
{ THandleStream class }
  ...
  protected
    FHandle: THandle;
    function GetCapabilities: TStreamCapabilities; override;
...
{ TProxySubrangeStream }
  ...
  public
    constructor Create(Stream: TStream; Offset, Size: Int64; OwnsStream: Boolean = False);
    destructor Destroy; override;
{$IF Sizeof(LongInt) <> Sizeof(NativeInt)}
    function Read(var Buffer; Count: Longint): Longint; override;
    function Write(const Buffer; Count: Longint): Longint; override;
...
{ TProxyAggregateStream }
  ...
  /// <summary> Helper class for reading concatenation of another streams.
  ///  For example, it is used by TMultipartFormData to aggregate form elements
  ///  and represent them as a single stream. </summary>
  TProxyAggregateStream = class(TStream)
  private
    FStreams: TList<TStream>;
    FOwnsStreams: array of Boolean;
...
{$ENDIF Sizeof(LongInt) <> Sizeof(NativeInt)}
    ...
    function AddStream(AStream: TStream; OwnsStream: Boolean = False): Integer;
...
{ TBasicAction }
  ...
  protected
    ...
    property SafeRef: TSafeRef read GetSafeRef;
  public
...
{$ENDIF MSWINDOWS}
  ...
  TTextReader = class
  protected
    function GetEndOfStream: Boolean; virtual; abstract;
  public
    ...
    function GetEnumerator: TTextReaderEnumerator; inline;
...
procedure TCollection_ResetID(Self: TCollection);
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        TIntConst(aLockList.Items[I]).Free;
        aLockList.Delete(I);
      end;
    end;
  finally
...
begin
  ...
  if (AName = '') or LClass.ClassNameIs('TApplication') then
    Exit(nil);
...
function TList.Add(Item: Pointer): NativeInt;
begin
  Result := FCount;
...
procedure TList.Delete(Index: NativeInt);
var
  Temp: Pointer;
begin
  if (Index < 0) or (Index >= FCount) then
    ListIndexError(Index, FCount - 1, Self);
  Temp := FList[Index];
  Dec(FCount);
...
procedure TList.Exchange(Index1, Index2: NativeInt);
var
  Item: Pointer;
begin
  if (Index1 < 0) or (Index1 >= FCount) then
    ListIndexError(Index1, FCount - 1, Self);
  if (Index2 < 0) or (Index2 >= FCount) then
    ListIndexError(Index2, FCount - 1, Self);
  Item := FList[Index1];
...
function TList.Get(Index: NativeInt): Pointer;
begin
  if NativeUInt(Index) >= NativeUInt(FCount) then
    ListIndexError(Index, FCount - 1, Self);
  Result := FList[Index];
end;
...
function TList.GetIsEmpty: Boolean;
begin
  Result := FCount = 0;
end;
...
function TList.IndexOf(Item: Pointer): NativeInt;
var
  P: PPointer;
begin
  P := Pointer(FList);
...
function TList.IndexOfItem(Item: Pointer; Direction: TDirection): NativeInt;
var
  P: PPointer;
begin
...
procedure TList.Insert(Index: NativeInt; Item: Pointer);
begin
  if (Index < 0) or (Index > FCount) then
    ListIndexError(Index, FCount, Self);
  if FCount = FCapacity then
    Grow;
  if Index < FCount then
...
begin
  ...
  begin
    ListIndexError(0, -1, Self);
    Result := nil;
  end;
end;

procedure TList.Move(CurIndex, NewIndex: NativeInt);
var
  Item: Pointer;
begin
  ...
  begin
    if (NewIndex < 0) or (NewIndex >= FCount) then
      ListIndexError(NewIndex, FCount - 1, Self);
    Item := Get(CurIndex);
    FList[CurIndex] := nil;
    Delete(CurIndex);
    Insert(NewIndex, nil);
...
procedure TList.Put(Index: NativeInt; Item: Pointer);
var
  Temp: Pointer;
begin
  if (Index < 0) or (Index >= FCount) then
...
function TList.Remove(Item: Pointer): NativeInt;
begin
  Result := RemoveItem(Item, TList.TDirection.FromBeginning);
end;

function TList.RemoveItem(Item: Pointer; Direction: TDirection): NativeInt;
begin
...
procedure TList.SetCapacity(NewCapacity: NativeInt);
begin
...
procedure TList.SetCount(NewCount: NativeInt);
var
  I: NativeInt;
  Temp: Pointer;
begin
...
procedure QuickSort(SortList: TPointerList; L, R: NativeInt;
...
begin
  ...
  try
    if (Index < 0) or (Index >= aList.Count) then
      ListIndexError(Index, aList.Count - 1, Self);
...
begin
  Result := FIndex < FCollection.FItems.Count - 1;
  if Result then
    Inc(FIndex);
end;
  ...
  begin
    ...
    try
      ...
      while FItems.Count > 0 do
        FItems.List[FItems.Count - 1].Free;
...
begin
  Notify(FItems[Index], cnDeleting);
  FItems[Index].Free;
end;
...
procedure TStrings.IndexError(AIndex, AMaxIndex: Integer);
begin
...
function TStrings.GetIsEmpty: Boolean;
begin
  Result := GetCount = 0;
end;
...
procedure WriteError(Ident: PResStringRec); overload;
begin
  raise EWriteError.CreateRes(Ident) at ReturnAddress;
end;

procedure WriteError; overload;
begin
  raise EWriteError.CreateRes(@SWriteError) at ReturnAddress;
end;
...
var
  ...
  s: string;
begin
  if stcOSErrorInfo in Capabilities then
  begin
    ErrorCode := GetLastError;
    if ErrorCode <> 0 then
      s := ': ' + SysErrorMessage(ErrorCode);
  end;
  raise EReadError.Create(SReadError + s) at ReturnAddress;
end;
...
begin
  ...
  raise EWriteError.Create(SWriteError + s) at ReturnAddress;
end;
...
begin
  Result := 0;
  repeat
    LCount := Min(Longint.MaxValue, Count);
    LReadCount := Read((PByte(@Buffer) + Result)^, Longint(LCount));
    Dec(Count, LReadCount);
    Inc(Result, LReadCount);
  until (Count <= 0) or (LCount <> LReadCount);
end;

function TStream.Write(const Buffer; Count: NativeInt): NativeInt;
var
  LCount,
  LWriteCount: NativeInt;
begin
  Result := 0;
  repeat
    LCount := Min(Longint.MaxValue, Count);
    LWriteCount := Write((PByte(@Buffer) + Result)^, Longint(LCount));
    Dec(Count, LWriteCount);
    Inc(Result, LWriteCount);
  until (Count <= 0) or (LCount <> LWriteCount);
end;
{$ENDIF Sizeof(LongInt) <> Sizeof(NativeInt)}
...
{$IF Sizeof(LongInt) <> Sizeof(NativeInt)}
function TStream.Read(Buffer: TBytes; Offset, Count: NativeInt): NativeInt;
begin
  Result := Read(Buffer[Offset], GetByteCount64(Buffer, Offset, Count));
end;

function TStream.Write(const Buffer: TBytes; Offset, Count: NativeInt): NativeInt;
begin
  Result := Write(Buffer[Offset], GetByteCount64(Buffer, Offset, Count));
end;
{$ENDIF Sizeof(LongInt) <> Sizeof(NativeInt)}
...
{$ENDIF SizeOf(NativeInt) = SizeOf(Int64)}
...
{$IF Sizeof(LongInt) <> Sizeof(NativeInt)}
function TStream.Read(var Buffer: TBytes; Count: NativeInt): NativeInt;
begin
  Result := Read(Buffer, 0, Count);
end;

function TStream.Write(const Buffer: TBytes; Count: NativeInt): NativeInt;
begin
  Result := Write(Buffer, 0, Count);
end;
{$ENDIF Sizeof(LongInt) <> Sizeof(NativeInt)}
...
begin
  { Perform a write directly. Most of the time this will succeed
    without the need to go into the WHILE loop. }
{$IF SizeOf(NativeInt) = SizeOf(Int64)}
  LTotalCount := Write64(Buffer, Offset, Count);
{$ELSE}
...
{$ENDIF SizeOf(NativeInt) = SizeOf(Int64)}
  ...
  begin
    { Try to write a contiguous block of <Count> size }
{$IF SizeOf(NativeInt) = SizeOf(Int64)}
    LWrittenCount := Write64(Buffer, Offset + LTotalCount, (Count - LTotalCount));
{$ELSE}
...
function TStream.GetCapabilities: TStreamCapabilities;
begin
  Result := [];
end;
...
begin
  ...
  begin
    ...
    SetLength(Buffer, BufferSize);
    AdjustSize := False;
    if stcPreallocateOptimization in Capabilities then
      try
        CurSize := Size;
        CurPos := Position;
...
function THandleStream.GetCapabilities: TStreamCapabilities;
begin
  Result := [stcOSErrorInfo];
end;
...
function TCustomMemoryStream.Read(var Buffer; Count: TNativeCount): TNativeCount;
begin
...
begin
  Result := Longint(Read(Buffer, NativeInt(Count)));
end;
{$ENDIF Sizeof(LongInt) <> Sizeof(NativeInt)}
...
function TMemoryStream.Write(const Buffer; Count: TNativeCount): TNativeCount;
var
  Pos: Int64;
begin
  if (FPosition >= 0) and (Count >= 0) then
...
function TMemoryStream.GetCapabilities: TStreamCapabilities;
begin
  Result := [stcPreallocateOptimization];
end;
...
function TPointerStream.Write(const Buffer; Count: TNativeCount): TNativeCount;
var
  Pos: Int64;
begin
  if FReadOnly then
...
begin
  ...
  if Offset >= 0 then
  begin
    if (Size = -1) and (Offset > Stream.Size) or
       (Size >= 0) and (Offset + Size > Stream.Size) then
...
destructor TProxySubrangeStream.Destroy;
begin
  if FOwnsStream then
    FStream.Free;
  inherited;
...
function TProxySubrangeStream.GetCapabilities: TStreamCapabilities;
begin
  Result := FStream.Capabilities - [stcPreallocateOptimization];
end;
...
function TProxySubrangeStream.Read(var Buffer; Count: TNativeCount): TNativeCount;
begin
  if FOffset >= 0 then
  begin
    if (FSize <> -1) and (FStream.Position + Count > FOffset + FSize) then
...
function TProxySubrangeStream.Write(const Buffer; Count: TNativeCount): TNativeCount;
begin
  if FOffset >= 0 then
  begin
    if (FSize <> -1) and (FStream.Position + Count > FOffset + FSize) then
...
begin
  ...
  begin
    ...
    case Origin of
      soBeginning:
        begin
          if (FSize <> -1) and (LOff > FSize) then
            LOff := FSize
          else if LOff < 0 then
            LOff := 0;
          Inc(LOff, FOffset);
      ...
      soCurrent:
        begin
          if (FSize <> -1) and (FStream.Position + LOff > FOffset + FSize) then
...
function TProxyAggregateStream.AddStream(AStream: TStream; OwnsStream: Boolean): Integer;
var
  LSize: Int64;
begin
  try
  ...
  SetLength(FOwnsStreams, FStreams.Count);
  FOwnsStreams[FStreams.Count - 1] := OwnsStream;
  Inc(FSize, LSize);
  Result := FStreams.Count - 1;
end;
...
var
  LStream: TStream;
begin
  LStream := FStreams[AIndex];
  try
    Dec(FSize, LStream.Size);
  finally
    if FOwnsStreams[AIndex] then
      LStream.Free;
    Delete(FOwnsStreams, AIndex, 1);
    FStreams.Delete(AIndex);
    FPosition := 0;
    FCurrentStream := -1;
  end;
end;
...
begin
  try
    for I := 0 to FStreams.Count - 1 do
      if FOwnsStreams[I] then
        FStreams[I].Free;
  finally
    SetLength(FOwnsStreams, 0);
    FStreams.Clear;
    FSize := 0;
    FPosition := 0;
    FCurrentStream := -1;
...
begin
  ...
  begin
    ...
    LLocPos := FStreams[FCurrentStream].Size;
    LAbsPos := FPosition;
  end else
  begin
    FCurrentStream := 0;
  ...
  while LAbsPos < FPosition do
  begin
    LLocSize := FStreams[FCurrentStream].Size;
    if FPosition - LAbsPos > LLocSize then
    begin
      Inc(LAbsPos, LLocSize);
    ...
    begin
      LLocPos := FPosition - LAbsPos;
      LAbsPos := FPosition;
    end;
  end;
  FCurrentStreamPos := FStreams[FCurrentStream].Seek(LLocPos, soBeginning);
end;

function TProxyAggregateStream.Read(var Buffer; Count: TNativeCount): TNativeCount;
var
  P: PByte;
  LTotRemains,
  LRead: TNativeCount;
...
begin
  if (FCurrentStream = -1) or
     (FCurrentStream < FStreams.Count) and
       (FCurrentStreamPos <> FStreams[FCurrentStream].Position) then
    SyncPosition;
  Result := 0;
  ...
  begin
    LTotRemains := Count - Result;
    LLocRemains := FStreams[FCurrentStream].Size - FStreams[FCurrentStream].Position;
    if LTotRemains < LLocRemains then
      LRead := LTotRemains
    else
      LRead := LLocRemains;
    LRead := FStreams[FCurrentStream].Read(P[Result], LRead);
    FCurrentStreamPos := FStreams[FCurrentStream].Position;
    Inc(FPosition, LRead);
    Inc(Result, LRead);
    if (Result < Count) and
       (FStreams[FCurrentStream].Position >= FStreams[FCurrentStream].Size) then
    begin
      Inc(FCurrentStream);
      if FCurrentStream < FStreams.Count then
      begin
        FStreams[FCurrentStream].Seek(0, soBeginning);
        FCurrentStreamPos := 0;
      end;
    end;
  end;
...
function TProxyAggregateStream.Write(const Buffer; Count: TNativeCount): TNativeCount;
begin
  raise EStreamError.CreateRes(@SCantWriteAggregateStreamError);
end;

{$IF Sizeof(LongInt) <> Sizeof(NativeInt)}
function TProxyAggregateStream.Read(var Buffer; Count: Longint): Longint;
begin
  Result := Longint(Read(Buffer, NativeInt(Count)));
end;

function TProxyAggregateStream.Write(const Buffer; Count: Longint): Longint;
begin
  Result := Longint(Write(Buffer, NativeInt(Count)));
end;
{$ENDIF Sizeof(LongInt) <> Sizeof(NativeInt)}
...
begin
  ...
  begin
    S := P;
    while not (P^ in ['.', '-', #0]) do Inc(P);
    if (P^ = #0) and (S = Pointer(NamePath)) then
      Name := NamePath
    else
    ...
    if (Found = nil) and SameText(Name, 'Owner') then                         { Do not translate }
      Found := Current;
  if (Found = nil) or (P^ = #0) then Exit(Found);
    if P^ = '.' then Inc(P);
    if P^ = '-' then Inc(P);
    if P^ = '>' then Inc(P);
    Current := Found;
  ...
  case ReadValue of
    ...
    vaString:
      begin
        ...
        Result := TEncoding.Default.GetString(Slice(LStatResult, L));
      end;
    vaLString:
      begin
        Read(L, SizeOf(Integer));
...
{$IFNDEF NEXTGEN}
class procedure TThread.NameThreadForDebugging(const AThreadName: AnsiString; AThreadID: TThreadID);
begin
  NameThreadForDebugging(string(AThreadName), AThreadID);
end;
{$ENDIF !NEXTGEN}

class procedure TThread.NameThreadForDebugging(const AThreadName: string; AThreadID: TThreadID);
{$IF Defined(MSWINDOWS)}
type
...
begin
  Result := (FIndex < MaxInt) and (FIndex < FComponent.FComponents.Count - 1);
  if Result then
    Inc(FIndex);
end;
...
var
  LSafeRef: TBasicAction.TSafeRef;
  LSubs: Boolean;
begin
  if FAction = nil then
    Exit(False);
  LSafeRef := FAction.SafeRef;
  LSubs := (AComponent <> nil) and (AComponent.FFreeNotifies <> nil) and
    AComponent.FFreeNotifies.Contains(FAction);
  FAction.ActionComponent := AComponent;
  try
    Result := FAction.Execute;
  finally
    if LSafeRef[0] <> nil then
      if LSubs then
        LSafeRef[0].FActionComponent := nil
      else
        LSafeRef[0].ActionComponent := nil;
  end;
end;
...
function TBasicAction.GetSafeRef: TSafeRef;
begin
  if FSafeRef = nil then
    SetLength(FSafeRef, 1);
  FSafeRef[0] := Self;
  Result := FSafeRef;
end;
...
var
  I: Integer;
begin
  if TMethod(Value) <> TMethod(OnExecute) then
  begin
...
constructor TTextReaderEnumerator.Create(AReader: TTextReader);
begin
  inherited Create;
  FReader := AReader;
end;
...
function TTextReader.GetEnumerator: TTextReaderEnumerator;
begin
  Result := TTextReaderEnumerator.Create(Self);
end;
...
function TStringReader.GetEndOfStream: Boolean;
begin
  Result := FIndex = -1;
end;
...
begin
  ...
  begin
    ...
    begin
      TLoginCredentialEventObject(FLoginHandlers.Objects[I]).Free;
