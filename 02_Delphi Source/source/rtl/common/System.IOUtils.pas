...
{$ENDIF POSIX}
  ...
  private
    type
      ...
      TEnumerator = class(TInterfacedObject, IEnumerator, IEnumerator<string>,
        IEnumerable<string>)
      private type
        TItem = class(TObject)
        private
      ...
      protected
        ...
        function IEnumerator<string>.GetCurrent = GetCurrentStr;
        function GetEnumerator: IEnumerator;
        function GetEnumeratorStr: IEnumerator<string>;
        function IEnumerable<string>.GetEnumerator = GetEnumeratorStr;
      public
        constructor Create(const Path, Pattern: string;
          const Callback: TDirectoryWalkProc; const Recursive: Boolean);
        destructor Destroy; override;
      end;
...
{$ENDIF}
    ...
    class function DoGetDirectoriesEnumerator(const Path, SearchPattern: string;
      const SearchOption: TSearchOption;
      const Predicate: TFilterPredicate): IEnumerable<string>; static;
    class function DoGetFilesEnumerator(const Path, SearchPattern: string;
      const SearchOption: TSearchOption;
      const Predicate: TFilterPredicate): IEnumerable<string>; static;
    class function DoGetFileSystemEntriesEnumerator(const Path, SearchPattern: string;
      const SearchOption: TSearchOption;
      const Predicate: TFilterPredicate): IEnumerable<string>; static;
...
{$ENDIF LINUX}
  ...
  private
    type
      TLinesEnumerator = class(TInterfacedObject, IEnumerator, IEnumerator<string>,
        IEnumerable<string>)
      private
        FPath: string;
        FEncoding: TEncoding;
      ...
      protected
        ...
        function IEnumerable<string>.GetEnumerator = GetEnumeratorStr;
      public
        constructor Create(const Path: string; Encoding: TEncoding);
        destructor Destroy; override;
      end;
...
{$ENDIF MSWINDOWS}
    ...
    class function GetLinesEnumerator(const Path: string): IEnumerable<string>;
        overload; static;
    class function GetLinesEnumerator(const Path: string;
        const Encoding: TEncoding): IEnumerable<string>; overload; static;
...
{ TDirectory.TEnumerator }

constructor TDirectory.TEnumerator.TItem.Create(const APath: string);
begin
  inherited Create;
  FPath := APath;
end;

destructor TDirectory.TEnumerator.TItem.Destroy;
begin
  FindClose(FSearchRec);
  inherited Destroy;
end;

constructor TDirectory.TEnumerator.Create(const Path, Pattern: string;
  const Callback: TDirectoryWalkProc; const Recursive: Boolean);
begin
  inherited Create;
  FPath := Path;
  ...
  FStack := TStack<TItem>.Create;
  FInit := True;
end;

destructor TDirectory.TEnumerator.Destroy;
begin
  Reset;
  FStack.Free;
  inherited Destroy;
...
procedure TDirectory.TEnumerator.Reset;
begin
...
function TDirectory.TEnumerator.MoveNext: Boolean;
var
  LItem: TItem;
  LMatch,
  LForw: Boolean;
begin
  ...
  repeat
    ...
    begin
      ...
      if not LForw and
         FRecursive and (LItem.FSearchRec.Attr and System.SysUtils.faDirectory <> 0) and
         (LItem.FSearchRec.Name <> TPath.FCCurrentDir) and
         (LItem.FSearchRec.Name <> TPath.FCParentDir) then
      begin
        var s: string := TPath.DoCombine(LItem.FPath, LItem.FSearchRec.Name, False);
        LItem := TItem.Create(s);
        s := TPath.DoCombine(LItem.FPath, '*', False); // DO NOT LOCALIZE
        if FindFirst(s, faAnyFile, LItem.FSearchRec) = 0 then
        begin
          FStack.Push(LItem);
          Result := True;
        end
      ...
      begin
        LForw := False;
        Result := FindNext(LItem.FSearchRec) = 0;
      end;
    end
    else if FInit then
    begin
      FInit := False;
      LItem := TItem.Create(FPath);
      var s: string := TPath.DoCombine(LItem.FPath, '*', False); // DO NOT LOCALIZE
      if FindFirst(s, faAnyFile, LItem.FSearchRec) = 0 then
      begin
        FStack.Push(LItem);
        Result := True;
    ...
    if Result then
    begin
      LMatch := TPath.DoMatchesPattern(LItem.FSearchRec.Name, FPattern);
      // call the callback method
      if LMatch and Assigned(FCallback) then
        LMatch := FCallback(LItem.FPath, LItem.FSearchRec);
    end
    else
    begin
      LMatch := False;
  ...
  until LMatch or not Result;
end;

function TDirectory.TEnumerator.GetCurrent: TObject;
begin
  Result := nil;
end;

function TDirectory.TEnumerator.GetEnumerator: IEnumerator;
begin
  Result := nil;
end;

function TDirectory.TEnumerator.GetCurrentStr: string;
var
  LItem: TItem;
begin
  if FStack.Count > 0 then
  begin
    LItem := FStack.Peek;
    Result := TPath.DoCombine(LItem.FPath, LItem.FSearchRec.Name, False);
  end
  else
    Result := '';
end;

function TDirectory.TEnumerator.GetEnumeratorStr: IEnumerator<string>;
begin
  Result := Self;
end;
...
class procedure TDirectory.Copy(const SourceDirName, DestDirName: string; IgnoreErrors: Boolean);
var
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            SetLength(ResultArray, Capacity);
          end;
          ResultArray[I] := TPath.DoCombine(Path, FileInfo.Name, False);
          Inc(I);
        end;
      end;
    end;
...
class function TDirectory.DoGetDirectoriesEnumerator(const Path, SearchPattern: string;
  const SearchOption: TSearchOption;
  const Predicate: TFilterPredicate): IEnumerable<string>;
var
  Callback: TDirectoryWalkProc;
begin
  Callback :=
    function (const Path: string; const FileInfo: TSearchRec): Boolean
    begin
      ...
      if (FileInfo.Attr and System.SysUtils.faDirectory <> 0) and
         (FileInfo.Name <> TPath.FCParentDir) and (FileInfo.Name <> TPath.FCCurrentDir) then
      begin
        Result := (not Assigned(Predicate)) or
                  (Assigned(Predicate) and Predicate(Path, FileInfo));
      end;
    end;

  Result := TEnumerator.Create(Path, SearchPattern, Callback,
    SearchOption = TSearchOption.soAllDirectories);
end;
...
class function TDirectory.DoGetFilesEnumerator(const Path, SearchPattern: string;
  const SearchOption: TSearchOption;
  const Predicate: TFilterPredicate): IEnumerable<string>;
var
  Callback: TDirectoryWalkProc;
...
class function TDirectory.DoGetFileSystemEntriesEnumerator(const Path, SearchPattern: string;
  const SearchOption: TSearchOption;
  const Predicate: TFilterPredicate): IEnumerable<string>;
var
  Callback: TDirectoryWalkProc;
...
class function TDirectory.GetDirectoriesEnumerator(const Path,
  SearchPattern: string): IEnumerable<string>;
begin
  Result := GetDirectoriesEnumerator(Path, SearchPattern, TSearchOption.soTopDirectoryOnly);
end;

class function TDirectory.GetDirectoriesEnumerator(const Path: string;
  const Predicate: TFilterPredicate): IEnumerable<string>;
begin
  Result := GetDirectoriesEnumerator(Path, '*', TSearchOption.soTopDirectoryOnly, Predicate); // DO NOT LOCALIZE
end;

class function TDirectory.GetDirectoriesEnumerator(
  const Path: string): IEnumerable<string>;
begin
  Result := GetDirectoriesEnumerator(Path, '*', TSearchOption.soTopDirectoryOnly); // DO NOT LOCALIZE
end;
...
begin
  ...
  Result := DoGetDirectoriesEnumerator(Path, '*', SearchOption, Predicate); // DO NOT LOCALIZE
end;

class function TDirectory.GetDirectoriesEnumerator(const Path,
  SearchPattern: string; const SearchOption: TSearchOption;
  const Predicate: TFilterPredicate): IEnumerable<string>;
begin
  CheckGetDirectoriesParameters(Path, SearchPattern);

  Result := DoGetDirectoriesEnumerator(Path, SearchPattern, SearchOption, Predicate);
end;

class function TDirectory.GetDirectoriesEnumerator(const Path,
  SearchPattern: string;
  const SearchOption: TSearchOption): IEnumerable<string>;
begin
  CheckGetDirectoriesParameters(Path, SearchPattern);

  Result := DoGetDirectoriesEnumerator(Path, SearchPattern, SearchOption, nil);
end;
...
class function TDirectory.GetFilesEnumerator(const Path,
  SearchPattern: string): IEnumerable<string>;
begin
  Result := GetFilesEnumerator(Path, SearchPattern, TSearchOption.soTopDirectoryOnly);
end;

class function TDirectory.GetFilesEnumerator(const Path: string;
  const Predicate: TFilterPredicate): IEnumerable<string>;
begin
  Result := GetFilesEnumerator(Path, '*', TSearchOption.soTopDirectoryOnly, Predicate); // DO NOT LOCALIZE
end;

class function TDirectory.GetFilesEnumerator(
  const Path: string): IEnumerable<string>;
begin
  Result := GetFilesEnumerator(Path, '*', TSearchOption.soTopDirectoryOnly); // DO NOT LOCALIZE
end;

class function TDirectory.GetFilesEnumerator(const Path, SearchPattern: string;
  const Predicate: TFilterPredicate): IEnumerable<string>;
begin
  Result := GetFilesEnumerator(Path, SearchPattern, TSearchOption.soTopDirectoryOnly, Predicate);
end;
...
begin
  ...
  Result := DoGetFilesEnumerator(Path, '*', SearchOption, Predicate); // DO NOT LOCALIZE
end;
...
begin
  ...
  Result := DoGetFilesEnumerator(Path, SearchPattern, SearchOption, Predicate);
end;
...
begin
  ...
  Result := DoGetFilesEnumerator(Path, SearchPattern, SearchOption, nil);
end;
...
class function TDirectory.GetFileSystemEntriesEnumerator(
  const Path: string): IEnumerable<string>;
begin
  Result := GetFileSystemEntriesEnumerator(Path, '*'); // DO NOT LOCALIZE
end;

class function TDirectory.GetFileSystemEntriesEnumerator(const Path: string;
  const Predicate: TFilterPredicate): IEnumerable<string>;
begin
  Result := GetFileSystemEntriesEnumerator(Path, '*', Predicate); // DO NOT LOCALIZE
end;

class function TDirectory.GetFileSystemEntriesEnumerator(const Path,
  SearchPattern: string): IEnumerable<string>;
begin
  CheckGetFileSystemEntriesParameters(Path, SearchPattern);

  Result := DoGetFileSystemEntriesEnumerator(Path, SearchPattern,
    TSearchOption.soTopDirectoryOnly, nil);
end;
...
begin
  ...
  Result := DoGetFileSystemEntriesEnumerator(Path, '*', SearchOption, Predicate); // DO NOT LOCALIZE
end;

{ TFile.TLinesEnumerator }

constructor TFile.TLinesEnumerator.Create(const Path: string;
  Encoding: TEncoding);
begin
  inherited Create;
  FPath := Path;
...
destructor TFile.TLinesEnumerator.Destroy;
begin
  FreeAndNil(FReader);
...
procedure TFile.TLinesEnumerator.Reset;
begin
  FreeAndNil(FReader);
  if FEncoding = nil then
    FReader := TStreamReader.Create(FPath, True)
  else
    FReader := TStreamReader.Create(FPath, FEncoding);
end;

function TFile.TLinesEnumerator.MoveNext: Boolean;
begin
...
function TFile.TLinesEnumerator.GetCurrent: TObject;
begin
  Result := nil;
end;

function TFile.TLinesEnumerator.GetEnumerator: IEnumerator;
begin
  Result := nil;
end;

function TFile.TLinesEnumerator.GetCurrentStr: string;
begin
  Result := FLine;
end;

function TFile.TLinesEnumerator.GetEnumeratorStr: IEnumerator<string>;
begin
  Result := Self;
end;
...
class function TFile.GetLinesEnumerator(const Path: string;
  const Encoding: TEncoding): IEnumerable<string>;
begin
  CheckReadAllLinesParameters(Path, Encoding, True);

  Result := TLinesEnumerator.Create(Path, Encoding);
end;

class function TFile.GetLinesEnumerator(
  const Path: string): IEnumerable<string>;
begin
  CheckReadAllLinesParameters(Path, nil, False);

  Result := TLinesEnumerator.Create(Path, nil);
end;
...
class function TPath.Combine(const Path1, Path2, Path3: string; const ValidateParams: Boolean): string;
begin
  Result := DoCombine(DoCombine(Path1, Path2, ValidateParams), Path3, ValidateParams);
end;

class function TPath.Combine(const Path1, Path2, Path3, Path4: string; const ValidateParams: Boolean): string;
begin
  Result := DoCombine(DoCombine(DoCombine(Path1, Path2, ValidateParams), Path3, ValidateParams), Path4, ValidateParams);
end;

class function TPath.Combine(const Paths: array of string; const ValidateParams: Boolean): string;
var
  i: Integer;
begin
  ...
  for i := 1 to High(Paths) do
    Result := DoCombine(Result, Paths[i], ValidateParams);
end;
...
begin
  if TOSVersion.Check(6) then
    try
      if SHGetKnownFolderPath(FOLDERID_Downloads, 0, 0, LPtr) = S_OK then
        Result := LPtr
      else
...
begin
  if TOSVersion.Check(6) then
    try
      if SHGetKnownFolderPath(FOLDERID_PublicDownloads, 0, 0, LPtr) = S_OK then
        Result := LPtr
      else
...
{$IFDEF POSIX}
begin
  // we have no better way
  Result := Combine(GetHomePath, 'Desktop'); // Do not localize
end;
{$ENDIF}
...
begin
  Result := ExtractFilePath(ParamStr(0));
  if (Result <> '') and (Result.Chars[Result.Length - 1] = DirectorySeparatorChar) then
