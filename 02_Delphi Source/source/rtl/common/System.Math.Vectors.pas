...
procedure TCubicBezierCurve.Init(const A, B, C, D: TPointF);
begin
  StartPoint := A;
  EndPoint := D;
  CX := 3 * (B.X - A.X);
...
function TCubicBezierCurve.GetPoint(Index, Total: Integer): TPointF;
begin
  if Index <= 0 then
    Result := StartPoint
  else
...
function TCubicBezierCurve.CurveLength(Segments: Integer = 6): Single;
var
  DT, T: Single;
  I: Integer;
  P: array[0..1] of TPointF;
  n: Integer;
begin
  DT := 1 / Segments;
  T := 0;
  ...
  begin
    T := T + DT;
    P[n] := GetPoint(T);
    Result := Result + (P[n] - P[1 - n]).Length;
    n := 1 - n;
  end;
  Result := Result + (EndPoint - P[1 - n]).Length;
end;
