...
var
  Permitted, GPSStarted, NetworkStarted, PassiveStarted: Boolean;

begin
  Result := False;
  Permitted := TPermissionsService.DefaultService.IsPermissionGranted(JStringToString(TJManifest_permission.JavaClass.ACCESS_COARSE_LOCATION));
  if Permitted then
  begin
    if FAccuracy > 0 then
...
begin
  ...
  if Assigned(FLastValue) then
    case Prop of
      ...
      TCustomLocationSensor.TProperty.ErrorRadius:
        if FLastValue.hasAccuracy then
          Result := FLastValue.getAccuracy;
    else
      Result := NaN;
    end;
end;
