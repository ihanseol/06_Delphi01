...
{ TNotification }
  ...
  public
    ...
    FireDate: TDateTime;
    /// <summary>Indicates whether or not a sound is played when the notification appears.</summary>
    /// <remarks>Playing sound depends on behavior of target platform.</remarks>
    EnableSound: Boolean;
    /// <summary>The file name of the sound to use for the notification.</summary>
    SoundName: string;
  ...
  public
    ...
    /// <summary>The file name of the sound to use for notifications posted to this channel.</summary>
    SoundName: string;
    constructor Create;
  end;
...
{ TBaseNotificationCenter }
  ...
  protected
    ...
    procedure DoPlatformInitialize(const ApplicationName: string); virtual;
...
{ TNotificationCenter }
  ...
  public
    ...
    /// With this overload the shortcut file is named after the .exe file and the notification toast will
    /// display the exe name (without extension) as the header line. To specify a custom shortcut filename,
    /// and so customise that toast header line, use the other overload.</remarks>
    ...
    /// <param name="ApplicationName">This is the name give to the shortcut file, and is therefore
    /// shown at the top of the notification. If not passed it will default to the .exe name.
    /// For example, if 'NotificationApp' is passed the shortcut file is called NotificationApp.lnk,
    /// but if nothing is passed and the app is called Project1.exe then the shortcut will be called
    /// Project1.lnk</param>
...
procedure TCustomNotificationCenter.DoPlatformInitialize(const ApplicationName: string);
begin
  if Supported then
    FPlatformNotificationCenter.DoPlatformInitialize(ApplicationName);
end;
...
procedure TCustomNotificationCenter.PlatformInitialize(const ApplicationName: string);
begin
  DoPlatformInitialize(ApplicationName);
end;
...
procedure TBaseNotificationCenter.DoPlatformInitialize(const ApplicationName: string);
begin
...
begin
  ...
  begin
    ...
    DestChannel.SoundName := SoundName;
  end
  else
    inherited;
end;
