...
type
  ...
  private
    ...
    function GetAsInstance: TRttiInstanceType; inline;
    function GetAsOrdinal: TRttiOrdinalType; inline;
    function GetAsRecord: TRttiRecordType; inline;
    function GetIsInstance: Boolean; inline;
    function GetIsOrdinal: Boolean; inline;
    function GetIsRecord: Boolean; inline;
    function GetHandle: PTypeInfo; inline;
    function GetAsSet: TRttiSetType; inline;
...
{$ENDIF}
  ...
  private type
    ...
    public
      constructor Create;
      destructor Destroy; override;
      function DefineOpenArray(AElType: PTypeInfo; AElCount: NativeInt): PTypeInfo; overload;
...
{$ENDIF}
    ...
    public
      ...
      destructor Destroy; override;
      procedure Seal;
      procedure Populate(AIsClas: Boolean; ASelfTypeHandle: PTypeInfo;
        const AParameters: TArray<TRttiParameter>; AReturnType: TRttiType);
  ...
  protected
    function GetInvokeInfo: TMethodImplementation.TInvokeInfo; virtual; abstract;
  public
    destructor Destroy; override;
  ...
  private
    ...
    constructor Create(APackage: TRttiPackage; AParent: TRttiObject; var P: PByte); override;
  protected
    function GetInvokeInfo: TMethodImplementation.TInvokeInfo; override;
  public
...
{$ENDIF CPUARM64 or LINUX64 or OSX64}
  ...
  private
    ...
    class function UseContext: IInterface; static;
...
function GetArrayValueFromTListHelperValue(const ACtx: TRttiContext;
  const AListHelperValue: TValue; out ACount: NativeInt): TValue;
...
procedure InsufficientRttiError;
begin
...
begin
  AValue.FAsMethod.Code := nil;
  AValue.FAsMethod.Data := nil;
end;
...
class function TValue.FromVariant(const Value: Variant; ArrayTypeInfo: PTypeInfo): TValue;
var
  DataPtr: Pointer;
begin
  case TVarData(Value).VType of
    ...
    begin
      if ArrayTypeInfo = nil then
        if VarIsType(Value, varArray or varByte) then
          ArrayTypeInfo := System.TypeInfo(TArray<Byte>)
        else
          raise EVariantTypeCastError.CreateRes(@SInvalidVarCast);
      DataPtr := nil;
      try
        DynArrayFromVariant(DataPtr, Value, ArrayTypeInfo);
        TValue.Make(@DataPtr, ArrayTypeInfo, Result);
      finally
        if DataPtr <> nil then
          DynArrayClear(DataPtr, ArrayTypeInfo);
      end;
    end
    else
      raise EVariantTypeCastError.CreateRes(@SInvalidVarCast);
...
{$ENDIF}
    ...
    vtPointer: Result := TValue.From<Pointer>(VarRec.VPointer);
...
function TValue.GetArrayLength: NativeInt;
begin
  if TypeInfo <> nil then
...
function TValue.GetArrayElement(Index: NativeInt): TValue;
begin
  if not IsArray then
    raise EInvalidCast.CreateRes(@SInvalidCast);
...
procedure TValue.SetArrayElement(Index: NativeInt; const AValue: TValue);
begin
  if not IsArray then
    raise EInvalidCast.CreateRes(@SInvalidCast);
  if (Index < 0) or (Index >= GetArrayLength) then
...
function TValue.GetReferenceToRawArrayElement(Index: NativeInt): Pointer;
var
  elSize: Integer;
begin
  if TypeInfo <> nil then
...
begin
  ...
  case FTypeInfo^.Kind of
    ...
    tkFloat:
      case GetTypeData(FTypeInfo)^.FloatType of
        ftSingle: Result := FloatToStr(FAsSingle, AFormatSettings);
        ftDouble:
        begin
          if FTypeInfo = System.TypeInfo(TDate) then
            Result := DateToStr(FAsDouble, AFormatSettings)
          else if FTypeInfo = System.TypeInfo(TTime) then
            Result := TimeToStr(FAsDouble, AFormatSettings)
          else if FTypeInfo = System.TypeInfo(TDateTime) then
            Result := DateTimeToStr(FAsDouble, AFormatSettings)
          else
            Result := FloatToStr(FAsDouble, AFormatSettings);
        end;
        ftExtended: Result := FloatToStr(FAsExtended, AFormatSettings);
        ftComp: Result := IntToStr(FAsSInt64);
        ftCurr: Result := CurrToStr(FAsCurr, AFormatSettings);
      end;
...
{$ENDIF}
    ...
    tkDynArray:
      try
        DataPtr := nil;
        try
          DynArrayFromVariant(DataPtr, v, ATarget);
          TValue.Make(@DataPtr, ATarget, AResult);
        finally
          if DataPtr <> nil then
            DynArrayClear(DataPtr, ATarget);
        end;
        Exit(True);
      except
        Exit(False);
...
{$ENDIF !NEXTGEN}
    ...
    tkDynArray:
      DynArrayToVariant(v, PPointer(ASource.GetReferenceToRawData)^, ASource.TypeInfo);
  else
    Exit(False);
  end;
...
const
  Conversions: array[TTypeKind,TTypeKind] of TConvertFunc = ( // [source, target]
    ...
    (
      ...
      // tkInterface, tkInt64, tkDynArray, tkUString, tkClassRef
      ConvFail, ConvFromVariant, ConvFromVariant, ConvFromVariant, ConvFail,
      // tkPointer, tkProcedure, tkMRecord
      ConvFail, ConvFail, ConvFail
    ),
    // tkArray
...
class function TRttiContext.UseContext: IInterface;
{$IFDEF USE_MONITOR_FOR_GLOBALCONTEXT}
begin
  TMonitor.Enter(PoolLock);
  try
...
procedure TRttiObject.BeforeDestruction;
begin
  if (FPackage <> nil) and (FPackage.FHandleToObject <> nil) then
    raise EInvalidOpException.CreateRes(@SInvalidRttiDestroy);
end;
...
constructor TRttiProperty.Create(APackage: TRttiPackage; AParent: TRttiObject;
  var P: PByte);
begin
  inherited Create(APackage, AParent, P);
  FPropertyType := GetDataType;
end;
...
function TRttiInstanceProperty.GetDataType: TRttiType;
begin
...
begin
  ...
  if Index <> Integer($80000000) then
    Result := Result + ' index ' + Index.ToString;
end;
...
function TRttiManagedField.GetDataType: TRttiType;
begin
...
function TRttiRecordField.GetDataType: TRttiType;
begin
...
function TRttiInvokableType.CreateImplementation(AUserData: Pointer;
  const ACallback: TMethodImplementationCallback): TMethodImplementation;
begin
...
function TRttiMethodType.GetInvokeInfo: TMethodImplementation.TInvokeInfo;
var
...
begin
  ...
  Info := TMethodImplementation.TInvokeInfo.Create(CallingConvention, True);
  try
    Info.Populate(False, TypeInfo(TObject), GetParameters, ReturnType);
    Info.Seal;
...
function TRttiProcedureType.GetInvokeInfo: TMethodImplementation.TInvokeInfo;
var
  Info: TMethodImplementation.TInvokeInfo;
begin
  if FInvokeInfo <> nil then
  ...
  Info := TMethodImplementation.TInvokeInfo.Create(CallingConvention, False);
  try
    Info.Populate(False, nil, GetParameters, ReturnType);
    Info.Seal;
    if AtomicCmpExchange(Pointer(FInvokeInfo), Pointer(Info), nil) = nil then
      Pointer(Info) := nil // for ARC-based plaforms this will ensure the local var doesn't affect the refcount
    else
...
begin
  ...
  try
    Info.Populate(IsClassMethod, Parent.Handle, GetParameters, ReturnType);
    Info.Seal;
    if AtomicCmpExchange(Pointer(FInvokeInfo), Pointer(Info), nil) = nil then
      Pointer(Info) := nil // for ARC-based plaforms this will ensure the local var doesn't affect the refcount
    else
...
function TRttiMember.GetPropName: string;
begin
  if Parent <> nil then
    Result := Parent.Name + '.'
  else
...
procedure TRttiMember.PropReadOnlyError;
begin
  raise EPropReadOnly.CreateFmt(SPropIsReadOnly, [GetPropName]) at ReturnAddress;
end;

procedure TRttiMember.PropWriteOnlyError;
begin
  raise EPropWriteOnly.CreateFmt(SPropIsWriteOnly, [GetPropName]) at ReturnAddress;
end;
...
function TRttiInstanceFieldClassic.GetDataType: TRttiType;
begin
...
function TRttiInstanceFieldEx.GetDataType: TRttiType;
begin
...
procedure TMethodImplementation.TInvokeInfo.Populate(AIsClas: Boolean;
  ASelfTypeHandle: PTypeInfo; const AParameters: TArray<TRttiParameter>; AReturnType: TRttiType);
var
  p: TRttiParameter;
begin
  if FHasSelf then
    if AIsClas then
      AddParameter(TypeInfo(TClass), False, True, False, False)
    else
      AddParameter(ASelfTypeHandle, False, True, False, False);

  for p in AParameters do
    if p.ParamType = nil then
      AddParameter(nil, True, True, False, False)
    else if pfArray in p.Flags then
    begin
      AddParameter(TypeInfos.DefineOpenArray(p.ParamType.Handle, 0), True,
        ([pfVar, pfOut] * p.Flags = []) or (p.ParamType.Handle = System.TypeInfo(TVarRec)),
        [pfOut, pfResult] * p.Flags <> [], True);
      AddParameter(TypeInfo(NativeInt), False, True, False, False);
    end
    else
      AddParameter(p.ParamType.Handle,
        ([pfVar, pfOut] * p.Flags <> []) or
          PassByRef(p.ParamType.Handle, FCallConv, pfConst in p.Flags),
        (pfConst in p.Flags) or
          (p.ParamType.Handle^.Kind in [tkRecord, tkMRecord, tkString, tkSet]) and ([pfVar, pfOut] * p.Flags = []),
        [pfOut, pfResult] * p.Flags <> [], False);

  if AReturnType <> nil then
    ReturnType := AReturnType.Handle;
end;
...
function TMethodImplementation.TRuntimeTypeInfos.DefineOpenArray(
  AElType: PTypeInfo; AElCount: NativeInt): PTypeInfo;
var
...
begin
  ...
  FTypeInfo := ATypeInfos.DefineOpenArray(GetArrayElType(FTypeInfo),
    PNativeInt(AHighLoc.GetArgLoc(AFrame))^ + 1);
  GetArg(AFrame, Value);
end;
...
procedure TMethodImplementation.TParamLoc.SetArg(AFrame: PInterceptFrame;
  const Value: TValue);
begin
  if (FTypeInfo <> nil) and ((Value.TypeInfo <> FTypeInfo) or Value.IsEmpty) then
  begin
    var LVal: TValue := Value.Cast(FTypeInfo);
    LVal.ExtractRawData(GetArgLoc(AFrame));
  end
  else
...
procedure GetRecCompareAddrs(AInfo: PTypeInfo; out PEQOper, PGetHashCode: Pointer);
var
  Context: TRttiContext;
  RttiType: TRttiType;
  RttiMethod: TRttiMethod;
begin
  ...
  Context := TRttiContext.Create;
  try
    RttiType := Context.GetType(AInfo);
    if RttiType <> nil then
      for RttiMethod in RttiType.GetDeclaredMethods do
      begin
        if RttiMethod.HasName('&op_Equality') and // do not localize
           (RttiMethod.MethodKind = mkOperatorOverload) and
           (RttiMethod.ReturnType.Handle = TypeInfo(Boolean)) and
           (RttiMethod.CallingConvention = ccReg) and
           (Length(RttiMethod.GetParameters) = 2) then
          PEQOper := RttiMethod.CodeAddress;
        if RttiMethod.HasName('GetHashCode') and // do not localize
           (RttiMethod.MethodKind = mkFunction) and
           (RttiMethod.ReturnType.Handle = TypeInfo(Integer)) and
           (RttiMethod.CallingConvention = ccReg) and
           (Length(RttiMethod.GetParameters) = 0) then
          PGetHashCode := RttiMethod.CodeAddress;
        if (PEQOper <> nil) and (PGetHashCode <> nil) then
          Break;
      end;
  finally
    Context.Free;
...
initialization
  ...
  System.TypInfo.GetRecCompareAddrs := @GetRecCompareAddrs;
