...
type
  ...
  private
    class function EQOperCompare(Self: Pointer; const Left, Right: T): Integer; static;
    class function TryUseRecEqualityOperator: Pointer; static;
  public
  ...
  private
    class function EQOperEquals(Self: Pointer; const Left, Right: T): Boolean; static;
    class function EQOperGetHashCode(Self: Pointer; const Value: T): Integer; static;
    class function TryUseRecEqualityOperator: Pointer; static;
  public
    class function _Default: Pointer; static; inline;
    ...
    class function Construct(const EqualityComparison: TEqualityComparison<T>):
      IEqualityComparer<T>; overload; static;
  ...
  end deprecated 'Use System.TNoRefCountObject';
  ...
  TPointerComparer = class(TCustomComparer<Pointer>)
  private
    class var
      FNoAlign: TCustomComparer<Pointer>;
      FAlign2: TCustomComparer<Pointer>;
      FAlign4: TCustomComparer<Pointer>;
      FAlign8: TCustomComparer<Pointer>;
      FAlign16: TCustomComparer<Pointer>;
      FAlign32: TCustomComparer<Pointer>;
      FAlign64: TCustomComparer<Pointer>;
  public
    class destructor Destroy;
    class function NoAlign: TPointerComparer;
    class function Align2: TPointerComparer;
    class function Align4: TPointerComparer;
    class function Align8: TPointerComparer;
    class function Align16: TPointerComparer;
    class function Align32: TPointerComparer;
    class function Align64: TPointerComparer;
    class function Ordinal: TPointerComparer; inline;
  end;
...
type
  ...
  TRecEqualityOperator<T> = function (const Left, Right: T): Boolean;
...
type
  ...
  TInfoFlag = (ifVariableSize, ifSelector);
  ...
  TTypeInfoSelector = function(info: PTypeInfo; size: NativeInt): Pointer;

  TThreeByteData = array [0..2] of Byte;

function MakeInstance(vtable: Pointer; info: PTypeInfo; sizeField: NativeInt): Pointer;
var
...
function Comparer_Selector_Integer(info: PTypeInfo; size: NativeInt): Pointer;
begin
...
function EqualityComparer_Selector_Integer(info: PTypeInfo; size: NativeInt): Pointer;
begin
  case GetTypeData(info)^.OrdType of
...
function Comparer_Selector_Float(info: PTypeInfo; size: NativeInt): Pointer;
begin
...
function EqualityComparer_Selector_Float(info: PTypeInfo; size: NativeInt): Pointer;
begin
  case GetTypeData(info)^.FloatType of
...
function Comparer_Selector_Binary(info: PTypeInfo; size: NativeInt): Pointer;
begin
  case size of
...
function EqualityComparer_Selector_Binary(info: PTypeInfo; size: NativeInt): Pointer;
begin
  case size of
{$IFDEF EABI_RECORD_ARRAY_BYREF}
    1:
...
function Comparer_Selector_Record(info: PTypeInfo; size: NativeInt): Pointer;
begin
...
function EqualityComparer_Selector_Record(info: PTypeInfo; size: NativeInt): Pointer;
begin
  case GetConstRecordArgMode(info) of
  rmByValueStack:
...
function Comparer_Selector_DynArray(info: PTypeInfo; size: NativeInt): Pointer;
begin
  Result := MakeInstance(@Comparer_Vtable_DynArray, info, GetTypeData(info)^.elSize);
end;

function EqualityComparer_Selector_DynArray(info: PTypeInfo; size: NativeInt): Pointer;
begin
...
function Comparer_Selector_String(info: PTypeInfo; size: NativeInt): Pointer;
begin
  case size of
...
function EqualityComparer_Selector_String(info: PTypeInfo; size: NativeInt): Pointer;
begin
  case size of
...
var
  ...
  I: Integer;
begin
  GetMem(inst, SizeOf(inst^) + Length(data) * SizeOf(Pointer) + (3 + Length(meths)) * SizeOf(Pointer));
  inst^.Vtable := PByte(inst) + SizeOf(TSimpleInstance) + Length(data) * SizeOf(Pointer);
  inst^.RefCount := 0;
  inst^.Size := size;
  inst^.Info := info;

  p := PPointer(PByte(inst) + SizeOf(TSimpleInstance));
  ...
  p^ := @MemRelease;
  Inc(p);
  for I := Low(meths) to High(meths) do
  begin
    p^ := meths[I];
    Inc(p);
  end;
...
function _GetExtInstanceData(self: Pointer; index: Integer): Pointer; inline;
const
  // SizeOf(TSimpleInstance)
  CDataOff = SizeOf(Pointer) * 2 + SizeOf(Integer) + SizeOf(NativeInt);
begin
  Result := PPointer(PByte(Self) + CDataOff + SizeOf(Pointer) * index)^;
end;
...
{ TNoAlignPointerComparer - TAlign64PointerComparer }

type
  TNoAlignPointerComparer = class(TPointerComparer)
  public
    function Compare(const Left, Right: Pointer): Integer; override;
    function Equals(const Left, Right: Pointer): Boolean;
      reintroduce; overload; override;
    function GetHashCode(const Value: Pointer): Integer;
      reintroduce; overload; override;
  end;
  TAlign2PointerComparer = class(TNoAlignPointerComparer)
  public
    function GetHashCode(const Value: Pointer): Integer; override;
  end;
  TAlign4PointerComparer = class(TNoAlignPointerComparer)
  public
    function GetHashCode(const Value: Pointer): Integer; override;
  end;
  TAlign8PointerComparer = class(TNoAlignPointerComparer)
  public
    function GetHashCode(const Value: Pointer): Integer; override;
  end;
  TAlign16PointerComparer = class(TNoAlignPointerComparer)
  public
    function GetHashCode(const Value: Pointer): Integer; override;
  end;
  TAlign32PointerComparer = class(TNoAlignPointerComparer)
  public
    function GetHashCode(const Value: Pointer): Integer; override;
  end;
  TAlign64PointerComparer = class(TNoAlignPointerComparer)
  public
    function GetHashCode(const Value: Pointer): Integer; override;
  end;

function TNoAlignPointerComparer.Compare(const Left, Right: Pointer): Integer;
begin
...
function TNoAlignPointerComparer.Equals(const Left, Right: Pointer): Boolean;
begin
  Result := Left = Right;
end;

function TNoAlignPointerComparer.GetHashCode(const Value: Pointer): Integer;
begin
{$IFDEF CPU64BITS}
...
function TAlign2PointerComparer.GetHashCode(const Value: Pointer): Integer;
begin
{$IFDEF CPU64BITS}
  Result := Integer(IntPtr(Self) shr 1) xor Integer(IntPtr(Self) shr 32);
{$ELSE !CPU64BITS}
  Result := Integer(Value);
...
function TAlign4PointerComparer.GetHashCode(const Value: Pointer): Integer;
begin
{$IFDEF CPU64BITS}
  Result := Integer(IntPtr(Self) shr 2) xor Integer(IntPtr(Self) shr 32);
{$ELSE !CPU64BITS}
  Result := Integer(Value);
...
function TAlign8PointerComparer.GetHashCode(const Value: Pointer): Integer;
begin
{$IFDEF CPU64BITS}
  Result := Integer(IntPtr(Self) shr 3) xor Integer(IntPtr(Self) shr 32);
{$ELSE !CPU64BITS}
  Result := Integer(Value);
...
function TAlign16PointerComparer.GetHashCode(const Value: Pointer): Integer;
begin
{$IFDEF CPU64BITS}
  Result := Integer(IntPtr(Self) shr 4) xor Integer(IntPtr(Self) shr 32);
{$ELSE !CPU64BITS}
  Result := Integer(Value);
...
function TAlign32PointerComparer.GetHashCode(const Value: Pointer): Integer;
begin
{$IFDEF CPU64BITS}
  Result := Integer(IntPtr(Self) shr 5) xor Integer(IntPtr(Self) shr 32);
{$ELSE !CPU64BITS}
  Result := Integer(Value);
...
function TAlign64PointerComparer.GetHashCode(const Value: Pointer): Integer;
begin
{$IFDEF CPU64BITS}
  Result := Integer(IntPtr(Self) shr 6) xor Integer(IntPtr(Self) shr 32);
{$ELSE !CPU64BITS}
  Result := Integer(Value);
...
class destructor TPointerComparer.Destroy;
begin
  FreeAndNil(FNoAlign);
  FreeAndNil(FAlign2);
  FreeAndNil(FAlign4);
...
class function TPointerComparer.NoAlign: TPointerComparer;
begin
  if FNoAlign = nil then
    FNoAlign := TNoAlignPointerComparer.Create;
  Result := TPointerComparer(FNoAlign);
end;

class function TPointerComparer.Align2: TPointerComparer;
begin
  if FAlign2 = nil then
    FAlign2 := TAlign2PointerComparer.Create;
  Result := TPointerComparer(FAlign2);
end;

class function TPointerComparer.Align4: TPointerComparer;
begin
  if FAlign4 = nil then
    FAlign4 := TAlign4PointerComparer.Create;
  Result := TPointerComparer(FAlign4);
end;

class function TPointerComparer.Align8: TPointerComparer;
begin
  if FAlign8 = nil then
    FAlign8 := TAlign8PointerComparer.Create;
  Result := TPointerComparer(FAlign8);
end;

class function TPointerComparer.Align16: TPointerComparer;
begin
  if FAlign16 = nil then
    FAlign16 := TAlign16PointerComparer.Create;
  Result := TPointerComparer(FAlign16);
end;

class function TPointerComparer.Align32: TPointerComparer;
begin
  if FAlign32 = nil then
    FAlign32 := TAlign32PointerComparer.Create;
  Result := TPointerComparer(FAlign32);
end;

class function TPointerComparer.Align64: TPointerComparer;
begin
  if FAlign64 = nil then
    FAlign64 := TAlign64PointerComparer.Create;
  Result := TPointerComparer(FAlign64);
end;

class function TPointerComparer.Ordinal: TPointerComparer;
begin
  Result := Align16;
end;
...
class function TComparer<T>.TryUseRecEqualityOperator: Pointer;
var
  pEQ, pHash: Pointer;
begin
  System.TypInfo.GetRecCompareAddrs(TypeInfo(T), pEQ, pHash);
  if pEQ <> nil then
    Result := _MakeExtInstance(TypeInfo(T), SizeOf(T), [pEQ], [@EQOperCompare])
  else
...
begin
  if (GetTypeKind(T) in [tkRecord, tkMRecord]) and
     Assigned(System.TypInfo.GetRecCompareAddrs) then
    Exit(TryUseRecEqualityOperator);
  Result := _LookupVtableInfo(giComparer, TypeInfo(T), SizeOf(T));
end;
...
class function TEqualityComparer<T>.EQOperEquals(Self: Pointer; const Left, Right: T): Boolean;
begin
  Result := TRecEqualityOperator<T>(_GetExtInstanceData(Self, 0))(Left, Right);
end;

class function TEqualityComparer<T>.EQOperGetHashCode(Self: Pointer; const Value: T): Integer;
begin
  Result := TRecGetHashCodeFunction(_GetExtInstanceData(Self, 1))(@Value);
end;

class function TEqualityComparer<T>.TryUseRecEqualityOperator: Pointer;
var
  pEQ, pHash: Pointer;
begin
  System.TypInfo.GetRecCompareAddrs(TypeInfo(T), pEQ, pHash);
  if (pEQ <> nil) and (pHash <> nil) then
    Result := _MakeExtInstance(TypeInfo(T), SizeOf(T), [pEQ, pHash],
      [@EQOperEquals, @EQOperGetHashCode])
  else
...
class function TEqualityComparer<T>.Construct(
  const EqualityComparison: TEqualityComparison<T>): IEqualityComparer<T>;
begin
  Result := TDelegatedEqualityComparer<T>.Create(EqualityComparison, nil);
end;
