...
type
  ...
  public type
    ...
    TReadType = (Read, ReadAsInteger, ReadAsBytes, ReadAsString, ReadAsDouble,
      ReadAsDateTime, ReadAsOid, ReadAsInt64);
  private class var
    FalseValue,
    TrueValue: TValue;
  ...
  private
    ...
    procedure InternalSetToken(ANewToken: TJsonToken; AUpdateIndex: Boolean);
  protected
    ...
    /// <param name='AValue'> The new value associated to the new token type </param>
    /// <param name='AUpdateIndex'> Indicates whether the index of the current TJsonPosition should be updated </param>
    procedure SetToken(ANewToken: TJsonToken; const AValue: TValue; AUpdateIndex: Boolean); overload; inline;
    /// <summary> Sets the current token </summary>
    /// <param name='ANewToken'> The new token type </param>
    /// <param name='AValue'> The new value associated to the new token type </param>
    /// <param name='AUpdateIndex'> Indicates whether the index of the current TJsonPosition should be updated </param>
    procedure SetToken<T>(ANewToken: TJsonToken; const AValue: T; AUpdateIndex: Boolean); overload; inline;
  public
    constructor Create;
...
begin
  ...
  case LTokenType of
    ...
    begin
      ...
      SetToken<TBytes>(TJsonToken.Bytes, Result, False);
      Exit(Result);
    end;
    TJsonToken.Null:
      Exit(Default(TBytes));
    ...
    begin
      ...
      Result := Oid.AsBytes;
      SetToken<TBytes>(TJsonToken.Oid, Result, False);
      Exit(Result);
    end;
    ...
    TJsonToken.String:
      begin
        ...
        begin
          ...
          Result := DateTime;
          SetToken<TDateTime>(TJsonToken.Date, DateTime, False);
        end;
      end;
    TJsonToken.Integer:
      begin
...
begin
  ...
  case LTokenType of
    ...
    TJsonToken.Float:
      begin
        ...
        if not (Value.TypeInfo^.Kind = TTypeKind.tkFloat) then
          SetToken<Double>(TJsonToken.Float, Result, False);
      end;
    TJsonToken.Null,
    TJsonToken.EndArray:
    ...
    TJsonToken.Decimal:
      begin
        Result := Value.AsType<TJsonDecimal128>.AsExtended;
        SetToken<Double>(TJsonToken.Float, Result, False);
      end;
    else
      Result := 0;
...
begin
  ...
  case LTokenType of
    ...
    TJsonToken.Float:
      begin
        ...
        if not (Value.TypeInfo^.Kind = TTypeKind.tkInteger) then
          SetToken<Integer>(TJsonToken.Integer, Result, False);
      end;
    TJsonToken.Null,
    TJsonToken.EndArray:
    ...
    TJsonToken.Decimal:
      begin
        Result := Trunc(Value.AsType<TJsonDecimal128>.AsExtended);
        SetToken<Integer>(TJsonToken.Integer, Result, False);
      end;
    else
      Result := 0;
...
begin
  FReadType := TReadType.ReadAsInt64;
    ...
    TJsonToken.Float:
      begin
        ...
        if not (Value.TypeInfo^.Kind = TTypeKind.tkInt64) then
          SetToken<Int64>(TJsonToken.Integer, Result, False);
      end;
    TJsonToken.Null,
    TJsonToken.EndArray:
      Result := Default(Integer);
...
begin
  ...
  case LTokenType of
    ...
    TJsonToken.EndArray:
      ...
      begin
        Result := Value.AsString;
        SetToken<string>(TJsonToken.String, Result, False);
      end
      else
        ErrorUnexp;
  end;
...
procedure TJsonReader.InternalSetToken(ANewToken: TJsonToken; AUpdateIndex: Boolean);
begin
  ...
  case ANewToken of
    ...
    begin
      ...
      FCurrentPosition.PropertyName := s;
    end;
...
begin
  FValue := AValue;
  InternalSetToken(ANewToken, True);
end;
...
begin
  FValue := AValue;
  InternalSetToken(ANewToken, AUpdateIndex);
end;

procedure TJsonReader.SetToken<T>(ANewToken: TJsonToken; const AValue: T; AUpdateIndex: Boolean);
begin
  TValue.Make<T>(AValue, FValue);
  InternalSetToken(ANewToken, AUpdateIndex);
end;
...
var
 ...
 function TryReadAsInt(const AValue: string): Boolean;
  ...
  begin
    ...
    Result := C = 0;
    if Result then
      SetToken<Integer>(TJsonToken.Integer, I, False)
    else
    begin
      Val(AValue, L, C);
      Result := C = 0;
      if Result then
        SetToken<Int64>(TJsonToken.Integer, L, False);
    end;
  end;
  ...
  var
    D: Extended;
  begin
    Result := TextToFloat(PChar(AValue), D, fvExtended, FFormatSettings);
    if Result then
      SetToken<Extended>(TJsonToken.Float, D, False);
  end;
...
begin
  ...
  case FReadType of
    ...
    TReadType.ReadAsDouble:
      ...
      if not ((StrScan(PChar(NumberStr), FFormatSettings.DecimalSeparator) = nil) and
              TryReadAsInt(NumberStr)) then
        if not TryReadAsFloat(NumberStr) then
          ErrorInvNum;
  end;
  ClearRecentString;
...
var
  ...
  procedure Error;
  begin
    raise EJsonReaderException.Create(Self, Format(SUnexpecteCharAfterValue, [LCurrentChar]));
  end;
...
begin
  ...
  SetToken<string>(TJsonToken.PropertyName, PropertyName, True);
  ...
  case FReadType of
    ...
    TReadType.ReadAsBytes:
      begin
        ...
        SetToken<TBytes>(TJsonToken.Bytes, Data, False);
      end;

    TReadType.ReadAsOid:
      begin
        Oid.AsString := LString;
        SetToken<TJsonOid>(TJsonToken.Oid, Oid, False);
      end;

    TReadType.ReadAsString:
      begin
        SetToken<string>(TJsonToken.String, LString, False);
        FQuoteChar := Quote;
      end;

  else
    if (FDateParseHandling = TJsonDateParseHandling.DateTime) and TryISO8601ToDate(LString, LDateTime, True) then
      SetToken<TDateTime>(TJsonToken.Date, LDateTime, False)
    else
    begin
      SetToken<string>(TJsonToken.String, LString, False);
      FQuoteChar := Quote;
...
begin
  ...
  begin
    ...
    SetToken<TJsonRegEx>(TJsonToken.RegEx, Re, False);
  end
  ...
  begin
    ...
    begin
      ...
      Ref.DB := '';
    SetToken<TJsonDBRef>(TJsonToken.DBRef, Ref, False);
  end
  ...
  begin
    ...
    SetToken<TJsonCodeWScope>(TJsonToken.CodeWScope, Cws, False);
  end
  ...
  begin
    ...
    ReadAsInt64
  else if LPropName = JsonExtDecimalPropertyName then
  begin
    Dec.AsString := ReadAsString;
    SetToken<TJsonDecimal128>(TJsonToken.Decimal, Dec, False);
  end
  else
    Result := False;
end;
...
begin
  ...
  if Len - FCharPos <= Len div 10 then
  begin
...
begin
  ...
  if FCurrent is TJSONNumber then
    if FReadType = TReadType.ReadAsInteger then
      SetToken(TJsonToken.Integer, TJSONNumber(FCurrent).AsInt)
    else if FReadType = TReadType.ReadAsInt64 then
      SetToken(TJsonToken.Integer, TJSONNumber(FCurrent).AsInt64)
    else
