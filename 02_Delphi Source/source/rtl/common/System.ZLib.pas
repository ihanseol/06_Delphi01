...
*                                                                            *
*  revision history                                                          *
*    2024.01.27  updated Zlib library version 1.3.1 and review               *
*                  header translation                                        *
*    2022.10.20  updated Zlib library version 1.2.13 and review              *
*                  header translation                                        *
...
const
  ZLIB_VERSION: MarshaledAString = '1.2.13'; // "1.3.1"
  ZLIB_VERNUM = $12D0; // 0x1310
  ZLIB_VER_MAJOR = 1;
  ZLIB_VER_MINOR = 2; // 3
  ZLIB_VER_REVSION = 13; // 1
...
{$IFDEF MSWINDOWS}
function inflateBack9Init_(var strm: z_stream;
  window: PByte; version: MarshaledAString;
  stream_size: Integer): Integer; cdecl;
  external;
function inflateBack9End(var strm: z_stream): Integer; cdecl;
  external;
function inflateBack9(var strm: z_stream;
  in_: in_func; in_desc: Pointer;
  out_: out_func; out_desc: Pointer): Integer; cdecl;
  external;
function inflateBack9Init(var strm: z_stream;
  window: PByte): Integer;
{$ENDIF MSWINDOWS}
...
type
  ...
  private
    ...
    FBuffer: TBytes;
    class procedure UpdateVal(prev, cur: LongWord; var val: Int64); static;
  protected
...
{$IFDEF MSWINDOWS}
  TZInflate9Stream = class(TCustomZStream)
  private
    FOwnsStream: Boolean;
    FWindow: TBytes;
...
function inflateBack9Init(var strm: z_stream;
  window: PByte): Integer;
begin
  Result := inflateBack9Init_(strm, window,
    ZLIB_VERSION, SizeOf(z_stream));
end;
{$ENDIF MSWINDOWS}
...
class procedure TCustomZStream.UpdateVal(prev, cur: LongWord; var val: Int64);
begin
  if cur < prev then
    val := val + Int64((High(LongWord) - prev) + cur)
  else
    val := val + Int64(cur - prev);
end;
...
var
  prevTotalIn: LongWord;
  prevTotalOut: LongWord;
begin
  ...
  begin
    prevTotalIn := FZStream.total_in;
    prevTotalOut := FZStream.total_out;
    try
      ZCompressCheckWithoutBufferError(deflate(FZStream, Z_NO_FLUSH));
    finally
      UpdateVal(prevTotalIn, FZStream.total_in, FTotalIn);
      UpdateVal(prevTotalOut, FZStream.total_out, FTotalOut);
    end;
...
begin
  if FTotalIn = 0 then result := 0
  else result := (1.0 - (FTotalOut / FTotalIn)) * 100.0;
end;
...
var
  ...
  localOffset: Int64;
begin
  if (NativeUInt(offset) < FTotalOut) and (Origin = soBeginning) then
  begin
  ...
  if ((Offset >= 0) and (Origin = soCurrent)) or
    (((NativeUInt(offset) >= FTotalOut)) and (Origin = soBeginning)) then
  begin
    localOffset := Offset;
    if (Origin = soBeginning) then Dec(localOffset, FTotalOut);
...
{$IFDEF MSWINDOWS}
function Input(in_desc: Pointer; var buf: PByte): Cardinal; cdecl;
var
  LCount: Longint;
begin
  with TZInflate9Stream(in_desc) do
  begin
    LCount := FStream.Read(FBuffer, Length(FBuffer));
    if LCount < 0 then
      LCount := 0;
    Result := LCount;
    buf := @FBuffer[0];
...
function Output(out_desc: Pointer; buf: PByte; length: Cardinal): Integer; cdecl;
begin
  with TZInflate9Stream(out_desc) do
  begin
    FUncompressedStream.Seek(0, soEnd);
    if FUncompressedStream.Write(buf^, length) = Integer(length) then
      Result := 0
    else
      Result := 1;
    DoProgress;
...
constructor TZInflate9Stream.Create(source: TStream);
begin
  Create(source, False);
end;

constructor TZInflate9Stream.Create(source: TStream; OwnsStream: Boolean);
begin
  inherited Create(source);
  SetLength(FWindow, $10000);
  FOwnsStream := OwnsStream;
  FUncompressedStream := TMemoryStream.Create;
  FZStream.next_in := @FBuffer[0];
  FZStream.avail_in := 0;
  ZDecompressCheckWithoutBufferError(inflateBack9Init(FZStream, @FWindow[0]));

  ZDecompressCheck(inflateBack9(FZStream, @Input, Self, @Output, Self));
  FUncompressedStream.Position := 0;
end;

destructor TZInflate9Stream.Destroy;
begin
  inflateBack9End(FZStream);
...
function TZInflate9Stream.GetSize: Int64;
begin
  Result := FUncompressedStream.Size;
end;

function TZInflate9Stream.Read(var buffer; count: Longint): Longint;
begin
  Result := FUncompressedStream.Read(buffer, count);
end;

function TZInflate9Stream.Write(const buffer; count: Longint): Longint;
begin
  raise EZDecompressionError.Create(SZInvalid);
end;

function TZInflate9Stream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
  Result := FUncompressedStream.Seek(Offset, Origin);
end;
{$ENDIF MSWINDOWS}
