...
{$IFDEF MSWINDOWS}
    ...
    FInitializeCOMFlags: DWORD;
{$ENDIF}
    FOnThreadTerminate: TProc<TThread>;
    FOnThreadStart: TProc<TThread>;
    ...
    FShutdown: Boolean;
    FMonitorThreadWakeEvent: TLightweightEvent;
    ...
    function GrowWorkerPool: Boolean;
    procedure IncIdleThreadCount; inline;
    procedure IncWorkRequestCount; inline;
    procedure SetMaxWorkerThreads2(const Value: Integer); inline;
    procedure SetMinWorkerThreads2(const Value: Integer); inline;
    ...
    procedure InitializeThread(AThread: TBaseWorkerThread);
    procedure FinalizeThread(AThread: TBaseWorkerThread);
  ...
  public
    ...
    ///  <summary>
    ///  Set maximum number of working threads in pool. Default value is ProcessorCount * 2.
    ///  Returns false if attempting to set a value less than or equal zero. Setting this
    ///  value too high can lead problems with other libraries and processes. If too many
    ///  threads execute at the same time, task/thread switching overhead can become restrictive.
    ///  <summary>
    function SetMaxWorkerThreads(Value: Integer): Boolean;
    ///  <summary>
    ///  Set minimum number of working threads in pool. Default value is Max(2, ProcessorCount div 4).
    ///  Returns false if attempting to set a value less than zero. The actual number of pool
    ///  threads could be less than this value depending on actual demand. Setting this to
    ///  too few threads could be less than optimal resource utilization.
    ///  <summary>
    function SetMinWorkerThreads(Value: Integer): Boolean;

    ///  <summary>
    ///  The maximum number of working threads in pool. Default value is ProcessorCount * 2.
    ///  <summary>
    property MaxWorkerThreads: Integer read FMaxLimitWorkerThreadCount write SetMaxWorkerThreads2;
    ///  <summary>
    ///  The minimum number of working threads in pool. Default value is Max(2, ProcessorCount div 4).
    ///  <summary>
    ...
    ///  <summary>
    ///  Specifies when tasks running by this pool threads will call TThread.Synchronize. Default value is False.
    ///  When Interactive is False and a task calls Synchronize, then deadlock happens. It is required to set
    ///  Interactive to True when a task calls Synchronize.
    ///  <summary>
    property Interactive: Boolean read FInteractive write FInteractive default False;
{$IFDEF MSWINDOWS}
    ///  <summary>
    ///  Specifies when tasks running by this pool threads will use Microsoft COM. Default value is False.
    ///  For example, MSXML, ADO components, TWICImage. It is required to set InitializeCOM to True when
    ///  a task will use COM services. This property is available only on Windows platform.
    ///  <summary>
    property InitializeCOM: Boolean read FInitializeCOM write FInitializeCOM default False;
    ///  <summary>
    ///  Specifies Microsoft COM initialization flags when InitializeCOM is True. Default value is
    ///  COINIT_MULTITHREADED. Corresponds to dwCoInit argument of CoInitializeEx. This property
    ///  is available only on Windows platform.
    ///  <summary>
    property InitializeCOMFlags: DWORD read FInitializeCOMFlags write FInitializeCOMFlags
      default COINIT_MULTITHREADED;
{$ENDIF}
    ///  <summary>
    ///  Specifies anonymous procedure, which will be called at start of this pool threads.
    ///  It allows to initialize thread. In contrast to InitializeCOM, it is platform independent.
    ///  <summary>
    property OnThreadStart: TProc<TThread> read FOnThreadStart write FOnThreadStart;
    ///  <summary>
    ///  Specifies anonymous procedure, which will be called at termination of this pool threads.
    ///  It allows to finalize thread. In contrast to InitializeCOM, it is platform independent.
    ///  <summary>
  ...
  protected
    ...
    function GetIsStartCanceled: Boolean;
...
uses
  System.SysConst, System.RTLConsts, System.Variants, System.Diagnostics, System.Math;
...
begin
  ...
  begin
    ...
    Completed := Task.Value.IsComplete or Task.Value.IsStartCanceled;
  ...
  begin
    ...
    try
      ...
      if InternalTasks[0].Value.FPool.Interactive and (TThread.Current.ThreadID = MainThreadID) then
      begin
        if Timeout < INFINITE then
          Timer := TStopwatch.StartNew;
        while (Event.WaitFor(0) = TWaitResult.wrTimeout) and ((Timeout = INFINITE) or (Timer.ElapsedMilliseconds < Timeout)) do
          CheckSynchronize(1);
        Result := Event.WaitFor(0) <> TWaitResult.wrTimeout;
      end
      else
...
begin
  ...
  begin
    ...
    if Task.Value = nil then
      raise EArgumentNilException.CreateRes(@sWaitNilTask);
    if (Result = -1) and (Task.Value.IsComplete or Task.Value.IsStartCanceled) then
      Result := I;
  end;
  if Result >= 0 then
    Tasks[Result].Wait(0)
  ...
  begin
    ...
    try
      ...
      Task.Value := TTask(Tasks[0]);
      if Task.Value.FPool.Interactive and (TThread.Current.ThreadID = MainThreadID) then
      begin
        if Timeout < INFINITE then
          Timer := TStopwatch.StartNew;
        while (Event.WaitFor(0) = TWaitResult.wrTimeout) and ((Timeout = INFINITE) or (Timer.ElapsedMilliseconds < Timeout)) do
          CheckSynchronize(1);
        WaitRes := Event.WaitFor(0);
      end
      else
...
function TTask.GetIsStartCanceled: Boolean;
var
  LFlags: TOptionStateFlags;
begin
  LFlags := FState * [TOptionStateFlag.Started, TOptionStateFlag.CallbackRun, TOptionStateFlag.Canceled, TOptionStateFlag.Faulted, TOptionStateFlag.Complete];
  Result := (LFlags = [TOptionStateFlag.Canceled, TOptionStateFlag.Started]) or (LFlags = [TOptionStateFlag.Canceled]);
end;
...
begin
  ...
  begin
    ...
    try
      EventsObj := TCompleteEventsWrapper(FCompleteEvents).FCompleteEvents;
      if (EventsObj is TCompleteEvents) and not IsComplete then
      begin
...
begin
  ...
  // only way for List not to be freed above, after moving the Free inside the
  // lock, is if TCompleteEventsWrapper(FCompleteEvents).Lock raises an exception,
  // in which case a memory leak is the least of the problems...
end;
...
var
  Timer: TStopwatch;
begin
  Result := IsComplete or IsStartCanceled;
  if not Result then
    if FPool.Interactive and (TThread.Current.ThreadID = MainThreadID) then
    begin
      if Timeout < INFINITE then
        Timer := TStopwatch.StartNew;
      while not IsComplete and ((Timeout = INFINITE) or (Timer.ElapsedMilliseconds < Timeout)) do
        CheckSynchronize(1);
      Result := IsComplete;
    end
    else
...
{$IFDEF MSWINDOWS}
  FInitializeCOMFlags := COINIT_MULTITHREADED;
{$ENDIF}
  FMonitorThreadWakeEvent := TLightweightEvent.Create;
end;
...
begin
  ...
  begin
    ...
    TThreadPool.FObjectCaches.Free;
{$IFDEF AUTOREFCOUNT}
...
procedure TThreadPool.SetMaxWorkerThreads2(const Value: Integer);
begin
...
procedure TThreadPool.SetMinWorkerThreads2(const Value: Integer);
begin
...
function TThreadPool.GrowWorkerPool: Boolean;
begin
  Result := False;
...
begin
  ...
  if FMonitorThreadStatus = [] then
    CreateMonitorThread;
  if not LHasWorkers and UnlimitedWorkerThreadsWhenBlocked then
    FMonitorThreadWakeEvent.SetEvent;
end;
...
procedure TThreadPool.InitializeThread(AThread: TBaseWorkerThread);
begin
  try
    if Assigned(OnThreadStart) then
      OnThreadStart(AThread);
{$IFDEF MSWINDOWS}
    if InitializeCOM then
      CoInitializeEx(nil, InitializeCOMFlags);
{$ENDIF}
  except
...
procedure TThreadPool.FinalizeThread(AThread: TBaseWorkerThread);
begin
  try
{$IFDEF MSWINDOWS}
    if InitializeCOM then
...
begin
  ThreadPool.InitializeThread(Self);
  ...
  try
    ...
    ThreadPool.FinalizeThread(Self);
  end;
end;
...
begin
  Lock;
  try
    for Result := 0 to Length(FArray) - 1 do
      if FArray[Result] = nil then
      begin
        FArray[Result] := Item;
        Exit;
      end;
    SetLength(FArray, GrowCollection(Result, Result + 1));
    FArray[Result] := Item;
  finally
    Unlock;
  end;
...
{ TParallel.TLoopState.TLoopStateFlag }
...
var
  ...
  i: Integer;
begin
  if FThreadPool.FQueuedRequestCount > 0 then
  begin
    WakeRetiredThread := False;
    TMonitor.Enter(FThreadPool.FQueue);
    try
      if (FThreadPool.FQueuedRequestCount >= FThreadPool.FLastQueuedRequestCount) and
         IsThrottledDelay(FThreadPool.FLastThreadCreationTick, FThreadPool.FWorkerThreadCount) and
         ((FThreadPool.FWorkerThreadCount < FThreadPool.FMaxLimitWorkerThreadCount) or
          FThreadPool.UnlimitedWorkerThreadsWhenBlocked and (FThreadPool.FIdleWorkerThreadCount = 0)) then
      begin
        ...
        else if FThreadPool.FWorkerThreadCount < FThreadPool.FMaxLimitWorkerThreadCount then
