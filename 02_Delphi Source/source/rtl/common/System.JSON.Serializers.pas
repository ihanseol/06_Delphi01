...
type
  ...
  TAttrKey = TPair<TRttiObject, TCustomAttributeClass>;
  ...
  private
    ...
    function GetRttiField(const ARttiType: TRttiType; const AFieldName: string): TRttiMember;
    function GetRttiProperty(const ARttiType: TRttiType; const APropertyName: string): TRttiMember;
  protected
...
begin
  ...
  FCachedAttributeConverters := TObjectDictionary<TJsonConverterClass, TJsonConverter>.Create([doOwnsValues]);
  ...
  FGUIDConverter := TJsonGUIDConverter.Create;
end;
...
begin
  ...
  case ATypeInf^.Kind of
    ...
    begin
      ...
      else if (ATypeInf = TypeInfo(System.TGUID)) and (JSONSerializationVersion >= 36) then
        Result.Converter := FGUIDConverter;
    end;
  end;
...
var
  LConverterClass: TJsonConverterClass;
begin
...
function TJsonSerializer.Deserialize(const AReader: TJsonReader;
  ...
  case AContract.ContractType of
    ...
    begin
      ...
      try
        ...
        if ExistingValue.IsEmpty then
          LObjectContract.DefaultCreator.Release(AValue)
        else
          AValue := TValue.Empty;
        raise;
      end;
  ...
  case AContract.ContractType of
    ...
    begin
      ...
      try
        ...
        if TJsonArrayContract(AContract).ItemContract.ContractType = TJsonContractType.Object then
        begin
          LObjectContract := TJsonObjectContract(TJsonArrayContract(AContract).ItemContract);
          for I := Low(LArray) to High(LArray) do
            LObjectContract.DefaultCreator.Release(LArray[I]);
        end;
        raise;
      end;
...
begin
  ...
  case TypeToRead(AContract) of
    ...
    TJsonReader.TReadType.ReadAsInt64:
      AReader.ReadAsInt64;
...
begin
  ...
  if Length(Params) > 0 then
    FParameterizedConstructor.Invoke(TValue.From<Pointer>(Result.GetReferenceToRawData), Params);
end;
...
begin
  ...
  for Attr in ARttiObject.GetAttributes do
    FAttributes.AddOrSetValue(TAttrKey.Create(ARttiObject, TCustomAttributeClass(Attr.ClassType)), Attr);
end;

class function TJsonInlineAttributes.GetAttribute(const ARttiObject: TRttiObject; const AAttributeClass: TCustomAttributeClass; AInherit: Boolean): TCustomAttribute;
begin
...
begin
  FAttributes.AddOrSetValue(TAttrKey.Create(ARttiObject, TCustomAttributeClass(AAttribute.ClassType)), AAttribute);
end;
...
begin
  inherited Create;
  FAttributes := TObjectDictionary<TAttrKey, TCustomAttribute>.Create([doOwnsValues]);
end;
...
function TJsonInlineAttributeProvider.GetAttribute(const AAttributeClass: TCustomAttributeClass; AInherit: Boolean): TCustomAttribute;
begin
...
function TJsonDynamicContractResolver.GetRttiField(const ARttiType: TRttiType; const AFieldName: string): TRttiMember;
begin
...
function TJsonDynamicContractResolver.GetRttiProperty(const ARttiType: TRttiType; const APropertyName: string): TRttiMember;
begin
...
begin
  FDynamicAttributes.AddAttribute(GetRttiField(GetRttiType(ATypeInf), AFieldName), JsonConverterAttribute.Create(AConverterClass));
end;

procedure TJsonDynamicContractResolver.SetFieldName(ATypeInf: PTypeInfo; const AFieldName, AResolvedName: string);
begin
  FDynamicAttributes.AddAttribute(GetRttiField(GetRttiType(ATypeInf), AFieldName), JsonNameAttribute.Create(AResolvedName));
end;
...
procedure TJsonDynamicContractResolver.SetPropertyName(ATypeInf: PTypeInfo; const APropertyName, AResolvedName: string);
begin
...
initialization
  ...
  LRegistered := TJSONMappers.RegisterLibrary('System.JSON.Serializers',
    ...
    begin
      AIntro := '[JsonSerialize(TJsonMemberSerialization.' + CSerItems[ASerItems] + ')]';
      AReqUnit := 'System.JSON.Serializers';
    end,
    ...
    procedure (const AItemType: string; var AIntro, AReqUnit, AInstantiation: string)
    begin
      var LConv := StringReplace(AItemType, '<', '', [TReplaceFlag.rfReplaceAll]);
      LConv := 'TJsonListConverter' + StringReplace(LConv, '>', '', [TReplaceFlag.rfReplaceAll]);
      AIntro := '[JsonConverter(' + LConv + ')]';
      AReqUnit := 'System.JSON.Converters';
      AInstantiation := LConv + ' = class(TJsonTypedListConverter<' + AItemType + '>)';
    end,
...
finalization
  if LRegistered then
    TJSONMappers.UnRegisterLibrary('System.JSON.Serializers');
