...
type
  ...
  /// <summary> .
  /// * ioNoTZIsLocal - is used when a date/time string does not have a timezone.
  ///   When ioNoTZIsLocal is specified, then the time is considering as local,
  ///   otherwise as UTC.
  /// * ioReturnUTC - when specified, then returning date/time is UTC, otherwise
  ///   local.
  /// </summary>
  TISO8601ToDateOption = (ioReturnUTC, ioNoTZIsLocal);
  TISO8601ToDateOptions = set of TISO8601ToDateOption;
...
/// </example>
function ISO8601ToDate(const AISODate: string; AReturnUTC: Boolean = True): TDateTime; overload;
function ISO8601ToDate(const AISODate: string; AOptions: TISO8601ToDateOptions): TDateTime; overload;
...
function TryISO8601ToDate(const AISODate: string; out Value: TDateTime; AReturnUTC: Boolean = True): Boolean; overload;
function TryISO8601ToDate(const AISODate: string; out Value: TDateTime; AOptions: TISO8601ToDateOptions): Boolean; overload;
...
begin
  ...
  LHasDateSep := DateString.IndexOf(SDateSeparator, Integer(LNegativeDate)) > 0;
...
begin
  ...
  begin
    ...
    begin
      PStart := P;
      LFractionalSecond := GetNextDTComp(P, PE, 0, P^, True, False, InvTime, TimeString, 10);
...
begin
  ...
  if not AHasTZ then
  begin
    { If we have no explicitly specified time zone }
    if ioNoTZIsLocal in AOptions then
    begin
      if ioReturnUTC in AOptions then
        Result := TTimeZone.Local.ToUniversalTime(Result);
    end
    else
    begin
      if not (ioReturnUTC in AOptions) then
        Result := TTimeZone.Local.ToLocalTime(Result);
    end;
  end
  else if ioReturnUTC in AOptions then
  begin
...
function ISO8601ToDate(const AISODate: string; AOptions: TISO8601ToDateOptions): TDateTime;
const
  STimePrefix = Char('T');
var
  ...
  HasTZ, AddDay, AddMinute, AddSecond: Boolean;
begin
  HourOffset := 0;
  MinuteOffset := 0;
  ...
  begin
    ...
    DecodeISO8601Time(AISODate, TimePosition + 2, Length(AISODate) - TimePosition - 1,
      Hour, Minute, Second, Millisecond, HourOffset, MinuteOffset, HasTZ);
  end
  else
  begin
    Hour := 0;
  ...
  Result := AdjustDateTime(Result, HasTZ, HourOffset, MinuteOffset, AOptions);
end;

function TryISO8601ToDate(const AISODate: string; out Value: TDateTime; AOptions: TISO8601ToDateOptions): Boolean;
begin
  try
    Value := ISO8601ToDate(AISODate, AOptions);
    Result := True;
  except
    Result := False;
  end;
...
const
  CISO8601ToDateOptions: array[Boolean] of TISO8601ToDateOptions = ([], [ioReturnUTC]);

function ISO8601ToDate(const AISODate: string; AReturnUTC: Boolean = True): TDateTime;
begin
  Result := ISO8601ToDate(AISODate, CISO8601ToDateOptions[AReturnUTC]);
end;

function TryISO8601ToDate(const AISODate: string; out Value: TDateTime; AReturnUTC: Boolean = True): Boolean;
begin
  Result := TryISO8601ToDate(AISODate, Value, CISO8601ToDateOptions[AReturnUTC]);
end;
...
var
  ...
  begin
    ...
    begin
      ...
      for I := 1 to 12 do
        if SameText(LToken, InvariantFormat.ShortMonthNames[I]) then
        begin
          LMonth := I;
          LFoundMonth := True;
          Result := True;
...
begin
  Self := RecodeDate(Self, AYear, AMonth, ADay);
end;

procedure TDateTimeHelper.SetDay(ADay: Word);
begin
  Self := RecodeDay(Self, ADay);
end;

procedure TDateTimeHelper.SetHour(AHour: Word);
begin
  Self := RecodeHour(Self, AHour);
end;
...
begin
  Self := RecodeMilliSecond(Self, AMilliSecond);
end;

procedure TDateTimeHelper.SetMinute(AMinute: Word);
begin
  Self := RecodeMinute(Self, AMinute);
end;
