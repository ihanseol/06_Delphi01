...
type
  ...
  private
    ...
    FStringCache: TDictionary<string, string>;
  ...
  public
    ...
    constructor Create(const Data: PByte; const Offset: Integer; const Range: Integer;
      const IsUTF8: Boolean; const IsANSI: Boolean = False); overload;
  ...
  public type
    ...
    /// <summary>
    ///   JSON Parser Options
    ///     IsUTF8 - data should be treated as UTF8 encoded
    ///     UseBool - Parser should create a TJSONBool object for each instance of "true" or "false" seen in the JSON data
    ///     RaiseExc - raise exception on parse failure instead of returning nil from ParseJSONValue
    ///     IsANSI - data should be treated as ANSI encoded. This is useful for application internal JSON exchange.
    /// </summary>
...
{$ENDIF !NEXTGEN}
  ...
  protected
    ...
    /// <summary> Returns the unsigned integer part of the number </summary>
    ...
    /// <summary> Returns the UInt64 part of the number </summary>
  ...
  public
    ...
    /// <summary> Constructor for a extended number </summary>
    /// <param name="value">extended to be represented as JSONNumber</param>
    constructor Create(const Value: Extended); overload;

    /// <summary> Constructor for a currency number </summary>
    /// <param name="value">currency to be represented as JSONNumber</param>
    ...
    /// <summary> Constructor for unsigned integer </summary>
    /// <param name="value">unsigned integer to be represented as JSONNumber</param>
    constructor Create(const Value: Cardinal); overload;

    /// <summary> Constructor for Int64 </summary>
    /// <param name="value">Int64 to be represented as JSONNumber</param>
    constructor Create(const Value: Int64); overload;

    /// <summary> Constructor for UInt64 </summary>
    /// <param name="value">UInt64 to be represented as JSONNumber</param>
    ...
    /// <summary> Returns the ubsigned integer part of the number </summary>
    /// <returns>Cardinal</returns>
    ...
    /// <summary> Returns the number as an UInt64 </summary>
    /// <returns>UInt64</returns>
  ...
  public
    ...
    /// <summary> Convenience method for adding a pair to current object. </summary>
    /// <param name="Str">- string: pair name</param>
    /// <param name="Val">- UInt64: pair value</param>
    ...
    /// <summary> Convenience method for adding a pair to current object. </summary>
    /// <param name="Str">- string: pair name</param>
    /// <param name="Val">- Cardinal: pair value</param>
    ...
    /// <summary> Convenience method for adding a pair to current object. </summary>
    /// <param name="Str">- string: pair name</param>
    /// <param name="Val">- Extended: pair value</param>
    function AddPair(const Str: string; const Val: Extended): TJSONObject; overload;

    /// <summary> Convenience method for adding a pair to current object. </summary>
    /// <param name="Str">- string: pair name</param>
    /// <param name="Val">- Currency: pair value</param>
  ...
  public type
    ...
    TTypeStartedProc = reference to procedure (const AName: string; const ASerItems: TItem;
      var AIntro, AReqUnit: string);
    TElemStartedProc = reference to procedure (const AName, AOrigName: string; const AElem: TItem;
      var AIntro, AReqUnit: string);
    TListStartedProc = reference to procedure (const AItemType: string;
      var AIntro, AReqUnit, AInstantiation: string);
    ...
    public
      FName: string;
      FMapArrayToList: TOptionality;
      FMapEntityToRecord: TOptionality;
      FMapElemToFields: TOptionality;
      FOnTypeStarted: TTypeStartedProc;
      FOnElemStarted: TElemStartedProc;
      FOnListStarted: TListStartedProc;
      FToObject: TToObjectFunc;
      FFromObject: TFromObjectFunc;
    end;
  private
    class var FLibraries: TObjectDictionary<string, TLibrary>;
    class var FDefLibraryName: string;
    class constructor Create;
    class destructor Destroy;
  public
    class function RegisterLibrary(
      const AName: string;
      const AMapArrayToList: TOptionality;
      const AMapEntityToRecord: TOptionality;
      const AMapElemToFields: TOptionality;
      const AOnTypeStarted: TTypeStartedProc;
      const AOnElemStarted: TElemStartedProc;
      const AOnListStarted: TListStartedProc;
      const AToObject: TToObjectFunc;
      const AFromObject: TFromObjectFunc
    ): Boolean;
    class procedure UnRegisterLibrary(const AName: string); static;
    class function LibraryByName(const AName: string): TLibrary;
    class function GetLibraries: TArray<string>;
    class procedure SetDefaultLibrary(const ALibraryName: string);
  end;

  TJSONMapper<T> = class(TJSONMappers)
  private type
    TEnumerator = class(TInterfacedObject, IEnumerator, IEnumerator<T>,
      IEnumerable<T>)
    private
      FMarshaller: TJSONMapper<T>;
      FArray: TJSONArray;
      FCurrentItem: T;
      FArrayOwned,
      FCurrentOwned: Boolean;
      FEnum: TJSONArray.TEnumerator;
    ...
    public
      constructor Create(AMarshaller: TJSONMapper<T>;
        AArray: TJSONArray; AArrayOwned, ACurrentOwned: Boolean);
      destructor Destroy; override;
    end;

  private
    class var FDefault: TJSONMapper<T>;
    FLibrary: TJSONMappers.TLibrary;
    class function GetDefault: TJSONMapper<T>; static;
    class destructor Destroy;
  public
  ...
  function CurrencyToJson(const Value: Currency): string;
  function JsonToFloat(const DotValue: string): Double;
  function JsonToCurrency(const DotValue: string): Currency;
...
{$IF SizeOf(Extended) < 10}
  // Values out of this range with Precision=15 are rounded up to "1.79769313486232E308".
  // Then StrToFloat will raise exception.
  if (Value < 1.7976931348623152E308) and (Value > -1.7976931348623152E308) then
    P := 15
  else
{$ENDIF}
    P := 17;
  L := FloatToText(Buffer, Value, fvExtended, ffGeneral, P, 0, JSONFormatSettings);
  Buffer[L] := #0;
...
var
  ...
  L: Integer;
begin
  L := FloatToText(Buffer, Value, fvCurrency, ffGeneral, 19, 0, JSONFormatSettings);
  Buffer[L] := #0;
...
function JsonToCurrency(const DotValue: string): Currency;
begin
  Result := StrToCurr(DotValue, JSONFormatSettings);
end;
...
begin
  ...
  FStringCache := TDictionary<string, string>.Create();
...
constructor TJSONByteReader.Create(const Data: PByte; const Offset: Integer;
  const Range: Integer; const IsUTF8, IsANSI: Boolean);
begin
  inherited Create;
  Init(Data, Offset, Range);
  FIsUTF8 := IsUTF8 and not IsANSI;
  FIsANSI := IsANSI and not IsUTF8;
  if IsUTF8 then
    ConsumeBOM;
end;
...
begin
  ...
  begin
    var LCached: string;
    if FStringCache.TryGetValue(ADest, LCached) then
      ADest := LCached
    else
...
begin
  ...
  Result := nil;
  LExc := False;
  Br := TJSONByteReader.Create(Data, Offset, ALength,
    TJSONParseOption.IsUTF8 in Options, TJSONParseOption.IsANSI in Options);
  try
    try
      Br.SkipWhitespaces;
...
constructor TJSONNumber.Create(const Value: Extended);
begin
  inherited Create(FloatToJson(Value));
end;

constructor TJSONNumber.Create(const Value: Currency);
begin
  inherited Create(CurrencyToJson(Value));
end;
...
constructor TJSONNumber.Create(const Value: UInt64);
begin
  inherited Create(UIntToStr(Value));
end;
...
constructor TJSONNumber.Create(const Value: Cardinal);
begin
  inherited Create(UIntToStr(Value));
end;
...
constructor TJSONPair.Create(const Str: string; const Value: UInt64);
begin
  Create(TJSONString.Create(Str), TJSONNumber.Create(Value));
end;
...
constructor TJSONPair.Create(const Str: string; const Value: Cardinal);
begin
  Create(TJSONString.Create(Str), TJSONNumber.Create(Value));
end;
...
constructor TJSONPair.Create(const Str: string; const Value: Extended);
begin
  Create(TJSONString.Create(Str), TJSONNumber.Create(Value));
end;

constructor TJSONPair.Create(const Str: string; const Value: Currency);
begin
  Create(TJSONString.Create(Str), TJSONNumber.Create(Value));
end;
...
begin
  if (Data = nil) or (Pos < 0) or (Pos >= Count) then
    Exit(-1);
  Br := TJSONByteReader.Create(PByte(Data), Pos, Count, True, False);
  try
...
begin
  Result := FIndex < FObject.FMembers.Count - 1;
  if Result then
    Inc(FIndex);
end;
...
begin
  Result := FIndex < FArray.FElements.Count - 1;
  if Result then
    Inc(FIndex);
end;
...
class constructor TJSONMappers.Create;
begin
  FLibraries := TObjectDictionary<string, TLibrary>.Create([doOwnsValues],
    TIStringComparer.Ordinal);
end;

class destructor TJSONMappers.Destroy;
begin
  FreeAndNil(FLibraries);
end;

class function TJSONMappers.RegisterLibrary(const AName: string;
  const AMapArrayToList: TOptionality;
  const AMapEntityToRecord: TOptionality;
  const AMapElemToFields: TOptionality;
  const AOnTypeStarted: TTypeStartedProc;
  ...
  const AOnListStarted: TListStartedProc;
  const AToObject: TToObjectFunc;
  const AFromObject: TFromObjectFunc): Boolean;
var
  LLib: TLibrary;
begin
  TMonitor.Enter(FLibraries);
  try
    if FLibraries.ContainsKey(AName) then
      Exit(False);
    LLib := TLibrary.Create;
    LLib.FName := AName;
    LLib.FMapArrayToList := AMapArrayToList;
    LLib.FMapEntityToRecord := AMapEntityToRecord;
    LLib.FMapElemToFields := AMapElemToFields;
    LLib.FOnTypeStarted := AOnTypeStarted;
    LLib.FOnElemStarted := AOnElemStarted;
    LLib.FOnListStarted := AOnListStarted;
    LLib.FToObject := AToObject;
    LLib.FFromObject := AFromObject;
    FLibraries.Add(AName, LLib);
  finally
    TMonitor.Exit(FLibraries);
  end;
  Result := True;
end;

class procedure TJSONMappers.UnRegisterLibrary(const AName: string);
begin
  if FLibraries = nil then
    Exit;
  TMonitor.Enter(FLibraries);
  try
    FLibraries.Remove(AName);
  finally
    TMonitor.Exit(FLibraries);
  end;
end;

class function TJSONMappers.LibraryByName(const AName: string): TLibrary;
begin
  TMonitor.Enter(FLibraries);
  try
    if AName = '' then
      if FLibraries.Count = 1 then
        Result := FLibraries.ToArray[0].Value
      else
        raise EJSONException.CreateRes(@SAmbigiousJSONSerializer)
    else if not FLibraries.TryGetValue(AName, Result) then
      raise EJSONException.CreateResFmt(@SUnknownJSONSerializer, [AName]);
  finally
    TMonitor.Exit(FLibraries);
  end;
end;

class function TJSONMappers.GetLibraries: TArray<string>;
var
  LLib: TLibrary;
  i: Integer;
begin
  TMonitor.Enter(FLibraries);
  try
    SetLength(Result, FLibraries.Count);
    i := 0;
    for LLib in FLibraries.Values do
    begin
      Result[i] := LLib.FName;
      Inc(i);
    end;
...
class procedure TJSONMappers.SetDefaultLibrary(const ALibraryName: string);
begin
  FDefLibraryName := ALibraryName;
end;

{ TJSONMapper<T> }

class function TJSONMapper<T>.GetDefault: TJSONMapper<T>;
var
  LMapper: TJSONMapper<T>;
begin
  if FDefault = nil then
  begin
    LMapper := TJSONMapper<T>.Create(FDefLibraryName);
    if AtomicCmpExchange(Pointer(FDefault), Pointer(LMapper), nil) <> nil then
      LMapper.Free
{$IFDEF AUTOREFCOUNT}
    else
      FDefault.__ObjAddRef
{$ENDIF AUTOREFCOUNT};
  end;
  Result := FDefault;
end;

class destructor TJSONMapper<T>.Destroy;
begin
  FreeAndNil(FDefault);
end;

constructor TJSONMapper<T>.Create(const ALibraryName: string);
var
  LInfo: PTypeInfo;
begin
  inherited Create;
  FLibrary := LibraryByName(ALibraryName);
  LInfo := TypeInfo(T);
  if not (LInfo^.Kind in [TTypeKind.tkClass, TTypeKind.tkRecord, TTypeKind.tkMRecord]) or
     (FLibrary.FMapEntityToRecord = TOptionality.No) and
       (LInfo^.Kind <> TTypeKind.tkClass) or
     (FLibrary.FMapEntityToRecord = TOptionality.Yes) and
       not (LInfo^.Kind in [TTypeKind.tkRecord, TTypeKind.tkMRecord]) then
    raise EJSONException.CreateResFmt(@STypeNotSupByJSONSerializer,
      [GetTypeName(LInfo), FLibrary.FName]);
end;

function TJSONMapper<T>.ToObject(const AValue: T): TJSONValue;
begin
  if (GetTypeKind(T) = tkClass) and (PPointer(@AValue)^ = nil) then
    Result := TJSONNull.Create
  else
    Result := FLibrary.FToObject(@AValue, TypeInfo(T));
end;

function TJSONMapper<T>.ToString(const AValue: T; AFormatIndent: Integer): string;
var
  LObj: TJSONValue;
begin
  LObj := ToObject(AValue);
  try
    if AFormatIndent > 0 then
      Result := LObj.Format(AFormatIndent)
    else
      Result := LObj.ToJSON;
  finally
    LObj.Free;
...
function TJSONMapper<T>.ToArray(const AValue: TArray<T>; AMaxCount: NativeInt): TJSONArray;
var
  LVal: T;
  i: NativeInt;
begin
  ...
  try
    i := 0;
    for LVal in AValue do
    begin
      if NativeUInt(i) >= NativeUInt(AMaxCount) then
        Break;
      Result.AddElement(ToObject(LVal));
      Inc(i);
    end;
  except
    Result.Free;
...
function TJSONMapper<T>.ToString(const AValue: TArray<T>; AMaxCount: NativeInt;
  AFormatIndent: Integer): string;
var
  LArr: TJSONArray;
begin
  LArr := ToArray(AValue, AMaxCount);
  try
    if AFormatIndent > 0 then
      Result := LArr.Format(AFormatIndent)
    else
      Result := LArr.ToJSON;
  finally
    LArr.Free;
...
function TJSONMapper<T>.FromObject(const AValue: TJSONValue): T;
begin
  if AValue is TJSONNull then
    Result := System.Default(T)
  else
    Result := FLibrary.FFromObject(AValue as TJSONObject, TypeInfo(T)).AsType<T>;
end;

function TJSONMapper<T>.FromObject(const AValue: string): T;
var
  LObj: TJSONValue;
begin
  LObj := TJSONValue.ParseJSONValue(AValue, True, True);
  try
    Result := FromObject(LObj);
  finally
    LObj.Free;
  end;
end;

function TJSONMapper<T>.FromArray(const AValue: TJSONArray): TArray<T>;
var
  i: Integer;
begin
  ...
  for i := 0 to AValue.Count - 1 do
    Result[i] := FromObject(AValue.Items[i]);
end;

function TJSONMapper<T>.FromArray(const AValue: string): TArray<T>;
var
  LArr: TJSONValue;
begin
  LArr := TJSONValue.ParseJSONValue(AValue, True, True);
  try
    if not (LArr is TJSONArray) then
      LArr := TJSONArray.Create(LArr);
    Result := FromArray(LArr as TJSONArray);
  finally
    LArr.Free;
  end;
end;

function TJSONMapper<T>.GetEnumerator(const AValue: TJSONArray;
  AArrayOwned, ACurrentOwned: Boolean): IEnumerator<T>;
begin
  Result := TEnumerator.Create(Self, AValue, AArrayOwned, ACurrentOwned);
end;

function TJSONMapper<T>.GetEnumerator(const AValue: string; ACurrentOwned: Boolean): IEnumerator<T>;
var
  LVal: TJSONValue;
begin
  LVal := TJSONValue.ParseJSONValue(AValue, True, True);
  try
    if not (LVal is TJSONArray) then
      LVal := TJSONArray.Create(LVal);
    Result := GetEnumerator(LVal as TJSONArray, True, ACurrentOwned);
  finally
    LVal.Free;
  end;
end;

{ TJSONMapper<T>.TEnumerator }

constructor TJSONMapper<T>.TEnumerator.Create(AMarshaller: TJSONMapper<T>;
  AArray: TJSONArray; AArrayOwned, ACurrentOwned: Boolean);
begin
  inherited Create;
  FMarshaller := AMarshaller;
  FArray := AArray;
  FArrayOwned := AArrayOwned;
  FCurrentOwned := ACurrentOwned;
  Reset;
end;

destructor TJSONMapper<T>.TEnumerator.Destroy;
begin
  ReleaseCurrent;
  FreeAndNil(FEnum);
  if FArrayOwned then
...
procedure TJSONMapper<T>.TEnumerator.ReleaseCurrent;
begin
  if FCurrentOwned and (GetTypeKind(T) = tkClass) then
    FreeAndNil(PObject(@FCurrentItem)^);
end;

procedure TJSONMapper<T>.TEnumerator.Reset;
begin
  FreeAndNil(FEnum);
  FEnum := FArray.GetEnumerator;
end;

function TJSONMapper<T>.TEnumerator.MoveNext: Boolean;
begin
  ReleaseCurrent;
  Result := FEnum.MoveNext;
end;

function TJSONMapper<T>.TEnumerator.GetCurrent: TObject;
begin
  Result := nil;
end;

function TJSONMapper<T>.TEnumerator.GetEnumerator: IEnumerator;
begin
  Result := nil;
end;

function TJSONMapper<T>.TEnumerator.GetCurrentT: T;
begin
  ReleaseCurrent;
  FCurrentItem := FMarshaller.FromObject(FEnum.Current as TJSONObject);
  Result := FCurrentItem;
end;

function TJSONMapper<T>.TEnumerator.GetEnumeratorT: IEnumerator<T>;
begin
  Result := Self;
end;
