...
// Utility method for loading a framework
function LoadFramework(const Fwk: string): THandle;
...
{$ENDIF AUTOREFCOUNT};
  ...
  try
    if not FrameworkDic.TryGetValue(Fwk, Result) then
    begin
      if Fwk = '' then
      ...
      Result := LoadLibrary(FrameworkNamePtr);
      FrameworkDic.Add(Fwk, Result);
    end;
  finally
...
begin
  Result := nil;
  FrameworkHandle := LoadFramework(Fwk);
