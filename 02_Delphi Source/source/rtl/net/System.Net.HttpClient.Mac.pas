...
function kCFNetworkProxiesValue(const AName: string): NSString;
begin
  Result := CocoaNSStringConst(NetworkLib, 'kCFNetworkProxies' + AName);
...
begin
  Result := kCFNetworkProxiesValue('HTTPEnable');
end;

function kCFNetworkProxiesHTTPProxy: NSString;
begin
  Result := kCFNetworkProxiesValue('HTTPProxy');
end;

function kCFNetworkProxiesHTTPPort: NSString;
begin
  Result := kCFNetworkProxiesValue('HTTPPort');
end;

function kCFNetworkProxiesHTTPSEnable: NSString;
begin
  Result := kCFNetworkProxiesValue('HTTPSEnable');
end;

function kCFNetworkProxiesHTTPSProxy: NSString;
begin
  Result := kCFNetworkProxiesValue('HTTPSProxy');
end;

function kCFNetworkProxiesHTTPSPort: NSString;
begin
  Result := kCFNetworkProxiesValue('HTTPSPort');
end;

function kCFNetworkProxiesSOCKSEnable: NSString;
begin
  Result := kCFNetworkProxiesValue('SOCKSEnable');
end;

function kCFNetworkProxiesSOCKSProxy: NSString;
begin
  Result := kCFNetworkProxiesValue('SOCKSProxy');
end;

function kCFNetworkProxiesSOCKSPort: NSString;
begin
  Result := kCFNetworkProxiesValue('SOCKSPort');
end;
...
begin
  ...
  begin
    ...
    if ProxySettings.Scheme.StartsWith('socks', True) then
    begin
      LDict.setValue(TNSNumber.OCClass.numberWithInt(1), kCFNetworkProxiesSOCKSEnable);
      LDict.setValue(StringToID(ProxySettings.Host), kCFNetworkProxiesSOCKSProxy);
      LDict.setValue(TNSNumber.OCClass.numberWithInt(ProxySettings.Port), kCFNetworkProxiesSOCKSPort);
    end;
    ...
    begin
      ...
      if LAllCookies.count > 0 then
        ...
        begin
          ...
          begin
            DecodeDate(LCookie.Expires, LYear, LMonth, LDayOfMonth);
            // See comment in System.DateUtils.HttpToDate
            if not ((LYear = 9999) and (LMonth = 12) and (LDayOfMonth = 31)) then
              LCookie.Expires := TTimeZone.Local.ToLocalTime(LCookie.Expires);
          end;
...
begin
  ...
  try
    ...
    LResponse.FRequest.DoReceiveDataProgress(LResponse.GetStatusCode, LResponse.GetContentLength,
      LResponse.FReaded, didReceiveData.bytes, didReceiveData.length, LAbort);
    if LAbort then
      dataTask.cancel;
  except
...
begin
  ...
  try
    ...
    try
      ...
      LResponse.FRequest.DoReceiveDataProgress(LResponse.GetStatusCode, LResponse.GetContentLength, 0, nil, 0, LAbort);
      if LAbort then
        LCompletionHandlerImpl(NSURLSessionResponseCancel)
      else
...
begin
  ...
  try
    ...
    LArray := LDict.allValues;
    for I := 0 to LArray.count - 1 do
    begin
      LDataDict := TNSDictionary.Wrap(LArray.objectAtIndex(I));
      ...
      begin
        ...
        if LResponse.GetStatusCode <> 0 then
          ...
          begin
            ...
            LAllow := True;
            if Assigned(FMacHTTPClient.OnRedirect) then
              FMacHTTPClient.OnRedirect(FMacHTTPClient, LResponse.FRequest, LResponse,
                LResponse.FRequest.Redirects, LAllow);
