...
type
  ...
  public
    ...
    /// <summary>Add a form data stream. A stream copy will be added</summary>
    ...
    /// <summary>Returns a form data boundary string</summary>
  ...
  public
    ...
    procedure ForAll(const AExtMask, ATypeMask: string; const AFunc: TIterateFunc);
    procedure ForExts(const AExtMask: string; const AFunc: TIterateFunc);
    procedure ForTypes(const ATypeMask: string; const AFunc: TIterateFunc);
...
procedure TMultipartFormData.BeginPart;
begin
  FSection := TStringStream.Create('', TEncoding.UTF8);
end;

procedure TMultipartFormData.EndPart;
begin
  try
...
procedure TMultipartFormData.AddField(const AField, AValue: string;
  const AContentType: string; AHeaders: TStrings);
var
  I: Integer;
begin
  AdjustLastBoundary;
  ...
  try
    ...
    if AHeaders <> nil then
      for I := 0 to AHeaders.Count - 1 do
        WriteStringLn(AHeaders.Names[I] + ': ' + AHeaders.ValueFromIndex[I]);
...
procedure TMultipartFormData.AddStream(const AField: string; AStream: TStream;
  AOwnsStream: Boolean; const AFileName, AContentType: string; AHeaders: TStrings);
var
  LLine, LType: string;
...
procedure TMultipartFormData.AddStream(const AField: string; AStream: TStream;
  const AFileName: string; const AContentType: string; AHeaders: TStrings);
var
  LStr: TMemoryStream;
begin
  LStr := TMemoryStream.Create;
  try
    LStr.CopyFrom(AStream);
  except
    LStr.Free;
    raise;
  end;
  AddStream(AField, LStr, True, AFileName, AContentType, AHeaders);
end;
...
begin
  ...
  AddStream(AField, LFileStream, True, ExtractFileName(AFilePath), AContentType, AHeaders);
end;

procedure TMultipartFormData.AddBytes(const AField: string;
  const ABytes: TBytes; const AFileName, AContentType: string; AHeaders: TStrings);
var
...
{$REGION 'MIME CONST'}
  ...
  AddType('rtf', 'application/rtf', TMimeTypes.TKind.Text); // do not localize
...
function TMimeTypes.NormalizeType(const AType: string): string;
var
  i: Integer;
begin
  Result := AType;
...
var
  ...
  LType: string;
begin
  LType := NormalizeType(AType);
  Result := FTypeDict.TryGetValue(LType, LInfo);
  if Result then
  begin
    AExt := LInfo.FExt;
    AKind := LInfo.FKind;
...
var
  LInfo, LExistInfo, LTmp1, LTmp2: TInfo;
  LType, LKey: string;
begin
  LType := NormalizeType(AType);
  if LType = '' then
  ...
  if FInfos.TryGetValue(LKey, LExistInfo) then
  begin
    LInfo.Free;
    if AIgnoreDups then
    begin
      if AKind <> TKind.Undefined then
        LExistInfo.FKind := AKind;
      Exit;
    end;
...
procedure TMimeTypes.ForAll(const AExtMask, ATypeMask: string; const AFunc: TIterateFunc);
var
  S: string;
  LExtMask: TMask;
  LTypeMask: TMask;
...
procedure TMimeTypes.ForExts(const AExtMask: string; const AFunc: TIterateFunc);
begin
  ForAll(AExtMask, '', AFunc);
end;

procedure TMimeTypes.ForTypes(const ATypeMask: string; const AFunc: TIterateFunc);
begin
