...
procedure SetCertStore(AStore: Pointer);
...
{$NODEFINE HttpCertWinInfoToTCertificate}
...
type
  ...
  private class var
    ...
    class function GetCertStore: HCERTSTORE; static;
    class procedure SetCertStore(AValue: HCERTSTORE); static;
    class function GetProxyConfig: PWinHttpCurrentUserIEProxyConfig; static;
  public
    class procedure Reset;
    class procedure LockHandleGC;
    class property Handle: THandle read FHandle;
    class property ProxyConfig: PWinHttpCurrentUserIEProxyConfig read GetProxyConfig;
    class property CertStore: HCERTSTORE read GetCertStore write SetCertStore;
  end;
...
begin
  ...
  try
    ...
    if LDataLength > High(DWORD) then
    begin
      LTotalLength := WINHTTP_IGNORE_REQUEST_TOTAL_LENGTH;
      LRequest.AddHeader(sContentLength, IntToStr(LDataLength));
    end
    else
      LTotalLength := LDataLength;
  ...
  // Send Request
  if not WinHttpSendRequest(LRequest.FWRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0, WINHTTP_NO_REQUEST_DATA, 0, LTotalLength, 0) then
...
var
  ...
  begin
    ...
    LpProxyConfig := TWinHttpLib.ProxyConfig;
...
begin
  ...
  FRequestLink.DoReceiveDataProgress(LStatusCode, LExpected, LReaded, nil, 0, LAbort);
  if not LAbort then
    repeat
      // Get the size of readed data in LSize
      if not WinHttpQueryDataAvailable(FWRequest, @LSize) then
        raise ENetHTTPResponseException.CreateResFmt(@SNetHttpRequestReadDataError,
          [GetLastError, SysErrorMessage(GetLastError, TWinHttpLib.Handle)]);
      ...
      FRequestLink.DoReceiveDataProgress(LStatusCode, LExpected, LReaded, Pointer(LBuffer), LDownloaded, LAbort);
...
class procedure TWinHttpLib.SetCertStore(AValue: HCERTSTORE);
begin
  FLock.Enter;
  try
    if FStore <> nil then
      CertCloseStore(FStore, CERT_CLOSE_STORE_FORCE_FLAG);
    FStore := AValue;
  finally
    FLock.Leave;
  end;
