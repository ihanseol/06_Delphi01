...
begin
  ...
  begin
    if FClient.ProxySettings.Scheme.StartsWith('socks', True) then
      LJProxyType := TJProxy_Type.JavaClass.SOCKS
    else
      LJProxyType := TJProxy_Type.JavaClass.HTTP;
    LJProxy := TJProxy.JavaClass.init(LJProxyType,
...
begin
  ...
  if LExpected = 0 then
    LExpected := -1;
  FRequest.DoReceiveDataProgress(LStatusCode, LExpected, LTotalReaded, nil, 0, LAbort);
  if not LAbort then
  begin
    LJBuffer := TJavaArray<System.Byte>.Create(BUFFERSIZE);
    try
      ...
      begin
        ...
        begin
          ...
          FRequest.DoReceiveDataProgress(LStatusCode, LExpected, LTotalReaded, LJBuffer.Data, LReaded, LAbort);
        end;
      end;
    finally
