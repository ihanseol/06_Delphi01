...
// -------------------------------------------------------------------------------- //
  ...
  private
    procedure DecomposeBaseScheme(const AURIStr: string; Pos, Limit, SlashCount: Integer);
    procedure DecomposeNoAuthorityScheme(const AURIStr: string; Pos, Limit, SlashCount: Integer);
...
// -------------------------------------------------------------------------------- //
  ...
  public
    ...
    function Execute(const ARequest: IURLRequest; const AContentStream: TStream; const AHeaders: TNetHeaders): IURLResponse; overload; virtual;
...
// -------------------------------------------------------------------------------- //
  ...
  public type
    TOnSetupClient = reference to procedure (AStream: TURLStream; const AClient: TURLClient);
    TOnSetupRequest = reference to procedure (AStream: TURLStream; const AClient: IURLRequest);
  private class var
    FOnSetupClient: TOnSetupClient;
    FOnSetupRequest: TOnSetupRequest;
  private
    FClient: TURLClient;
    FRequest: IURLRequest;
    FModule: THandle;
  ...
  public
    ...
    constructor Create(const AURL: string; const AProvider: TAsyncReadStream.TStreamer;
      ASynchronizeProvide: Boolean = True; AFreeOnCompletion: Boolean = True);
    destructor Destroy; override;
    class property OnSetupClient: TOnSetupClient read FOnSetupClient write FOnSetupClient;
    class property OnSetupRequest: TOnSetupRequest read FOnSetupRequest write FOnSetupRequest;
  end;
...
begin
  ...
  begin
    FQuery := FQuery + Params[I].Name;
    if Params[I].Value <> #0 then
      FQuery := FQuery + '=' + Params[I].Value
    else
      Params[I].Value := '';
    FQuery := FQuery + '&';
  end;
  if FQuery <> '' then
    FQuery := FQuery.Substring(0, FQuery.Length - 1); //Remove last '&'
end;
...
begin
        ...
        begin
          ...
          begin
            ...
            if LHasUsername then
              FUsername := FUsername + '%40' + LEncodedUsername
            else
              FUsername := LEncodedUsername;

            if LPasswordColonOffset <> LCompDelimiterOffset then
            begin
              LHasPassword := True;
              FPassword := TNetEncoding.URL.EncodeAuth(AURIStr.Substring(LPasswordColonOffset + 1,
                LCompDelimiterOffset - (LPasswordColonOffset + 1)));
            end;
            LHasUsername := True;
          end
          else
            FPassword := FPassword + '%40' + TNetEncoding.URL.EncodeAuth(AURIStr.Substring(Pos, LCompDelimiterOffset - Pos));
  ...
  begin
    ...
    Query := AURIStr.Substring(Pos + 1, LQueryDelimiterOffset - (Pos + 1));
...
function TryEncode(const AStr: string): string;
begin
  // Encode value when it is not encoded, otherwise preserve original encoding
  if TNetEncoding.URL.URLDecode(AStr) = AStr then
    Result := TNetEncoding.URL.EncodeQuery(AStr)
  else
    Result := AStr;
end;
...
begin
  ...
  begin
    FParams[I].Name := TryEncode(FParams[I].Name);
    FParams[I].Value := TryEncode(FParams[I].Value);
  end;
  QueryChanged;
end;
...
var
  ...
  Pos: Integer;
begin
  if Scheme.Equals(SCHEME_HTTP) or Scheme.Equals(SCHEME_HTTPS) then
    LParts := Value.Split([Char('&')])
  else
    LParts := Value.Split([Char(';'), Char('&')]);
  ...
  begin
    ...
    if Pos > 0 then
    begin
      FParams[I].Name := TryEncode(LParts[I].Substring(0, Pos));
      FParams[I].Value := TryEncode(LParts[I].Substring(Pos + 1));
    end
    else
    begin
      FParams[I].Name := TryEncode(LParts[I]);
      FParams[I].Value := #0;
    end;
  end;
  QueryChanged;
...
function TURLClient.Execute(const ARequest: IURLRequest;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        LStream := nil;
        if Assigned(OnSetupClient) then
          OnSetupClient(Self, FClient);
        FRequest := FClient.GetRequest('GET', LURI);
        if Assigned(OnSetupRequest) then
          OnSetupRequest(Self, FRequest);
      end
      else
...
begin
  if FClient <> nil then
    FClient.Execute(FRequest, Self, nil);
end;
