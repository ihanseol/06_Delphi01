...
// ------------------------------------------------------------------------------------------------------------------ //
  ...
  private
    ...
    procedure DoOnReceiveDataEx(const Sender: TObject; AContentLength, AReadCount: Int64;
      AChunk: Pointer; AChunkLength: Cardinal; var AAbort: Boolean);
    procedure DoOnRedirect(const Sender: TObject; const ARequest: IHTTPRequest;
      const AResponse: IHTTPResponse; ARedirections: Integer; var AAllow: Boolean);
    procedure DoOnUpdateCookie(const Sender: TObject; const ACookie: TCookie;
      const AURL: TURI; var AAllow: Boolean);
    ...
    procedure SetOnReceiveDataEx(const Value: TReceiveDataExEvent);
    ...
    procedure SetOnRedirect(const Value: THTTPRedirectEvent);
    procedure SetOnUpdateCookie(const Value: THTTPUpdateCookieEvent);
  ...
  published
    /// <summary>Property to indicate if the requests are going to be Synchronous or Asynchronous.
    /// Default value is False.</summary>
    property Asynchronous: Boolean read FAsynchronous write FAsynchronous default False;
    /// <summary>Specifies if Events should run in the context of the main thread (True)
    /// or in the context of an arbitrary thread. It is recomended to set it to False
    /// on Android platform to avoid deadlock. Default value is True. </summary>
    ...
    property OnReceiveDataEx: TReceiveDataExEvent read FOnReceiveDataEx write SetOnReceiveDataEx;
...
function TNetHTTPClient.Delete(const AURL: string; const ASource,
  AResponseContent: TStream; const AHeaders: TNetHeaders): IHTTPResponse;
begin
  try
    if FAsynchronous then
      Result := FHttpClient.BeginDelete(DoOnAsyncRequestCompleted, AURL, ASource, AResponseContent, AHeaders) as IHTTPResponse
    else
    begin
      Result := FHttpClient.Delete(AURL, ASource, AResponseContent, AHeaders);
      DoOnRequestCompleted(Self, Result);
    end;
  except
    on E: Exception do
...
procedure TNetHTTPClient.DoOnReceiveDataEx(const Sender: TObject; AContentLength,
...
begin
  ...
  begin
    ...
    HandleEvent(SynchronizeEvents,
      procedure
      begin
        FOnReceiveDataEx(Self, AContentLength, AReadCount, AChunk, AChunkLength, LAbort);
      end);
    AAbort := LAbort;
  end;
end;
...
procedure TNetHTTPClient.DoOnRedirect(const Sender: TObject; const ARequest: IHTTPRequest;
  const AResponse: IHTTPResponse; ARedirections: Integer; var AAllow: Boolean);
var
  LAllow: Boolean;
begin
  ...
  begin
    ...
    HandleEvent(SynchronizeEvents,
      procedure
      begin
        FOnRedirect(Self, ARequest, AResponse, ARedirections, LAllow);
      end);
    AAllow := LAllow;
  end;
end;

procedure TNetHTTPClient.DoOnUpdateCookie(const Sender: TObject; const ACookie: TCookie;
  const AURL: TURI; var AAllow: Boolean);
var
  LAllow: Boolean;
begin
  ...
  begin
    ...
    HandleEvent(SynchronizeEvents,
      procedure
      begin
        FOnUpdateCookie(Self, ACookie, AURL, LAllow);
      end);
    AAllow := LAllow;
  end;
end;
...
procedure TNetHTTPClient.SetOnReceiveDataEx(const Value: TReceiveDataExEvent);
begin
  FOnReceiveDataEx := Value;
  if Assigned(FOnReceiveDataEx) then
    FHttpClient.OnReceiveDataEx := DoOnReceiveDataEx
  else
    FHttpClient.OnReceiveDataEx := nil;
end;

procedure TNetHTTPClient.SetOnRedirect(const Value: THTTPRedirectEvent);
begin
  FOnRedirect := Value;
  if Assigned(FOnRedirect) then
    FHttpClient.OnRedirect := DoOnRedirect
  else
    FHttpClient.OnRedirect := nil;
end;

procedure TNetHTTPClient.SetOnUpdateCookie(const Value: THTTPUpdateCookieEvent);
begin
  FOnUpdateCookie := Value;
  if Assigned(FOnUpdateCookie) then
    FHttpClient.OnUpdateCookie := DoOnUpdateCookie
  else
    FHttpClient.OnUpdateCookie := nil;
end;
...
procedure TNetHTTPRequest.DoOnReceiveDataEx(const Sender: TObject; AContentLength,
  AReadCount: Int64; AChunk: Pointer; AChunkLength: Cardinal; var AAbort: Boolean);
var
  LAbort: Boolean;
begin
...
begin
  ...
  try
    ...
    if Assigned(FOnReceiveDataEx) then
      Result.OnReceiveDataEx := DoOnReceiveDataEx;
...
procedure TNetHTTPRequest.SetOnReceiveDataEx(const Value: TReceiveDataExEvent);
begin
  FOnReceiveDataEx := Value;
  if FHttpRequest <> nil then
    if Assigned(FOnReceiveDataEx) then
      FHttpRequest.OnReceiveDataEx := DoOnReceiveDataEx
    else
      FHttpRequest.OnReceiveDataEx := nil;
end;
